...
Log::Log()
    : AppenderId(0), lowestLogLevel(LOG_LEVEL_FATAL), _ioContext(nullptr),
      _strand(nullptr) {
...
void Log::CreateAppenderFromConfig(std::string const &appenderName) {
  ...
  AppenderType type = AppenderType(
      Firelands::StringTo<uint8>(tokens[0]).value_or(APPENDER_INVALID));
  LogLevel level = LogLevel(
      Firelands::StringTo<uint8>(tokens[1]).value_or(LOG_LEVEL_INVALID));

  if (level > LOG_LEVEL_FATAL) {
    fmt::print(
        stderr,
        "Log::CreateAppenderFromConfig: Wrong Log Level {} for appender {}\n",
        level, name);
    return;
  }
  ...
  if (size > 2) {
    if (Optional<uint8> flagsVal = Firelands::StringTo<uint8>(tokens[2])) {
      ...
      fmt::print(
          stderr,
          "Log::CreateAppenderFromConfig: Unknown flags '{}' for appender {}\n",
          tokens[2], name);
      return;
    }
  }
  ...
  if (factoryFunction == appenderFactory.end()) {
    fmt::print(
        stderr,
        "Log::CreateAppenderFromConfig: Unknown type '{}' for appender {}\n",
        tokens[0], name);
    return;
  }
...
void Log::CreateLoggerFromConfig(std::string const &appenderName) {
  ...
  if (tokens.size() != 2) {
    fmt::print(
        stderr,
        "Log::CreateLoggerFromConfig: Wrong config option Logger.{}={}\n", name,
        options);
    return;
  }
  ...
  for (std::string_view appendName :
       Firelands::Tokenize(tokens[1], ' ', false)) {
    if (Appender *appender = GetAppenderByName(appendName)) {
      logger->addAppender(appender->getId(), appender);
    } else {
      fmt::print(stderr,
                 "Error while configuring Appender {} in Logger {}. Appender "
                 "does not exist\n",
                 appendName, name);
    }
  }
...
void Log::ReadLoggersFromConfig() {
  ...
  if (loggers.find(LOGGER_ROOT) == loggers.end()) {
    ...
    AppenderConsole *appender = new AppenderConsole(
        NextAppenderId(), "Console", LOG_LEVEL_DEBUG, APPENDER_FLAGS_NONE,
...
void Log::_outMessage(std::string const &filter, LogLevel level,
                      std::string &&message) {
...
void Log::_outCommand(std::string &&message, std::string &&param1) {
  write(Firelands::make_unique<LogMessage>(
      LOG_LEVEL_INFO, "commands.gm", std::move(message), std::move(param1)));
}
...
void Log::write(std::unique_ptr<LogMessage> &&msg) const {
  ...
  if (_ioContext) {
    ...
    Firelands::Asio::post(
        *_ioContext, Firelands::Asio::bind_executor(
                         *_strand, [logOperation]() { logOperation->call(); }));
  } else
...
void Log::outCharDump(char const *str, uint32 accountId, uint64 guid,
                      char const *name) {
  if (!str || !ShouldLog("entities.player.dump", LOG_LEVEL_INFO))
    return;

  std::ostringstream ss;
  ss << "== START DUMP == (account: " << accountId << " guid: " << guid
     << " name: " << name << ")\n"
     << str << "\n== END DUMP ==\n";

  std::unique_ptr<LogMessage> msg(
      new LogMessage(LOG_LEVEL_INFO, "entities.player.dump", ss.str()));
...
bool Log::ShouldLog(std::string const &type, LogLevel level) const {
  ...
  // Don't even look for a logger if the LogLevel is lower than lowest log
