...
AppenderFile::AppenderFile(uint8 id, std::string const &name, LogLevel level,
                           AppenderFlags flags,
  ...
  if (extraArgs.empty())
    throw InvalidAppenderArgsException(Firelands::StringFormat(
        "Log::CreateAppenderFromConfig: Missing file name for appender %s\n",
        name.c_str()));
  ...
  if (extraArgs.size() > 2) {
    if (Optional<uint32> size = Firelands::StringTo<uint32>(extraArgs[2])) {
      ...
      throw InvalidAppenderArgsException(Firelands::StringFormatFmt(
          "Log::CreateAppenderFromConfig: Invalid size '{}' for appender {}",
          extraArgs[2], name));
    }
  }
...
void AppenderFile::_write(LogMessage const *message) {
  ...
  if (_dynamicName) {
    ...
    snprintf(namebuf, FIRELANDS_PATH_MAX, _fileName.c_str(),
...
FILE *AppenderFile::OpenFile(std::string const &filename,
                             std::string const &mode, bool backup) {
