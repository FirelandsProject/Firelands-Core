    ...
    _fileSize(0)
{
    if (args.size() < 4)
    {
        throw InvalidAppenderArgsException(Firelands::StringFormatFmt("Log::CreateAppenderFromConfig: Missing file name for appender {}", name));
    }
    ...
    if (5 < args.size())
    {
        if (Optional<uint32> size = Firelands::StringTo<uint32>(args[5]))
        {
            _maxFileSize = *size;
        }
        else
        {
            throw InvalidAppenderArgsException(Firelands::StringFormatFmt("Log::CreateAppenderFromConfig: Invalid size '{}' for appender {}", args[5], name));
        }
    }
