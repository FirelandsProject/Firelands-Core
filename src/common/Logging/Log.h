...
public:
    ...
    void Initialize(Firelands::Asio::IoContext* ioContext = nullptr);
    ...
    {
        _outMessage(filter, level, Firelands::StringFormatFmt(fmt, std::forward<Args>(args)...));
    }
    ...
    void outCommand(uint32 account, std::string_view fmt, Args&&... args)
    {
        ...
        _outCommand(Firelands::StringFormatFmt(fmt, std::forward<Args>(args)...), std::to_string(account));
    }
