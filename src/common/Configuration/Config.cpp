...
void AddKey(std::string const &optionName, std::string const &optionKey,
            std::string_view fileName, bool isOptional,
...
bool ParseFile(std::string const &file, bool isOptional, bool isReload) {
  ...
  if (in.fail()) {
    ...
    std::string message = "Config::LoadFile: Failed open " +
                          std::string(isOptional ? "optional " : "") +
                          " file '" + file + "'";
  ...
  while (in.good()) {
    ...
    // read line error
    if (!in.good() && !in.eof()) {
      std::string msg = "> Config::LoadFile: Failure to read line number " +
    ...
    auto entry =
        Firelands::String::Trim(line.substr(0, equal_pos), in.getloc());
    auto value = Firelands::String::Trim(
...
bool ConfigMgr::LoadAppConfigs(bool isReload /*= false*/) {
  ...
  if (!LoadInitial(_filename + ".dist", isReload)) {
    LOG_ERROR("server.loading",
              "> Config::LoadAppsConfig: .dist file not found.");
    return false;
  }
...
bool ConfigMgr::LoadModulesConfigs(bool isReload /*= false*/,
  ...
  for (auto const &distFileName : _additonalFiles) {
    ...
    isExistDistConfig =
        LoadAdditionalFile(moduleConfigPath + distFileName, false, isReload);
    ...
    isExistDefaultConfig =
        LoadAdditionalFile(moduleConfigPath + defaultFileName, true, isReload);
