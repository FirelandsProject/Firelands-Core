...
Optional<int32> MoneyStringToMoney(std::string_view moneyString)
{
    ...
    for (std::string_view token : Firelands::Tokenize(moneyString, ' ', false))
    {
        uint32 unit;
        ...
        Optional<uint32> amount = Firelands::StringTo<uint32>(token.substr(0, token.length() - 1));
        if (amount)
        {
            money += (unit * *amount);
        }
...
void Firelands::Impl::HexStrToByteArray(std::string_view str, uint8* out, size_t outlen, bool reverse /*= false*/)
{
