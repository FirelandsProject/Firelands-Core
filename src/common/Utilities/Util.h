...
template <class T, class U> inline T CalculatePct(T base, U pct) {
  return T(base * static_cast<float>(pct) / 100.0f);
}

template <class T, class U> inline T AddPct(T &base, U pct) {
  return base += CalculatePct(base, pct);
}

template <class T, class U> inline T ApplyPct(T &base, U pct) {
  return base = CalculatePct(base, pct);
}
...
FC_COMMON_API bool Utf8toWStr(char const *utf8str, size_t csize, wchar_t *wstr,
                              size_t &wsize);
...
FC_COMMON_API bool WStrToUtf8(wchar_t const *wstr, size_t size,
                              std::string &utf8str);
...
inline bool isExtendedLatinCharacter(wchar_t wchar) {
  ...
  if (wchar >= 0x00F8 &&
      wchar <=
          0x00FE) // LATIN SMALL LETTER O WITH STROKE - LATIN SMALL LETTER THORN
  {
    return true;
  }
...
inline wchar_t wcharToUpper(wchar_t wchar) {
  ...
  if (wchar >= 0x0101 &&
      wchar <= 0x012F) // LATIN SMALL LETTER A WITH MACRON - LATIN SMALL LETTER
...
inline wchar_t wcharToLower(wchar_t wchar) {
  ...
  if (wchar >= 0x0410 &&
      wchar <= 0x042F) // CYRILLIC CAPITAL LETTER A - CYRILLIC CAPITAL LETTER YA
  {
    return wchar_t(uint16(wchar) + 0x0020);
  }
...
FC_COMMON_API std::string ByteArrayToHexStr(uint8 const *bytes, size_t length,
                                            bool reverse = false);
FC_COMMON_API void HexStrToByteArray(std::string_view str, uint8 *out,
                                     size_t outlen, bool reverse = false);
} // namespace Firelands::Impl

template <typename Container>
std::string ByteArrayToHexStr(Container const &c, bool reverse = false) {
  return Firelands::Impl::ByteArrayToHexStr(std::data(c), std::size(c),
                                            reverse);
}

template <size_t Size>
void HexStrToByteArray(std::string_view str, std::array<uint8, Size> &buf,
                       bool reverse = false) {
  Firelands::Impl::HexStrToByteArray(str, buf.data(), Size, reverse);
}
...
inline bool StringStartsWith(std::string_view haystack,
...
FC_COMMON_API bool StringContainsStringI(std::string_view haystack,
                                         std::string_view needle);
...
public:
  ...
  template <typename... Args> void Register(Args &&...args) {
...
public:
  ...
  [[nodiscard]] inline bool IsEqual(uint32 p1 = 0, uint32 p2 = 0,
                                    uint32 p3 = 0) const {
    return (part[0] == p1 && part[1] == p2 && part[2] == p3);
  }

  [[nodiscard]] inline bool HasFlag(uint32 p1 = 0, uint32 p2 = 0,
                                    uint32 p3 = 0) const {
    return (part[0] & p1 || part[1] & p2 || part[2] & p3);
  }
