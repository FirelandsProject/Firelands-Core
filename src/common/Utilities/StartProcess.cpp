...
template <typename T> class TCLogSink {
  T callback_;
...
public:
  ...
  TCLogSink(T callback) : callback_(std::move(callback)) {}
...
static int CreateChildProcess(T waiter, std::string const &executable,
  ...
  if (!secure) {
    LOG_TRACE(logger, "Starting process \"{}\" with arguments: \"{}\".",
              executable.c_str(),
              boost::algorithm::join(argsVector, " ").c_str());
  }
  ...
  child c = [&]() {
    if (inputFile) {
      // With binding stdin
      return child{exe = boost::filesystem::absolute(executable).string(),
                   args = argsVector,
  ...
  auto outInfo = MakeTCLogSink(
      [&](std::string const &msg) { LOG_INFO(logger, "{}", msg.c_str()); });

  auto outError = MakeTCLogSink(
      [&](std::string const &msg) { LOG_ERROR(logger, "{}", msg.c_str()); });
...
public:
  explicit AsyncProcessResultImplementation(std::string executable_,
      ...
      : executable(std::move(executable_)), args(std::move(args_)),
        logger(std::move(logger_)), input_file(input_file_), is_secure(secure),
...
StartAsyncProcess(std::string executable, std::vector<std::string> args,
  ...
  auto handle = std::make_shared<AsyncProcessResultImplementation>(
      std::move(executable), std::move(args), std::move(logger),
      std::move(input_file), secure);
