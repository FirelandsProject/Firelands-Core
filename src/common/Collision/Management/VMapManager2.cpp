...
namespace VMAP
{
    ...
    bool VMapManager2::_loadMap(unsigned int mapId, const std::string& basePath, uint32 tileX, uint32 tileY)
    {
        ...
        if (instanceTree == iInstanceMapTrees.end())
        {
            if (thread_safe_environment)
            {
                ...
                ABORT("Invalid mapId {} tile [{}, {}] passed to VMapManager2 after startup in thread unsafe environment",
                       mapId, tileX, tileY);
        }
    ...
    */
    ...
    void VMapManager2::releaseModelInstance(const std::string &filename)
    {
        ...
        if (model == iLoadedModelFiles.end())
        {
            LOG_ERROR("maps", "VMapManager2: trying to unload non-loaded file '{}'", filename);
            return;
        }
        if (model->second.decRefCount() == 0)
        {
            LOG_DEBUG("maps", "VMapManager2: unloading file '{}'", filename);
