...
namespace MMAP
{
    static char const* const MAP_FILE_NAME_FORMAT = "{}mmaps/{}.mmap";
    static char const* const TILE_FILE_NAME_FORMAT = "{}mmaps/{}{}{}.mmtile";
    ...
    bool MMapManager::loadMapData(std::string const& basePath, uint32 mapId)
    {
        ...
        if (!file)
        {
            LOG_DEBUG("maps", "MMAP:loadMapData: Error: Could not open mmap file '{}'", fileName.c_str());
            return false;
        }
        ...
        if (count != 1)
        {
            LOG_DEBUG("maps", "MMAP:loadMapData: Error: Could not read params from file '{}'", fileName.c_str());
            return false;
        }
        ...
        {
            dtFreeNavMesh(mesh);
            LOG_ERROR("maps", "MMAP:loadMapData: Failed to initialize dtNavMesh for mmap {} from file {}", mapId, fileName.c_str());
            return false;
        }

        LOG_DEBUG("maps", "MMAP:loadMapData: Loaded {}.mmap", mapId);
    ...
    bool MMapManager::loadMapImpl(std::string const& basePath, uint32 mapId, int32 x, int32 y)
    {
        ...
        {
            LOG_ERROR("maps", "MMAP:loadMap: Bad header in mmap {}{}{}.mmtile", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        {
            LOG_ERROR("maps", "MMAP:loadMap: {}{}{}.mmtile was built with generator v{}, expected v{}",
        ...
        {
            LOG_ERROR("maps", "MMAP:loadMap: {}{}{}.mmtile has corrupted data size", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        if (!result)
        {
            LOG_ERROR("maps", "MMAP:loadMap: Bad header or data in mmap {}{}{}.mmtile", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        if (dtStatusSucceed(mmap->navMesh->addTile(data, fileHeader.size, DT_TILE_FREE_DATA, 0, &tileRef)))
        {
            ...
            ++loadedTiles;
            LOG_DEBUG("maps", "MMAP:loadMap: Loaded mmtile {}[{}, {}] into {}[{}, {}]", mapId, x, y, mapId, header->x, header->y);
            return true;
        }
        else
        {
            LOG_ERROR("maps", "MMAP:loadMap: Could not load {}{}{}.mmtile into navmesh", mapId, x, y);
            dtFree(data);
            return false;
        }
    }
    ...
    bool MMapManager::loadMapInstanceImpl(std::string const& basePath, uint32 mapId, uint32 instanceId)
    {
        ...
        if (dtStatusFailed(query->init(mmap->navMesh, 1024)))
        {
            ...
            LOG_ERROR("maps", "MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for mapId {} instanceId {}", mapId, instanceId);
            return false;
        }

        LOG_DEBUG("maps", "MMAP:GetNavMeshQuery: created dtNavMeshQuery for mapId {} instanceId {}", mapId, instanceId);
    ...
    bool MMapManager::unloadMapImpl(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (itr == loadedMMaps.end())
        {
            ...
            LOG_DEBUG("maps", "MMAP:unloadMap: Asked to unload not loaded navmesh map. {}{}{}.mmtile", mapId, x, y);
            return false;
        }
        ...
        {
            // file may not exist, therefore not loaded
            LOG_DEBUG("maps", "MMAP:unloadMap: Asked to unload not loaded navmesh tile. {}{}{}.mmtile", mapId, x, y);
            return false;
        }
        ...
        if (dtStatusFailed(mmap->navMesh->removeTile(tileRefItr->second, nullptr, nullptr)))
        {
            ...
            LOG_ERROR("maps", "MMAP:unloadMap: Could not unload {}{}{}.mmtile from navmesh", mapId, x, y);
            ABORT();
        }
        else
        {
            ...
            --loadedTiles;
            LOG_DEBUG("maps", "MMAP:unloadMap: Unloaded mmtile {}[{}, {}] from {}", mapId, x, y, mapId);
            return true;
        }
    ...
    bool MMapManager::unloadMapImpl(uint32 mapId)
    {
        ...
        {
            // file may not exist, therefore not loaded
            LOG_DEBUG("maps", "MMAP:unloadMap: Asked to unload not loaded navmesh map {}", mapId);
            return false;
        }
        ...
        delete mmap;
        itr->second = nullptr;
        LOG_DEBUG("maps", "MMAP:unloadMap: Unloaded {}.mmap", mapId);
    ...
    bool MMapManager::unloadMapInstance(uint32 mapId, uint32 instanceId)
    {
        ...
        {
            // file may not exist, therefore not loaded
            LOG_DEBUG("maps", "MMAP:unloadMapInstance: Asked to unload not loaded navmesh map {}", mapId);
            return false;
        }
        ...
        if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end())
        {
            LOG_DEBUG("maps", "MMAP:unloadMapInstance: Asked to unload not loaded dtNavMeshQuery mapId {} instanceId {}", mapId, instanceId);
            return false;
        }
        ...
        LOG_DEBUG("maps", "MMAP:unloadMapInstance: Unloaded mapId {} instanceId {}", mapId, instanceId);
