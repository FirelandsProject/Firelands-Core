    ...
    */
    ...
    bool StaticMapTree::InitMap(const std::string &fname, VMapManager2* vm)
    {
        ...
        else
        {
            ...
            ModelSpawn spawn;
#ifdef VMAP_DEBUG
            LOG_DEBUG("maps", "StaticMapTree::InitMap() : map isTiled: {}", static_cast<uint32>(iIsTiled));
#endif
            if (!iIsTiled && ModelSpawn::readFromFile(rf, spawn))
            {
                WorldModel* model = vm->acquireModelInstance(iBasePath, spawn.name);
                VMAP_DEBUG_LOG("maps", "StaticMapTree::InitMap() : loading {}", spawn.name);
                if (model)
                {
                ...
                {
                    success = false;
                    VMAP_ERROR_LOG("misc", "StaticMapTree::InitMap() : could not acquire WorldModel pointer for '{}'", spawn.name);
                }
            }
                ...
                for (uint32 i=0; i<numSpawns && result; ++i)
                {
                    ...
                    if (result)
                    {
                        ...
                        else
                        {
                            ...
                            VMAP_ERROR_LOG("misc", "StaticMapTree::UnloadMapTile() : trying to unload non-referenced model '{}' (ID:{})", spawn.name, spawn.ID);
