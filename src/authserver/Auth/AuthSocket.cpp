        ...
        if (result)
        {
            ...
            if (!locked)
            {
                ...
                else
                {
                    ...
                    FIRELANDS_ASSERT(gmod.GetNumBytes() <= 32);
            ...
            if (QueryResult* loginfail = LoginDatabase.PQuery("SELECT `id`, `failed_logins` FROM `account` WHERE `username` = '%s'", _safelogin.c_str()))
            {
                ...
                if (failed_logins >= MaxWrongPassCount)
                {
                    ...
                    if (WrongPassBanType)
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO `account_banned` VALUES ('%u',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','Firelands authserver','Failed login autoban',1)",
                    ...
                    else
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO `ip_banned` VALUES ('%s',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','Firelands authserver','Failed login autoban')",
