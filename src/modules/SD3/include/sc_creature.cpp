...
Unit* ScriptedAI::DoSelectLowestHpFriendly(float fRange, uint32 uiMinHPDiff)
{
    ...
    Firelands::MostHPMissingInRangeCheck u_check(m_creature, fRange, uiMinHPDiff);
    Firelands::UnitLastSearcher<Firelands::MostHPMissingInRangeCheck> searcher(pUnit, u_check);
...
std::list<Creature*> ScriptedAI::DoFindFriendlyCC(float fRange)
{
    ...
    Firelands::FriendlyCCedInRangeCheck u_check(m_creature, fRange);
    Firelands::CreatureListSearcher<Firelands::FriendlyCCedInRangeCheck> searcher(pList, u_check);
...
std::list<Creature*> ScriptedAI::DoFindFriendlyMissingBuff(float fRange, uint32 uiSpellId)
{
    ...
    Firelands::FriendlyMissingBuffInRangeCheck u_check(m_creature, fRange, uiSpellId);
    Firelands::CreatureListSearcher<Firelands::FriendlyMissingBuffInRangeCheck> searcher(pList, u_check);
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float fMinimumRange)
{
    ...
    Firelands::AnyPlayerInObjectRangeCheck check(m_creature, fMinimumRange);
    Firelands::PlayerSearcher<Firelands::AnyPlayerInObjectRangeCheck> searcher(pPlayer, check);
...
// It is assumed the information is found elswehere and can be handled by firelands. So far no luck finding such information/way to extract it.
enum
{
