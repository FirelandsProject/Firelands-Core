...
struct npc_eye_of_acherus : public CreatureScript
{
    ...
    struct  npc_eye_of_acherusAI : public ScriptedAI
    {
        npc_eye_of_acherusAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
            ...
            m_creature->SetPhaseMask(3, true);                  // HACK as firelands cannot handle auras proberly, also HACK below
            Reset();
        }
        ...
        {
            m_creature->CastSpell(m_creature, 52694, true);     // HACK - Remove this when firelands supports proper spell casting
        }
        ...
        void UpdateAI(const uint32 /*uiDiff*/) override
        {
            ...
            if (Player* pPlayer = m_creature->GetCharmerOrOwnerPlayerOrPlayerItself())
            {
                m_creature->SetPhaseMask(2, true);              // HACK remove when summon spells and auras are implemented properly in firelands
