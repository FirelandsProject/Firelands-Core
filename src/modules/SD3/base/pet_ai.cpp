...
{
    if (!m_creature->IsAlive())                             // should not be needed, IsAlive is checked in firelands before calling UpdateAI
    {
        return;
    }

    // UpdateAllies() is done in the generic PetAI in Firelands, but we can't do this from script side.
    // Unclear what side effects this has, but is something to be resolved from Firelands.
    ...
    if (m_creature->getVictim())                            // in combat
    {
        if (!m_creature->getVictim()->IsTargetableForAttack())
        {
            ...
            // doesn't normally reach this, because of how petAi is designed in Firelands. CombatStop
