...
namespace LuaGlobalFunctions
{
     ...
     *
     * The result will be either `Firelands`, `cFirelands`, or `TrinityCore`.
     *
     ...
     *
     * - for Firelands returns the realmID as it is stored in the core.
     ...
     * - For cFirelands returns the date and time of the last revision, e.g. `2015-09-06 13:18:50`
     * - for Firelands returns the version number as string, e.g. `21000`
     *
     ...
     *
     * On Firelands and cFirelands low GUIDs are unique only on the same map.
...
#elif defined(MANGOS) || defined(FIRELANDS)
    ...
    int PerformIngameSpawn(lua_State* L)
    {
        ...
        if (spawntype == 2) // Spawn object
        {
            if (save)
            {
                ...
                // DEBUG_LOG(GetFirelandsString(LANG_GAMEOBJECT_CURRENT), gInfo->name, db_lowGUID, x, y, z, o);
