...
namespace LuaUnit
{
    ...
    {
#if defined(MANGOS) || defined(CMANGOS) || defined(FIRELANDS)
    ...
    */
    ...
    int GetFriendlyUnitsInRange(lua_State* L, Unit* unit)
    {
        ...
        Cell::VisitAllObjects(unit, searcher, range);
#elif CMANGOS
        Firelands::AnyFriendlyUnitInObjectRangeCheck checker(unit, nullptr, range);
        Firelands::UnitListSearcher<Firelands::AnyFriendlyUnitInObjectRangeCheck> searcher(list, checker);
        Cell::VisitGridObjects(unit, searcher, range);
#else
        Firelands::AnyFriendlyUnitInObjectRangeCheck checker(unit, range);
        Firelands::UnitListSearcher<Firelands::AnyFriendlyUnitInObjectRangeCheck> searcher(list, checker);
        Cell::VisitGridObjects(unit, searcher, range);
#endif
    ...
    int GetUnfriendlyUnitsInRange(lua_State* L, Unit* unit)
    {
        ...
        Cell::VisitAllObjects(unit, searcher, range);
#else
        Firelands::AnyUnfriendlyUnitInObjectRangeCheck checker(unit, range);
        Firelands::UnitListSearcher<Firelands::AnyUnfriendlyUnitInObjectRangeCheck> searcher(list, checker);
        Cell::VisitGridObjects(unit, searcher, range);
#endif
        ElunaUtil::ObjectGUIDCheck guidCheck(unit->GET_GUID());
        list.remove_if(guidCheck);
...
#if (defined(CMANGOS) || defined(MANGOS) || defined(FIRELANDS)) && (defined(WOTLK) || defined(CATA))
