...
SqlPreparedStatement* SqlConnection::GetStmt(uint32 nIndex)
{
    ...
    if (m_holder[nIndex] == NULL)
    {
        ...
        FIRELANDS_ASSERT(fmt.length());
        // allocate SQlPreparedStatement object
        pStmt = CreateStatement(fmt);
        // prepare statement
        if (!pStmt->prepare())
        {
            FIRELANDS_ASSERT(false && "Unable to prepare SQL statement");
            return NULL;
        }
...
SqlStatement Database::CreateStatement(SqlStatementID& index, const char* fmt)
{
    ...
    if (!index.initialized())
    {
        ...
        FIRELANDS_ASSERT(_guard.locked());
...
{
    FIRELANDS_ASSERT(!m_pTrans);   // if we will get a nested transaction request - we MUST fix code!!!
