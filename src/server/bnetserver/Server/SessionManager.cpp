...
bool Battlenet::SessionManager::StartNetwork(Firelands::Asio::IoContext& ioContext, std::string const& bindIp, uint16 port, int threadCount /*= 1*/)
{
...
Battlenet::Session* Battlenet::SessionManager::GetSession(uint32 accountId, uint32 gameAccountId) const
{
    ...
    return Firelands::Containers::MapGetValuePtr(_sessions, { accountId, gameAccountId });
}

std::list<Battlenet::Session*>* Battlenet::SessionManager::GetSessions(uint32 accountId) const
{
    boost::shared_lock<boost::shared_mutex> lock(_sessionMutex);
    return Firelands::Containers::MapGetValuePtr(_sessionsByAccountId, accountId);
}
