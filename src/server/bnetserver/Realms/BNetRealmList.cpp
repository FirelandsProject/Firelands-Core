    ...
    if (result)
    {
        do
        {
            try
            {
                ...
                LOG_TRACE("server.bnetserver", "Realm \"%s\" at %s:%u.", name.c_str(), externalAddressString.c_str(), port);
            }
            catch (std::exception& ex)
            {
                LOG_ERROR("server.bnetserver", "BNetRealmList::UpdateRealms has thrown an exception: %s", ex.what());
                ABORT();
            }
