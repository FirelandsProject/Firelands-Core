...
void WorldListener::HandleOpen()
{
    ...
    {
        LOG_FATAL("server.ipc", "Could not bind to WorldserverListenPort %u. Exception: %s. Shutting down bnetserver.", _worldListenPort, ex.what());
        abort();
    }

    _poller->add(*_worldSocket);
    LOG_INFO("server.ipc", "Listening on connections from worldservers on port %u...", _worldListenPort);
}

void WorldListener::HandleClose()
{
    _worldSocket->close();
    LOG_INFO("server.ipc", "Shutting down connections from worldservers...");
}
