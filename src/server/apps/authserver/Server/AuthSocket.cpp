...
{
    LOG_DEBUG("server.authserver", "'{}:{}' Accepting connection", socket().getRemoteAddress(), socket().getRemotePort());
}
...
#define MAX_AUTH_LOGON_CHALLENGES_IN_A_ROW 3
    ...
    while (1)
    {
        ...
        if (_cmd == 16) // REALM_LIST
        {
            ...
            {
                LOG_WARN("server.authserver", "Got {} REALM_LIST in a row from '{}', possible ongoing DoS", challengesInARow, socket().getRemoteAddress());
                socket().shutdown();
        ...
        if (_cmd == AUTH_LOGON_CHALLENGE)
        {
            ...
            {
                LOG_WARN("server.authserver", "Got {} AUTH_LOGON_CHALLENGE in a row from '{}', possible ongoing DoS", challengesInARow, socket().getRemoteAddress());
                socket().shutdown();
                return;
            }
        }
        ...
        for (i = 0; i < AUTH_TOTAL_COMMANDS; ++i)
        {
            ...
            {
                LOG_DEBUG("server.authserver", "Received unauthorized command {} recv length {}", (uint32)_cmd, (uint32)socket().recv_len());
                return;
            }

            LOG_DEBUG("server.authserver", "Received command {}, length {}", (uint32)_cmd, (uint32)socket().recv_len());
            if (!(*this.*table[i].handler)())
            {
                LOG_DEBUG("server.authserver", "Command handler failed for cmd {} recv length {}", (uint32)_cmd, (uint32)socket().recv_len());
                return;
            }
            break;
        }
        ...
        {
            LOG_ERROR("server.authserver", "Got unknown command {}", (uint32)_cmd);
            socket().shutdown();
            return;
        }
    }
...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    LOG_DEBUG("server.authserver", "[AuthChallenge] got header, body is {} bytes", remaining);
    ...
    LOG_DEBUG("server.authserver", "[AuthChallenge] got full packet, {} bytes", ch->size);
    LOG_DEBUG("server.authserver", "[AuthChallenge] name({}): '{}'", ch->I_len, ch->I);
    ...
    if (result)
    {
        ...
        LOG_DEBUG("server.authserver", "'{}:{}' [AuthChallenge] Banned ip tries to login!", socket().getRemoteAddress(), socket().getRemotePort());
    }
    else
    {
        ...
        if (res2)
        {
            ...
            {
                LOG_DEBUG("server.authserver", "[AuthChallenge] Account '{}' is locked to IP - '{}'", _login, fields[3].GetCString());
                LOG_DEBUG("server.authserver", "[AuthChallenge] Player address is '{}'", ip_address);
                ...
                LOG_DEBUG("server.authserver", "[AuthChallenge] Account '{}' is not locked to ip", _login);
            ...
            if (!locked)
            {
                ...
                if (banresult)
                {
                    ...
                    {
                        pkt << uint8(WOW_FAIL_BANNED);
                        LOG_DEBUG("server.authserver", "'{}:{}' [AuthChallenge] Banned account {} tried to login!", socket().getRemoteAddress(), socket().getRemotePort(), _login);
                    }
                    else
                    {
                        pkt << uint8(WOW_FAIL_SUSPENDED);
                        LOG_DEBUG("server.authserver", "'{}:{}' [AuthChallenge] Temporarily banned account {} tried to login!", socket().getRemoteAddress, socket().getRemotePort(), _login);
                    }
                }
                else
                {
                    ...
                    LOG_DEBUG("network.opcode", "database authentication values: v='{}' s='{}'", databaseV, databaseS);
                    ...
                    LOG_DEBUG("server.authserver", "'{}:{}' [AuthChallenge] account {} is using '{}{}{}{}' locale ({})", socket().getRemoteAddress(), socket().getRemotePort(),
                        _login, ch->country[3], ch->country[2], ch->country[1], ch->country[0], GetLocaleByName(_localizationName)
                    );
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (_expversion == NO_VALID_EXP_FLAG)
    {
        ...
        LOG_DEBUG("network.opcode", "Client with invalid version, patching is not implemented");
        socket().shutdown();
        return true;
    }
    ...
    {
        LOG_DEBUG("server.authserver", "'{}:{}' User '{}' successfully authenticated", socket().getRemoteAddress(), socket().getRemotePort(), _login);
    ...
    else
    {
        ...
        LOG_DEBUG("server.authserver", "'{}:{}' [AuthChallenge] account {} tried to login with invalid password!", socket().getRemoteAddress(), socket().getRemotePort(), _login);
                ...
                if (failed_logins >= MaxWrongPassCount)
                {
                    ...
                    else
                    {
                        ...
                        LOG_DEBUG("server.authserver", "'{}:{}' [AuthChallenge] IP {} got banned for '{}' seconds because account {} failed to authenticate '{}' times",
                            socket().getRemoteAddress(), socket().getRemotePort(), socket().getRemoteAddress(), WrongPassBanTime, _login, failed_logins);
                    }
                }
            }
        }
...
{
    LOG_DEBUG("server.authserver", "Entering _HandleReconnectChallenge");
    if (socket().recv_len() < sizeof(sAuthLogonChallenge_C))
        return false;
    ...
    uint16 remaining = ((sAuthLogonChallenge_C*)&buf[0])->size;
    LOG_DEBUG("server.authserver", "[ReconnectChallenge] got header, body is {} bytes", remaining);
    ...
    // Read the remaining of the packet
    socket().recv((char*)&buf[4], remaining);
    LOG_DEBUG("server.authserver", "[ReconnectChallenge] got full packet, {} bytes", ch->size);
    LOG_DEBUG("server.authserver", "[ReconnectChallenge] name({}): '{}'", ch->I_len, ch->I);
    ...
    if (!result)
    {
        LOG_ERROR("server.authserver", "'{}:{}' [ERROR] user {} tried to login and we cannot find his session key in the database.", socket().getRemoteAddress(), socket().getRemotePort(), _login);
        socket().shutdown();
        return false;
    }
...
bool AuthSocket::_HandleReconnectProof()
{
    ...
    {
        LOG_ERROR("server.authserver", "'{}:{}' [ERROR] user {} tried to login, but session is invalid.", socket().getRemoteAddress(), socket().getRemotePort(), _login);
        socket().shutdown();
        return false;
    }
}
...
bool AuthSocket::_HandleRealmList()
{
    ...
    if (!result)
    {
        LOG_ERROR("server.authserver", "'{}:{}' [ERROR] user {} tried to login but we cannot find him in the database.", socket().getRemoteAddress(), socket().getRemotePort(), _login);
        socket().shutdown();
        return false;
    }
...
{
    LOG_DEBUG("server.authserver", "Entering _HandleXferResume");
...
void Patcher::LoadPatchMD5(char* szFileName)
{
    ...
    LOG_DEBUG("network.opcode", "Loading patch info from {}\n", path);

    if (!pPatch)
    {
        LOG_ERROR("server.authserver", "Error loading patch {}\n", path);
        return;
    }
