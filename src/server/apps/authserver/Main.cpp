...
class AuthServerSignalHandler : public Firelands::SignalHandler
{
public:
...
extern int main(int argc, char **argv)
{
    ...
    char const* cfg_file = _FIRELANDS_REALM_CONFIG;
    int c = 1;
    while (c < argc)
    {
    ...
    LOG_INFO("server.authserver", "{} (authserver)", GitRevision::GetFullVersion());
    LOG_INFO("server.authserver", "<Ctrl-C> to stop.\n");
    LOG_INFO("server.authserver", "Using configuration file {}.", cfg_file);

    LOG_WARN("server.authserver", "{} (Library: {})", OPENSSL_VERSION_TEXT, SSLeay_version(SSLEAY_VERSION));
    ...
    LOG_DEBUG("server.authserver", "Max allowed open files is {}", ACE::max_handles());
    ...
    {
        LOG_ERROR("server.authserver", "Auth server can not bind to {}:{}", bind_ip.c_str(), rmport);
        return 1;
    }
...
#ifdef _WIN32
    {
        ...
        if (Aff > 0)
        {
            ...
            if (GetProcessAffinityMask(hProcess, &appAff, &sysAff))
            {
                ...
                if (!curAff)
                    LOG_ERROR("server.authserver", "Processors marked in UseProcessors bitmask (hex) {} not accessible for authserver. Accessible processors bitmask (hex): {}", Aff, appAff);
                else if (SetProcessAffinityMask(hProcess, curAff))
                    LOG_INFO("server.authserver", "Using processors (bitmask, hex): {}", curAff);
                else
                    LOG_ERROR("server.authserver", "Can't set used processors (hex): {}", curAff);
            }
        ...
        if (Prio)
        {
            if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
                LOG_INFO("server.authserver", "The auth server process priority class has been set to HIGH");
            else
                LOG_ERROR("server.authserver", "Can't set auth server process priority class.");
