...
char serviceLongName[] = "FirelandsCata world service";
char serviceDescription[] = "FirelandsCata World of Warcraft emulator world service";
/*
...
variables_map GetConsoleArguments(int argc, char** argv, fs::path& configFile, [[maybe_unused]] std::string& configService)
{
    ...
    all.add_options()
        ...
        ("dry-run,d", "Dry run")
        ("config,c", value<fs::path>(&configFile)->default_value(fs::path(sConfigMgr->GetConfigPath() + std::string(_FIRELANDS_CORE_CONFIG))), "use <arg> as configuration file");
