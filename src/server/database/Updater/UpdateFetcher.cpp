...
UpdateFetcher::UpdateFetcher(
    ...
    : _sourceDirectory(Firelands::make_unique<Path>(sourceDirectory)),
      _apply(apply), _applyFile(applyFile), _retrieve(retrieve) {}
...
void UpdateFetcher::FillFileListRecursively(Path const &path,
  ...
  for (directory_iterator itr(path); itr != end; ++itr) {
    if (is_directory(itr->path())) {
      ...
      LOG_TRACE("sql.updates", "Added locale file \"{}\".",
                itr->path().filename().generic_string().c_str());
  ...
  do {
    ...
    if (!is_directory(p)) {
      LOG_WARN("sql.updates",
               "DBUpdater: Given update include directory \"{}\" does not "
               "exist, skipped!",
    ...
    DirectoryEntry const entry = {
        p, AppliedFileEntry::StateConvert(fields[1].Get<std::string>())};
    directories.push_back(entry);

    LOG_TRACE("sql.updates", "Added applied file \"{}\" from remote.",
...
UpdateFetcher::AppliedFileStorage UpdateFetcher::ReceiveAppliedFiles() const {
  ...
  do {
    ...
    AppliedFileEntry const entry = {
        fields[0].Get<std::string>(), fields[1].Get<std::string>(),
...
std::string
UpdateFetcher::ReadSQLUpdate(boost::filesystem::path const &file) const {
...
UpdateResult
UpdateFetcher::Update(bool const redundancyChecks, bool const allowRehash,
  ...
  for (auto const &availableQuery : available) {
    LOG_DEBUG("sql.updates", "Checking update \"{}\"...",
              availableQuery.first.filename().generic_string().c_str());

    AppliedFileStorage::const_iterator iter =
        applied.find(availableQuery.first.filename().string());
    if (iter != applied.end()) {
      ...
      // If the update is in an archived directory and is marked as archived in
      // our database, skip redundancy checks (archived updates never change).
      if (!archivedRedundancy && (iter->second.state == ARCHIVED) &&
          (availableQuery.second == ARCHIVED)) {
        LOG_DEBUG("sql.updates", ">> Update is archived and marked as archived "
                                 "in database, skipping redundancy checks.");
        applied.erase(iter);
        continue;
      }
    }
    ...
    std::string const hash =
        CalculateSHA1Hash(ReadSQLUpdate(availableQuery.first));
    ...
    if (iter == applied.end()) {
      ...
      if (hashIter != hashToName.end()) {
        ...
        for (localeIter = available.begin();
             ...
             (localeIter->first.filename().string() != hashIter->second);
             ++localeIter)
          ;
        ...
        if (localeIter != available.end()) {
          LOG_WARN("sql.updates",
                   ">> It seems like the update \"{}\" \'{}\' was renamed, but "
                   "the old file is still there! "
                   "Treating it as a new file! (It is probably an unmodified "
                   "copy of the file \"{}\")",
        ...
        // It is safe to treat the file as renamed here
          ...
          LOG_INFO("sql.updates", ">> Renaming update \"{}\" to \"{}\" \'{}\'.",
                   hashIter->second.c_str(),
                   availableQuery.first.filename().string().c_str(),
                   hash.substr(0, 7).c_str());

          RenameEntry(hashIter->second,
                      availableQuery.first.filename().string());
    ...
    // Rehash the update entry if it exists in our database with an empty hash.
      ...
      LOG_INFO("sql.updates", ">> Re-hashing update \"{}\" \'{}\'...",
               availableQuery.first.filename().string().c_str(),
               hash.substr(0, 7).c_str());
    } else {
      // If the hash of the files differs from the one stored in our database,
      // reapply the update (because it changed).
      if (iter->second.hash != hash) {
        LOG_INFO("sql.updates",
                 ">> Reapplying update \"{}\" \'{}\' -> \'{}\' (it changed)...",
                 availableQuery.first.filename().string().c_str(),
                 iter->second.hash.substr(0, 7).c_str(),
                 hash.substr(0, 7).c_str());
      } else {
        // If the file wasn't changed and just moved, update its state (if
        // necessary).
        if (iter->second.state != availableQuery.second) {
          LOG_DEBUG(
              "sql.updates", ">> Updating the state of \"{}\" to \'{}\'...",
              availableQuery.first.filename().string().c_str(),
              AppliedFileEntry::StateConvert(availableQuery.second).c_str());

          UpdateState(availableQuery.first.filename().string(),
                      availableQuery.second);
        }
    ...
    uint32 speed = 0;
    AppliedFileEntry const file = {availableQuery.first.filename().string(),
  ...
  if (!applied.empty()) {
    ...
    for (auto const &entry : applied) {
      ...
      if (doCleanup)
        LOG_INFO("sql.updates", "Deleting orphaned entry \'{}\'...",
                 entry.first.c_str());
    }
    ...
    if (doCleanup)
      ...
      LOG_ERROR("sql.updates",
                "Cleanup is disabled! There were  " SZFMTD
                " dirty files applied to your database, "
                "but they are now missing in your source directory!",
                applied.size());
    }
  }

  return UpdateResult(importedUpdates, countRecentUpdates,
                      countArchivedUpdates);
}
...
uint32 UpdateFetcher::Apply(Path const &path) const {
  ...
  return uint32(
      std::chrono::duration_cast<std::chrono::milliseconds>(Time::now() - begin)
          .count());
}
...
void UpdateFetcher::UpdateEntry(AppliedFileEntry const &entry,
  ...
  std::string const update =
      ...
      entry.name + "\", \"" + entry.hash + "\", \'" + entry.GetStateAsString() +
      "\', " + std::to_string(speed) + ")";
...
bool UpdateFetcher::PathCompare::operator()(
    LocaleFileEntry const &left, LocaleFileEntry const &right) const {
