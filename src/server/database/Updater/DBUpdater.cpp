...
bool DBUpdaterUtil::CheckExecutable() {
  ...
  if (!is_regular_file(exe)) {
    ...
    LOG_FATAL("sql.updates",
              "Didn't find any executable MySQL binary at \'{}\' or in path, "
              "correct the path in the *.conf (\"MySQLExecutable\").",
...
template <> std::string DBUpdater<LoginDatabaseConnection>::GetBaseFile() {
  return BuiltInConfig::GetSourceDirectory() +
         "/data/sql/base/auth_database.sql";
}
...
template <> std::string DBUpdater<WorldDatabaseConnection>::GetBaseFile() {
...
template <> std::string DBUpdater<CharacterDatabaseConnection>::GetBaseFile() {
  return BuiltInConfig::GetSourceDirectory() +
         "/data/sql/base/characters_database.sql";
}

template <>
bool DBUpdater<CharacterDatabaseConnection>::IsEnabled(
...
template <> std::string DBUpdater<HotfixDatabaseConnection>::GetBaseFile() {
...
template <class T> bool DBUpdater<T>::Create(DatabaseWorkerPool<T> &pool) {
  LOG_INFO("sql.updates",
           "Database \"{}\" does not exist, do you want to create it? [yes "
           "(default) / no]: ",
  ...
  LOG_INFO("sql.updates", "Creating database \"{}\"...",
           pool.GetConnectionInfo()->database.c_str());
  ...
  if (!file.is_open()) {
    LOG_FATAL("sql.updates", "Failed to create temporary query file \"{}\"!",
  ...
  try {
    DBUpdater<T>::ApplyFile(pool, pool.GetConnectionInfo()->host,
                            pool.GetConnectionInfo()->user,
                            pool.GetConnectionInfo()->password,
                            pool.GetConnectionInfo()->port_or_socket, "", temp);
  } catch (UpdateException &) {
    LOG_FATAL("sql.updates",
              "Failed to create database {}! Does the user (named in *.conf) "
              "have `CREATE` privileges on the MySQL server?",
              pool.GetConnectionInfo()->database.c_str());
...
template <class T> bool DBUpdater<T>::Update(DatabaseWorkerPool<T> &pool) {
  if (!DBUpdaterUtil::CheckExecutable())
    return false;

  LOG_INFO("sql.updates", "Updating {} database...",
           DBUpdater<T>::GetTableName().c_str());

  Path const sourceDirectory(BuiltInConfig::GetSourceDirectory());

  if (!is_directory(sourceDirectory)) {
    LOG_ERROR("sql.updates",
              "DBUpdater: Given source directory {} does not exist, skipped!",
  ...
  std::string const info = Firelands::StringFormat(
      "Containing " SZFMTD " new and " SZFMTD " archived updates.",
      result.recent, result.archived);

  if (!result.updated)
    LOG_INFO("sql.updates", ">> {} database is up-to-date! {}",
             DBUpdater<T>::GetTableName().c_str(), info.c_str());
  else
    LOG_INFO("sql.updates", ">> Applied " SZFMTD " {}. {}", result.updated,
             result.updated == 1 ? "query" : "queries", info.c_str());
...
template <class T> bool DBUpdater<T>::Populate(DatabaseWorkerPool<T> &pool) {
  ...
  LOG_INFO("sql.updates", "Database {} is empty, auto populating it...",
           DBUpdater<T>::GetTableName().c_str());
  ...
  if (!exists(base)) {
    ...
    case LOCATION_REPOSITORY: {
      LOG_ERROR(
          "sql.updates",
          ">> Base file \"{}\" is missing, try to clone the source again.",
    ...
    case LOCATION_DOWNLOAD: {
      ...
      LOG_ERROR(
          "sql.updates",
          ">> File \"{}\" is missing, download it from "
          "\"https://github.com/FirelandsProject/Firelands-Core/releases\""
          " uncompress it and place the file \"{}\" in the directory \"{}\".",
...
void DBUpdater<T>::ApplyFile(DatabaseWorkerPool<T> &pool, Path const &path) {
  DBUpdater<T>::ApplyFile(pool, pool.GetConnectionInfo()->host,
                          ...
                          pool.GetConnectionInfo()->database, path);
}

template <class T>
void DBUpdater<T>::ApplyFile(DatabaseWorkerPool<T> &pool,
                             std::string const &host, std::string const &user,
                             std::string const &password,
                             std::string const &port_or_socket,
                             std::string const &database, Path const &path) {
  ...
  int const ret =
      Firelands::StartProcess(DBUpdaterUtil::GetCorrectedMySQLExecutable(),
                              args, "sql.updates", path.generic_string(), true);

  if (ret != EXIT_SUCCESS) {
    LOG_FATAL("sql.updates",
              "Applying of file \'{}\' to database \'{}\' failed!"
              " If you are a user, pull the latest revision from the "
              "repository. If you are a developer, fix your sql query.",
