...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool, std::string_view modulesList /*= {}*/)
{
    ...
    {
        auto checkTable = DBUpdater<T>::Retrieve(pool, Firelands::StringFormatFmt("SHOW TABLES LIKE '{}'", tableName));
        if (!checkTable)
        {
    ...
    std::string const info = Firelands::StringFormatFmt("Containing {} new and {} archived updates.", result.recent, result.archived);
