...
bool TransactionTask::Execute() {
  ...
  if (errorCode == ER_LOCK_DEADLOCK) {
    ...
    // Make sure only 1 async thread retries a transaction so they don't keep
    // dead-locking each other
    std::lock_guard<std::mutex> lock(_deadlockLock);

    for (uint32 loopDuration = 0, startMSTime = getMSTime();
      ...
      LOG_WARN("sql.sql",
               "Deadlocked SQL Transaction, retrying. Loop timer: {} ms, "
               "Thread Id: {}",
               loopDuration, threadId.c_str());
    }
