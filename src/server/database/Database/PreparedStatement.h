...
public:
    ...
    template<typename T>
    inline Firelands::Types::is_default<T> SetData(const uint8 index, T value)
    {
    ...
    // Set enums
    template<typename T>
    inline Firelands::Types::is_enum_v<T> SetData(const uint8 index, T value)
    {
...
protected:
    template<typename T>
    Firelands::Types::is_non_string_view_v<T> SetValidData(const uint8 index, T const& value);
