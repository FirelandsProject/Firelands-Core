...
#ifdef FIRELANDS_STRICT_DATABASE_TYPE_CHECKS
    if (IsNumeric() && data.raw)
    {
...
T Field::GetData() const
{
    ...
    if (data.raw)
        ...
        result = Firelands::StringTo<T>(data.value);
    ...
    if constexpr (std::is_same_v<T, double>)
    {
        if (data.raw && !IsType(DatabaseFieldTypes::Decimal))
            ...
            result = Firelands::StringTo<float>(data.value);
    }
    ...
    if constexpr (std::is_same_v<T, uint32>)
    {
        ...
        {
            auto signedResult = Firelands::StringTo<int32>(data.value);
