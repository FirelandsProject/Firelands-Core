...
DatabaseWorkerPool<T>::DatabaseWorkerPool()
  ...
  WPFatal(mysql_get_client_version() >= MIN_MYSQL_CLIENT_VERSION,
          "Firelands Core does not support MySQL versions below 5.1");
  WPFatal(mysql_get_client_version() == MYSQL_VERSION_ID,
          "Used MySQL library version ({} id {}) does not match the version id "
          "used to compile Firelands Core (id {}).",
...
template <class T> uint32 DatabaseWorkerPool<T>::Open() {
  ...
  if (!error) {
    LOG_INFO("sql.driver",
             "DatabasePool '{}' opened successfully. " SZFMTD
  ...
  for (auto &connections : _connections) {
    for (auto &connection : connections) {
      ...
      for (size_t i = 0; i < preparedSize; ++i) {
        ...
        // (each connection only has prepared statements of it's own type
        // sync/async)
...
void DatabaseWorkerPool<T>::CommitTransaction(SQLTransaction<T> transaction) {
  ...
  case 0:
    ...
    return;
  case 1:
    LOG_DEBUG("sql.driver", "Warning: Transaction only holds 1 query, consider "
...
void DatabaseWorkerPool<T>::DirectCommitTransaction(
  ...
  if (errorCode == ER_LOCK_DEADLOCK) {
    // todo: handle multiple sync threads deadlocking in a similar way as async
    // threads
    uint8 loopBreaker = 5;
...
template <class T>
unsigned long DatabaseWorkerPool<T>::EscapeString(char *to, char const *from,
...
template <class T> char const *DatabaseWorkerPool<T>::GetDatabaseName() const {
  return _connectionInfo->database.c_str();
}

template <class T> void DatabaseWorkerPool<T>::Execute(char const *sql) {
...
template <class T> void DatabaseWorkerPool<T>::DirectExecute(char const *sql) {
  if (Firelands::IsFormatEmptyOrNull(sql))
    return;
