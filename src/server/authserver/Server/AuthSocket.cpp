...
{
    LOG_DEBUG("server.authserver", "'%s:%d' Accepting connection", socket().getRemoteAddress().c_str(), socket().getRemotePort());
}
...
void AuthSocket::OnRead()
{
    ...
    while (1)
    {
        ...
        if (_cmd == 16) // REALM_LIST
        {
            ...
            {
                LOG_WARN("server.authserver", "Got %u REALM_LIST in a row from '%s', possible ongoing DoS", challengesInARow, socket().getRemoteAddress().c_str());
                socket().shutdown();
        ...
        if (_cmd == AUTH_LOGON_CHALLENGE)
        {
            ...
            {
                LOG_WARN("server.authserver", "Got %u AUTH_LOGON_CHALLENGE in a row from '%s', possible ongoing DoS", challengesInARow, socket().getRemoteAddress().c_str());
                socket().shutdown();
                return;
            }
        }
        ...
        for (i = 0; i < AUTH_TOTAL_COMMANDS; ++i)
        {
            ...
            {
                LOG_DEBUG("server.authserver", "Received unauthorized command %u recv length %u", (uint32)_cmd, (uint32)socket().recv_len());
                return;
            }

            LOG_DEBUG("server.authserver", "Received command %u, length %u", (uint32)_cmd, (uint32)socket().recv_len());
        ...
        {
            LOG_ERROR("server.authserver", "Got unknown command %u", (uint32)_cmd);
            socket().shutdown();
            return;
        }
    }
        ...
        if (res2)
        {
            ...
            if (!locked)
            {
                ...
                else
                {
                    ...
                    LOG_DEBUG("network.opcode", "database authentication values: v='%s' s='%s'", databaseV.c_str(), databaseS.c_str());
                    ...
                    LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] account %s is using '%c%c%c%c' locale (%u)", socket().getRemoteAddress().c_str(), socket().getRemotePort(),
...
void Patcher::LoadPatchMD5(char *szFileName)
{
    ...
    LOG_DEBUG("network.opcode", "Loading patch info from %s\n", path.c_str());
