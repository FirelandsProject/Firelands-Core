...
bool StartDB();
void StopDB();
void SignalHandler(std::weak_ptr<Firelands::Asio::IoContext> ioContextRef,
                   boost::system::error_code const &error, int signalNumber);
void KeepDatabaseAliveHandler(
    std::weak_ptr<Firelands::Asio::DeadlineTimer> dbPingTimerRef,
    int32 dbPingInterval, boost::system::error_code const &error);
void BanExpiryHandler(
    std::weak_ptr<Firelands::Asio::DeadlineTimer> banExpiryCheckTimerRef,
    int32 banExpiryCheckInterval, boost::system::error_code const &error);
...
int main(int argc, char **argv) {
  ...
  auto configFile = fs::path(sConfigMgr->GetConfigPath() +
                             std::string(_FIRELANDS_REALM_CONFIG));
  ...
  Firelands::Banner::Show(
      "authserver",
      [](std::string text) { LOG_INFO("server.authserver", text); },
      []() {
        LOG_INFO("server.authserver", "> Using configuration file       {}",
  ...
  signals.async_wait(std::bind(
      &SignalHandler, std::weak_ptr<Firelands::Asio::IoContext>(ioContext),
  ...
  std::shared_ptr<Firelands::Asio::DeadlineTimer> dbPingTimer =
  ...
  dbPingTimer->async_wait(
      std::bind(&KeepDatabaseAliveHandler,
                std::weak_ptr<Firelands::Asio::DeadlineTimer>(dbPingTimer),
  ...
  banExpiryCheckTimer->async_wait(std::bind(
      &BanExpiryHandler,
      std::weak_ptr<Firelands::Asio::DeadlineTimer>(banExpiryCheckTimer),
...
void SignalHandler(std::weak_ptr<Firelands::Asio::IoContext> ioContextRef,
                   ...
                   int /*signalNumber*/) {
  if (!error) {
    if (std::shared_ptr<Firelands::Asio::IoContext> ioContext =
            ioContextRef.lock()) {
      ioContext->stop();
    }
  }
...
void KeepDatabaseAliveHandler(
    ...
    int32 dbPingInterval, boost::system::error_code const &error) {
  if (!error) {
    if (std::shared_ptr<Firelands::Asio::DeadlineTimer> dbPingTimer =
            dbPingTimerRef.lock()) {
...
void BanExpiryHandler(
    ...
    int32 banExpiryCheckInterval, boost::system::error_code const &error) {
  if (!error) {
    if (std::shared_ptr<Firelands::Asio::DeadlineTimer> banExpiryCheckTimer =
      ...
      banExpiryCheckTimer->async_wait(
          std::bind(&BanExpiryHandler, banExpiryCheckTimerRef,
                    banExpiryCheckInterval, std::placeholders::_1));
    }
  }
}
