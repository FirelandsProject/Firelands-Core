...
bool LoadRealmInfo(Firelands::Asio::IoContext& ioContext)
{
    ...
    if (!externalAddress)
    {
        LOG_ERROR("server.worldserver", "Could not resolve address %s", fields[2].Get<std::string>().c_str());
        return false;
    }
    ...
    if (!localAddress)
    {
        LOG_ERROR("server.worldserver", "Could not resolve address %s", fields[3].Get<std::string>().c_str());
        return false;
    }
    ...
    if (!localSubmask)
    {
        LOG_ERROR("server.worldserver", "Could not resolve address %s", fields[4].Get<std::string>().c_str());
        return false;
    }
...
bool StartDB()
{
    ...
    realm.Id.Region = (*realmIdQuery)[0].Get<uint8>();
    realm.Id.Site = (*realmIdQuery)[1].Get<uint8>();
