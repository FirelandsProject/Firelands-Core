...
bool RASession::CheckPassword(const std::string &user,
  ...
  std::transform(safe_user.begin(), safe_user.end(), safe_user.begin(),
                 ::toupper);
  Utf8ToUpperOnlyLatin(safe_user);

  std::string safe_pass = pass;
  Utf8ToUpperOnlyLatin(safe_pass);
  std::transform(safe_pass.begin(), safe_pass.end(), safe_pass.begin(),
                 ::toupper);
  ...
  if (PreparedQueryResult result = LoginDatabase.Query(stmt)) {
    Firelands::Crypto::SRP6::Salt salt =
        (*result)[0].Get<Binary, Firelands::Crypto::SRP6::SALT_LENGTH>();
    Firelands::Crypto::SRP6::Verifier verifier =
        (*result)[1].Get<Binary, Firelands::Crypto::SRP6::VERIFIER_LENGTH>();

    if (Firelands::Crypto::SRP6::CheckLogin(safe_user, safe_pass, salt,
                                            verifier))
      return true;
  }
...
bool RASession::ProcessCommand(std::string &command) {
  ...
  CliCommandHolder *cmd =
      new CliCommandHolder(this, command.c_str(), &RASession::CommandPrint,
