    ...
    bool Load(char const* fn, SqlDbc* sql, bool force)
    {
        ...
        if (sql)
        {
            ...
            if (result)
            {
                ...
                {
                    LOG_ERROR("server.loading", "Invalid index pos for dbc:'{}'", sql->sqlTableName);
                    return false;
                }
            }
        }
            ...
            if (indexTable.asT)
            {
                ...
                do
                {
                    ...
                    if (sql->indexPos >= 0)
                    {
                        ...
                        {
                            LOG_ERROR("server.loading", "Index {} already exists in dbc:'{}'", id, sql->sqlTableName);
                            return false;
                        }
                    ...
                    for (; columnNumber < sql->formatString->size(); ++columnNumber)
                    {
                        ...
                        {
                            LOG_ERROR("server.loading", "Incorrect sql format string '{}' at char {}", sql->sqlTableName, columnNumber);
                            return false;
                        }
                    }
                    ...
                    {
                        LOG_ERROR("server.loading", "SQL and DBC format strings are not matching for table: '{}'", sql->sqlTableName);
                        return false;
                    }
