...
static void RegeneratePool(QuestPool &pool) {
  ...
  ASSERT(pool.numActive <= pool.members.size(),
         "Quest Pool %u requests %u spawns, but has only %u members.",
         pool.poolId, pool.numActive, uint32(pool.members.size()));
...
static void SaveToDB(QuestPool const &pool,
  ...
  delStmt->SetData(0, pool.poolId);
  trans->Append(delStmt);

  for (uint32 questId : pool.activeQuests) {
    CharacterDatabasePreparedStatement *insStmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_POOL_QUEST_SAVE);
    insStmt->SetData(0, pool.poolId);
    insStmt->SetData(1, questId);
...
void QuestPoolMgr::LoadFromDB() {
  ...
  std::unordered_map<uint32, std::pair<std::vector<QuestPool> *, uint32>>
      lookup; // poolId -> (vector, index)
  ...
  // load template data from world DB
  {
    QueryResult result = WorldDatabase.Query(
        "SELECT qpm.questId, qpm.poolId, qpm.poolIndex, qpt.numActive FROM "
        "quest_pool_members qpm LEFT JOIN quest_pool_template qpt ON "
        "qpm.poolId = qpt.poolId");
    if (!result) {
      LOG_INFO(
          "server.loading",
    ...
    do {
      ...
      if (fields[2].IsNull()) {
        LOG_ERROR("sql.sql",
                  "Table `quest_pool_members` contains reference to "
                  "non-existing pool %u. Skipped.",
                  fields[1].Get<uint32>());
        continue;
      }
      ...
      if (!quest) {
        LOG_ERROR("sql.sql",
                  "Table `quest_pool_members` contains reference to "
                  "non-existing quest %u. Skipped.",
                  questId);
        continue;
      }
      if (!quest->IsDailyOrWeekly() && !quest->IsMonthly()) {
        LOG_ERROR("sql.sql",
                  "Table `quest_pool_members` contains reference to quest %u, "
      ...
      if (!pair.first) {
        pair.first = (quest->IsDaily()    ? &_dailyPools
                      : quest->IsWeekly() ? &_weeklyPools
                                          : &_monthlyPools);
  ...
  // load saved spawns from character DB
  {
    QueryResult result = CharacterDatabase.Query(
        "SELECT pool_id, quest_id FROM pool_quest_save");
    if (result) {
      ...
      do {
        ...
        if (it == lookup.end() || !it->second.first) {
          LOG_ERROR("sql.sql",
                    "Table `pool_quest_save` contains reference to "
                    "non-existant quest pool %u. Deleted.",
                    poolId);
  ...
  for (auto pair : lookup) {
    ...
    if (pool.members.size() < pool.numActive) {
      LOG_ERROR(
          "sql.sql",
          "Table `quest_pool_template` contains quest pool %u requesting %u "
          "spawns, but only has %zu members. Requested spawns reduced.",
          pool.poolId, pool.numActive, pool.members.size());
    ...
    if (!doRegenerate) {
      ...
      for (size_t i = pool.members.size(); (i--);) {
        ...
        if (member.empty()) {
          LOG_ERROR("sql.sql",
                    "Table `quest_pool_members` contains no entries at index "
                    "%zu for quest pool %u. Index removed.",
                    i, pool.poolId);
        ...
        // now check if all other members also have the same status, and warn if
        // not
        for (auto it = member.begin(); (++it) != member.end();) {
          ...
          if (status != otherStatus)
            LOG_WARN("sql.sql",
                     "Table `pool_quest_save` %s quest %u (in pool %u, index "
                     "%zu) saved, but its index is%s active (because quest %u "
                     "is%s in the table). Set quest %u to %sactive.",
                     (status ? "does not have" : "has"), *it, pool.poolId, i,
                     (status ? "" : " not"), member[0], (status ? "" : " not"),
                     *it, (status ? "" : "in"));
          if (otherStatus)
      ...
      for (uint32 quest : pool.activeQuests)
        LOG_WARN("sql.sql",
                 "Table `pool_quest_save` has saved quest %u for pool %u, but "
                 "that quest is not part of the pool. Skipped.",
                 quest, pool.poolId);
      ...
      if (activeCount != pool.numActive) {
        ...
        LOG_ERROR(
            "sql.sql",
            "Table `pool_quest_save` has %u active members saved for pool %u, "
            "which requests %u active members. Pool spawns re-generated.",
            activeCount, pool.poolId, pool.numActive);
      }
    }
    ...
    for (QuestPool::Member const &member : pool.members) {
      for (uint32 quest : member) {
        ...
        if (ref) {
          LOG_ERROR("sql.sql",
                    "Table `quest_pool_members` lists quest %u as member of "
                    "pool %u, but it is already a member of pool %u. Skipped.",
                    quest, pool.poolId, ref->poolId);
          continue;
        }
        ref = &pool;
      }
  ...
  LOG_INFO(
      "server.loading",
      ">> Loaded %zu daily, %zu weekly and %zu monthly quest pools in %u ms",
      _dailyPools.size(), _weeklyPools.size(), _monthlyPools.size(),
...
bool QuestPoolMgr::IsQuestActive(uint32 questId) const {
  ...
  return (it->second->activeQuests.find(questId) !=
          it->second->activeQuests.end());
}
