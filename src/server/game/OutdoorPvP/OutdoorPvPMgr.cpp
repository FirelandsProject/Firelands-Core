...
void OutdoorPvPMgr::InitOutdoorPvP()
{
    ...
    do
    {
        ...
        data->ScriptId = sObjectMgr->GetScriptId(fields[1].Get<cstring>());
    ...
    for (uint8 i = 1; i < MAX_OUTDOORPVP_TYPES; ++i)
    {
        ...
        if (!pvp)
        {
            LOG_ERROR("outdoorpvp", "Could not initialize OutdoorPvP object for type ID {}; got NULL pointer from script.", uint32(i));
            continue;
        }
...
void OutdoorPvPMgr::HandlePlayerEnterZone(Player* player, uint32 zoneid)
{
    ...
    LOG_DEBUG("outdoorpvp", "Player {} entered outdoorpvp id {}", player->GetGUIDLow(), itr->second->GetTypeId());
}
...
void OutdoorPvPMgr::HandlePlayerLeaveZone(Player* player, uint32 zoneid)
{
    ...
    LOG_DEBUG("outdoorpvp", "Player {} left outdoorpvp id {}", player->GetGUIDLow(), itr->second->GetTypeId());
}
