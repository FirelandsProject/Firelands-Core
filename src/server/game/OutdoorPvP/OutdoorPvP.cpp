...
public:
  DefenseMessageBuilder(uint32 zoneId, uint32 id) : _zoneId(zoneId), _id(id) {}
...
OPvPCapturePoint::OPvPCapturePoint(OutdoorPvP *pvp)
    : m_capturePointSpawnId(0), m_capturePoint(nullptr), m_maxValue(0.0f),
...
bool OPvPCapturePoint::AddObject(uint32 type, uint32 entry, uint32 map,
                                 Position const &pos,
                                 QuaternionData const &rot) {
  if (ObjectGuid::LowType guid =
          sObjectMgr->AddGameObjectData(entry, map, pos, rot, 0)) {
...
bool OPvPCapturePoint::AddCreature(uint32 type, uint32 entry, uint32 map,
  ...
  if (ObjectGuid::LowType guid =
          sObjectMgr->AddCreatureData(entry, map, pos, spawntimedelay)) {
...
bool OPvPCapturePoint::SetCapturePointData(uint32 entry, uint32 map,
                                           Position const &pos,
                                           QuaternionData const &rot) {
...
void OPvPCapturePoint::DeleteSpawns() {
  for (std::map<uint32, ObjectGuid::LowType>::iterator i = m_Objects.begin();
       i != m_Objects.end(); ++i)
    DelObject(i->first);
  for (std::map<uint32, ObjectGuid::LowType>::iterator i = m_Creatures.begin();
       i != m_Creatures.end(); ++i)
    DelCreature(i->first);
  DelCapturePoint();
}
...
void OutdoorPvP::DeleteSpawns() {
  ...
  for (auto itr = m_GoScriptStore.begin(); itr != m_GoScriptStore.end();
       ++itr) {
  ...
  for (auto itr = m_CreatureScriptStore.begin();
       itr != m_CreatureScriptStore.end(); ++itr) {
...
bool OPvPCapturePoint::Update(uint32 diff) {
  ...
  Firelands::PlayerListSearcher<Firelands::AnyPlayerInObjectRangeCheck>
      searcher(m_capturePoint, players, checker);
...
bool OutdoorPvP::HandleGossipOption(Player *player, Creature *creature,
                                    uint32 id) {
  for (OPvPCapturePointMap::iterator itr = m_capturePoints.begin();
       itr != m_capturePoints.end(); ++itr)
...
int32 OPvPCapturePoint::HandleOpenGo(Player * /*player*/, GameObject *go) {
  std::map<ObjectGuid::LowType, uint32>::iterator itr =
      m_ObjectTypes.find(go->GetSpawnId());
