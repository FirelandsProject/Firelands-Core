...
bool AuctionBotBuyer::Initialize() {
  ...
  // load Check interval
  _checkInterval =
      sAuctionBotConfig->GetConfig(CONFIG_AHBOT_BUYER_RECHECK_INTERVAL) *
      MINUTE;
  LOG_DEBUG("ahbot", "AHBot buyer interval is {} minutes",
            _checkInterval / MINUTE);
  return true;
}

void AuctionBotBuyer::LoadConfig() {
  for (int i = 0; i < MAX_AUCTION_HOUSE_TYPE; ++i) {
    _houseConfig[i].BuyerEnabled =
        sAuctionBotConfig->GetConfigBuyerEnabled(AuctionHouseType(i));
    if (_houseConfig[i].BuyerEnabled)
...
// Collects information about item counts and minimum prices to SameItemInfo and
// updates EligibleItems - a list with new items eligible for bot to buy and bid
// Returns count of items in AH that were eligible for being bought or bidded on
// by ahbot buyer (EligibleItems size)
uint32 AuctionBotBuyer::GetItemInformation(BuyerConfiguration &config) {
  ...
  for (AuctionHouseObject::AuctionEntryMap::const_iterator itr =
           house->GetAuctionsBegin();
       itr != house->GetAuctionsEnd(); ++itr) {
  ...
  LOG_DEBUG("ahbot",
            "AHBot: {} items added to buyable/biddable vector for ah type: {}",
            count, config.GetHouseType());
  LOG_DEBUG("ahbot", "AHBot: SameItemInfo size = {}",
            (uint32)config.SameItemInfo.size());
  return count;
}
...
bool AuctionBotBuyer::RollBuyChance(BuyerItemInfo const *ahInfo,
  ...
  float itemPrice =
      float(item->GetTemplate()->GetSellPrice()
                ? item->GetTemplate()->GetSellPrice()
                : GetVendorPrice(item->GetTemplate()->GetQuality()));
  // The AH cut needs to be added to the price, but we dont want a 100% chance
  // to buy if the price is exactly AH default
  itemPrice *= 1.4f;

  // This value is between 0 and 100 and is used directly as the chance to buy
  // or bid Value equal or above 100 means 100% chance and value below 0 means
  // 0% chance
  float chance = std::min(
      100.f, std::pow(100.f, 1.f + (1.f - itemBuyPrice / itemPrice) /
  ...
  if (ahInfo) {
    ...
    LOG_DEBUG("ahbot", "AHBot: buyout average: {} items with buyout: {}",
              avgBuyPrice, ahInfo->BuyItemCount);

    // If there are more than 5 items on AH of this entry, try weigh in the
    // average buyout price
  ...
  LOG_DEBUG("ahbot", "AHBot: {} BUY! chance = {}, price = {}, buyprice = {}.",
            win ? "WIN" : "LOSE", chance, uint32(itemPrice),
            uint32(itemBuyPrice));
  return win;
}
...
bool AuctionBotBuyer::RollBidChance(BuyerItemInfo const *ahInfo,
  ...
  // 0% chance
  float chance = std::min(
      100.f, std::pow(100.f, 1.f + (1.f - itemBidPrice / itemPrice) /
                                       sAuctionBotConfig->GetConfig(
                                           CONFIG_AHBOT_BUYER_CHANCE_FACTOR)));
  ...
  if (ahInfo) {
    ...
    LOG_DEBUG("ahbot", "AHBot: Bid average: {} biddable item count: {}",
              avgBidPrice, ahInfo->BidItemCount);
  ...
  float rand = frand(0.f, 100.f);
  bool win = rand <= chance;
  LOG_DEBUG("ahbot", "AHBot: {} BID! chance = {}, price = {}, bidprice = {}.",
            win ? "WIN" : "LOSE", chance, uint32(itemPrice),
            uint32(itemBidPrice));
  return win;
}
...
void AuctionBotBuyer::PrepareListOfEntry(BuyerConfiguration &config) {
  // now - 5 seconds to leave out all old entries but keep the ones just updated
  // a moment ago

  for (CheckEntryMap::iterator itr = config.EligibleItems.begin();
       itr != config.EligibleItems.end();) {
  ...
  LOG_DEBUG("ahbot", "AHBot: EligibleItems size = {}",
            (uint32)config.EligibleItems.size());
}
...
void AuctionBotBuyer::BuyAndBidItems(BuyerConfiguration &config) {
  ...
  if (items.size() > sAuctionBotConfig->GetItemPerCycleBoost()) {
    ...
    LOG_DEBUG("ahbot", "AHBot: Boost value used for Buyer! (if this happens "
  ...
  while (cycles && itr != items.end()) {
    ...
    if (!auction) {
      LOG_DEBUG("ahbot",
                "AHBot: Entry {} doesn't exists, perhaps bought already?",
    ...
    if (itr->second.LastChecked &&
        (GameTime::GetGameTime() - itr->second.LastChecked) <= _checkInterval) {
      LOG_DEBUG("ahbot", "AHBot: In time interval wait for entry {}!",
                auction->Id);
      ++itr;
      continue;
    }
    ...
    BuyerItemInfoMap::const_iterator sameItemItr =
        config.SameItemInfo.find(item->GetEntry());
    ...
    // - Better bid than buy since the item is bought by bot if no player bids
    // after Otherwise bid if roll for bid was successful
...
uint32 AuctionBotBuyer::GetChanceMultiplier(uint32 quality) {
  switch (quality) {
  case ITEM_QUALITY_POOR:
    return sAuctionBotConfig->GetConfig(
...
void AuctionBotBuyer::BuyEntry(AuctionEntry *auction,
                               AuctionHouseObject *auctionHouse) {
  LOG_DEBUG("ahbot", "AHBot: Entry {} bought at {}g", auction->Id,
            float(auction->buyout) / GOLD);
...
void AuctionBotBuyer::PlaceBidToEntry(AuctionEntry *auction, uint32 bidPrice) {
  LOG_DEBUG("ahbot", "AHBot: Bid placed to entry {}, {}g", auction->Id,
            float(bidPrice) / GOLD);
