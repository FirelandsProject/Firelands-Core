...
uint32 AuctionBotBuyer::GetItemInformation(BuyerConfiguration &config)
{
    ...
    LOG_DEBUG("ahbot", "AHBot: SameItemInfo size = {}", (uint32)config.SameItemInfo.size());
    return count;
}
...
bool AuctionBotBuyer::RollBuyChance(BuyerItemInfo const *ahInfo, Item const *item, AuctionEntry const *auction, uint32 /*bidPrice*/)
{
    ...
    if (ahInfo)
    {
        ...
        LOG_DEBUG("ahbot", "AHBot: buyout average: {} items with buyout: {}", avgBuyPrice, ahInfo->BuyItemCount);
    ...
    LOG_DEBUG("ahbot", "AHBot: {} BUY! chance = {}, price = {}, buyprice = {}.", win ? "WIN" : "LOSE", chance, uint32(itemPrice), uint32(itemBuyPrice));
    return win;
}
...
bool AuctionBotBuyer::RollBidChance(BuyerItemInfo const *ahInfo, Item const *item, AuctionEntry const *auction, uint32 bidPrice)
{
    ...
    if (ahInfo)
    {
        ...
        LOG_DEBUG("ahbot", "AHBot: Bid average: {} biddable item count: {}", avgBidPrice, ahInfo->BidItemCount);
    ...
    float rand = frand(0.f, 100.f);
    bool win = rand <= chance;
    LOG_DEBUG("ahbot", "AHBot: {} BID! chance = {}, price = {}, bidprice = {}.", win ? "WIN" : "LOSE", chance, uint32(itemPrice), uint32(itemBidPrice));
    return win;
}
...
void AuctionBotBuyer::PrepareListOfEntry(BuyerConfiguration &config)
{
    ...
    LOG_DEBUG("ahbot", "AHBot: EligibleItems size = {}", (uint32)config.EligibleItems.size());
}
...
void AuctionBotBuyer::BuyAndBidItems(BuyerConfiguration &config)
{
    ...
    while (cycles && itr != items.end())
    {
        ...
        if ((auction->buyout && successBid && bidPrice >= auction->buyout) || (successBuy && (!successBid || urand(1, 5) == 1)))
