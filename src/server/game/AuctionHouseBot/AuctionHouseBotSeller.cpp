...
bool AuctionBotSeller::Initialize() {
  ...
  LOG_DEBUG("ahbot", "Forced Inclusion {} items", (uint32)includeItems.size());
  LOG_DEBUG("ahbot", "Forced Exclusion {} items", (uint32)excludeItems.size());

  LOG_DEBUG("ahbot", "Loading npc vendor items for filter..");
  CreatureTemplateContainer const *creatures =
      sObjectMgr->GetCreatureTemplates();
  for (CreatureTemplateContainer::const_iterator it = creatures->begin();
       it != creatures->end(); ++it)
    if (VendorItemData const *data =
            sObjectMgr->GetNpcVendorItemList(it->first))
      for (VendorItem const &it2 : data->m_items)
        npcItems.insert(it2.item);

  LOG_DEBUG("ahbot", "Npc vendor filter has {} items", (uint32)npcItems.size());

  LOG_DEBUG("ahbot", "Loading loot items for filter..");
  QueryResult result = WorldDatabase.PQuery(
      "SELECT `item` FROM `creature_loot_template` WHERE `Reference` = 0 UNION "
      "SELECT `item` FROM `disenchant_loot_template` WHERE `Reference` = 0 "
      "UNION "
      "SELECT `item` FROM `fishing_loot_template` WHERE `Reference` = 0 UNION "
      "SELECT `item` FROM `gameobject_loot_template` WHERE `Reference` = 0 "
      "UNION "
      ...
      "SELECT `item` FROM `pickpocketing_loot_template` WHERE `Reference` = 0 "
      "UNION "
      "SELECT `item` FROM `prospecting_loot_template` WHERE `Reference` = 0 "
      "UNION "
      "SELECT `item` FROM `reference_loot_template` WHERE `Reference` = 0 "
      "UNION "
  ...
  LOG_DEBUG("ahbot", "Loot filter has {} items", (uint32)lootItems.size());
  ...
  for (uint32 itemId = 0; itemId < sItemStore.GetNumRows(); ++itemId) {
    ...
    if (includeItems.count(itemId)) {
      _itemPool[prototype->GetQuality()][prototype->GetClass()].push_back(
          itemId);
      ++itemsAdded;
      continue;
    }
    ...
    case ITEM_CLASS_CONSUMABLE:
      allowZero = sAuctionBotConfig->GetConfig(
          CONFIG_AHBOT_CLASS_CONSUMABLE_ALLOW_ZERO);
      break;
    case ITEM_CLASS_CONTAINER:
      allowZero =
          sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_CONTAINER_ALLOW_ZERO);
      break;
    case ITEM_CLASS_WEAPON:
      allowZero =
          sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_WEAPON_ALLOW_ZERO);
      break;
    case ITEM_CLASS_GEM:
      allowZero =
          sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_GEM_ALLOW_ZERO);
      break;
    case ITEM_CLASS_ARMOR:
      allowZero =
          sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_ARMOR_ALLOW_ZERO);
      break;
    case ITEM_CLASS_REAGENT:
      allowZero =
          sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_REAGENT_ALLOW_ZERO);
      break;
    ...
    case ITEM_CLASS_TRADE_GOODS:
      allowZero =
          sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_TRADEGOOD_ALLOW_ZERO);
      break;
    case ITEM_CLASS_RECIPE:
      allowZero =
          sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_RECIPE_ALLOW_ZERO);
      break;
    case ITEM_CLASS_QUIVER:
      allowZero =
          sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_QUIVER_ALLOW_ZERO);
      break;
    case ITEM_CLASS_QUEST:
      allowZero =
          sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_QUEST_ALLOW_ZERO);
      break;
    case ITEM_CLASS_KEY:
      allowZero =
          sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_KEY_ALLOW_ZERO);
      break;
    case ITEM_CLASS_MISCELLANEOUS:
      allowZero =
          sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_MISC_ALLOW_ZERO);
      break;
    case ITEM_CLASS_GLYPH:
      allowZero =
          sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_GLYPH_ALLOW_ZERO);
      break;
    default:
      allowZero = false;
    }

    // Filter out items with no buy/sell price unless otherwise flagged in the
    // config.
    if (!allowZero) {
    ...
    case ITEM_CLASS_WEAPON: {
      if (uint32 value =
              sAuctionBotConfig->GetConfig(CONFIG_AHBOT_ITEM_MIN_ITEM_LEVEL))
        if (prototype->GetBaseItemLevel() < value)
          continue;
      if (uint32 value =
              sAuctionBotConfig->GetConfig(CONFIG_AHBOT_ITEM_MAX_ITEM_LEVEL))
        if (prototype->GetBaseItemLevel() > value)
          continue;
      if (int32 value =
              sAuctionBotConfig->GetConfig(CONFIG_AHBOT_ITEM_MIN_REQ_LEVEL))
        if (prototype->GetRequiredLevel() < value)
          continue;
      if (int32 value =
              sAuctionBotConfig->GetConfig(CONFIG_AHBOT_ITEM_MAX_REQ_LEVEL))
        if (prototype->GetRequiredLevel() > value)
          continue;
      if (uint32 value =
              sAuctionBotConfig->GetConfig(CONFIG_AHBOT_ITEM_MIN_SKILL_RANK))
        if (prototype->GetRequiredSkillRank() < value)
          continue;
      if (uint32 value =
              sAuctionBotConfig->GetConfig(CONFIG_AHBOT_ITEM_MAX_SKILL_RANK))
        if (prototype->GetRequiredSkillRank() > value)
          continue;
      break;
    }
    ...
    case ITEM_CLASS_MISCELLANEOUS:
      if (prototype->GetSubClass() == ITEM_SUBCLASS_JUNK_MOUNT) {
        if (int32 value = sAuctionBotConfig->GetConfig(
        ...
        if (int32 value = sAuctionBotConfig->GetConfig(
          ...
          if (prototype->GetRequiredLevel() > value)
            continue;
        if (uint32 value = sAuctionBotConfig->GetConfig(
      ...
      if (prototype->GetFlags() & ITEM_FLAG_HAS_LOOT) {
        // skip any not locked lootable items (mostly quest specific or reward
        // cases)
  ...
  LOG_DEBUG("ahbot",
            "AuctionHouseBot seller will use {} items to fill auction house "
            "(according your config choices)",
            itemsAdded);
  ...
  for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i)
    LOG_DEBUG("ahbot", "\t\t{}\t{}\t{}\t{}\t{}\t{}\t{}",
              (uint32)_itemPool[0][i].size(), (uint32)_itemPool[1][i].size(),
              (uint32)_itemPool[2][i].size(), (uint32)_itemPool[3][i].size(),
              (uint32)_itemPool[4][i].size(), (uint32)_itemPool[5][i].size(),
...
void AuctionBotSeller::LoadItemsQuantity(SellerConfiguration &config) {
  uint32 ratio =
      sAuctionBotConfig->GetConfigItemAmountRatio(config.GetHouseType());

  for (uint32 i = 0; i < MAX_AUCTION_QUALITY; ++i) {
    uint32 amount = sAuctionBotConfig->GetConfig(
        AuctionBotConfigUInt32Values(CONFIG_AHBOT_ITEM_GRAY_AMOUNT + i));
    config.SetItemsAmountPerQuality(AuctionQuality(i),
                                    std::lroundf(amount * ratio / 100.f));
  }

  // Set Stack Quantities
  config.SetRandomStackRatioPerClass(
      ITEM_CLASS_CONSUMABLE,
      sAuctionBotConfig->GetConfig(
          CONFIG_AHBOT_CLASS_RANDOMSTACKRATIO_CONSUMABLE));
  config.SetRandomStackRatioPerClass(
      ITEM_CLASS_CONTAINER, sAuctionBotConfig->GetConfig(
  ...
  config.SetRandomStackRatioPerClass(
      ITEM_CLASS_REAGENT, sAuctionBotConfig->GetConfig(
  ...
  config.SetRandomStackRatioPerClass(
      ITEM_CLASS_GENERIC, sAuctionBotConfig->GetConfig(
  ...
  for (uint32 j = 0; j < MAX_AUCTION_QUALITY; ++j) {
    ...
    for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i) {
      ...
      uint32 weightedAmount = std::lroundf(
          classPrio / float(totalPrioPerQuality[j]) * qualityAmount);
      config.SetItemsAmountPerClass(AuctionQuality(j), ItemClass(i),
                                    weightedAmount);
    }
  }

  // do some assert checking, GetItemAmount must always return 0 if selected
  // _itemPool is empty
  for (uint32 j = 0; j < MAX_AUCTION_QUALITY; ++j) {
    for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i) {
      if (_itemPool[j][i].empty())
        ASSERT(config.GetItemsAmountPerClass(AuctionQuality(j), ItemClass(i)) ==
               0);
    }
  }
}
...
void AuctionBotSeller::LoadSellerValues(SellerConfiguration &config) {
  ...
  for (uint32 i = 0; i < MAX_AUCTION_QUALITY; ++i) {
    uint32 amount = sAuctionBotConfig->GetConfig(
        AuctionBotConfigUInt32Values(CONFIG_AHBOT_ITEM_GRAY_PRICE_RATIO + i));
    config.SetPriceRatioPerQuality(AuctionQuality(i),
                                   std::lroundf(amount * ratio / 100.f));
  }

  config.SetPriceRatioPerClass(
      ITEM_CLASS_CONSUMABLE,
  ...
  config.SetPriceRatioPerClass(
      ITEM_CLASS_WEAPON,
      sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_WEAPON_PRICE_RATIO));
  config.SetPriceRatioPerClass(
      ITEM_CLASS_GEM,
      sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_GEM_PRICE_RATIO));
  config.SetPriceRatioPerClass(
      ITEM_CLASS_ARMOR,
      sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_ARMOR_PRICE_RATIO));
  config.SetPriceRatioPerClass(
      ITEM_CLASS_REAGENT,
  ...
  config.SetPriceRatioPerClass(
      ITEM_CLASS_RECIPE,
      sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_RECIPE_PRICE_RATIO));
  config.SetPriceRatioPerClass(
      ITEM_CLASS_MONEY,
      sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_MONEY_PRICE_RATIO));
  config.SetPriceRatioPerClass(
      ITEM_CLASS_QUIVER,
      sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_QUIVER_PRICE_RATIO));
  config.SetPriceRatioPerClass(
      ITEM_CLASS_QUEST,
      sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_QUEST_PRICE_RATIO));
  config.SetPriceRatioPerClass(
      ITEM_CLASS_KEY,
      sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_KEY_PRICE_RATIO));
  config.SetPriceRatioPerClass(
  ...
  config.SetPriceRatioPerClass(
      ITEM_CLASS_GLYPH,
      sAuctionBotConfig->GetConfig(CONFIG_AHBOT_CLASS_GLYPH_PRICE_RATIO));
...
uint32 AuctionBotSeller::SetStat(SellerConfiguration &config) {
  AllItemsArray itemsSaved(MAX_AUCTION_QUALITY,
                           std::vector<uint32>(MAX_ITEM_CLASS));

  AuctionHouseObject *auctionHouse =
      sAuctionMgr->GetAuctionsMap(config.GetHouseType());
  ...
  for (uint32 j = 0; j < MAX_AUCTION_QUALITY; ++j) {
    for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i) {
      config.SetMissedItemsPerClass((AuctionQuality)j, (ItemClass)i,
                                    itemsSaved[j][i]);
      count += config.GetMissedItemsPerClass((AuctionQuality)j, (ItemClass)i);
    }
  }

  LOG_DEBUG("ahbot", "AHBot: Missed Item       "
...
// getRandomArray is used to make viable the possibility to add any of missed
// item in place of first one to last one.
bool AuctionBotSeller::GetItemsToSell(SellerConfiguration &config,
  ...
  for (uint32 j = 0; j < MAX_AUCTION_QUALITY; ++j) {
    for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i) {
      // if _itemPool for chosen is empty, MissedItemsPerClass will return 0
      // here (checked at startup)
      if (config.GetMissedItemsPerClass(AuctionQuality(j), ItemClass(i)) >
          addedItem[j][i]) {
...
void AuctionBotSeller::SetPricesOfItem(ItemTemplate const *itemProto,
                                       ...
                                       uint32 &buyp, uint32 &bidp,
                                       uint32 stackCount) {
  uint32 classRatio =
      config.GetPriceRatioPerClass(ItemClass(itemProto->GetClass()));
  uint32 qualityRatio =
      config.GetPriceRatioPerQuality(AuctionQuality(itemProto->GetQuality()));
  ...
  if (buyPrice == 0) {
    if (sellPrice > 0)
      ...
      float divisor = ((itemProto->GetClass() == ITEM_CLASS_WEAPON ||
                        itemProto->GetClass() == ITEM_CLASS_ARMOR)
                           ? 284.0f
                           : 80.0f);
      float tempLevel =
          (itemProto->GetBaseItemLevel() == 0 ? 1.0f
                                              : itemProto->GetBaseItemLevel());
      float tempQuality =
          (itemProto->GetQuality() == 0 ? 1.0f : itemProto->GetQuality());

      buyPrice = tempLevel * tempQuality *
                 static_cast<float>(GetBuyModifier(itemProto)) * tempLevel /
                 divisor;
    }
  }
  ...
  if (sellPrice == 0)
    sellPrice =
        (buyPrice > 10 ? buyPrice / GetSellModifier(itemProto) : buyPrice);

  if (sAuctionBotConfig->GetConfig(CONFIG_AHBOT_BUYPRICE_SELLER))
    buyPrice = sellPrice;

  float basePriceFloat = buyPrice * stackCount /
                         (itemProto->GetClass() == 6
                              ? 200.0f
                              : static_cast<float>(itemProto->GetBuyCount()));
  basePriceFloat *= priceRatio;

  float range = basePriceFloat * 0.04f;

  buyp = static_cast<uint32>(
      frand(basePriceFloat - range, basePriceFloat + range) + 0.5f);
  if (buyp == 0)
    buyp = 1;

  float bidPercentage =
      frand(sAuctionBotConfig->GetConfig(CONFIG_AHBOT_BIDPRICE_MIN),
...
uint32
AuctionBotSeller::GetStackSizeForItem(ItemTemplate const *itemProto,
                                      SellerConfiguration &config) const {
  if (config.GetRandomStackRatioPerClass(ItemClass(itemProto->GetClass())) >
      urand(0, 99))
...
// Determine the multiplier for the sell price of any weapon without a buy
// price.
...
// Return the modifier by which the item's level and quality will be modified by
// to derive a relatively accurate price.
...
void AuctionBotSeller::SetItemsRatio(uint32 al, uint32 ho, uint32 ne) {
  sAuctionBotConfig->SetConfig(CONFIG_AHBOT_ALLIANCE_ITEM_AMOUNT_RATIO,
                               std::max(al, 100000u));
  sAuctionBotConfig->SetConfig(CONFIG_AHBOT_HORDE_ITEM_AMOUNT_RATIO,
                               std::max(ho, 100000u));
  sAuctionBotConfig->SetConfig(CONFIG_AHBOT_NEUTRAL_ITEM_AMOUNT_RATIO,
                               std::max(ne, 100000u));
...
void AuctionBotSeller::SetItemsAmount(uint32 (&vals)[MAX_AUCTION_QUALITY]) {
  ...
  sAuctionBotConfig->SetConfig(CONFIG_AHBOT_ITEM_WHITE_AMOUNT,
                               vals[AUCTION_QUALITY_WHITE]);
  sAuctionBotConfig->SetConfig(CONFIG_AHBOT_ITEM_GREEN_AMOUNT,
                               vals[AUCTION_QUALITY_GREEN]);
  sAuctionBotConfig->SetConfig(CONFIG_AHBOT_ITEM_BLUE_AMOUNT,
                               vals[AUCTION_QUALITY_BLUE]);
  sAuctionBotConfig->SetConfig(CONFIG_AHBOT_ITEM_PURPLE_AMOUNT,
                               vals[AUCTION_QUALITY_PURPLE]);
  sAuctionBotConfig->SetConfig(CONFIG_AHBOT_ITEM_ORANGE_AMOUNT,
                               vals[AUCTION_QUALITY_ORANGE]);
  sAuctionBotConfig->SetConfig(CONFIG_AHBOT_ITEM_YELLOW_AMOUNT,
                               vals[AUCTION_QUALITY_YELLOW]);
...
void AuctionBotSeller::AddNewAuctions(SellerConfiguration &config) {
  ...
  // If there is large amount of items missed we can use boost value to get fast
  // filled AH
  if (config.LastMissedItem > sAuctionBotConfig->GetItemPerCycleBoost()) {
    items = sAuctionBotConfig->GetItemPerCycleBoost();
    LOG_DEBUG("ahbot", "AHBot: Boost value used to fill AH! (if this happens "
                       "often adjust both ItemsPerCycle in worldserver.conf)");
  } else
  ...
  AllItemsArray allItems(MAX_AUCTION_QUALITY,
                         std::vector<uint32>(MAX_ITEM_CLASS));
  // Main loop
  // getRandomArray will give what categories of items should be added (return
  // true if there is at least 1 items missed)
  ...
  while (GetItemsToSell(config, itemsToSell, allItems) && items > 0) {
    ...
    ItemToSell const &sellItem =
        Firelands::Containers::SelectRandomContainerElement(itemsToSell);

    // Set itemId with random item ID for selected categories and color, from
    // _itemPool table
    uint32 itemId = Firelands::Containers::SelectRandomContainerElement(
        _itemPool[sellItem.Color][sellItem.Itemclass]);
    ++allItems[sellItem.Color]
              [sellItem.Itemclass]; // Helper table to avoid rescan from DB in
                                    // this loop. (has we add item in random
                                    // orders)
    ...
    ItemTemplate const *prototype = sObjectMgr->GetItemTemplate(itemId);
    if (!prototype) {
      LOG_DEBUG("ahbot", "AHBot: Unknown item {} auction creating attempt.",
                itemId);
      continue;
    }
    ...
    if (!item) {
      LOG_ERROR(
          "ahbot",
          "AHBot: Item::CreateItem() returned nullptr for item {} (stack: {})",
          itemId, stackCount);
      return;
    }

    // Update the just created item so that if it needs random properties it has
    // them. Ex:  Notched Shortsword of Stamina will only generate as a Notched
    // Shortsword without this.
    ...
    auctionEntry->expire_time =
        GameTime::GetGameTime() +
        urand(config.GetMinTime(), config.GetMaxTime()) * HOUR;
...
bool AuctionBotSeller::Update(AuctionHouseType houseType) {
  if (sAuctionBotConfig->GetConfigItemAmountRatio(houseType) > 0) {
    LOG_DEBUG("ahbot", "AHBot: {} selling ...",
              AuctionBotConfig::GetHouseTypeName(houseType));
