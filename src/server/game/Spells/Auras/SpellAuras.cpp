...
void AuraApplication::_HandleEffect(uint8 effIndex, bool apply)
{
    ...
    LOG_DEBUG("spells", "AuraApplication::_HandleEffect: %u, apply: %u: amount: %d", aurEff->GetAuraType(), apply, aurEff->GetAmount());
                    ...
                    case 12043: // Presence of Mind
                        ...
                        if (AuraEffect const* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_MAGE, 2120, 0))
                        {
                            ...
                            switch (aurEff->GetId())
                            {
                                ...
                                default:
                                    LOG_ERROR("spells", "Aura::HandleAuraSpecificMods: Unknown rank of Arcane Potency (%d) found", aurEff->GetId());
                            }
                            if (spellId)
        ...
        else
        {
            ...
            switch (GetSpellInfo()->Effects[effIndex].Effect)
            {
                ...
                case SPELL_EFFECT_APPLY_AREA_AURA_RAID:
                {
                    ...
                    if (!isolated)
                    {
                        Firelands::AnyGroupedUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius, GetSpellInfo()->Effects[effIndex].Effect == SPELL_EFFECT_APPLY_AREA_AURA_RAID);
                        Firelands::UnitListSearcher<Firelands::AnyGroupedUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList, u_check);
                ...
                case SPELL_EFFECT_APPLY_AREA_AURA_FRIEND:
                {
                    ...
                    if (!isolated)
                    {
                        Firelands::AnyFriendlyUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius);
                        Firelands::UnitListSearcher<Firelands::AnyFriendlyUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList, u_check);
                        GetUnitOwner()->VisitNearbyObject(radius, searcher);
                    }
                    break;
                }
                ...
                {
                    Firelands::AnyAoETargetUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius); // No GetCharmer in searcher
                    Firelands::UnitListSearcher<Firelands::AnyAoETargetUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList, u_check);
                    GetUnitOwner()->VisitNearbyObject(radius, searcher);
                    break;
                }
...
void DynObjAura::FillTargetMap(std::map<Unit*, uint8> & targets, Unit* /*caster*/)
{
    ...
    for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
    {
        ...
        {
            Firelands::AnyFriendlyUnitInObjectRangeCheck u_check(GetDynobjOwner(), dynObjOwnerCaster, radius,  GetSpellInfo()->Effects[effIndex].ImplicitTargetConditions);
            Firelands::UnitListSearcher<Firelands::AnyFriendlyUnitInObjectRangeCheck> searcher(GetDynobjOwner(), targetList, u_check);
            GetDynobjOwner()->VisitNearbyObject(radius, searcher);
        }
        else
        {
            Firelands::AnyAoETargetUnitInObjectRangeCheck u_check(GetDynobjOwner(), dynObjOwnerCaster, radius, GetSpellInfo()->Effects[effIndex].ImplicitTargetConditions);
            Firelands::UnitListSearcher<Firelands::AnyAoETargetUnitInObjectRangeCheck> searcher(GetDynobjOwner(), targetList, u_check);
            GetDynobjOwner()->VisitNearbyObject(radius, searcher);
        }
