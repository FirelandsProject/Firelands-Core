...
int32 SpellScript::GetTrueDamage()
{
    ...
    {
        LOG_ERROR("scripts", "Script: `%s` Spell: `%u`: function SpellScript::GetHitHeal was called while spell not in after-hit phase!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }
...
bool AuraScript::_Validate(SpellInfo const* entry)
{
    for (std::list<CheckAreaTargetHandler>::iterator itr = DoCheckAreaTarget.begin(); itr != DoCheckAreaTarget.end();  ++itr)
        if (!entry->HasAreaAuraEffect())
            LOG_ERROR("scripts", "Spell `%u` of script `%s` does not have area aura effect - handler bound to hook `DoCheckAreaTarget` of AuraScript won't be executed", entry->Id, m_scriptName->c_str());
    ...
    for (std::list<EffectUpdateHandler>::iterator itr = OnEffectUpdate.begin(); itr != OnEffectUpdate.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            LOG_ERROR("scripts", "Spell `%u` Effect `%s` of script `%s` did not match dbc effect data - handler bound to hook `OnEffectUpdate` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());
