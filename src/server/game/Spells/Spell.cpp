...
SpellCastTargets::SpellCastTargets()
    : m_targetMask(0), m_objectTarget(nullptr), m_itemTarget(nullptr),
      m_itemTargetEntry(0), m_elevation(0.f), m_speed(0.f) {}

SpellCastTargets::SpellCastTargets(
    Unit *caster,
    WorldPackets::Spells::SpellCastRequest const &spellCastRequest)
    : m_targetMask(spellCastRequest.Target.Flags), m_objectTarget(nullptr),
      m_itemTarget(nullptr), m_itemTargetEntry(0), m_elevation(0.0f),
      m_speed(0.0f) {
...
void SpellCastTargets::Write(WorldPackets::Spells::SpellTargetData &data) {
  ...
  if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION) {
    ...
    data.SrcLocation->Transport =
        m_src._transportGUID; // relative position guid here - transport for
                              // example
    if (!m_src._transportGUID.IsEmpty())
  ...
  if (m_targetMask & TARGET_FLAG_DEST_LOCATION) {
    ...
    data.DstLocation->Transport =
        m_dst._transportGUID; // relative position guid here - transport for
                              // example
    if (!m_dst._transportGUID.IsEmpty())
...
void SpellCastTargets::OutDebug() const {
  ...
  if (m_targetMask & (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_CORPSE_MASK |
                      TARGET_FLAG_GAMEOBJECT_MASK))
    LOG_DEBUG("spells", "Object target: %s",
              m_objectTargetGUID.ToString().c_str());
  if (m_targetMask & TARGET_FLAG_ITEM)
    LOG_DEBUG("spells", "Item target: %s", m_itemTargetGUID.ToString().c_str());
  if (m_targetMask & TARGET_FLAG_TRADE_ITEM)
    LOG_DEBUG("spells", "Trade item target: %s",
              m_itemTargetGUID.ToString().c_str());
  if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION)
    LOG_DEBUG(
        "spells",
        "Source location: transport guid:%s trans offset: %s position: %s",
        m_src._transportGUID.ToString().c_str(),
        m_src._transportOffset.ToString().c_str(),
        m_src._position.ToString().c_str());
  if (m_targetMask & TARGET_FLAG_DEST_LOCATION)
    LOG_DEBUG(
        "spells",
        "Destination location: transport guid:%s trans offset: %s position: %s",
        m_dst._transportGUID.ToString().c_str(),
...
Spell::Spell(Unit *caster, SpellInfo const *info, TriggerCastFlags triggerFlags,
             ObjectGuid originalCasterGUID, bool skipCheck)
  ...
  if (m_attackType == RANGED_ATTACK)
    ...
    if ((m_caster->getClassMask() & CLASSMASK_WAND_USERS) != 0 &&
      ...
      if (Item *pItem = m_caster->ToPlayer()->GetWeaponForAttack(RANGED_ATTACK))
        m_spellSchoolMask =
            SpellSchoolMask(1 << pItem->GetTemplate()->GetDamageType());
...
void Spell::RecalculateDelayMomentForDst() {
  ...
  m_caster->m_Events.ModifyEventTime(_spellEvent,
                                     GetDelayStart() + m_delayMoment);
}
...
void Spell::SelectImplicitChannelTargets(
  ...
  if (!channeledSpell) {
    LOG_DEBUG("spells",
              "Spell::SelectImplicitChannelTargets: cannot find channel spell "
  ...
  case TARGET_UNIT_CHANNEL_TARGET: {
    WorldObject *target = ObjectAccessor::GetUnit(
        *m_caster, m_originalCaster->GetChannelObjectGuid());
  ...
  case TARGET_DEST_CHANNEL_TARGET:
    ...
    else if (WorldObject *target = ObjectAccessor::GetWorldObject(
                 *m_caster, m_originalCaster->GetChannelObjectGuid())) {
      CallScriptObjectTargetSelectHandlers(target, effIndex, targetType);
      if (target) {
...
void Spell::SelectImplicitNearbyTargets(
  ...
  if (!target) {
    LOG_DEBUG("spells",
              "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script "
              "hook for spell Id %u set nullptr target, effect %u",
              m_spellInfo->Id, effIndex);
    SendCastResult(SPELL_FAILED_BAD_IMPLICIT_TARGETS);
    finish(false);
    return;
  ...
  case TARGET_OBJECT_TYPE_UNIT:
    if (Unit *unit = target->ToUnit())
      ...
      LOG_DEBUG("spells",
                "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect "
                "script hook for spell Id %u set object of wrong type, "
                "expected unit, got %s, effect %u",
  ...
  case TARGET_OBJECT_TYPE_GOBJ:
    if (GameObject *gobjTarget = target->ToGameObject())
      ...
      LOG_DEBUG("spells",
                "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect "
                "script hook for spell Id %u set object of wrong type, "
                "expected gameobject, got %s, effect %u",
                m_spellInfo->Id, target->GetGUID().GetTypeName(), effMask);
      SendCastResult(SPELL_FAILED_BAD_IMPLICIT_TARGETS);
      finish(false);
      return;
  ...
  case TARGET_OBJECT_TYPE_CORPSE:
    if (Corpse *corpse = target->ToCorpse())
      ...
      LOG_DEBUG("spells",
                "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect "
                "script hook for spell Id %u set object of wrong type, "
                "expected corpse, got %s, effect %u",
                m_spellInfo->Id, target->GetGUID().GetTypeName(), effMask);
      SendCastResult(SPELL_FAILED_BAD_IMPLICIT_TARGETS);
      finish(false);
      return;
...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex,
  ...
  // Workaround for some spells that don't have RadiusEntry set in dbc (but
  // SpellRange instead)
  if (G3D::fuzzyEq(radius, 0.f))
    radius = m_spellInfo->GetMaxRange(m_spellInfo->IsPositiveEffect(effIndex),
                                      m_caster, this);

  radius *= m_spellValue->RadiusMod;

  if (uint32 containerTypeMask = GetSearcherTypeMask(objectType, condList)) {
    float coneRads = coneAngle * (M_PI / 180);
    Firelands::WorldObjectSpellConeTargetCheck check(coneSrc, coneRads, radius,
                                                     m_caster, m_spellInfo,
                                                     selectionType, condList);
    Firelands::WorldObjectListSearcher<
        Firelands::WorldObjectSpellConeTargetCheck>
        searcher(m_caster, targets, check, containerTypeMask);
    SearchTargets<Firelands::WorldObjectListSearcher<
        Firelands::WorldObjectSpellConeTargetCheck>>(
  ...
  case TARGET_UNIT_TARGET_ALLY_OR_RAID:
    if (Unit *targetedUnit = m_targets.GetUnitTarget()) {
      if (!m_caster->IsInRaidWith(targetedUnit))
        ...
        SearchAreaTargets(
            targets, radius, targetedUnit, referer, targetType.GetObjectType(),
...
void Spell::SelectImplicitCasterDestTargets(
  ...
  case TARGET_DEST_DB:
    if (SpellTargetPosition const *st =
      ...
      LOG_DEBUG("spells", "SPELL: unknown target coordinates for spell ID %u",
                m_spellInfo->Id);
      if (WorldObject *target = m_targets.GetObjectTarget())
  ...
  case TARGET_DEST_CASTER_FISHING: {
    ...
    float x, y, z;
    float angle =
        float(rand_norm()) * static_cast<float>(M_PI * 35.0f / 180.0f) -
        static_cast<float>(M_PI * 17.5f / 180.0f);
    m_caster->GetClosePoint(x, y, z, DEFAULT_PLAYER_BOUNDING_RADIUS, dist,
                            angle);

    float ground = m_caster->GetMapHeight(x, y, z);
    float liquidLevel = VMAP_INVALID_HEIGHT_VALUE;
    LiquidData liquidData;
    if (m_caster->GetMap()->GetLiquidStatus(m_caster->GetPhaseShift(), x, y, z,
  ...
  case TARGET_DEST_CASTER_GROUND_2:
    dest._position.m_positionZ = m_caster->GetMapWaterOrGroundLevel(
        dest._position.GetPositionX(), dest._position.GetPositionY(),
...
void Spell::SelectImplicitTargetDestTargets(
    SpellEffIndex effIndex, SpellImplicitTargetInfo const &targetType) {
  ASSERT(m_targets.GetObjectTarget() &&
         "Spell::SelectImplicitTargetDestTargets - no explicit object target "
         "available!");
...
void Spell::SelectImplicitChainTargets(
  ...
  if (maxTargets > 1) {
    ...
    std::list<WorldObject *> targets;
    SearchChainTargets(targets, maxTargets - 1, target,
                       targetType.GetObjectType(), targetType.GetCheckType(),
                       m_spellInfo->Effects[effIndex].ImplicitTargetConditions,
...
void Spell::SelectImplicitTrajTargets(
  ...
  float srcToDestDelta =
      m_targets.GetDstPos()->m_positionZ - srcPos.m_positionZ;

  std::list<WorldObject *> targets;
  Firelands::WorldObjectSpellTrajTargetCheck check(
      dist2d, &srcPos, m_caster, m_spellInfo, targetType.GetCheckType(),
      m_spellInfo->Effects[effIndex].ImplicitTargetConditions);
  Firelands::WorldObjectListSearcher<Firelands::WorldObjectSpellTrajTargetCheck>
      searcher(m_caster, targets, check, GRID_MAP_TYPE_MASK_ALL);
  SearchTargets<Firelands::WorldObjectListSearcher<
      Firelands::WorldObjectSpellTrajTargetCheck>>(
      searcher, GRID_MAP_TYPE_MASK_ALL, m_caster, &srcPos, dist2d);
  if (targets.empty())
    return;
  ...
  // We should check if triggered spell has greater range (which is true in many
  // cases, and initial spell has too short max range) limit max range to 300
  // yards, sometimes triggered spells can have 50000yds
  float bestDist = m_spellInfo->GetMaxRange(false);
  if (SpellInfo const *triggerSpellInfo =
          sSpellMgr->GetSpellInfo(m_spellInfo->Effects[effIndex].TriggerSpell))
    bestDist =
        std::min(std::max(bestDist, triggerSpellInfo->GetMaxRange(false)),
                 std::min(dist2d, 300.0f));
  ...
  for (; itr != targets.end(); ++itr) {
    ...
    if (Unit *unit = (*itr)->ToUnit()) {
      ...
      if (Creature *creatureTarget = unit->ToCreature()) {
        if (!(creatureTarget->GetCreatureTemplate()->type_flags &
    ...
    const float horizontalDistToTraj =
        std::fabs(objDist2d * std::sin(srcPos.GetRelativeAngle(*itr)));
    const float sizeFactor =
        std::cos((horizontalDistToTraj / size) * (M_PI / 2.0f));
    const float distToHitPoint = std::max(
        objDist2d * std::cos(srcPos.GetRelativeAngle(*itr)) - size * sizeFactor,
        0.0f);
  ...
  if (dist2d > bestDist) {
    float x = m_targets.GetSrcPos()->m_positionX +
              std::cos(m_caster->GetOrientation()) * bestDist;
...
void Spell::SearchTargets(SEARCHER &searcher, uint32 containerMask,
  ...
  bool searchInGrid = (containerMask & (GRID_MAP_TYPE_MASK_CREATURE |
                                        GRID_MAP_TYPE_MASK_GAMEOBJECT)) != 0;
  bool searchInWorld = (containerMask & (GRID_MAP_TYPE_MASK_CREATURE |
                                         GRID_MAP_TYPE_MASK_PLAYER |
                                         GRID_MAP_TYPE_MASK_CORPSE)) != 0;
...
WorldObject *Spell::SearchNearbyTarget(float range,
  ...
  if (!containerTypeMask)
    return nullptr;
  Firelands::WorldObjectSpellNearbyTargetCheck check(
      range, m_caster, m_spellInfo, selectionType, condList);
  Firelands::WorldObjectLastSearcher<
      Firelands::WorldObjectSpellNearbyTargetCheck>
      searcher(m_caster, target, check, containerTypeMask);
  SearchTargets<Firelands::WorldObjectLastSearcher<
      Firelands::WorldObjectSpellNearbyTargetCheck>>(
...
void Spell::SearchAreaTargets(std::list<WorldObject *> &targets, float range,
  ...
  if (!containerTypeMask)
    return;
  Firelands::WorldObjectSpellAreaTargetCheck check(
      range, position, m_caster, referer, m_spellInfo, selectionType, condList);
  Firelands::WorldObjectListSearcher<Firelands::WorldObjectSpellAreaTargetCheck>
      searcher(m_caster, targets, check, containerTypeMask);
  SearchTargets<Firelands::WorldObjectListSearcher<
      Firelands::WorldObjectSpellAreaTargetCheck>>(searcher, containerTypeMask,
...
GameObject *Spell::SearchSpellFocus() {
  ...
  Firelands::GameObjectFocusCheck check(m_caster,
                                        m_spellInfo->RequiresSpellFocus);
  Firelands::GameObjectSearcher<Firelands::GameObjectFocusCheck> searcher(
      m_caster, focus, check);
  SearchTargets<Firelands::GameObjectSearcher<Firelands::GameObjectFocusCheck>>(
      searcher, GRID_MAP_TYPE_MASK_GAMEOBJECT, m_caster, m_caster,
...
void Spell::prepareDataForTriggerSystem() {
  ...
  if (m_spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER &&
      (m_spellInfo->SpellFamilyFlags[0] &
       ...
       m_spellInfo->Id ==
           57879 || // Snake Trap - done this way to avoid double proc
       m_spellInfo->SpellFamilyFlags[2] &
           0x00024000)) // Explosive and Immolation Trap
  {
    m_procAttacker |= PROC_FLAG_TRAP_ACTIVATION;

    // also fill up other flags (DoAllEffectOnTarget only fills up flag if both
    // are not set)
...
public:
  ProcReflectDelayed(Unit *owner, ObjectGuid casterGuid)
      : _victim(owner), _casterGuid(casterGuid) {}
  ...
  bool Execute(uint64 /*e_time*/, uint32 /*p_time*/) override {
    ...
    uint32 const typeMaskActionTarget =
        PROC_FLAG_TAKE_HARMFUL_SPELL | PROC_FLAG_TAKE_HARMFUL_ABILITY;
    uint32 const spellTypeMask =
        PROC_SPELL_TYPE_DAMAGE | PROC_SPELL_TYPE_NO_DMG_HEAL;
    uint32 const spellPhaseMask = PROC_SPELL_PHASE_NONE;
    uint32 const hitMask = PROC_HIT_REFLECT;

    caster->ProcSkillsAndAuras(_victim, typeMaskActor, typeMaskActionTarget,
                               spellTypeMask, spellPhaseMask, hitMask, nullptr,
                               nullptr, nullptr);
    return true;
  }
...
void Spell::AddUnitTarget(Unit *target, uint32 effectMask,
  ...
  for (uint32 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
    if (!m_spellInfo->Effects[effIndex].IsEffect() ||
        !CheckEffectTarget(target, effIndex, losPosition))
  ...
  if (checkIfValid)
    if (m_spellInfo->CheckTarget(m_caster, target,
                                 implicit ||
                                     m_caster->GetEntry() == WORLD_TRIGGER) !=
        SPELL_CAST_OK) // skip stealth checks for GO casts
      return;
  ...
  auto const &ihit =
      std::find_if(std::begin(m_UniqueTargetInfo), std::end(m_UniqueTargetInfo),
                   [targetGUID](TargetInfo const &target) {
  ...
  if (m_originalCaster) {
    ...
    // Totems inherit their owner's spell hit rating so we use the player as
    // primary reference
    ...
    if (!hasTargetInfo || targetInfo.missCondition == SPELL_MISS_IMMUNE)
      targetInfo.missCondition = caster->SpellHitResult(
          target, m_spellInfo,
  ...
  if (hasTargetInfo && hasReflectData &&
      targetInfo.missCondition == SPELL_MISS_REFLECT)
    return;
  ...
  if (m_spellInfo->Speed > 0.0f && m_caster != target) {
    ...
    if (!m_spellInfo->HasAttribute(SPELL_ATTR9_SPECIAL_DELAY_CALCULATION))
      targetInfo.timeDelay =
          uint64(std::floor(dist / m_spellInfo->Speed * 1000.0f));
    else
  ...
  if (targetInfo.missCondition == SPELL_MISS_REFLECT) {
    ...
    targetInfo.reflectResult = m_caster->SpellHitResult(
        m_caster, m_spellInfo, false); // can't reflect twice

    // Proc spell reflect aura when missile hits the original target
    target->m_Events.AddEvent(
        new ProcReflectDelayed(target, m_originalCasterGUID),
        target->m_Events.CalculateTime(targetInfo.timeDelay));
...
void Spell::AddCorpseTarget(Corpse *corpse, uint32 effectMask) {
  {
    ...
    if (m_spellInfo->Speed > 0.0f) {
      // calculate spell incoming interval
      float dist =
          m_caster->GetDistance(corpse->GetPositionX(), corpse->GetPositionY(),
                                corpse->GetPositionZ());
      if (dist < 5.0f)
        dist = 5.0f;
...
void Spell::DoAllEffectOnTarget(TargetInfo *target) {
  ...
  Unit *unit = m_caster->GetGUID() == target->targetGUID
                   ? m_caster
                   : ObjectAccessor::GetUnit(*m_caster, target->targetGUID);
  if (!unit && !target->targetGUID
  ...
  if (!unit) {
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
      if (farMask & (1 << i))
        HandleEffects(unit, nullptr, nullptr, nullptr, i,
  ...
  if (getState() == SPELL_STATE_DELAYED && !IsPositive() &&
      (GameTime::GetGameTimeMS() - target->timeDelay) <=
          unit->m_lastSanctuaryTime)
  ...
  bool const canTriggerCasterProcs = [&]() {
    ...
    if (!(CanExecuteTriggersOnHit(mask) || missInfo == SPELL_MISS_IMMUNE ||
  ...
  {
    if (target->reflectResult ==
        SPELL_MISS_NONE) // If reflected spell hit caster -> do all effect on
                         // him
      spellHitTarget = m_caster;
  }

  // Ensure that a player target is put in combat by a taunt, even if they
  // result immune clientside
  if ((missInfo == SPELL_MISS_IMMUNE || missInfo == SPELL_MISS_IMMUNE2) &&
      m_caster->IsPlayer() && unit->IsPlayer() &&
      m_caster->IsValidAttackTarget(unit))
    unit->SetInCombatWith(m_caster->ToPlayer());

  bool enablePvP = false; // need to check PvP state before spell effects, but
  ...
  // it now
  if ((canTriggerCasterProcs || canTriggerTargetProcs) && !procAttacker &&
      !procVictim) {
    bool positive = true;
    if (m_damage > 0)
      positive = false;
  ...
  if (m_healing > 0) {
    ...
    HealInfo healInfo(caster, unitTarget, addhealth, m_spellInfo,
                      m_spellInfo->GetSchoolMask());
    caster->HealBySpell(healInfo, crit);
    unitTarget->GetThreatManager().ForwardThreatForAssistingMe(
        caster, float(healInfo.GetEffectiveHeal()) * 0.5f, m_spellInfo);
    m_healing = healInfo.GetEffectiveHeal();

    // Do triggers for unit
    if (canTriggerCasterProcs || canTriggerTargetProcs)
      caster->ProcSkillsAndAuras(unitTarget, procAttacker, procVictim,
                                 PROC_SPELL_TYPE_HEAL, PROC_SPELL_PHASE_HIT,
                                 hitMask, this, nullptr, &healInfo);
  }
  // Do damage and triggers
      ...
      if (Creature *target = damageInfo.target->ToCreature())
        if (caster->IsCreature() && !caster->IsCharmedOwnedByPlayerOrPlayer())
          if (target->GetNoNpcDamageBelowPctHealthValue() != 0.0f)
            if (target->GetHealthPct() <=
                target->GetNoNpcDamageBelowPctHealthValue())
              damageInfo.damage = 0;
    ...
    if (canTriggerCasterProcs || canTriggerTargetProcs) {
      DamageInfo spellDamageInfo(damageInfo, SPELL_DIRECT_DAMAGE, m_attackType,
                                 hitMask);
      caster->ProcSkillsAndAuras(unitTarget, procAttacker, procVictim,
                                 PROC_SPELL_TYPE_DAMAGE, PROC_SPELL_PHASE_HIT,
                                 hitMask, this, &spellDamageInfo, nullptr);
      ...
      if (caster->IsSummon() && (caster->IsGuardian() || caster->IsPet()))
        if (Unit *summoner = caster->ToTempSummon()->GetSummoner())
          summoner->ProcSkillsAndAuras(
              unitTarget, procAttacker, procVictim, PROC_SPELL_TYPE_DAMAGE,
              PROC_SPELL_PHASE_HIT, hitMask, this, &spellDamageInfo, nullptr);
  ...
  // Passive spell hits/misses or active spells only misses (only triggers)
    ...
    if (canTriggerCasterProcs || canTriggerTargetProcs) {
      ...
      caster->ProcSkillsAndAuras(
          unitTarget, procAttacker, procVictim, PROC_SPELL_TYPE_NO_DMG_HEAL,
          PROC_SPELL_PHASE_HIT, hitMask, this, &spellNoDamageInfo, nullptr);
    ...
    if (missInfo == SPELL_MISS_RESIST &&
      ...
      m_caster->RemoveAurasWithInterruptFlags(
          SpellAuraInterruptFlags::Interacting, 0, this);
  ...
  if (missInfo != SPELL_MISS_EVADE && spellHitTarget &&
      !m_caster->IsFriendlyTo(unit) &&
      (!IsPositive() || m_spellInfo->HasEffect(SPELL_EFFECT_DISPEL))) {
    if (Unit *unitCaster = m_caster->ToUnit())
      unitCaster->AtTargetAttacked(unit, m_spellInfo->CausesInitialThreat() &&
                                             !unitCaster->IsIgnoringCombat());
  ...
  if (missInfo == SPELL_MISS_NONE &&
      m_spellInfo->HasAttribute(SPELL_ATTR7_INTERRUPT_ONLY_NONPLAYER) &&
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit *unit, uint32 effectMask,
  ...
  if (m_spellInfo->Speed &&
      unit->IsImmunedToSpell(m_spellInfo, m_caster, effectMask))
  ...
  if (Player *player = unit->ToPlayer()) {
    player->StartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_SPELL_TARGET,
                                  m_spellInfo->Id);
    player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET,
                                      m_spellInfo->Id, 0, 0, m_caster);
    player->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET2, m_spellInfo->Id);
  }

  if (Player *player = m_caster->ToPlayer()) {
    player->StartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_SPELL_CASTER,
                                  m_spellInfo->Id);
    player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL2,
                                      m_spellInfo->Id, 0, 0, unit);
  }
  ...
  if (m_caster != unit) {
    ...
    if (m_caster->_IsValidAttackTarget(unit, m_spellInfo))
      unit->RemoveAurasWithInterruptFlags(
          SpellAuraInterruptFlags::HostileActionReceived);
      ...
      if (m_spellInfo->Speed > 0.0f && unit->GetTypeId() == TYPEID_PLAYER &&
          !IsPositive() && m_spellInfo->Id != 63881 && m_spellInfo->Id != 45034)
        return SPELL_MISS_EVADE;
      ...
      if (m_originalCaster && unit->IsInCombat() &&
          m_spellInfo->CausesInitialThreat()) {
        if (m_originalCaster->HasFlag(
                UNIT_FIELD_FLAGS,
                UNIT_FLAG_PLAYER_CONTROLLED)) // only do explicit combat
                                              // forwarding for PvP enabled
                                              // units
          m_originalCaster->GetCombatManager().InheritCombatStatesFrom(
              unit); // for creature v creature combat, the threat forward does
                     // it for us
        unit->GetThreatManager().ForwardThreatForAssistingMe(
            m_originalCaster, 0.0f, nullptr, true);
      }
    }
  }
  ...
  DiminishingGroup const diminishGroup =
      m_spellInfo->GetDiminishingReturnsGroupForSpell(triggered);
  ...
  if (diminishGroup && aura_effmask) {
    ...
    DiminishingReturnsType type =
        m_spellInfo->GetDiminishingReturnsGroupType(triggered);
    // Increase Diminishing on unit, current informations for actually casts
    // will use values above
    if (type == DRTYPE_ALL ||
        (type == DRTYPE_PLAYER && unit->IsAffectedByDiminishingReturns()))
    ...
    if (m_originalCaster) {
      ...
      m_spellAura = Aura::TryRefreshStackOrCreate(
          ...
          (aurSpellInfo == m_spellInfo) ? &m_spellValue->EffectBasePoints[0]
                                        : &basePoints[0],
          m_CastItem, ObjectGuid::Empty, &refresh);
      if (m_spellAura) {
        ...
        // check whatever effects we're going to apply, diminishing returns only
        // apply to negative aura effects
        bool positive = true;
        ...
        if (m_spellValue->AuraDuration &&
            m_spellValue->AuraDuration != duration)
          duration = m_spellValue->AuraDuration;

        // unit is immune to aura if it was diminished to 0 duration
        if (!positive && !unit->ApplyDiminishingToDuration(
                             aurSpellInfo, triggered, duration,
                             m_originalCaster, diminishLevel)) {
          m_spellAura->Remove();
          bool found = false;
          for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            if (effectMask & (1 << i) &&
          ...
          if (DynamicObject *dynObj =
                  m_originalCaster->GetDynObject(m_spellInfo->Id))
            dynObj->SetDuration(duration);

          if (m_spellInfo->IsChanneled() && refresh &&
              m_spellInfo->IsRollingDurationOver()) {
            SendChannelStart(m_spellAura->GetMaxDuration() -
                             m_spellAura->GetRolledOverDuration());
            SendChannelUpdate(m_spellAura->GetMaxDuration());

            // A side-effect of SendChannelStart() is to set the spell's timer
            // to the value passed as parameter. However for channeled dot
            // clipping to work properly, we need to roll over the duration. So
            // we need to re-update timer with a proper value.
  ...
  for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS;
       ++effectNumber)
    if (effectMask & (1 << effectNumber))
      HandleEffects(unit, nullptr, nullptr, nullptr, effectNumber,
                    SPELL_EFFECT_HANDLE_HIT_TARGET);
  ...
  if (!m_hitTriggerSpells.empty()) {
    ...
    for (auto i = m_hitTriggerSpells.begin(); i != m_hitTriggerSpells.end();
      ...
      if (CanExecuteTriggersOnHit(effMask, i->triggeredByAura) &&
        ...
        m_caster->CastSpell(unit, i->triggeredSpell->Id,
...
void Spell::DoAllEffectOnTarget(GOTargetInfo *target) {
  ...
  for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS;
       ++effectNumber)
    if (effectMask & (1 << effectNumber))
      HandleEffects(nullptr, nullptr, go, nullptr, effectNumber,
                    SPELL_EFFECT_HANDLE_HIT_TARGET);
...
void Spell::DoAllEffectOnTarget(ItemTargetInfo *target) {
  ...
  for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS;
       ++effectNumber)
    if (effectMask & (1 << effectNumber))
      HandleEffects(nullptr, target->item, nullptr, nullptr, effectNumber,
                    SPELL_EFFECT_HANDLE_HIT_TARGET);
...
void Spell::DoAllEffectOnTarget(CorpseTargetInfo *target) {
  ...
  for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS;
       ++effectNumber)
    if (effectMask & (1 << effectNumber))
      HandleEffects(nullptr, nullptr, nullptr, corpse, effectNumber,
                    SPELL_EFFECT_HANDLE_HIT_TARGET);
...
bool Spell::UpdateChanneledTargetList() {
  ...
  if (channelAuraMask) {
    ...
    range += std::min(
        ...
        range * 0.1f); // 10% but no more than MAX_SPELL_RANGE_TOLERANCE
  }
...
void Spell::prepare(SpellCastTargets const &targets,
  ...
  if (m_caster->IsControlledByPlayer() && !m_spellInfo->IsPassive() &&
  ...
  // Don't allow to interrupt channeled spells that are not allowed to be
  // cancelled
  if (Spell *curSpell = m_caster->GetCurrentSpell(CURRENT_CHANNELED_SPELL)) {
    if (curSpell->m_spellInfo->HasAttribute(SPELL_ATTR0_CANT_CANCEL) &&
        !m_spellInfo->HasAttribute(SPELL_ATTR4_CAN_CAST_WHILE_CASTING)) {
  ...
  if ((_triggeredCastFlags & TRIGGERED_IGNORE_COMBO_POINTS) || m_CastItem ||
      !m_caster->IsMovedByClient())
    m_needComboPoints = false;

  MountResult mountResult = MountResult::Ok;
  uint32 param1 = 0, param2 = 0;
  SpellCastResult result = CheckCast(true, &param1, &param2, &mountResult);
  // target is checked in too many locations and with different results to
  // handle each of them handle just the general SPELL_FAILED_BAD_TARGETS result
  // which is the default result for most DBC target checks
  if (_triggeredCastFlags & TRIGGERED_IGNORE_TARGET_CHECK &&
  ...
  // don't allow channeled spells / spells with cast time to be cast while
  // moving exception are only channeled spells that have no casttime and
  // SPELL_ATTR5_CAN_CHANNEL_WHEN_MOVING (even if they are interrupted on
  // moving, spells with almost immediate effect get to have their effect
  // processed before movement interrupter kicks in) don't cancel spells which
  // are affected by a SPELL_AURA_CAST_WHILE_WALKING effect
  if (((m_spellInfo->IsChanneled() || m_casttime) &&
      ...
      !m_caster->HasAuraTypeWithAffectMask(SPELL_AURA_CAST_WHILE_WALKING,
                                           m_spellInfo) &&
    ...
    if ((m_spellInfo->IsChanneled() || m_casttime) &&
        ...
        m_caster->isMoving() &&
        m_spellInfo->InterruptFlags.HasFlag(SpellInterruptFlags::Movement)) {
      // 1. Has casttime, 2. Or doesn't have flag to allow movement during
      // channel
  ...
  if (m_casttime > 0 && m_caster->IsCreature() &&
      !m_spellInfo->IsNextMeleeSwingSpell() && !IsAutoRepeat() &&
  ...
  if ((_triggeredCastFlags & TRIGGERED_CAST_DIRECTLY) &&
    ...
    // commented out !m_spellInfo->StartRecoveryTime, it forces instant spells
    // with global cooldown to be processed in spell::update as a result a spell
    // that passed CheckCast and should be processed instantly may suffer from
    // this delayed process the easiest bug to observe is LoS check in
    // AddUnitTarget, even if spell passed the CheckCast LoS check the situation
    // can change in spell::update because target could be relocated in the
    // meantime, making the spell fly to the air (no targets can be registered,
    // so no effects processed, nothing in combat log)
    if (!m_casttime &&
        /*!m_spellInfo->StartRecoveryTime && */ GetCurrentContainer() ==
  ...
  case SPELL_STATE_CASTING:
    for (std::list<TargetInfo>::const_iterator ihit =
      ...
      if (interruptingSpell != nullptr) {
        // However if we are refreshing a channeled spell, only removed owned
        // aura if the new target isn't the old one.
        if (m_spellInfo->Id == interruptingSpell->m_spellInfo->Id &&
            m_spellInfo->IsChanneled()) {
          // Different target, ease up on the core and already remove auras
          // if same target, do nothing. If we miss with the new spell, auras
          // will get removed anyways
          removeOwnedAura = (*ihit).targetGUID !=
                            interruptingSpell->m_targets.GetUnitTargetGUID();
        }
      }
    ...
    // Only send channel updates if we are interrupt with a spell that is not
    // the one currently channeled.
    sendChannelUpdate &= interruptingSpell == nullptr ||
                         m_spellInfo->Id != interruptingSpell->m_spellInfo->Id;
...
void Spell::_cast(bool skipCheck) {
  ...
  if (!skipCheck) {
    auto cleanupSpell = [this, modOwner](SpellCastResult res,
                                         uint32 *p1 = nullptr,
                                         uint32 *p2 = nullptr) {
      ...
      if (modOwner) {
        if (TradeData *my_trade = modOwner->GetTradeData()) {
          if (!my_trade->IsInAcceptProcess()) {
            // Spell will be cast after completing the trade. Silently ignore at
            // this place
    ...
    // check diminishing returns (again, only after finish cast bar, tested on
    // retail)
    if (Unit *target = m_targets.GetUnitTarget()) {
      uint8 aura_effmask = 0;
      for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
      ...
      if (aura_effmask) {
        ...
        if (m_spellInfo->GetDiminishingReturnsGroupForSpell(triggered)) {
          ...
          if (type == DRTYPE_ALL ||
              (type == DRTYPE_PLAYER &&
               target->IsAffectedByDiminishingReturns())) {
            Unit *caster = m_originalCaster ? m_originalCaster : m_caster;
  ...
  if (m_spellInfo->HasAttribute(SPELL_ATTR1_DISMISS_PET))
    if (Creature *pet =
            ObjectAccessor::GetCreature(*m_caster, m_caster->GetPetGUID()))
  ...
  if (Player *player = m_caster->ToPlayer()) {
    if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CAST_ITEM) && m_CastItem) {
      player->StartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_ITEM,
                                    m_CastItem->GetEntry());
      player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_USE_ITEM,
                                        m_CastItem->GetEntry());
    }

    player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL,
                                      m_spellInfo->Id);
  }
  ...
  if ((m_spellInfo->Speed > 0.0f && !m_spellInfo->IsChanneled()) ||
      m_spellInfo->HasAttribute(SPELL_ATTR4_UNK4)) {
  ...
  if (modOwner) {
    ...
    // Clear spell cooldowns after every spell is cast if .cheat cooldown is
    // enabled.
  ...
  uint32 procAttacker = m_procAttacker;
  if (!procAttacker &&
      !m_spellInfo->HasAttribute(SPELL_ATTR3_CANT_TRIGGER_CASTER_PROCS)) {
    if (m_spellInfo->HasAttribute(SPELL_ATTR3_TREAT_AS_PERIODIC))
      procAttacker |= PROC_FLAG_DEAL_PERIODIC;
    else if (m_spellInfo->HasAttribute(SPELL_ATTR0_ABILITY)) {
      if (IsPositive())
  ...
  if (!(_triggeredCastFlags & TRIGGERED_IGNORE_AURA_INTERRUPT_FLAGS) &&
      !m_spellInfo->HasAttribute(
          SPELL_ATTR2_IGNORE_ACTION_AURA_INTERRUPT_FLAGS))
    m_originalCaster->RemoveAurasWithInterruptFlags(
        SpellAuraInterruptFlags::ActionDelayed);

  m_originalCaster->RemoveAurasWithInterruptFlags(
      SpellAuraInterruptFlags::ActionDelayed);
  m_originalCaster->ProcSkillsAndAuras(
      nullptr, procAttacker, PROC_FLAG_NONE, PROC_SPELL_TYPE_MASK_ALL,
      PROC_SPELL_PHASE_CAST, hitMask, this, nullptr, nullptr);

  // Call CreatureAI hook OnSuccessfulSpellCast
  if (Creature *caster = m_originalCaster->ToCreature())
    if (CreatureAI *casterAI = caster->AI())
      casterAI->OnSpellCastFinished(GetSpellInfo(),
...
void Spell::handle_immediate() {
  ...
  if (m_spellInfo->IsChanneled()) {
    ...
    if (duration > 0) {
      ...
      m_caster->AddInterruptMask(m_spellInfo->ChannelInterruptFlags,
                                 m_spellInfo->ChannelInterruptFlags2);
  ...
  // consider spell hit for some spells without target, so they may proc on
  // finish phase correctly
...
void Spell::_handle_immediate_phase() {
  ...
  for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j) {
    ...
    HandleEffects(nullptr, nullptr, nullptr, nullptr, j,
...
void Spell::_handle_finish_phase() {
  if (m_caster->IsMovedByClient()) {
    ...
    if (m_spellInfo->PowerType == POWER_HOLY_POWER &&
        mover->getClass() == CLASS_PALADIN)
      HandleHolyPower(mover);
  }

  if (m_caster->m_extraAttacks &&
      m_spellInfo->HasEffect(SPELL_EFFECT_ADD_EXTRA_ATTACKS)) {
    if (Unit *victim = ObjectAccessor::GetUnit(
            *m_caster, m_targets.GetOrigUnitTargetGUID()))
  ...
  m_originalCaster->ProcSkillsAndAuras(
      nullptr, procAttacker, PROC_FLAG_NONE, PROC_SPELL_TYPE_MASK_ALL,
      PROC_SPELL_PHASE_FINISH, m_hitMask, this, nullptr, nullptr);
}
...
void Spell::update(uint32 difftime) {
  ...
  if (m_targets.GetUnitTargetGUID() && !m_targets.GetUnitTarget()) {
    LOG_DEBUG("spells", "Spell %u is cancelled due to removal of target.",
              m_spellInfo->Id);
    cancel();
    return;
  }
  ...
  // with the exception of spells affected with SPELL_AURA_CAST_WHILE_WALKING
  // effect
  if ((m_caster->GetTypeId() == TYPEID_PLAYER && m_timer != 0) &&
    ...
    if (!m_spellInfo->IsNextMeleeSwingSpell() && !IsAutoRepeat() &&
        !IsTriggered() &&
        !(IsChannelActive() && m_spellInfo->IsMoveAllowedChannel())) {
      // if charmed by creature, trust the AI not to cheat and allow the cast to
      // proceed
      // @todo this is a hack, "creature" movesplines don't differentiate
      // turning/moving right now however, checking what type of movement the
      // spline is for every single spline would be really expensive
  ...
  case SPELL_STATE_CASTING: {
    ...
    if (m_timer == 0) {
      ...
      // We call the hook here instead of in Spell::finish because we only want
      // to call it for completed channeling. Everything else is handled by
      // interrupts
      if (Creature *creatureCaster = m_caster->ToCreature())
        if (CreatureAI *casterAI = creatureCaster->AI())
          casterAI->OnSpellCastFinished(m_spellInfo,
...
void Spell::finish(bool ok, bool sendChannelUpdate /* = false */) {
  ...
  if (m_caster->GetTypeId() == TYPEID_UNIT && m_caster->IsSummon()) {
    ...
    if (spellInfo && spellInfo->SpellIconID == 2056) {
      LOG_DEBUG("spells", "Statue %d is unsummoned in spell %d finish",
                m_caster->GetGUID().GetCounter(), m_spellInfo->Id);
      // Avoid infinite loops with setDeathState(JUST_DIED) being called over
      // and over It might make sense to do this check in Unit::setDeathState()
      // and all overloaded functions
...
void Spell::WriteCastResultInfo(WorldPacket &data, Player *caster,
  ...
  case SPELL_FAILED_TOO_MANY_OF_ITEM: {
    if (param1)
      ...
      uint32 item = 0;
      for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS && !item;
           ++effIndex)
  ...
  case SPELL_FAILED_PREVENTED_BY_MECHANIC:
    data << uint32(
        spellInfo->GetAllEffectsMechanicMask()); // SpellMechanic.dbc id
    break;
...
void Spell::SendCastResult(SpellCastResult result, uint32 *param1 /*= nullptr*/,
                           uint32 *param2 /*= nullptr*/) const {
  if (result == SPELL_CAST_OK)
    return;
  ...
  if (m_caster->ToPlayer()
          ->IsLoading()) // don't send cast results at loading time
    return;

  SendCastResult(m_caster->ToPlayer(), m_spellInfo, m_cast_count, result,
                 m_customError, param1, param2);
}
...
void Spell::SendSpellStart() {
  ...
  uint32 schoolImmunityMask =
      m_casttime != 0 ? m_caster->GetSchoolImmunityMask() : 0;
  uint32 mechanicImmunityMask =
      m_casttime != 0 ? m_spellInfo->GetMechanicImmunityMask(m_caster) : 0;
  ...
  if (castFlags & CAST_FLAG_POWER_LEFT_SELF)
    castData.RemainingPower =
        m_caster->GetPower(Powers(m_spellInfo->PowerType));
  ...
  if (castFlags & CAST_FLAG_RUNE_LIST) // rune cooldowns list
  {
    ...
    if (Player *player = m_caster->ToPlayer()) {
      ...
      castData.RemainingRunes->Count =
          player->GetRunesState(); // runes state after
      for (uint8 i = 0; i < MAX_RUNES; ++i) {
        // float casts ensure the division is performed on floats as we need
        // float result
        float baseCd = float(uint32(RUNE_BASE_COOLDOWN));
        castData.RemainingRunes->Cooldowns.push_back(
            (baseCd - float(player->GetRuneCooldown(i))) / baseCd *
...
void Spell::SendSpellGo() {
  ...
  if (castFlags & CAST_FLAG_RUNE_LIST) // rune cooldowns list
  {
    ...
    /// @todo There is a crash caused by a spell with CAST_FLAG_RUNE_LIST cast
    /// by a creature
    // The creature is the mover of a player, so HandleCastSpellOpcode uses it
    // as the caster
    if (Player *player = m_caster->ToPlayer()) {
      castData.RemainingRunes->Start = m_runesState; // runes state before
      castData.RemainingRunes->Count =
...
void Spell::UpdateSpellCastDataTargets(
    WorldPackets::Spells::SpellHitInfo &data) {
  ...
  for (TargetInfo &targetInfo : m_UniqueTargetInfo) {
    ...
    if (targetInfo.missCondition == SPELL_MISS_NONE) // hits
    {
      ...
      data.MissStatus.emplace_back(targetInfo.targetGUID,
...
void Spell::UpdateSpellHealPrediction(
  ...
  /*
  * @todo: implement
  if (AuraEffect const* beaconOfLightEffect =
  m_caster->GetDummyAuraEffect(SPELLFAMILY_PALADIN, 3032, EFFECT_0))
  {
      if (beaconOfLightEffect->GetCasterGUID() == m_caster->GetGUID())
      {
          predict.Type = 2;
          predict.BeaconGUID =
  beaconOfLightEffect->GetBase()->GetUnitOwner()->GetGUID();
      }
  }
  */
  ...
  for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex) {
    ...
    case SPELL_EFFECT_HEAL_PCT:
      predict.Points += m_caster->SpellHealingBonusDone(
          target, m_spellInfo, effect.CalcValue(m_caster, nullptr, target),
          HEAL, effIndex);
      break;
    case SPELL_EFFECT_APPLY_AURA: {
      ...
      case SPELL_AURA_PERIODIC_HEAL:
        predict.Points +=
            m_caster->SpellHealingBonusDone(
                target, m_spellInfo,
                effect.CalcValue(m_caster, nullptr, target), HEAL, effIndex) *
            m_spellInfo->GetMaxTicks();
        break;
      case SPELL_AURA_PERIODIC_TRIGGER_SPELL:
        if (SpellInfo const *spell =
                sSpellMgr->GetSpellInfo(effect.TriggerSpell))
          for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            if (spell->Effects[i].Effect == SPELL_EFFECT_HEAL ||
              ...
              predict.Points +=
                  m_caster->SpellHealingBonusDone(
                      target, spell,
                      spell->Effects[i].CalcValue(m_caster, nullptr, target),
                      HEAL, i) *
                  m_spellInfo->GetMaxTicks();
        break;
      default:
...
void Spell::SendChannelStart(uint32 duration) {
  ...
  if (!channelTarget && !m_spellInfo->NeedsExplicitUnitTarget()) {
    ...
    if (m_UniqueTargetInfo.size() + m_UniqueGOTargetInfo.size() == 1)
      ...
      // We assume that when we have a channel spell with more than one unique
      // unit target that there is a caster and a target. We don't want the
      // caster but the target so we are looking for a non-caster entry within
      // the target list
  ...
  if (castFlags & CAST_FLAG_IMMUNITY) {
    ...
    packet.InterruptImmunities->SchoolImmunities =
        schoolImmunityMask; // CastSchoolImmunities
    packet.InterruptImmunities->Immunities =
        mechanicImmunityMask; // CastImmunities
  }
...
void Spell::SendResurrectRequest(Player *target) {
  // Get creature name for resurrection. Player names are being received via
  // ResurrectOffererGUID and send a nul-terminator only.
  ...
  resurrectRequest.Sickness =
      !m_caster->IsPlayer() &&
      m_caster->IsSpiritHealer(); // "you'll be afflicted with resurrection
                                  // sickness"
  resurrectRequest.UseTimer =
      !m_spellInfo->HasAttribute(SPELL_ATTR3_IGNORE_RESURRECTION_TIMER);
...
void Spell::TakeCastItem() {
  ...
  if (!proto) {
    ...
    // prototype
    LOG_ERROR("spells", "Cast item has no item prototype %s",
              m_CastItem->GetGUID().ToString().c_str());
    return;
  }
...
void Spell::TakePower() {
  ...
  bool hit = true;
  if (m_caster->GetTypeId() == TYPEID_PLAYER &&
      m_spellInfo->HasAttribute(SPELL_ATTR1_DISCOUNT_POWER_ON_MISS)) {
  ...
  if (powerType == POWER_RUNE && m_caster->IsPlayer()) {
    ...
    if (ObjectGuid targetGUID = m_targets.GetUnitTargetGUID()) {
      auto itr =
          std::find_if(m_UniqueTargetInfo.begin(), m_UniqueTargetInfo.end(),
                       [&targetGUID](Spell::TargetInfo const &targetInfo) {
...
void Spell::TakeAmmo() {
  if (m_attackType == RANGED_ATTACK && m_caster->GetTypeId() == TYPEID_PLAYER) {
    ...
    if (!pItem || pItem->IsBroken() ||
        pItem->GetTemplate()->GetSubClass() == ITEM_SUBCLASS_WEAPON_WAND)
      return;
...
void Spell::TakeRunePower(SpellMissInfo hitInfo) {
  ...
  std::array<int32, NUM_RUNE_TYPES> runeCost =
  ...
  // If the spell does not require any rune because of power cost spell mods,
  // the Death Knight wont gain any Runic Power (Freezing Fog for example)
  if (!consumeRunes && !runeCostData->NoRuneCost())
    return;

  consumeRunes = hitInfo == SPELL_MISS_NONE || hitInfo == SPELL_MISS_MISS;
  // Death Strike is special - it consumes runes on dodge and parry as well,
  // because the healing component still goes through. There is no flag to
  // control this and it's probably tied to how Blizzard applies spell effects
  // and auras, which we KNOW we do wrong.
  ...
  for (uint32 i = 0; i < MAX_RUNES; ++i) {
    ...
    if (!player->GetRuneCooldown(i) && runeCost[rune] > 0) {
      player->SetRuneCooldown(i, consumeRunes ? uint32(RUNE_BASE_COOLDOWN)
  ...
  if (consumeRunes) {
    if (int32 runicPowerGain = runeCostData->RunicPower *
                               sWorld->getRate(RATE_POWER_RUNICPOWER_INCOME)) {
      Unit::AuraEffectList const &bonusPct =
          m_caster->GetAuraEffectsByType(SPELL_AURA_MOD_RUNE_REGEN_SPEED);
      ...
      if (Player *modOwner = m_caster->GetSpellModOwner())
        modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_COST, runicPowerGain,
                                const_cast<Spell *>(this));
...
void Spell::HandleThreatSpells() {
  ...
  for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin();
    ...
    // for negative spells threat gets distributed among affected targets
      ...
      target->GetThreatManager().AddThreat(m_caster, threatToAdd, m_spellInfo,
                                           true);
    }
  }
  LOG_DEBUG("spells",
            "Spell %u, added an additional %f threat for %s %u target(s)",
            m_spellInfo->Id, threat, IsPositive() ? "assisting" : "harming",
...
void Spell::HandleHolyPower(Player *caster) {
  ...
  m_powerCost = caster->GetPower(
      POWER_HOLY_POWER); // Always use all the holy power we have
  ...
  if (ObjectGuid targetGUID = m_targets.GetUnitTargetGUID()) {
    for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin();
      ...
      if (ihit->targetGUID == targetGUID) {
        if (ihit->missCondition != SPELL_MISS_NONE &&
            ihit->missCondition != SPELL_MISS_MISS)
          hit = false;
...
void Spell::HandleEffects(Unit *pUnitTarget, Item *pItemTarget,
                          GameObject *pGoTarget, Corpse *pCorpseTarget,
                          uint32 i, SpellEffectHandleMode mode) {
...
SpellCastResult Spell::CheckCast(bool strict, uint32 *param1 /*= nullptr*/,
                                 uint32 *param2 /*= nullptr*/,
                                 MountResult *mountResult /*= nullptr*/) {
  // check death state
  if (!m_caster->IsAlive() && !m_spellInfo->IsPassive() &&
      !(m_spellInfo->HasAttribute(SPELL_ATTR0_CASTABLE_WHILE_DEAD) ||
        (IsTriggered() && !m_triggeredByAuraSpell)))
    return SPELL_FAILED_CASTER_DEAD;

  // Prevent cheating in case the player has an immunity effect and tries to
  // interact with a non-allowed gameobject. The error message is handled by the
  // client so we don't report anything here
  ...
  if (!m_spellInfo->IsPassive()) {
    if (m_caster->GetTypeId() == TYPEID_PLAYER) {
      ...
      if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_AURASTATE)) {
        // These two auras check SpellFamilyName defined by dbc class data
        // instead of current spell SpellFamilyName
        if (m_caster->HasAuraType(
            ...
            !m_caster->HasAuraTypeWithFamilyFlags(
                SPELL_AURA_DISABLE_CASTING_EXCEPT_ABILITIES,
                sChrClassesStore.AssertEntry(m_caster->getClass())
                    ->SpellClassSet,
      ...
      if (!IsIgnoringCooldowns() && m_caster->ToPlayer()->GetLastPotionId() &&
          m_CastItem &&
  ...
  if (!(_triggeredCastFlags & TRIGGERED_IGNORE_BLOCKED_SPELL_FAMILY))
    for (AuraEffect const *blocked :
         m_caster->GetAuraEffectsByType(SPELL_AURA_BLOCK_SPELL_FAMILY))
      if (!blocked->GetMiscValue() ||
          uint32(blocked->GetMiscValue()) == m_spellInfo->SpellFamilyName)
  ...
  // update code) Do not cancel spells which are affected by a
  // SPELL_AURA_CAST_WHILE_WALKING effect
  if (m_caster->GetTypeId() == TYPEID_PLAYER &&
    ...
    if ((!m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING_FAR) ||
         m_spellInfo->Effects[0].Effect != SPELL_EFFECT_STUCK) &&
        (IsAutoRepeat() ||
         m_spellInfo->HasAuraInterruptFlag(SpellAuraInterruptFlags::Standing)))
  ...
  // check spell cast conditions from database
  {
    ConditionSourceInfo condInfo =
        ConditionSourceInfo(m_caster, m_targets.GetObjectTarget());
    if (!sConditionMgr->IsObjectMeetingNotGroupedConditions(
            CONDITION_SOURCE_TYPE_SPELL, m_spellInfo->Id, condInfo)) {
  ...
  // to caster instead also, such casts shouldn't be sent to client
  if (!(m_spellInfo->IsPassive() && (!m_targets.GetUnitTarget() ||
    ...
    if (m_originalCaster &&
        m_caster->GetEntry() !=
            WORLD_TRIGGER) // Do a simplified check for gameobject casts
      caster = m_originalCaster;

    SpellCastResult castResult = m_spellInfo->CheckExplicitTarget(
        caster, m_targets.GetObjectTarget(), m_targets.GetItemTarget());
    if (castResult != SPELL_CAST_OK)
      return castResult;
  }

  if (Unit *target = m_targets.GetUnitTarget()) {
    SpellCastResult castResult = m_spellInfo->CheckTarget(
        m_caster, target,
        m_caster->GetEntry() ==
            WORLD_TRIGGER); // skip stealth checks for GO casts
    if (castResult != SPELL_CAST_OK)
      return castResult;
    ...
    if (target != m_caster) {
      ...
      if (m_caster->GetEntry() !=
          WORLD_TRIGGER) // Ignore LOS for gameobjects casts (wrongly cast by a
                         // trigger)
      {
        WorldObject *losTarget = m_caster;
        if (IsTriggered() && m_triggeredByAuraSpell)
          if (DynamicObject *dynObj =
                  m_caster->GetDynObject(m_triggeredByAuraSpell->Id))
            losTarget = dynObj;
        ...
        if (!m_spellInfo->HasAttribute(SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) &&
            ...
            !DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id,
                                       nullptr, SPELL_DISABLE_LOS) &&
            !target->IsWithinLOSInMap(losTarget, LINEOFSIGHT_ALL_CHECKS,
                                      VMAP::ModelIgnoreFlags::M2))
  ...
  if (m_targets.HasDst()) {
    ...
    if (!m_spellInfo->HasAttribute(SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) &&
        !m_spellInfo->HasAttribute(SPELL_ATTR5_SKIP_CHECKCAST_LOS_CHECK) &&
        !DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id, nullptr,
                                   SPELL_DISABLE_LOS) &&
  ...
  if (m_caster->GetTypeId() == TYPEID_UNIT ||
    ...
    SpellCastResult locRes = m_spellInfo->CheckLocation(
        m_caster->GetMapId(), zone, area, m_caster->ToPlayer());
  ...
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    ...
    case SPELL_EFFECT_CHARGE: {
      ...
      if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_AURAS) &&
          m_caster->HasUnitState(UNIT_STATE_ROOT))
      ...
      if (GetSpellInfo()->NeedsExplicitUnitTarget()) {
        ...
        // first we must check to see if the target is in LoS. A path can
        // usually be built but LoS matters for charge spells
        if (!target->IsWithinLOSInMap(
                m_caster)) // Do full LoS/Path check. Don't exclude m2
          return SPELL_FAILED_LINE_OF_SIGHT;

        float objSize = target->GetCombatReach();
        float range = m_spellInfo->GetMaxRange(true, m_caster, this) * 1.5f +
        ...
        m_preGeneratedPath->ShortenPathUntilDist(PositionToVector3(target),
                                                 objSize); // move back
      }
      break;
    }
    ...
    case SPELL_EFFECT_OPEN_LOCK: {
        ...
        if (TradeData *pTrade = m_caster->ToPlayer()->GetTradeData())
          pTempItem = pTrade->GetTraderData()->GetItem(
              TradeSlots(m_targets.GetItemTargetGUID()
                             .GetRawValue())); // at this point item target guid
                                               // contains the trade slot
      ...
      if (m_spellInfo->Effects[i].TargetA.GetTarget() ==
              TARGET_GAMEOBJECT_ITEM_TARGET &&
          !m_targets.GetGOTarget() &&
          (!pTempItem || !pTempItem->GetTemplate()->GetLockID() ||
           !pTempItem->IsLocked()))
        return SPELL_FAILED_BAD_TARGETS;
      ...
      if (m_spellInfo->Id != 1842 ||
        ...
        if (m_caster->ToPlayer()
                ->InBattleground() && // In Battleground players can use only
                                      // flags and banners
    ...
    case SPELL_EFFECT_SUMMON: {
      ...
      case SUMMON_CATEGORY_PET:
        if (!m_spellInfo->HasAttribute(SPELL_ATTR1_DISMISS_PET) &&
            m_caster->GetPetGUID())
          return SPELL_FAILED_ALREADY_HAVE_SUMMON;
        [[fallthrough]]; //  check both GetPetGUID() and GetCharmGUID for
                         //  SUMMON_CATEGORY_PET*/
    ...
    case SPELL_EFFECT_SUMMON_PET: {
      ...
      {
        if (m_caster->GetTypeId() == TYPEID_PLAYER) {
          if (strict) // starting cast, trigger pet stun (cast by pet so it
        ...
        if (map->IsDungeon()) {
          ...
          if (map->IsRaid())
            if (InstancePlayerBind *targetBind =
                    target->GetBoundInstance(mapId, difficulty))
              if (InstancePlayerBind *casterBind =
  ...
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    ...
    case SPELL_AURA_AOE_CHARM: {
      ...
      if (Unit *target = m_targets.GetUnitTarget()) {
        ...
        if (target->GetOwner() &&
            target->GetOwner()->GetTypeId() == TYPEID_PLAYER)
      ...
      if (m_originalCaster && m_originalCaster->GetTypeId() == TYPEID_PLAYER &&
        ...
        if (AreaTableEntry const *area =
          ...
          if (area->Flags & AREA_FLAG_NO_FLY_ZONE || (Bf && !Bf->CanFlyIn()))
            return (_triggeredCastFlags & TRIGGERED_DONT_REPORT_CAST_ERROR)
                       ? SPELL_FAILED_DONT_REPORT
    ...
    case SPELL_AURA_PERIODIC_MANA_LEECH: {
      ...
      if (m_targets.GetUnitTarget() &&
          m_targets.GetUnitTarget()->GetPowerType() != POWER_MANA)
        return SPELL_FAILED_BAD_TARGETS;
      else if (!m_targets.GetUnitTarget() &&
               m_caster->GetPowerType() != POWER_MANA)
        return SPELL_FAILED_BAD_TARGETS;
    ...
    if (!nonAuraEffectMask && (approximateAuraEffectMask & (1 << i)) &&
      ...
      if (Unit *target = m_targets.GetUnitTarget())
        if (!target->IsHighestExclusiveAuraEffect(
                m_spellInfo, AuraType(m_spellInfo->Effects[i].ApplyAuraName),
                m_spellInfo->Effects[i].CalcValue(
                    m_caster, &m_spellValue->EffectBasePoints[i]),
  ...
  if (m_targets.GetTargetMask() & TARGET_FLAG_TRADE_ITEM) {
    ...
    // Item target guid contains trade slot until
    // m_targets.UpdateTradeSlotItem() is called
...
SpellCastResult Spell::CheckPetCast(Unit *target) {
  ...
  // this would be zero until ::prepare normally, we set it here (it gets reset
  // in ::prepare)
  ...
  if (m_spellInfo->StartRecoveryCategory > 0)
    if (m_caster->GetCharmInfo() &&
        m_caster->GetSpellHistory()->HasGlobalCooldown(m_spellInfo))
      return SPELL_FAILED_NOT_READY;
...
SpellCastResult Spell::CheckCasterAuras(uint32 *param1) const {
  ...
  // these attributes only show the spell as usable on the client when it has
  // related aura applied still they need to be checked against certain
  // mechanics

  // SPELL_ATTR5_USABLE_WHILE_STUNNED by default only MECHANIC_STUN (ie no
  // sleep, knockout, freeze, etc.)
  bool usableWhileStunned =
  ...
  bool usableWhileFeared =
      m_spellInfo->HasAttribute(SPELL_ATTR5_USABLE_WHILE_FEARED);

  // SPELL_ATTR5_USABLE_WHILE_CONFUSED by default only disorient (ie no
  // polymorph)
  bool usableWhileConfused =
  ...
  if ((m_spellInfo->Id == 33206 || m_spellInfo->Id == 47788) &&
      !m_caster->HasAura(63248))
  ...
  // (ie not when it's called from a script) Breaks for example PlayerAI when
  // charmed
  /*
  if (m_caster->GetCharmerGUID())
  {
      if (Unit* charmer = m_caster->GetCharmer())
          if (charmer->GetUnitBeingMoved() != m_caster &&
  !CheckSpellCancelsCharm(param1)) result = SPELL_FAILED_CHARMED;
  }
  */

  // spell has attribute usable while having a cc state, check if caster has
  // allowed mechanic auras, another mechanic types must prevent cast spell
  auto mechanicCheck = [&](AuraType type) -> SpellCastResult {
    ...
    for (AuraEffect const *aurEff : auras) {
      uint32 const mechanicMask =
          aurEff->GetSpellInfo()->GetAllEffectsMechanicMask();
      if (mechanicMask &&
          !(mechanicMask & GetSpellInfo()->GetAllowedMechanicMask())) {
        foundNotMechanic = true;

        // fill up aura mechanic info to send client proper error message
        if (param1) {
          *param1 =
              aurEff->GetSpellInfo()->Effects[aurEff->GetEffIndex()].Mechanic;
          if (!*param1)
...
bool Spell::CheckSpellCancelsAuraEffect(AuraType auraType,
                                        uint32 *param1) const {
  // Checking auras is needed now, because you are prevented by some state but
  // the spell grants immunity.
  Unit::AuraEffectList const &auraEffects =
...
SpellCastResult Spell::CheckArenaAndRatedBattlegroundCastRules() const {
  ...
  if (isRatedBattleground &&
      m_spellInfo->HasAttribute(SPELL_ATTR9_USABLE_IN_RATED_BATTLEGROUNDS))
    return SPELL_CAST_OK;
  ...
  if (m_spellInfo->HasAttribute(SPELL_ATTR4_NOT_USABLE_IN_ARENA_OR_RATED_BG))
    return isArena ? SPELL_FAILED_NOT_IN_ARENA
  ...
  if (isArena &&
      spellCooldown > 10 * MINUTE * IN_MILLISECONDS) // not sure if still needed
    return SPELL_FAILED_NOT_IN_ARENA;
...
bool Spell::CanAutoCast(Unit *target) {
  ...
  for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    ...
    AuraType const &auraType =
        AuraType(GetSpellInfo()->Effects[i].ApplyAuraName);
    Unit::AuraEffectList const &auras = target->GetAuraEffectsByType(auraType);
    for (Unit::AuraEffectList::const_iterator auraIt = auras.begin();
         auraIt != auras.end(); ++auraIt) {
      if (GetSpellInfo()->Id == (*auraIt)->GetSpellInfo()->Id)
        return false;

      switch (sSpellMgr->CheckSpellGroupStackRules(GetSpellInfo(),
                                                   (*auraIt)->GetSpellInfo())) {
      ...
      case SPELL_GROUP_STACK_RULE_EXCLUSIVE_FROM_SAME_CASTER:
        if (GetCaster() == (*auraIt)->GetCaster())
          return false;
        break;
      case SPELL_GROUP_STACK_RULE_EXCLUSIVE_SAME_EFFECT: // this one has further
                                                         // checks, but i don't
                                                         // think they're
                                                         // necessary for
                                                         // autocast logic
  ...
  if (result == SPELL_CAST_OK || result == SPELL_FAILED_UNIT_NOT_INFRONT) {
    // do not check targets for ground-targeted spells (we target them on top of
...
SpellCastResult Spell::CheckRange(bool strict) const {
  ...
  if (m_spellInfo->RangeEntry &&
      m_spellInfo->RangeEntry->Flags != SPELL_RANGE_MELEE && !strict)
    maxRange += std::min(
        MAX_SPELL_RANGE_TOLERANCE,
        maxRange * 0.1f); // 10% but no more than MAX_SPELL_RANGE_TOLERANCE
  ...
  if (target && target != m_caster) {
    ...
    if (minRange > 0.0f && m_caster->GetExactDistSq(target) < minRange &&
        !target->HasFlag(UNIT_FIELD_FLAGS_2,
                         UNIT_FLAG2_IGNORE_SPELL_MIN_RANGE_RESTRICTIONS))
  ...
  if (m_targets.HasDst() && !m_targets.HasTraj()) {
    if (m_caster->GetExactDistSq(m_targets.GetDstPos()) > maxRange)
      return SPELL_FAILED_OUT_OF_RANGE;
    if (minRange > 0.0f &&
        m_caster->GetExactDistSq(m_targets.GetDstPos()) < minRange)
      return SPELL_FAILED_TOO_CLOSE;
  }
...
std::pair<float, float> Spell::GetMinMaxRange(bool strict) const {
  ...
  if (m_spellInfo->RangeEntry) {
    ...
    if (m_spellInfo->RangeEntry->Flags & SPELL_RANGE_MELEE) {
      rangeMod = m_caster->GetMeleeRange(
          target
              ? target
              : m_caster); // when the target is not a unit, take the caster's
                           // combat reach as the target's combat reach.
    } else {
      float meleeRange = 0.0f;
      if (m_spellInfo->RangeEntry->Flags & SPELL_RANGE_RANGED)
        meleeRange = m_caster->GetMeleeRange(
            target
                ? target
                : m_caster); // when the target is not a unit, take the caster's
                             // combat reach as the target's combat reach.
      ...
      minRange =
          m_caster->GetSpellMinRangeForTarget(target, m_spellInfo) + meleeRange;
      maxRange = m_caster->GetSpellMaxRangeForTarget(target, m_spellInfo);

      if (target || m_targets.GetCorpseTarget()) {
        rangeMod =
            m_caster->GetCombatReach() +
            (target ? target->GetCombatReach() : m_caster->GetCombatReach());

        if (minRange > 0.0f &&
            !(m_spellInfo->RangeEntry->Flags & SPELL_RANGE_RANGED))
          minRange += rangeMod;
      }
    }
  ...
  if (m_spellInfo->HasAttribute(SPELL_ATTR0_REQ_AMMO) &&
      m_caster->GetTypeId() == TYPEID_PLAYER)
    if (Item *ranged =
            m_caster->ToPlayer()->GetWeaponForAttack(RANGED_ATTACK, true))
...
SpellCastResult Spell::CheckPower() const {
  ...
  if (m_spellInfo->PowerType >= MAX_POWERS) {
    LOG_ERROR("spells", "Spell::CheckPower: Unknown power type '%d'",
...
SpellCastResult Spell::CheckItems(uint32 *param1 /*= nullptr*/,
  ...
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    ...
    case SPELL_EFFECT_CREATE_ITEM_2: {
      ...
      Unit *target =
          m_targets.GetUnitTarget() ? m_targets.GetUnitTarget() : m_caster;
      if (target && target->GetTypeId() == TYPEID_PLAYER && !IsTriggered() &&
    ...
    case SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC: {
      ...
      if (targetItem->GetTemplate()->GetBaseItemLevel() <
      ...
      if (targetItem->GetOwner() != m_caster) {
        ...
        if (enchantEntry->GetFlags().HasFlag(
                SpellItemEnchantmentFlags::Soulbound))
          return SPELL_FAILED_NOT_TRADEABLE;
      }
      break;
    }
    case SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY: {
      ...
      // Apply item level restriction if the enchanting spell has max level
      // restrition set
      if (m_CastItem && m_spellInfo->MaxLevel > 0) {
        if (int32(item->GetTemplate()->GetBaseItemLevel()) <
            m_CastItem->GetTemplate()->GetRequiredLevel())
          return SPELL_FAILED_LOWLEVEL;
    ...
    case SPELL_EFFECT_DISENCHANT: {
      ...
      if (item_quality > 4 || item_quality < 2)
        return SPELL_FAILED_CANT_BE_DISENCHANTED;
      if (itemProto->GetClass() != ITEM_CLASS_WEAPON &&
          itemProto->GetClass() != ITEM_CLASS_ARMOR)
        return SPELL_FAILED_CANT_BE_DISENCHANTED;
    ...
    case SPELL_EFFECT_PROSPECTING: {
      ...
      uint32 item_prospectingskilllevel =
          item->GetTemplate()->GetRequiredSkillRank();
    ...
    case SPELL_EFFECT_CREATE_MANA_GEM: {
      ...
      if (Item *pitem = player->GetItemByEntry(item_id)) {
        for (uint8 x = 0; x < pProto->Effects.size(); ++x)
          if (pProto->Effects[x].Charges != 0 &&
              pitem->GetSpellCharges(x) == pProto->Effects[x].Charges)
            return SPELL_FAILED_ITEM_AT_MAX_CHARGES;
      }
      break;
    }
...
void Spell::Delayed() // only called in DealDamage()
{
  ...
  LOG_DEBUG("spells", "Spell %u partially interrupted for (%d) ms at damage",
...
bool Spell::UpdatePointers() {
  ...
  if (m_castItemGUID && m_caster->GetTypeId() == TYPEID_PLAYER) {
    ...
    // check if the item is really the same, in case it has been wrapped for
    // example
...
bool Spell::CheckEffectTarget(Unit const *target, uint32 eff,
                              Position const *losPosition) const {
  ...
  {
    if (losPosition)
      return target->IsWithinLOS(
          losPosition->GetPositionX(), losPosition->GetPositionY(),
          losPosition->GetPositionZ(), LINEOFSIGHT_ALL_CHECKS,
          VMAP::ModelIgnoreFlags::M2);
    else {
      ...
      if (!caster)
        caster = m_caster;
      if (target != m_caster &&
          !target->IsWithinLOSInMap(caster, LINEOFSIGHT_ALL_CHECKS,
                                    VMAP::ModelIgnoreFlags::M2))
        return false;
    }
    break;
...
bool Spell::IsFocusDisabled() const {
  return ((_triggeredCastFlags & TRIGGERED_IGNORE_SET_FACING) != 0 ||
          (m_spellInfo->IsChanneled() && !m_spellInfo->HasAttribute(SpellAttr1(
...
bool Spell::IsAutoActionResetSpell() const {
  ...
  if (!m_casttime &&
      m_spellInfo->HasAttribute(SPELL_ATTR6_NOT_RESET_SWING_IF_INSTANT))
    return false;
  ...
  case SPELL_DAMAGE_CLASS_MAGIC:
  case SPELL_DAMAGE_CLASS_MELEE:
    // Spells with damage class magic or melee reset swing timers when any
    // interrupt flag is given
...
bool Spell::IsNeedSendToClient() const {
  return m_spellInfo->SpellVisual[0] || m_spellInfo->SpellVisual[1] ||
         m_spellInfo->IsChanneled() ||
         m_spellInfo->HasAttribute(SPELL_ATTR8_AURA_SEND_AMOUNT) ||
...
bool Spell::HaveTargetsForEffect(uint8 effect) const {
  ...
  for (std::list<CorpseTargetInfo>::const_iterator itr =
           m_UniqueCorpseTargetInfo.begin();
       itr != m_UniqueCorpseTargetInfo.end(); ++itr)
    if (itr->effectMask & (1 << effect))
      return true;
  ...
  if (m_Spell->IsDeletable()) {
    ...
    LOG_ERROR("spells",
              ...
              (m_Spell->GetCaster()->GetTypeId() == TYPEID_PLAYER ? "Player"
                                                                  : "Creature"),
              m_Spell->GetCaster()->GetGUID().GetCounter(),
...
bool SpellEvent::Execute(uint64 e_time, uint32 p_time) {
  ...
  case SPELL_STATE_DELAYED: {
    ...
    if (m_Spell->GetDelayStart() != 0) {
      ...
      {
          // evented channeled spell is processed separately, cast once after
      delay, and not destroyed till finish
          // check, if we have casting anything else except this channeled spell
      and autorepeat if (m_Spell->GetCaster()->IsNonMeleeSpellCast(false, true,
      true))
          {
          ...
          else
          {
              ...
              ((Player*)m_Spell->GetCaster())->SetSpellModTakingSpell(m_Spell,
      true);
...
void Spell::HandleLaunchPhase() {
  ...
  for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    ...
    HandleEffects(nullptr, nullptr, nullptr, nullptr, i,
...
void Spell::DoAllEffectOnLaunchTarget(TargetInfo &targetInfo,
  ...
  // This will only cause combat - the target will engage once the projectile
  // hits (in DoAllEffectOnTarget)
    ...
    if (targetInfo.effectMask & (1 << i)) {
      ...
      if (m_damage > 0) {
        if (m_spellInfo->Effects[i].IsTargetingArea() ||
            ...
            m_spellInfo->Effects[i].IsEffect(
                SPELL_EFFECT_PERSISTENT_AREA_AURA)) {
          m_damage =
              int32(float(m_damage) * unit->GetTotalAuraMultiplierByMiscMask(
                                          SPELL_AURA_MOD_AOE_DAMAGE_AVOIDANCE,
                                          m_spellInfo->SchoolMask));
          if (m_caster->GetTypeId() != TYPEID_PLAYER)
            m_damage = int32(float(m_damage) *
  ...
  if (m_originalCaster) {
    if (!critChance)
      critChance = m_originalCaster->SpellCritChanceDone(
          m_spellInfo, m_spellSchoolMask, m_attackType);
    critChance =
        unit->SpellCritChanceTaken(m_originalCaster, m_spellInfo,
                                   m_spellSchoolMask, critChance, m_attackType);
  }
  ...
  for (int j = 0; j < MAX_LOCK_CASE; ++j) {
    ...
    case LOCK_KEY_SKILL: {
      ...
      if (skillId != SKILL_NONE) {
        ...
        // add the effect base points modifier from the spell cast (cheat lock /
        // skeleton key etc.)
...
void Spell::CallScriptBeforeCastHandlers() {
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
       ++scritr) {
    (*scritr)->_PrepareScriptCall(SPELL_SCRIPT_HOOK_BEFORE_CAST);
    auto hookItrEnd = (*scritr)->BeforeCast.end(),
...
void Spell::CallScriptOnCastHandlers() {
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
    ...
    auto hookItrEnd = (*scritr)->OnCast.end(),
...
void Spell::CallScriptAfterCastHandlers() {
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
    ...
    auto hookItrEnd = (*scritr)->AfterCast.end(),
...
SpellCastResult Spell::CallScriptCheckCastHandlers() {
  ...
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
    ...
    auto hookItrEnd = (*scritr)->OnCheckCast.end(),
...
void Spell::CallScriptOnSpellStartHandlers() {
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
    ...
    auto hookItrEnd = (*scritr)->OnSpellStart.end(),
         hookItr = (*scritr)->OnSpellStart.begin();
    for (; hookItr != hookItrEnd; ++hookItr)
      (*hookItr).Call();
...
bool Spell::CallScriptEffectHandlers(SpellEffIndex effIndex,
  ...
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
       ++scritr) {
    std::list<SpellScript::EffectNameHook>::iterator effItr, effEndItr;
    ...
    for (; effItr != effEndItr; ++effItr)
      ...
      if (!(*scritr)->_IsEffectPrevented(effIndex) &&
          (*effItr).Filter(m_spellInfo, effIndex))
...
void Spell::CallScriptSuccessfulDispel(SpellEffIndex effIndex) {
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
    ...
    auto hookItrEnd = (*scritr)->OnEffectSuccessfulDispel.end(),
         hookItr = (*scritr)->OnEffectSuccessfulDispel.begin();
    for (; hookItr != hookItrEnd; ++hookItr)
...
void Spell::CallScriptBeforeHitHandlers() {
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
    ...
    auto hookItrEnd = (*scritr)->BeforeHit.end(),
...
void Spell::CallScriptOnHitHandlers() {
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
    ...
    auto hookItrEnd = (*scritr)->OnHit.end(),
...
void Spell::CallScriptAfterHitHandlers() {
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
    ...
    auto hookItrEnd = (*scritr)->AfterHit.end(),
...
void Spell::CallScriptObjectAreaTargetSelectHandlers(
  ...
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
    ...
    auto hookItrEnd = (*scritr)->OnObjectAreaTargetSelect.end(),
         hookItr = (*scritr)->OnObjectAreaTargetSelect.begin();
    for (; hookItr != hookItrEnd; ++hookItr)
      if (hookItr->Filter(m_spellInfo, effIndex) &&
...
void Spell::CallScriptObjectTargetSelectHandlers(
  ...
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
    ...
    auto hookItrEnd = (*scritr)->OnObjectTargetSelect.end(),
...
void Spell::CallScriptDestinationTargetSelectHandlers(
  ...
  for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end();
    ...
    auto hookItrEnd = (*scritr)->OnDestinationTargetSelect.end(),
...
bool Spell::CheckScriptEffectImplicitTargets(uint32 effIndex,
  ...
  for (auto itr = m_loadedScripts.begin(); itr != m_loadedScripts.end();
       ++itr) {
    auto targetSelectHookEnd = (*itr)->OnObjectTargetSelect.end(),
    ...
    for (; targetSelectHookItr != targetSelectHookEnd; ++targetSelectHookItr)
      if (((*targetSelectHookItr)
               .Filter(m_spellInfo, SpellEffIndex(effIndex)) &&
           !(*targetSelectHookItr)
                .Filter(m_spellInfo, SpellEffIndex(effIndexToCheck))) ||
          (!(*targetSelectHookItr)
                .Filter(m_spellInfo, SpellEffIndex(effIndex)) &&
           (*targetSelectHookItr)
               .Filter(m_spellInfo, SpellEffIndex(effIndexToCheck))))
    ...
    auto areaTargetSelectHookEnd = (*itr)->OnObjectAreaTargetSelect.end(),
...
bool Spell::CanExecuteTriggersOnHit(uint8 effMask,
  ...
  // proc on a cast spell with TARGET_UNIT_CASTER
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    if ((effMask & (1 << i)) &&
        (!only_on_caster ||
...
void Spell::TriggerGlobalCooldown() {
  ...
  // such mods
  if (Player *modOwner = m_caster->GetSpellModOwner())
    modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_GLOBAL_COOLDOWN, gcd,
                            this);
  ...
  if (unitTarget) {
    ...
    case TARGET_CHECK_ENEMY:
      if (unitTarget->IsTotem())
        return false;
      if (!target->IsCorpse() &&
          !_caster->_IsValidAttackTarget(unitTarget, _spellInfo))
        return false;
      break;
    case TARGET_CHECK_ALLY:
      if (unitTarget->IsTotem())
        return false;
      if (!target->IsCorpse() &&
          !_caster->_IsValidAssistTarget(unitTarget, _spellInfo))
        return false;
      break;
    case TARGET_CHECK_PARTY:
      if (unitTarget->IsTotem())
    ...
    case TARGET_CHECK_TAP:
      if (_referer->GetTypeId() != TYPEID_UNIT ||
...
bool WorldObjectSpellAreaTargetCheck::operator()(WorldObject *target) {
  if (target->ToGameObject()) {
    ...
    bool isInRange = target->ToGameObject()->IsInRange(
        _position->GetPositionX(), _position->GetPositionY(),
        _position->GetPositionZ(), _range);
    if (!isInRange)
      return false;
  } else if (target->ToUnit()) {
    float hitboxSum =
        (_spellInfo->HasAttribute(SPELL_ATTR5_TREAT_AS_AREA_EFFECT) &&
         _spellInfo->SpellFamilyName != SPELLFAMILY_GENERIC)
            ? target->ToUnit()->GetMeleeRange(_caster)
            : 0.f;
    bool isInsideCylinder =
        target->IsWithinDist2d(_position, _range + hitboxSum) &&
        std::abs(target->GetPositionZ() - _position->GetPositionZ()) <=
            (_range + hitboxSum);
    if (!isInsideCylinder)
      return false;
  }
...
WorldObjectSpellConeTargetCheck::WorldObjectSpellConeTargetCheck(
    Position const &coneSrc, float coneAngle, float range, Unit *caster,
    SpellInfo const *spellInfo, SpellTargetCheckTypes selectionType,
    ConditionContainer *condList)
    : WorldObjectSpellAreaTargetCheck(range, caster, caster, caster, spellInfo,
                                      selectionType, condList),
      _coneSrc(coneSrc), _coneAngle(coneAngle) {}
...
bool WorldObjectSpellConeTargetCheck::operator()(WorldObject *target) {
  if (_spellInfo->HasAttribute(SPELL_ATTR0_CU_CONE_BACK)) {
    ...
    if (!_caster->IsWithinBoundaryRadius(target->ToUnit()))
      ...
      if (_coneSrc.HasInArc(_coneAngle, target) !=
          G3D::fuzzyGe(_coneAngle, 0.f))
        return false;
  }
