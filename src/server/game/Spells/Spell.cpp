...
void SpellCastTargets::OutDebug() const
{
    ...
    if (m_targetMask & (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_CORPSE_MASK | TARGET_FLAG_GAMEOBJECT_MASK))
        LOG_INFO("spells", "Object target: {} ", m_objectTargetGUID);
    if (m_targetMask & TARGET_FLAG_ITEM)
        LOG_INFO("spells", "Item target: {} ", m_itemTargetGUID);
    if (m_targetMask & TARGET_FLAG_TRADE_ITEM)
        LOG_INFO("spells", "Trade item target: {} ", m_itemTargetGUID);
    if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION)
        LOG_INFO("spells", "Source location: transport guid:{} trans offset: %s position: %s", m_src._transportGUID, m_src._transportOffset.ToString().c_str(), m_src._position.ToString().c_str());
    if (m_targetMask & TARGET_FLAG_DEST_LOCATION)
        LOG_INFO("spells", "Destination location: transport guid:{} trans offset: %s position: %s", m_dst._transportGUID, m_dst._transportOffset.ToString().c_str(), m_dst._position.ToString().c_str());
    if (m_targetMask & TARGET_FLAG_STRING)
...
void Spell::SelectImplicitNearbyTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    switch (targetType.GetObjectType())
    {
        case TARGET_OBJECT_TYPE_UNIT:
        {
            ...
            {
                LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id %u set object of wrong type, expected unit, got %s, effect %u", m_spellInfo->Id, GetLogNameForGuid(target->GetGUID()), effMask);
                return;
            }
            break;
        }
        case TARGET_OBJECT_TYPE_GOBJ:
            ...
            {
                LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id %u set object of wrong type, expected gameobject, got %s, effect %u", m_spellInfo->Id, GetLogNameForGuid(target->GetGUID()), effMask);
                return;
            }
            break;
        case TARGET_OBJECT_TYPE_DEST:
...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    {
        Firelands::WorldObjectSpellConeTargetCheck check(coneAngle, radius, m_caster, m_spellInfo, selectionType, condList);
        ...
        if (!targets.empty())
        {
            ...
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
                Firelands::Containers::RandomResizeList(targets, maxTargets);
...
void Spell::SelectImplicitAreaTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    if (!unitTargets.empty())
    {
        ...
        if (maxSize && power != -1)
        {
            if (Powers(power) == POWER_HEALTH)
            {
                ...
                {
                    unitTargets.sort(Firelands::HealthPctOrderPred());
            ...
            else
            {
                ...
                if (unitTargets.size() > maxSize)
                {
                    unitTargets.sort(Firelands::PowerPctOrderPred((Powers)power));
                    unitTargets.resize(maxSize);
                }
            }
        }
        ...
        // Other special target selection goes here
        if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            Firelands::Containers::RandomResizeList(unitTargets, maxTargets);
    ...
    {
        if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            Firelands::Containers::RandomResizeList(gObjTargets, maxTargets);
...
void Spell::SelectImplicitTrajTargets()
{
    ...
    std::list<WorldObject*> targets;
    Firelands::WorldObjectSpellTrajTargetCheck check(dist2d, m_targets.GetSrcPos(), m_caster, m_spellInfo);
    ...
    SearchTargets<Firelands::WorldObjectListSearcher<Firelands::WorldObjectSpellTrajTargetCheck> > (searcher, GRID_MAP_TYPE_MASK_ALL, m_caster, m_targets.GetSrcPos(), dist2d);
    if (targets.empty())
        return;
        ...
        if (m_originalCaster)
        {
            ...
            case 5211:     // Bash
                if (unit && unit->ToCreature() && unit->isAlive() && (unit->ToCreature()->GetCreatureTemplate()->MechanicImmuneMask & (1 << (MECHANIC_STUN - 1))))
                {
                    ...
                    {
                        LOG_INFO("spells", "immune to stun and interrupt, should not happen");
                        return SPELL_MISS_IMMUNE;
                    }
...
void Spell::finish(bool ok, Spell* interruptSpell)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_UNIT && m_caster->ToCreature()->isSummon())
    {
        ...
        {
            LOG_DEBUG("spells", "Statue %d is unsummoned in spell %d finish", m_caster->GetGUIDLow(), m_spellInfo->Id);
...
void Spell::HandleThreatSpells()
{
    ...
    LOG_DEBUG("spells", "Spell %u, added an additional %f threat for %s %u target(s)", m_spellInfo->Id, threat, m_spellInfo->_IsPositiveSpell() ? "assisting" : "harming", uint32(m_UniqueTargetInfo.size()));
}
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_spellInfo->RequiresSpellFocus)
    {
        CellCoord p(Firelands::ComputeCellCoord(m_caster->GetPositionX(), m_caster->GetPositionY()));
        Cell cell(p);

        GameObject* ok = NULL;
        Firelands::GameObjectFocusCheck go_check(m_caster, m_spellInfo->RequiresSpellFocus);
        Firelands::GameObjectSearcher<Firelands::GameObjectFocusCheck> checker(m_caster, ok, go_check);

        TypeContainerVisitor<Firelands::GameObjectSearcher<Firelands::GameObjectFocusCheck>, GridTypeMapContainer > object_checker(checker);
