...
void Spell::EffectDummy(SpellEffIndex effIndex)
{
        ...
        case SPELLFAMILY_PALADIN:
            switch (m_spellInfo->Id)
            {
                ...
                case 31789:                                 // Righteous Defense (step 1)
                {
                    ...
                    {
                        Unit* attacker = Firelands::Containers::SelectRandomContainerElement(attackers);
    ...
    if (spell_id)
    {
        ...
        if (!spellInfo)
        {
            LOG_ERROR("spells", "EffectDummy of spell %u: triggering unknown spell id %i\n", m_spellInfo->Id, spell_id);
            return;
        }
...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        LOG_DEBUG("spells", "Spell::EffectTriggerSpell spell %u tried to trigger unknown spell %u", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTriggerMissileSpell(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        LOG_DEBUG("spells", "Spell::EffectTriggerMissileSpell spell %u tried to trigger unknown spell %u", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    if (!m_targets.HasDst())
    {
        LOG_ERROR("spells", "Spell::EffectTeleportUnits - does not have destination for spellId %u.", m_spellInfo->Id);
        return;
    }
...
void Spell::EffectUnlearnSpecialization(SpellEffIndex effIndex)
{
    ...
    LOG_DEBUG("spells", "Spell: Player %u has unlearned spell %u from NpcGUID: %u", player->GetGUIDLow(), spellToUnlearn, m_caster->GetGUIDLow());
}
...
void Spell::EffectHeal(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        else if (m_spellInfo->TargetAuraState == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND, m_spellInfo, m_caster))
        {
            ...
            if (!targetAura)
            {
                LOG_ERROR("spells", "Target(GUID:" UI64FMTD ") has aurastate AURA_STATE_SWIFTMEND but no matching aura.", unitTarget->GetGUID());
                return;
            }
...
void Spell::EffectEnergize(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->Id == 45051)
    {
        ...
        if (!avalibleElixirs.empty())
        {
            ...
            m_caster->CastSpell(unitTarget, Firelands::Containers::SelectRandomContainerElement(avalibleElixirs), true, m_CastItem);
        }
    }
}
...
void Spell::SendLoot(uint64 guid, LootType loottype)
{
    ...
    if (gameObjTarget)
    {
        ...
        {
            LOG_ERROR("spells", "Possible hacking attempt: Player %s [guid: %u] tried to loot a gameobject [entry: %u id: %u] which is on respawn time without being in GM mode!",
        ...
        switch (gameObjTarget->GetGoType())
        {
            ...
            case GAMEOBJECT_TYPE_CHEST:
                ...
                {
                    LOG_DEBUG("spells", "Chest ScriptStart id %u for GO %u", gameObjTarget->GetGOInfo()->chest.eventId, gameObjTarget->GetDBTableGUIDLow());
...
void Spell::EffectLearnSpell(SpellEffIndex effIndex)
{
    ...
    LOG_DEBUG("spells", "Spell: Player %u has learned spell %u from NpcGUID=%u", player->GetGUIDLow(), spellToLearn, m_caster->GetGUIDLow());
}
...
void Spell::EffectEnchantItemPrismatic(SpellEffIndex effIndex)
{
    ...
    // support only enchantings with add socket in this slot
    {
        ...
        if (!add_socket)
        {
            LOG_ERROR("spells", "Spell::EffectEnchantItemPrismatic: attempt apply enchant spell %u with SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC (%u) but without ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET (%u), not suppoted yet.",
...
void Spell::EffectEnchantItemTmp(SpellEffIndex effIndex)
{
    ...
    if (!enchant_id)
    {
        LOG_ERROR("spells", "Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) have 0 as enchanting id", m_spellInfo->Id, effIndex);
        return;
    }
    ...
    if (!pEnchant)
    {
        LOG_ERROR("spells", "Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) have not existed enchanting id %u ", m_spellInfo->Id, effIndex, enchant_id);
        return;
    }
...
void Spell::EffectStuck(SpellEffIndex /*effIndex*/)
{
    ...
    LOG_INFO("spells", "Player %s (guid %u) used auto-unstuck future at map %u (%f, %f, %f)", player->GetName().c_str(), player->GetGUIDLow(), player->GetMapId(), player->GetPositionX(), player->GetPositionY(), player->GetPositionZ());
...
void Spell::EffectForceDeselect(SpellEffIndex /*effIndex*/)
{
    ...
    Firelands::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, m_caster->GetMap()->GetVisibilityRange());
    Firelands::UnitListSearcher<Firelands::AnyUnfriendlyUnitInObjectRangeCheck> searcher(m_caster, targets, u_check);
...
void Spell::EffectPlayMusic(SpellEffIndex effIndex)
{
    ...
    {
        LOG_ERROR("spells", "EffectPlayMusic: Sound (Id: %u) not exist in spell %u.", soundid, m_spellInfo->Id);
        return;
    }
...
void Spell::EffectPlaySound(SpellEffIndex effIndex)
{
    ...
    if (!sSoundEntriesStore.LookupEntry(soundId))
    {
        LOG_ERROR("spells", "EffectPlaySound: Sound (Id: %u) not exist in spell %u.", soundId, m_spellInfo->Id);
        return;
    }
