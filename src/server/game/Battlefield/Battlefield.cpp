...
void Battlefield::HandlePlayerEnterZone(Player *player, uint32 /*zone*/) {
  ...
  // If full of players > announce to player that BF is full and kick him after
  // a few second if he desn't leave
  if (IsWarTime()) {
...
void Battlefield::InvitePlayersInQueueToWar() {
  for (uint8 team = 0; team < BG_TEAMS_COUNT; ++team) {
    for (auto itr = m_PlayersInQueue[team].begin();
...
void Battlefield::InvitePlayerToWar(Player *player) {
  ...
  m_InvitedPlayers[player->GetTeamId()][player->GetGUID()] =
      GameTime::GetGameTime() + m_TimeForAcceptInvite;
  player->GetSession()->SendBfInvitePlayerToWar(m_Guid, m_ZoneId,
...
void Battlefield::InitStalker(uint32 entry, Position const &pos) {
  if (Creature *creature = SpawnCreature(entry, pos))
    ...
    LOG_ERROR("bg.battlefield",
              ...
              "entry {}), zone messages will be unavailable!",
              entry);
}
...
Group *Battlefield::GetFreeBfRaid(TeamId TeamId) {
  for (auto itr = m_Groups[TeamId].begin(); itr != m_Groups[TeamId].end();
       ++itr)
...
BfGraveyard *Battlefield::GetGraveyardById(uint32 id) const {
  if (id < m_GraveyardList.size()) {
    if (BfGraveyard *graveyard = m_GraveyardList.at(id))
      ...
      LOG_ERROR("bg.battlefield",
                "Battlefield::GetGraveyardById Id:{} does not exist.", id);
  } else
    LOG_ERROR("bg.battlefield",
              "Battlefield::GetGraveyardById Id:{} could not be found.", id);
...
float BfGraveyard::GetDistance(Player *player) {
  WorldSafeLocsEntry const *safeLoc =
      sWorldSafeLocsStore.LookupEntry(m_GraveyardId);
...
void BfGraveyard::RelocateDeadPlayers() {
  ...
  for (GuidSet::const_iterator itr = m_ResurrectQueue.begin();
    ...
    if (closestGrave)
      player->TeleportTo(player->GetMapId(), closestGrave->Loc.X,
...
bool BfGraveyard::HasNpc(ObjectGuid guid) {
  ...
  return (m_SpiritGuide[TEAM_ALLIANCE] == guid ||
          m_SpiritGuide[TEAM_HORDE] == guid);
}
...
Creature *Battlefield::SpawnCreature(uint32 entry, Position const &pos) {
  ...
  if (!creature->Create(m_Map->GenerateLowGuid<HighGuid::Unit>(), m_Map, entry,
                        pos)) {
    LOG_ERROR("bg.battlefield",
...
GameObject *Battlefield::SpawnGameObject(uint32 entry, Position const &pos,
  ...
  if (!go->Create(m_Map->GenerateLowGuid<HighGuid::GameObject>(), entry, m_Map,
                  pos, rot, 255, GO_STATE_READY)) {
    LOG_ERROR("bg.battlefield",
              "Battlefield::SpawnGameObject: Gameobject template {} could not "
              "be found in the database! Battlefield has not been created!",
              entry);
    LOG_ERROR("bg.battlefield",
              "Battlefield::SpawnGameObject: Could not create gameobject "
              "template {}! Battlefield has not been created!",
              entry);
    delete go;
    return nullptr;
  }
...
BfCapturePoint::BfCapturePoint(Battlefield *battlefield)
    : m_Bf(battlefield), m_capturePointSpawnId() {
  m_team = TEAM_NEUTRAL;
  m_value = 0;
  m_minValue = 0.0f;
  m_maxValue = 0.0f;
...
GuidSet::iterator BfCapturePoint::HandlePlayerLeave(Player *player) {
  if (m_capturePointSpawnId)
    if (GameObject *capturePoint = m_Bf->GetGameObject(m_capturePointSpawnId))
      player->SendUpdateWorldState(
          capturePoint->GetGOInfo()->capturePoint.worldState1, 0);
...
void BfCapturePoint::SendChangePhase() {
  ...
  if (GameObject *capturePoint = m_Bf->GetGameObject(m_capturePointSpawnId)) {
    ...
    SendUpdateWorldState(capturePoint->GetGOInfo()->capturePoint.worldState1,
                         1);
    ...
    SendUpdateWorldState(capturePoint->GetGOInfo()->capturePoint.worldstate3,
                         m_neutralValuePct);
  }
}
...
bool BfCapturePoint::SetCapturePointData(GameObject *capturePoint) {
  ...
  LOG_DEBUG("bg.battlefield", "Creating capture point {}",
            capturePoint->GetEntry());

  m_capturePointSpawnId =
      ObjectGuid(HighGuid::GameObject, capturePoint->GetEntry(),
                 capturePoint->GetGUID().GetCounter());

  // check info existence
  GameObjectTemplate const *goinfo = capturePoint->GetGOInfo();
  if (goinfo->type != GAMEOBJECT_TYPE_CAPTURE_POINT) {
    LOG_ERROR("misc", "OutdoorPvP: GO {} is not a capture point!",
              capturePoint->GetEntry());
    return false;
  }
...
bool BfCapturePoint::Update(uint32 diff) {
  ...
  if (GameObject *capturePoint = m_Bf->GetGameObject(m_capturePointSpawnId)) {
    ...
    Firelands::PlayerListSearcher<Firelands::AnyPlayerInObjectRangeCheck>
        searcher(capturePoint, players, checker);
  ...
  float fact_diff = ((float)m_activePlayers[TEAM_ALLIANCE].size() -
                     (float)m_activePlayers[TEAM_HORDE].size()) *
