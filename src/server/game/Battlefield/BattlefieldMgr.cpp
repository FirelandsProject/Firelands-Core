...
BattlefieldMgr::~BattlefieldMgr() {
  for (BattlefieldSet::iterator itr = _battlefieldSet.begin();
       itr != _battlefieldSet.end(); ++itr)
    delete *itr;
...
void BattlefieldMgr::InitBattlefield() {
  ...
  if (!tb->SetupBattlefield()) {
    ...
    LOG_DEBUG("bg.battlefield",
              "Battlefield: Tol Barad successfully initiated.");
  }
}
...
void BattlefieldMgr::HandlePlayerEnterZone(Player *player, uint32 zoneId) {
  ...
  LOG_DEBUG("bg.battlefield", "Player {} entered battlefield id {}",
            player->GetGUID().GetCounter(), bf->GetTypeId());
}
...
void BattlefieldMgr::HandlePlayerLeaveZone(Player *player, uint32 zoneId) {
  ...
  LOG_DEBUG("bg.battlefield", "Player {} left battlefield id {}",
            player->GetGUID().GetCounter(), itr->second->GetTypeId());
}
