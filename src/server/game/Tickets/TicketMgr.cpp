...
inline float GetAge(uint64 t) {
  return float(GameTime::GetGameTime() - t) / DAY;
}
...
GmTicket::GmTicket(Player *player)
    : _type(TICKET_TYPE_OPEN), _posX(0), _posY(0), _posZ(0), _mapId(0),
      _createTime(GameTime::GetGameTime()),
      _lastModifiedTime(GameTime::GetGameTime()), _completed(false),
      _escalatedStatus(TICKET_UNASSIGNED), _viewed(false), _needResponse(false),
      _needMoreHelp(false) {
...
bool GmTicket::LoadFromDB(Field *fields) {
  ...
  // escalated, viewed, needMoreHelp
  uint8 index = 0;
  _id = fields[index].Get<uint32>();
  _type = TicketType(fields[++index].Get<uint8>());
  _playerGuid = ObjectGuid(HighGuid::Player, fields[++index].Get<uint32>());
  ...
  _closedBy = ObjectGuid(uint64(fields[++index].Get<int32>()));
  _assignedTo = ObjectGuid(HighGuid::Player, fields[++index].Get<uint32>());
...
std::string GmTicket::FormatMessageString(ChatHandler &handler,
                                          ...
                                          char const *szCompletedName) const {
  std::stringstream ss;
  ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTGUID, _id);
  ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTNAME,
                                _playerName.c_str());
  ...
  if (szCompletedName)
    ss << handler.PGetParseString(LANG_COMMAND_TICKETCOMPLETED,
                                  szCompletedName);
  return ss.str();
}
...
void TicketMgr::LoadTickets() {
  ...
  LOG_INFO("server.loading", ">> Loaded %u GM tickets in %u ms", count,
...
void TicketMgr::LoadSurveys() {
  ...
  LOG_INFO("server.loading", ">> Loaded GM Survey count from database in %u ms",
           GetMSTimeDiffToNow(oldMSTime));
}
