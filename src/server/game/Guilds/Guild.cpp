...
void Guild::NewsLogEntry::SaveToDB(CharacterDatabaseTransaction& trans) const
{
    ...
    stmt->SetData(++index, GetPlayerGuid().GetCounter());
...
void Guild::Member::AddActivity(uint64 activity)
{
    ...
    stmt->SetData(2, m_guid.GetCounter());
    CharacterDatabase.Execute(stmt);
}
...
bool Guild::Member::LoadFromDB(Field* fields)
{
    ...
    m_bankWithdrawMoney = fields[13].Get<uint64>();
    ...
    if (!m_zoneId)
    {
        LOG_DEBUG("guild", "%s has broken zone-data", m_guid.ToString());
...
void Guild::Member::LoadProfessionDataFromDB(ObjectGuid guid)
{
    ...
    if (result)
    {
        do
        {
            ...
            stmt2->SetData(0, guid.GetCounter());
            ...
            if (result2)
            {
                do
                {
                    ...
                    uint32 spellId = fields2[0].Get<uint32>();
                    bool disabled = fields2[2].Get<bool>();
                    if (disabled)
                        continue;
...
bool Guild::Member::CheckStats() const
{
    if (m_level < 1)
    {
        LOG_ERROR("guild", "%s has a broken data in field `characters`.`level`, deleting him from guild!", m_guid.ToString());
        return false;
    }

    if (m_class < CLASS_WARRIOR || m_class >= MAX_CLASSES)
    {
        LOG_ERROR("guild", "%s has a broken data in field `characters`.`class`, deleting him from guild!", m_guid.ToString());
        return false;
    }
    return true;
}
...
void Guild::Member::UpdateBankMoneyWithdrawValue(CharacterDatabaseTransaction& trans, uint64 amount)
{
    ...
    stmt->SetData(0, m_guid.GetCounter());
    stmt->SetData(1, m_bankWithdrawMoney);
    CharacterDatabase.ExecuteOrAppend(trans, stmt);
}
...
void Guild::Member::ResetWeekActivityAndReputation()
{
    ...
    stmt2->SetData(0, m_guid.GetCounter());
    trans->Append(stmt2);
...
bool Guild::LoadFromDB(Field* fields)
{
    ...
    m_leaderGuid = ObjectGuid(HighGuid::Player, fields[2].Get<uint32>());
    ...
    _currChallengeCount[GUILD_CHALLENGE_TYPE_DUNGEON] = uint8(fields[16].Get<uint32>());
    _currChallengeCount[GUILD_CHALLENGE_TYPE_RAID] = uint8(fields[17].Get<uint32>());
    _currChallengeCount[GUILD_CHALLENGE_TYPE_RATED_BG] = uint8(fields[18].Get<uint32>());
    _weeklyBonusMoney = fields[19].Get<uint64>();
...
bool Guild::LoadMemberFromDB(Field* fields)
{
    ...
    Member* member = new Member(m_id, playerGuid, fields[2].Get<uint8>());
...
void Guild::LoadGuildNewsLogFromDB(Field* fields)
{
    ...
    m_newsLog->LoadEvent(new NewsLogEntry(
        ...
        ObjectGuid(HighGuid::Player, fields[3].Get<uint32>()), // player guid
...
bool Guild::AddMember(CharacterDatabaseTransaction& trans, ObjectGuid guid, uint8 rankId)
{
    ...
    else
    {
        ...
        if (!keepGuildReputation)
        {
            ...
            stmt2->SetData(0, uint16(FACTION_GUILD));
            stmt2->SetData(1, 0);
            stmt2->SetData(2, uint16(FACTION_GUILD));
...
void Guild::DeleteMember(CharacterDatabaseTransaction& trans, ObjectGuid guid, bool isDisbanding, bool isKicked, bool canDeleteGuild)
{
    ...
    if (Member* member = GetMember(guid))
    {
        if (!isDisbanding)
        {
            ...
            stmt->SetData(3, uint32(::GameTime::GetGameTime()));
            CharacterDatabase.Execute(stmt);
        }
        delete member;
    }
...
bool Guild::_ModifyBankMoney(CharacterDatabaseTransaction& trans, uint64 amount, bool add, bool fromCashFlow /*= false*/)
{
    ...
    stmt->SetData(1, _weeklyBonusMoney);
    stmt->SetData(2, m_id);
    trans->Append(stmt);
    return true;
}
...
void Guild::SendGuildRanksUpdate(ObjectGuid setterGuid, ObjectGuid targetGuid, uint32 rank)
{
    ...
    LOG_DEBUG("network", "SMSG_GUILD_RANKS_UPDATE [Broadcast] Target: %s, Issuer: %s, RankId: %u",
        targetGuid.ToString(), setterGuid.ToString(), rank);
}
...
void Guild::CompleteChallenge(uint8 challengeType, Player* source)
{
    ...
    stmt->SetData(0, _currChallengeCount[GUILD_CHALLENGE_TYPE_DUNGEON]);
    stmt->SetData(1, _currChallengeCount[GUILD_CHALLENGE_TYPE_RAID]);
    stmt->SetData(2, _currChallengeCount[GUILD_CHALLENGE_TYPE_RATED_BG]);
    stmt->SetData(3, m_id);
    trans->Append(stmt);
    CharacterDatabase.CommitTransaction(trans);
}
