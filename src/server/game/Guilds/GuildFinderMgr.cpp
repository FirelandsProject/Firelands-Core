...
MembershipRequest::MembershipRequest()
    : _guildId(0), _availability(0), _classRoles(0), _interests(0),
      _time(GameTime::GetGameTime()) {}

MembershipRequest::MembershipRequest(ObjectGuid playerGUID, uint32 guildId,
                                     uint32 availability, uint32 classRoles,
                                     uint32 interests, std::string comment,
                                     time_t submitTime)
    : _comment(std::move(comment)), _guildId(guildId), _playerGUID(playerGUID),
      _availability(availability), _classRoles(classRoles),
...
void GuildFinderMgr::LoadGuildSettings() {
  ...
  QueryResult result = CharacterDatabase.Query(
      "SELECT gfgs.guildId, gfgs.availability, gfgs.classRoles, "
      "gfgs.interests, gfgs.level, gfgs.listed, gfgs.comment, c.race "
  ...
  if (!result) {
    LOG_INFO("server.loading",
             ">> Loaded 0 guild finder guild-related settings. Table "
             "`guild_finder_guild_settings` is empty.");
    return;
  }
  ...
  do {
    ...
    if (ChrRacesEntry const *raceEntry =
            sChrRacesStore.LookupEntry(fields[7].Get<uint8>()))
      if (raceEntry->BaseLanguage == 1)
        guildTeam = TEAM_HORDE;

    _guildSettings[guildId] =
        LFGuildSettings(listed, guildTeam, guildId, classRoles, availability,
                        interests, level, comment);

    ++count;
  } while (result->NextRow());

  LOG_INFO("server.loading",
           ">> Loaded %u guild finder guild-related settings in %u ms.", count,
...
void GuildFinderMgr::LoadMembershipRequests() {
  ...
  //                                                      0         1 2 3 4 5 6
  QueryResult result =
      CharacterDatabase.Query("SELECT guildId, playerGuid, availability, "
                              "classRole, interests, comment, submitTime "
                              "FROM guild_finder_applicant");

  if (!result) {
    LOG_INFO("server.loading", ">> Loaded 0 guild finder membership requests. "
                               "Table `guild_finder_applicant` is empty.");
    return;
  }
  ...
  do {
    ...
    MembershipRequest request(ObjectGuid(HighGuid::Player, playerId), guildId,
                              availability, classRoles, interests, comment,
                              time_t(submitTime));
  ...
  LOG_INFO("server.loading",
           ">> Loaded %u guild finder membership requests in %u ms.", count,
           GetMSTimeDiffToNow(oldMSTime));
}

void GuildFinderMgr::AddMembershipRequest(uint32 guildGuid,
                                          MembershipRequest const &request) {
  _membershipRequests[guildGuid].push_back(request);

  CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction();
  CharacterDatabasePreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_REP_GUILD_FINDER_APPLICANT);
  stmt->SetData(0, request.GetGuildId());
  stmt->SetData(1, request.GetPlayerGUID().GetCounter());
...
void GuildFinderMgr::RemoveAllMembershipRequestsFromPlayer(uint32 playerId) {
  for (MembershipRequestStore::iterator itr = _membershipRequests.begin();
       itr != _membershipRequests.end(); ++itr) {
    std::vector<MembershipRequest>::iterator itr2 = itr->second.begin();
    for (; itr2 != itr->second.end(); ++itr2)
      if (itr2->GetPlayerGUID().GetCounter() == playerId)
        break;
    ...
    CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction();
    CharacterDatabasePreparedStatement *stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_FINDER_APPLICANT);
    stmt->SetData(0, itr2->GetGuildId());
    stmt->SetData(1, itr2->GetPlayerGUID().GetCounter());
    trans->Append(stmt);
...
void GuildFinderMgr::RemoveMembershipRequest(uint32 playerId, uint32 guildId) {
  std::vector<MembershipRequest>::iterator itr =
      _membershipRequests[guildId].begin();
...
std::list<MembershipRequest>
GuildFinderMgr::GetAllMembershipRequestsForPlayer(uint32 playerGuid) {
  std::list<MembershipRequest> resultSet;
  for (MembershipRequestStore::const_iterator itr = _membershipRequests.begin();
       itr != _membershipRequests.end(); ++itr) {
    std::vector<MembershipRequest> const &guildReqs = itr->second;
    for (std::vector<MembershipRequest>::const_iterator itr2 =
             guildReqs.begin();
         itr2 != guildReqs.end(); ++itr2) {
...
LFGuildStore GuildFinderMgr::GetGuildsMatchingSetting(LFGuildPlayer &settings,
                                                      TeamId faction) {
  LFGuildStore resultSet;
  for (LFGuildStore::const_iterator itr = _guildSettings.begin();
       itr != _guildSettings.end(); ++itr) {
...
bool GuildFinderMgr::HasRequest(uint32 playerId, uint32 guildId) {
  for (std::vector<MembershipRequest>::const_iterator itr =
           _membershipRequests[guildId].begin();
       itr != _membershipRequests[guildId].end(); ++itr)
    if (itr->GetPlayerGUID() == playerId)
      return true;
  return false;
}

void GuildFinderMgr::SetGuildSettings(uint32 guildGuid,
...
void GuildFinderMgr::DeleteGuild(uint32 guildId) {
  ...
  // Notify the guild master the list changed (even if he's not a GM any more,
  // not sure if needed)
  if (Guild *guild = sGuildMgr->GetGuildById(guildId))
    SendApplicantListUpdate(*guild);
}
