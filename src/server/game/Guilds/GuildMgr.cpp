...
void GuildMgr::LoadGuilds()
{
    ...
    LOG_INFO("server.loading", "Loading Guild News...");
    {
        ...
        else
        {
            ...
            LOG_INFO("server.loading", ">> Loaded {} guild new logs in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }
...
void GuildMgr::LoadGuildXpForLevel()
{
    ...
    do
    {
        ...
        uint32 requiredXP   = fields[1].Get<uint64>();

        if (level >= sWorld->getIntConfig(CONFIG_GUILD_MAX_LEVEL))
        {
            LOG_INFO("misc", "Unused (> Guild.MaxLevel in worldserver.conf) level {} in `guild_xp_for_level` table, ignoring.", uint32(level));
            continue;
        }
    ...
    for (uint8 level = 1; level < sWorld->getIntConfig(CONFIG_GUILD_MAX_LEVEL); ++level)
    {
        ...
        {
            LOG_ERROR("sql.sql", "Level {} does not have XP for guild level data. Using data of level [{}] + 1660000.", level+1, level);
            GuildXPperLevel[level] = GuildXPperLevel[level - 1] + 1660000;
        }
    }

    LOG_INFO("server.loading", ">> Loaded {} xp for guild level definitions in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void GuildMgr::LoadGuildRewards()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("server.loading", "Guild rewards constains not existing item entry {}", reward.Entry);
            continue;
        }

        if (reward.AchievementId != 0 && (!sAchievementMgr->GetAchievement(reward.AchievementId)))
        {
            LOG_ERROR("server.loading", "Guild rewards constains not existing achievement entry {}", reward.AchievementId);
            continue;
        }

        if (reward.Standing >= MAX_REPUTATION_RANK)
        {
            LOG_ERROR("server.loading", "Guild rewards contains wrong reputation standing {}, max is {}", uint32(reward.Standing), MAX_REPUTATION_RANK - 1);
            continue;
        }
    ...
    LOG_INFO("server.loading", ">> Loaded {} guild reward definitions in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}
