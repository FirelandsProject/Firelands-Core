...
bool ObjectMgr::LoadTrinityStrings(const char* table, int32 min_value, int32 max_value)
{
    ...
    if (min_value == MIN_TRINITY_STRING_ID)
        TC_LOG_INFO("server.loading", ">> Loaded %u Firelands strings from table %s in %u ms", count, table, GetMSTimeDiffToNow(oldMSTime));
    else
...
const char *ObjectMgr::GetTrinityString(int32 entry, LocaleConstant locale_idx) const
{
    ...
    if (entry > 0)
        TC_LOG_ERROR("sql.sql", "Entry %i not found in `firelands_string` table.", entry);
    else
        TC_LOG_ERROR("sql.sql", "Firelands string entry %i not found in DB.", entry);
    return "<error>";
}
...
bool LoadTrinityStrings(const char* table, int32 start_value, int32 end_value)
{
    ...
    {
        TC_LOG_ERROR("sql.sql", "Table '%s' load attempted with range (%d - %d) reserved by Firelands, strings not loaded.", table, start_value, end_value+1);
        return false;
    }
