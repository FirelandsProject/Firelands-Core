...
void ObjectMgr::LoadGossipMenuItemsLocales()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %lu gossip_menu_option locale strings in %u ms", (unsigned long)_gossipMenuItemsLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPointOfInterestLocales()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %lu points_of_interest locale strings in %u ms", (unsigned long)_pointOfInterestLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
    ...
    if (!factionTemplate)
        LOG_ERROR("sql.sql", "Creature (Entry: %u) has non-existing faction_A template (%u).", cInfo->Entry, cInfo->faction_A);

    factionTemplate = sFactionTemplateStore.LookupEntry(cInfo->faction_H);
    if (!factionTemplate)
        LOG_ERROR("sql.sql", "Creature (Entry: %u) has non-existing faction_H template (%u).", cInfo->Entry, cInfo->faction_H);
    ...
    if (cInfo->equipmentId > 0)                          // 0 no equipment
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table `creature_template` lists creature (Entry: %u) with `equipment_id` %u not found in table `creature_equip_template`, set to no equipment.", cInfo->Entry, cInfo->equipmentId);
    ...
    {
        LOG_ERROR("sql.sql", "Table `creature_template` lists creature (Entry: %u) with `exp` %u. Ignored and set to 0.", cInfo->Entry, cInfo->expansion);
...
void ObjectMgr::LoadEquipmentTemplates()
{
    ...
    do
    {
        ...
        for (uint8 i = 0; i < MAX_EQUIPMENT_ITEMS; ++i)
        {
            ...
            if (!dbcItem)
            {
                LOG_ERROR("sql.sql", "Unknown item (entry=%u) in creature_equip_template.itemEntry%u for entry = %u, forced to 0.",
            ...
            {
                LOG_ERROR("sql.sql", "Item (entry=%u) in creature_equip_template.itemEntry%u for entry = %u is not equipable in a hand, forced to 0.",
                    equipmentInfo.ItemEntry[i], i+1, entry);
                equipmentInfo.ItemEntry[i] = 0;
            }
        }
...
void ObjectMgr::LoadCreatures()
{
    ...
    do
    {
        ...
        if (data.spawnMask & ~spawnMasks[data.mapid])
            LOG_ERROR("sql.sql", "Table `creature` have creature (GUID: %u) that have wrong spawn mask %u including not supported difficulty modes for map (Id: %u) spawnMasks[data.mapid]: %u.", guid, data.spawnMask, data.mapid, spawnMasks[data.mapid]);
        ...
        {
            LOG_ERROR("sql.sql", "Table `creature` have creature (GUID: %u Entry: %u) with `spawndist`< 0, set to 0.", guid, data.id);
            data.spawndist = 0.0f;
        }
        else if (data.movementType == RANDOM_MOTION_TYPE)
        {
            if (data.spawndist == 0.0f)
            {
                LOG_ERROR("sql.sql", "Table `creature` have creature (GUID: %u Entry: %u) with `MovementType`=1 (random movement) but with `spawndist`=0, replace by idle movement type (0).", guid, data.id);
        ...
        else if (data.movementType == IDLE_MOTION_TYPE)
        {
            if (data.spawndist != 0.0f)
            {
                LOG_ERROR("sql.sql", "Table `creature` have creature (GUID: %u Entry: %u) with `MovementType`=0 (idle) have `spawndist`<>0, set to 0.", guid, data.id);
                data.spawndist = 0.0f;
            }
        }
    ...
    LOG_INFO("server.loading", ">> Loaded %zu creatures in %u ms", _creatureDataStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
    ...
    do
    {
        ...
        switch (teleporterType)
        {
            case TELEPORTER_TYPE_CREATURE:
                ...
                {
                    LOG_ERROR("sql.sql", "Table `instance_teleporter_dest` has teleporter with non existing entry %u for creature teleporter type, skipped.", teleporterId);
                    continue;
                }
                break;
            case TELEPORTER_TYPE_GAMEOBJECT:
                ...
                {
                    LOG_ERROR("sql.sql", "Table `instance_teleporter_dest` has teleporter with non existing entry %u for gameobject teleporter type, skipped.", teleporterId);
                    continue;
                }
                break;
            default:
                LOG_ERROR("sql.sql", "Table `instance_teleporter_dest` has unhandled teleporter type %u for teleporter %u, skipped.", teleporterType, teleporterId);
                continue;
        }
...
bool ObjectMgr::MoveCreData(uint32 guid, uint32 mapId, Position pos)
{
    ...
    if (Map* map = sMapMgr->CreateBaseMap(mapId))
    {
        ...
        if (!map->Instanceable() && map->IsGridLoaded(data.posX, data.posY))
        {
            ...
            {
                LOG_ERROR("misc", "MoveCreData: Cannot add creature guid %u to map", guid);
                delete creature;
                return false;
            }
        }
...
void ObjectMgr::LoadGameobjects()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table `gameobject` has gameobject (GUID: %u Entry: %u) with invalid rotation2 (%f) value, skip", guid, data.id, data.rotation2);
            continue;
        }

        if (data.rotation3 < -1.0f || data.rotation3 > 1.0f)
        {
            LOG_ERROR("sql.sql", "Table `gameobject` has gameobject (GUID: %u Entry: %u) with invalid rotation3 (%f) value, skip", guid, data.id, data.rotation3);
            continue;
        }
    ...
    LOG_INFO("server.loading", ">> Loaded %zu gameobjects in %u ms", _gameObjectDataStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemLocales()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %lu Item locale strings in %u ms", (unsigned long)_itemLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %u item templates from Item-sparse.db2 and %u from database in %u ms", sparseCount, dbCount, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPetLevelInfo()
{
    ...
    for (PetLevelInfoContainer::iterator itr = _petInfoStore.begin(); itr != _petInfoStore.end(); ++itr)
    {
        ...
        {
            LOG_ERROR("sql.sql", "Creature %u does not have pet stats data for Level 1 or %u! Must be exist!", itr->first, maxLevel);
            exit(1);
        }
        ...
        for (uint8 level = 1; level < maxLevel; ++level)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Creature %u has no full data set for Level %i pet stats data, using approximated (from default pet progression) data", itr->first,level+1);
...
void ObjectMgr::LoadPlayerInfo()
{
    ...
    LOG_INFO("server.loading", "Loading Player Create Spell Data...");
    {
        ...
        if (!result)
        {
            LOG_ERROR("server.loading", ">> Loaded 0 player create spells. DB table `%s` is empty.", sWorld->getBoolConfig(CONFIG_START_ALL_SPELLS) ? "playercreateinfo_spell_custom" : "playercreateinfo_spell");
        }
        else
        {
            uint32 count = 0;
            ...
            do
            {
                ...
                if (current_race >= MAX_RACES)
                {
                    LOG_ERROR("sql.sql", "Wrong race %u in `playercreateinfo_spell` table, ignoring.", current_race);
                    continue;
                }
                ...
                if (current_class >= MAX_CLASSES)
                {
                    LOG_ERROR("sql.sql", "Wrong class %u in `playercreateinfo_spell` table, ignoring.", current_class);
                    continue;
                }
                ...
                {
                    LOG_ERROR("sql.sql", "Wrong race: %u, class: %u combination in `playercreateinfo_spell` table, ignoring.", current_race, current_class);
                    continue;
                }
    ...
    for (QuestMap::iterator iter = _questTemplates.begin(); iter != _questTemplates.end(); ++iter)
    {
        ...
        if (qinfo->Flags & QUEST_FLAGS_AUTO_REWARDED)
        {
            ...
            for (int j = 1; j < QUEST_REWARD_CHOICES_COUNT; ++j )
            {
                ...
                {
                    LOG_ERROR("sql.sql", "Quest %u has `RewardChoiceItemId%d` = %u but item from `RewardChoiceItemId%d` can't be rewarded with quest flag QUEST_FLAGS_AUTO_REWARDED.",
        ...
        if (qinfo->RequiredRaces)
            ...
            if (!(qinfo->RequiredRaces & RACEMASK_ALL_PLAYABLE))
                {
                    LOG_ERROR("sql.sql", "Quest %u does not contain any playable races in `RequiredRaces` (%u), value set to 0 (all races).", qinfo->GetQuestId(), qinfo->RequiredRaces);
        ...
        if (qinfo->SourceItemId)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Quest %u has `SourceItemId` = %u but `SourceItemIdCount` = 0, set to 1 but need fix in DB.",
        ...
        for (uint8 j = 0; j < QUEST_SOURCE_ITEM_IDS_COUNT; ++j)
        {
            ...
            if (id)
            {
                if (!sObjectMgr->GetItemTemplate(id))
                {
                    LOG_ERROR("sql.sql", "Quest %u has `RequiredSourceItemId%d` = %u but item with entry %u does not exist, quest can't be done.",
                        qinfo->GetQuestId(), j+1, id, id);
                    // no changes, quest can't be done for this requirement
                }
            }
            else
            {
                ...
                {
                    LOG_ERROR("sql.sql", "Quest %u has `RequiredSourceItemId%d` = 0 but `RequiredSourceItemCount%d` = %u.",
        ...
        for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
        {
            ...
            if (id)
            {
                ...
                if (!spellInfo)
                {
                    LOG_ERROR("sql.sql", "Quest %u has `ReqSpellCast%d` = %u but spell %u does not exist, quest can't be done.",
                        qinfo->GetQuestId(), j+1, id, id);
                    continue;
                }
                ...
                if (!qinfo->RequiredNpcOrGo[j])
                {
                    ...
                    {
                        LOG_ERROR("sql.sql", "Quest %u has `ReqSpellCast%d` = %u and RequiredNpcOrGo%d = 0 but spell %u does not have SPELL_EFFECT_QUEST_COMPLETE or SPELL_EFFECT_SEND_EVENT effect for this quest, quest can't be done.",
        ...
        if (qinfo->RewardSpell)
        {
            ...
            if (!spellInfo)
            {
                LOG_ERROR("sql.sql", "Quest %u has `RewardSpell` = %u but spell %u does not exist, spell removed as display reward.",
            ...
            {
                LOG_ERROR("sql.sql", "Quest %u has `RewardSpell` = %u but spell %u is talent, quest will not have a spell reward.",
                    qinfo->GetQuestId(), qinfo->RewardSpell, qinfo->RewardSpell);
                qinfo->RewardSpell = 0;                        // no spell reward will display for this quest
            }
        }
        ...
        if (qinfo->NextQuestIdChain)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Quest %u has `NextQuestIdChain` = %u but quest %u does not exist, quest chain will not work.",
...
void ObjectMgr::LoadQuestLocales()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %lu Quest locale strings in %u ms", (unsigned long)_questLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            case SCRIPT_COMMAND_TALK:
            {
                ...
                {
                    LOG_ERROR("sql.sql", "Table `%s` has out of range text id (dataint = %i expected %u-%u) in SCRIPT_COMMAND_TALK for script id %u",
...
void ObjectMgr::ValidateSpellScripts()
{
    ...
    for (SpellScriptsContainer::iterator itr = _spellScriptsStore.begin(); itr != _spellScriptsStore.end();)
    {
        ...
        for (std::vector<std::pair<SpellScriptLoader *, SpellScriptsContainer::iterator> >::iterator sitr = SpellScriptLoaders.begin(); sitr != SpellScriptLoaders.end(); ++sitr)
        {
            ...
            {
                LOG_ERROR("scripts", "Functions GetSpellScript() and GetAuraScript() of script `%s` do not return objects - script skipped",  GetScriptName(sitr->second->second));
                valid = false;
            }
            if (spellScript)
            {
...
void ObjectMgr::LoadPageTextLocales()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %lu PageText locale strings in %u ms", (unsigned long)_pageTextLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadInstanceEncounters()
{
    ...
    do
    {
        ...
        if (lastEncounterDungeon)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Table `instance_encounters` has an encounter %u (%s) marked as final for invalid dungeon id %u, set to 0!", entry, dungeonEncounter->encounterName, lastEncounterDungeon);
        ...
        if (lastEncounterDungeon)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Table `instance_encounters` specified encounter %u (%s) as last encounter but %u (%s) is already marked as one, skipped!", entry, dungeonEncounter->encounterName, itr->second->id, itr->second->encounterName);
                continue;
            }
        ...
        case ENCOUNTER_CREDIT_KILL_CREATURE:
            {
                ...
                if (!creatureInfo)
                {
                    LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid creature (entry %u) linked to the encounter %u (%s), skipped!", creditEntry, entry, dungeonEncounter->encounterName);
                    continue;
                }
        ...
        case ENCOUNTER_CREDIT_CAST_SPELL:
            ...
            {
                LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid spell (entry %u) linked to the encounter %u (%s), skipped!", creditEntry, entry, dungeonEncounter->encounterName);
                continue;
            }
            break;
        default:
            LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid credit type (%u) for encounter %u (%s), skipped!", creditType, entry, dungeonEncounter->encounterName);
            continue;
        }
    ...
    LOG_ERROR("server.loading", ">> Loaded %u instance encounters in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadNpcTextLocales()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %lu NpcText locale strings in %u ms", (unsigned long)_npcTextLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGraveyardZones()
{
    ...
    do
    {
        ...
        if (!entry)
        {
            LOG_ERROR("sql.sql", "Table `game_graveyard_zone` has a record for not existing graveyard (WorldSafeLocs.dbc id) %u, skipped.", safeLocId);
            continue;
        }
        ...
        if (!areaEntry)
        {
            LOG_ERROR("sql.sql", "Table `game_graveyard_zone` has a record for not existing zone id (%u), skipped.", zoneId);
            continue;
        }

        if (areaEntry->zone != 0 && zoneId != 4815 && zoneId != 5004 && zoneId != 5144 && zoneId != 5145 && zoneId !=33 && zoneId != 5287)
        {
            LOG_ERROR("sql.sql", "Table `game_graveyard_zone` has a record for subzone id (%u) instead of zone, skipped.", zoneId);
            continue;
        }

        if (team != 0 && team != HORDE && team != ALLIANCE)
        {
            LOG_ERROR("sql.sql", "Table `game_graveyard_zone` has a record for non player faction (%u), skipped.", team);
            continue;
        }

        if (!AddGraveYardLink(safeLocId, zoneId, team, false))
            LOG_ERROR("sql.sql", "Table `game_graveyard_zone` has a duplicate record for Graveyard (ID: %u) and Zone (ID: %u), skipped.", safeLocId, zoneId);
    } while (result->NextRow());
...
WorldSafeLocsEntry const* ObjectMgr::GetClosestGraveYard(float x, float y, float z, uint32 MapId, uint32 team)
{
    ...
    for (; range.first != range.second; ++range.first)
    {
        ...
        if (!entry)
        {
            LOG_ERROR("sql.sql", "Table `game_graveyard_zone` has record for not existing graveyard (WorldSafeLocs.dbc id) %u, skipped.", data.safeLocId);
            continue;
        }
...
void ObjectMgr::SetHighestGuids()
{
    ...
    LOG_INFO("server.loading", ">> generate %u free guid", GenrateFreeItemGuid());
...
void ObjectMgr::LoadGameObjectLocales()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %lu gameobject locale strings in %u ms", (unsigned long)_gameObjectLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadRewardOnKill()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table `creature_onkill_reward` have data for not existed creature entry (%u), skipped", creature_id);
            continue;
        }
        ...
        if (rewOnKill.RepFaction1)
        {
            ...
            if (!factionEntry1)
            {
                LOG_ERROR("sql.sql", "Faction (faction.dbc) %u does not exist but is used in `creature_onkill_reward`", rewOnKill.RepFaction1);
                continue;
            }
        }
        ...
        if (rewOnKill.RepFaction2)
        {
            ...
            if (!factionEntry2)
            {
                LOG_ERROR("sql.sql", "Faction (faction.dbc) %u does not exist but is used in `creature_onkill_reward`", rewOnKill.RepFaction2);
                continue;
            }
        }
        ...
        if (rewOnKill.CurrencyId1)
        {
            ...
            {
                LOG_ERROR("sql.sql", "CurrenctType (CurrenctTypes.dbc) %u does not exist but is used in `creature_onkill_reward`", rewOnKill.CurrencyId1);
                continue;
            }
        }
        ...
        if (rewOnKill.CurrencyId2)
        {
            ...
            {
                LOG_ERROR("sql.sql", "CurrenctType (CurrenctTypes.dbc) %u does not exist but is used in `creature_onkill_reward`", rewOnKill.CurrencyId2);
                continue;
            }
        }
        ...
        if (rewOnKill.CurrencyId3)
        {
            ...
            {
                LOG_ERROR("sql.sql", "CurrenctType (CurrenctTypes.dbc) %u does not exist but is used in `creature_onkill_reward`", rewOnKill.CurrencyId3);
                 continue;
             }
         }
...
void ObjectMgr::LoadPointsOfInterest()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table `points_of_interest` (Entry: %u) have invalid coordinates (X: %f Y: %f), ignored.", point_id, POI.x, POI.y);
            continue;
        }
...
void ObjectMgr::LoadGameobjectQuestRelations()
{
    ...
    for (QuestRelations::iterator itr = _goQuestRelations.begin(); itr != _goQuestRelations.end(); ++itr)
    {
        ...
        if (!goInfo)
            LOG_ERROR("sql.sql", "Table `gameobject_questrelation` have data for not existed gameobject entry (%u) and existed quest %u", itr->first, itr->second);
        else if (goInfo->type != GAMEOBJECT_TYPE_QUESTGIVER)
            LOG_ERROR("sql.sql", "Table `gameobject_questrelation` have data gameobject entry (%u) for quest %u, but GO is not GAMEOBJECT_TYPE_QUESTGIVER", itr->first, itr->second);
    }
}
...
void ObjectMgr::LoadGameobjectInvolvedRelations()
{
    ...
    for (QuestRelations::iterator itr = _goQuestInvolvedRelations.begin(); itr != _goQuestInvolvedRelations.end(); ++itr)
    {
        GameObjectTemplate const* goInfo = GetGameObjectTemplate(itr->first);
        if (!goInfo)
            LOG_ERROR("sql.sql", "Table `gameobject_involvedrelation` have data for not existed gameobject entry (%u) and existed quest %u", itr->first, itr->second);
        else if (goInfo->type != GAMEOBJECT_TYPE_QUESTGIVER)
            LOG_ERROR("sql.sql", "Table `gameobject_involvedrelation` have data gameobject entry (%u) for quest %u, but GO is not GAMEOBJECT_TYPE_QUESTGIVER", itr->first, itr->second);
...
void ObjectMgr::LoadCreatureQuestRelations()
{
    ...
    for (QuestRelations::iterator itr = _creatureQuestRelations.begin(); itr != _creatureQuestRelations.end(); ++itr)
    {
        ...
        if (!cInfo)
            LOG_ERROR("sql.sql", "Table `creature_questrelation` have data for not existed creature entry (%u) and existed quest %u", itr->first, itr->second);
        else if (!(cInfo->npcflag & UNIT_NPC_FLAG_QUESTGIVER))
            LOG_ERROR("sql.sql", "Table `creature_questrelation` has creature entry (%u) for quest %u, but npcflag does not include UNIT_NPC_FLAG_QUESTGIVER", itr->first, itr->second);
    }
}
...
void ObjectMgr::LoadCreatureInvolvedRelations()
{
    ...
    for (QuestRelations::iterator itr = _creatureQuestInvolvedRelations.begin(); itr != _creatureQuestInvolvedRelations.end(); ++itr)
    {
        CreatureTemplate const* cInfo = GetCreatureTemplate(itr->first);
        if (!cInfo)
            LOG_ERROR("sql.sql", "Table `creature_involvedrelation` have data for not existed creature entry (%u) and existed quest %u", itr->first, itr->second);
        else if (!(cInfo->npcflag & UNIT_NPC_FLAG_QUESTGIVER))
            LOG_ERROR("sql.sql", "Table `creature_involvedrelation` has creature entry (%u) for quest %u, but npcflag does not include UNIT_NPC_FLAG_QUESTGIVER", itr->first, itr->second);
...
void ObjectMgr::LoadReservedPlayersNames()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("misc", "Table `reserved_name` have invalid name: %s", name.c_str());
            continue;
        }
...
bool ObjectMgr::LoadFirelandsStrings(const char* table, int32 min_value, int32 max_value)
{
    ...
    if (start_value < 0)
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table '%s' attempt loaded with invalid range (%d - %d), strings not loaded.", table, min_value, max_value);
            return false;
        }
    ...
    if (!result)
    {
        if (min_value == MIN_FIRELANDS_STRING_ID)              // error only in case internal strings
            LOG_ERROR("server.loading", ">> Loaded 0 trinity strings. DB table `%s` is empty. Cannot continue.", table);
        else
            LOG_INFO("server.loading", ">> Loaded 0 string templates. DB table `%s` is empty.", table);
    ...
    do
    {
        ...
        if (entry == 0)
        {
            LOG_ERROR("sql.sql", "Table `%s` contain reserved entry 0, ignored.", table);
            continue;
        }
        else if (entry < start_value || entry >= end_value)
        {
            LOG_ERROR("sql.sql", "Table `%s` contain entry %i out of allowed range (%d - %d), ignored.", table, entry, min_value, max_value);
            continue;
        }
        ...
        {
            LOG_ERROR("sql.sql", "Table `%s` contain data for already loaded entry  %i (from another table?), ignored.", table, entry);
            continue;
        }
    ...
    if (min_value == MIN_FIRELANDS_STRING_ID)
        LOG_INFO("server.loading", ">> Loaded %u Firelands strings from table %s in %u ms", count, table, GetMSTimeDiffToNow(oldMSTime));
    else
        LOG_INFO("server.loading", ">> Loaded %u string templates from %s in %u ms", count, table, GetMSTimeDiffToNow(oldMSTime));
...
const char *ObjectMgr::GetFirelandsString(int32 entry, LocaleConstant locale_idx) const
{
    ...
    if (entry > 0)
        LOG_ERROR("sql.sql", "Entry %i not found in `firelands_string` table.", entry);
    else
        LOG_ERROR("sql.sql", "Firelands string entry %i not found in DB.", entry);
    return "<error>";
}
...
void ObjectMgr::LoadResearchSitesInfo()
{
    ...
    do
    {
        ...
        if (!rs)
        {
            LOG_ERROR("misc", "ResearchSite %u defined in `research_sites` does not exist in DBC", entry);
            continue;
        }
        ...
        switch (state)
        {
            case DIGSITE_CORRUPTED:
                LOG_ERROR("misc", "ResearchSite %u defined in `research_sites` is corrupted", entry);
                continue;
            case DIGSITE_DISABLE:
                LOG_ERROR("misc", "ResearchSite %u defined in `research_sites` is disable", entry);
                break;
            case DIGSITE_ENABLE:
            default:
                break;
    ...
    while (result->NextRow());
    LOG_ERROR("misc", ">> Loaded %u Research Dig Sites info", count);
}
...
void ObjectMgr::LoadGossipMenu()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table gossip_menu entry %u are using non-existing text_id %u", gMenu.entry, gMenu.text_id);
            continue;
        }
    ...
    LOG_INFO("server.loading", ">> Loaded %u gossip_menu entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGossipMenuItems()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table `gossip_menu_option` for menu %u, id %u has unknown icon id %u. Replacing with GOSSIP_ICON_CHAT", gMenuItem.MenuId, gMenuItem.OptionIndex, gMenuItem.OptionIcon);
        ...
        if (gMenuItem.OptionBroadcastTextId)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Table `gossip_menu_option` for menu %u, id %u has non-existing or incompatible OptionBroadcastTextId %u, ignoring.", gMenuItem.MenuId, gMenuItem.OptionIndex, gMenuItem.OptionBroadcastTextId);
                gMenuItem.OptionBroadcastTextId = 0;
            }
        }

        if (gMenuItem.OptionType >= GOSSIP_OPTION_MAX)
            LOG_ERROR("sql.sql", "Table `gossip_menu_option` for menu %u, id %u has unknown option id %u. Option will not be used", gMenuItem.MenuId, gMenuItem.OptionIndex, gMenuItem.OptionType);

        if (gMenuItem.ActionPoiId && !GetPointOfInterest(gMenuItem.ActionPoiId))
        {
            LOG_ERROR("sql.sql", "Table `gossip_menu_option` for menu %u, id %u use non-existing action_poi_id %u, ignoring", gMenuItem.MenuId, gMenuItem.OptionIndex, gMenuItem.ActionPoiId);
        ...
        if (gMenuItem.BoxBroadcastTextId)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Table `gossip_menu_option` for menu %u, id %u has non-existing or incompatible BoxBroadcastTextId %u, ignoring.", gMenuItem.MenuId, gMenuItem.OptionIndex, gMenuItem.BoxBroadcastTextId);
    ...
    LOG_INFO("server.loading", ">> Loaded %u gossip_menu_option entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
void ObjectMgr::FillMissingBroadcastTexts()
{
    ...
    for (GossipMenuItemsContainer::iterator gItr = _gossipMenuItemsStore.begin(); gItr != _gossipMenuItemsStore.end(); ++gItr)
    {
        if (!gItr->second.OptionBroadcastTextId)
        {
            if (uint32 bctId = sObjectMgr->FindBroadcastTextId(gItr->second.OptionText))
            {
                ...
                ++count;
                LOG_INFO("server.loading", "ObjectMgr::FillMissingBroadcastTexts: Found broadcast text (Id: %u) for gossipmenu option (MenuId: %u, Id: %u).", bctId, gItr->second.MenuId, gItr->second.OptionIndex);
        ...
        if (!gItr->second.BoxText.empty() && !gItr->second.BoxBroadcastTextId)
        {
            if (uint32 bctId = sObjectMgr->FindBroadcastTextId(gItr->second.BoxText))
            {
                ...
                ++count;
                LOG_INFO("server.loading", "ObjectMgr::FillMissingBroadcastTexts: Found broadcast text (Id: %u) for gossipmenu option box (MenuId: %u, Id: %u).", bctId, gItr->second.MenuId, gItr->second.OptionIndex);
    ...
    LOG_INFO("server.loading", ">> Found %u missing broadcast texts for gossipmenu options in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
 }
...
void ObjectMgr::LoadBroadcastTexts()
{
    ...
    do
    {
        ...
        if (bct.SoundId)
        {
            ...
            {
                LOG_INFO("sql.sql", "BroadcastText (Id: %u) in table `broadcast_text` has SoundId %u but sound does not exist. Skipped.", bct.Id, bct.SoundId);
        ...
        {
            LOG_INFO("sql.sql", "BroadcastText (Id: %u) in table `broadcast_text` using Language %u but Language does not exist. Skipped.", bct.Id, bct.Language);
            // don't load bct of higher expansions
            continue;
        }
        ...
        if (bct.EmoteId0)
        {
            ...
            {
                LOG_INFO("sql.sql", "BroadcastText (Id: %u) in table `broadcast_text` has EmoteId0 %u but emote does not exist. Skipped.", bct.Id, bct.EmoteId0);
                // don't load bct of higher expansions
                continue;
            }
        }
        ...
        if (bct.EmoteId1)
        {
            ...
            {
                LOG_INFO("sql.sql", "BroadcastText (Id: %u) in table `broadcast_text` has EmoteId1 %u but emote does not exist. Skipped.", bct.Id, bct.EmoteId1);
                // don't load bct of higher expansions
                continue;
            }
        }
        ...
        if (bct.EmoteId2)
        {
            ...
            {
                LOG_INFO("sql.sql", "BroadcastText (Id: %u) in table `broadcast_text` has EmoteId2 %u but emote does not exist. Skipped.", bct.Id, bct.EmoteId2);
                // don't load bct of higher expansions
                continue;
            }
        }
    ...
    LOG_INFO("server.loading", ">> Loaded %u broadcast texts in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadBroadcastTextLocales()
{
    ...
    do
    {
        ...
        {
            LOG_INFO("sql.sql", "BroadcastText (Id: %u) in table `locales_broadcast_text` does not exist or is incompatible (bad expansions). Skipped!", id);
            // don't load bct of higher expansions
            continue;
        }
    ...
    LOG_INFO("server.loading", ">> Loaded %u broadcast text locales in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadBroadcastTextHelpers()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %u entries to broadcast text helper storage in %u ms", _broadcastTextHelperStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadBroadcastGroups()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %u broadcast text groups in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
bool ObjectMgr::IsVendorItemValid(uint32 vendor_entry, uint32 id, int32 maxcount, uint32 incrtime, uint32 ExtendedCost, uint8 type, Player* player, std::set<uint32>* skip_vendors, uint32 ORnpcflag) const
{
        ...
        (type == ITEM_VENDOR_TYPE_CURRENCY && !sCurrencyTypesStore.LookupEntry(id)))
    {
        if (player)
            ...
            LOG_ERROR("sql.sql", "Table `(game_event_)npc_vendor` for Vendor (Entry: %u) have in item list non-existed item (%u, type %u), ignore", vendor_entry, id, type);
        return false;
    }
    ...
    if (ExtendedCost && !sItemExtendedCostStore.LookupEntry(ExtendedCost))
    {
        if (player)
            ...
            LOG_ERROR("sql.sql", "Table `(game_event_)npc_vendor` have Item (Entry: %u) with wrong ExtendedCost (%u) for vendor (%u), ignore", id, ExtendedCost, vendor_entry);
        return false;
    }
    ...
    if (type == ITEM_VENDOR_TYPE_ITEM) // not applicable to currencies
    {
        if (maxcount > 0 && incrtime == 0)
        {
            if (player)
                ...
                LOG_ERROR("sql.sql", "Table `(game_event_)npc_vendor` has `maxcount` (%u) for item %u of vendor (Entry: %u) but `incrtime`=0, ignore", maxcount, id, vendor_entry);
            return false;
        }
        else if (maxcount == 0 && incrtime > 0)
        {
            if (player)
                ...
                LOG_ERROR("sql.sql", "Table `(game_event_)npc_vendor` has `maxcount`=0 for item %u of vendor (Entry: %u) but `incrtime`<>0, ignore", id, vendor_entry);
            return false;
        }
    }
    ...
    if (vItems->FindItemCostPair(id, ExtendedCost, type))
    {
        if (player)
            ...
            LOG_ERROR("sql.sql", "Table `npc_vendor` has duplicate items %u (with extended cost %u, type %u) for vendor (Entry: %u), ignoring", id, ExtendedCost, type, vendor_entry);
        return false;
    }
...
bool LoadFirelandsStrings(const char* table, int32 start_value, int32 end_value)
{
    ...
    {
        LOG_ERROR("sql.sql", "Table '%s' load attempted with range (%d - %d) reserved by Firelands, strings not loaded.", table, start_value, end_value+1);
        return false;
    }
...
void ObjectMgr::LoadFactionChangeAchievements()
{
    ...
    do
    {
        ...
        if (!sAchievementMgr->GetAchievement(alliance))
            LOG_ERROR("sql.sql", "Achievement %u referenced in `player_factionchange_achievement` does not exist, pair skipped!", alliance);
        else if (!sAchievementMgr->GetAchievement(horde))
            LOG_ERROR("sql.sql", "Achievement %u referenced in `player_factionchange_achievement` does not exist, pair skipped!", horde);
        else
...
void ObjectMgr::LoadFactionChangeItems()
{
    ...
    do
    {
        ...
        if (!GetItemTemplate(alliance))
            LOG_ERROR("sql.sql", "Item %u referenced in `player_factionchange_items` does not exist, pair skipped!", alliance);
        else if (!GetItemTemplate(horde))
            LOG_ERROR("sql.sql", "Item %u referenced in `player_factionchange_items` does not exist, pair skipped!", horde);
        else
...
void ObjectMgr::LoadFactionChangeSpells()
{
    ...
    do
    {
        ...
        if (!sSpellMgr->GetSpellInfo(alliance))
            LOG_ERROR("sql.sql", "Spell %u referenced in `player_factionchange_spells` does not exist, pair skipped!", alliance);
        else if (!sSpellMgr->GetSpellInfo(horde))
            LOG_ERROR("sql.sql", "Spell %u referenced in `player_factionchange_spells` does not exist, pair skipped!", horde);
        else
...
void ObjectMgr::LoadFactionChangeReputations()
{
    ...
    do
    {
        ...
        if (!sFactionStore.LookupEntry(alliance))
            LOG_ERROR("sql.sql", "Reputation %u referenced in `player_factionchange_reputations` does not exist, pair skipped!", alliance);
        else if (!sFactionStore.LookupEntry(horde))
            LOG_ERROR("sql.sql", "Reputation %u referenced in `player_factionchange_reputations` does not exist, pair skipped!", horde);
        else
...
void ObjectMgr::LoadHotfixData()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %u hotfix info entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadMissingKeyChains()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %u KeyChain entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadReportQuestData()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %u report quest info entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadFactionChangeTitles()
{
    ...
    do
    {
        ...
        if (!sCharTitlesStore.LookupEntry(alliance))
            LOG_ERROR("sql.sql", "Title %u referenced in `player_factionchange_title` does not exist, pair skipped!", alliance);
        else if (!sCharTitlesStore.LookupEntry(horde))
            LOG_ERROR("sql.sql", "Title %u referenced in `player_factionchange_title` does not exist, pair skipped!", horde);
        else
...
void ObjectMgr::LoadPhaseDefinitions()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql", "Flags defined in phase_definitions in zoneId %d and entry %u does contain PHASE_FLAG_OVERWRITE_EXISTING and PHASE_FLAG_NEGATE_PHASE. Setting flags to PHASE_FLAG_OVERWRITE_EXISTING", PhaseDefinition.zoneId, PhaseDefinition.entry);
    ...
    LOG_INFO("server.loading", ">> Loaded %u phasing definitions in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadSpellPhaseInfo()
{
    ...
    do
    {
        ...
        if (!spell)
        {
            LOG_ERROR("sql.sql", "Spell %u defined in `spell_phase` does not exists, skipped.", spellPhaseInfo.spellId);
            continue;
        }

        if (!spell->HasAura(SPELL_AURA_PHASE))
        {
            LOG_ERROR("sql.sql", "Spell %u defined in `spell_phase` does not have aura effect type SPELL_AURA_PHASE, useless value.", spellPhaseInfo.spellId);
            continue;
        }
    ...
    while (result->NextRow());
    LOG_INFO("server.loading", ">> Loaded %u spell dbc infos in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
