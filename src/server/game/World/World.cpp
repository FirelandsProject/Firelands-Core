...
void World::LoadConfigSettings(bool reload)
{
    if (reload)
    {
        ...
        {
            LOG_ERROR("misc", "World settings reload fail: can't read settings from %s.", sConfigMgr->GetFilename().c_str());
            return;
        }
        sLog->LoadFromConfig();
    }
    ...
    {
        LOG_ERROR("server.loading", "StartPlayerMoney (%i) must be in range 0.." UI64FMTD ". Set to %u.", m_int_configs[CONFIG_START_PLAYER_MONEY], uint64(MAX_MONEY_AMOUNT), 0);
    ...
    {
        LOG_ERROR("server.loading", "Currency.StartValorPoints (%i) must be >= 0, set to default 0.", m_int_configs[CONFIG_CURRENCY_START_VALOR_POINTS]);
    ...
    {
        LOG_ERROR("server.loading", "Currency.ConquestPointsSoloArenaReward (%i) must be > 0, set to default 50.", m_int_configs[CONFIG_CURRENCY_CONQUEST_POINTS_SOLO_ARENA_REWARD]);
    ...
    {
        LOG_ERROR("server.loading", "Battleground.Exception.Starttime (%i) can't be load. Set to 13.", m_int_configs[CONFIG_BATTLEGROUND_EXCEPTION_START]);
    ...
    {
        LOG_ERROR("server.loading", "Battleground.Exception.Endtime (%i) can't be load. Set to 6.", m_int_configs[CONFIG_BATTLEGROUND_EXCEPTION_END]);
...
void World::SetInitialWorldSettings()
{
    ...
    {
        LOG_ERROR("server.loading", "Correct *.map files not found in path '%smaps' or *.vmtree/*.vmtile files in '%svmaps'. Please place *.map/*.vmtree/*.vmtile files in appropriate directories or correct the DataDir value in the worldserver.conf file.", m_dataPath.c_str(), m_dataPath.c_str());
        exit(1);
    }
    ...
    LOG_INFO("server.loading", "Loading Spell Pre Nerf corrections...");
    ...
    LOG_INFO("server.loading", "Loading Spell Phase Dbc Info...");
    ...
    LOG_INFO("server.loading", "Loading Equipment templates...");
    ...
    LOG_INFO("server.loading", "Loading Instance Teleportations Destinations Data...");
    ...
    LOG_INFO("server.loading", "Loading Quests Relations...");
    ...
    LOG_INFO("server.loading", "Loading Player Corpses...");
    ...
    LOG_INFO("server.loading", "Loading Research Dig Sites info...");
    ...
    LOG_INFO("server.loading", "Deleting expired auctions...");
    ...
    LOG_INFO("server.loading", "Loading Phase definitions...");
    ...
    LOG_INFO("server.loading", "Loading Scripts text locales...");      // must be after Load*Scripts calls
    ...
    LOG_INFO("server.loading", "Loading Transport NPCs...");
    sMapMgr->LoadTransportNPCs();

    LOG_INFO("server.loading", "Loading Transport GOs...");
    ...
    LOG_INFO("server.loading", "Calculate next weekly LFR looted reset time...");
    ...
    LOG_INFO("misc", "Initializing Opcodes...");
    opcodeTable.Initialize();

    LOG_INFO("misc", "Initializing Performance logger...");
    sPerfLog->Initialize();

    LOG_INFO("misc", "Initializing Info Manager...");
    sInfoMgr->Initialize();

    LOG_INFO("misc", "Loading hotfix info...");
    sObjectMgr->LoadHotfixData();

    LOG_INFO("server.loading", "Loading missing KeyChains...");
    sObjectMgr->LoadMissingKeyChains();

    LOG_INFO("misc", "Loading report quest info...");
    sObjectMgr->LoadReportQuestData();

#ifdef FIRELANDS_DEBUG
    LOG_INFO("server.loading", "Validate all player spells...");
...
void World::RecordTimeDiff(const char *text, ...)
{
    ...
    if (diff > m_int_configs[CONFIG_MIN_LOG_UPDATE])
    {
        ...
        va_end(ap);
        LOG_INFO("misc", "Difftime %s: %u.", str, diff);
    }
...
void World::Update(uint32 diff)
{
    ...
    if (m_int_configs[CONFIG_INTERVAL_LOG_UPDATE] && diff > m_int_configs[CONFIG_MIN_LOG_UPDATE])
    {
        ...
        {
            LOG_DEBUG("misc", "Update time diff: %u. Players online: %u.", m_updateTimeSum / m_updateTimeCount, GetActiveSessionCount());
...
void World::SendWorldText(int32 string_id, ...)
{
    ...
    Firelands::WorldWorldTextBuilder wt_builder(string_id, 0, &ap);
...
{
    Firelands::WorldWorldTextBuilder wt_builder(string_id, text, NULL);
    Firelands::LocalizedPacketListDo<Firelands::WorldWorldTextBuilder> wt_do(wt_builder);
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        if (!itr->second || !itr->second->GetPlayer() || !itr->second->GetPlayer()->IsInWorld())
...
void World::ShutdownCancel()
{
    ...
    LOG_DEBUG("misc", "Server %s cancelled.", (m_ShutdownMask & SHUTDOWN_MASK_RESTART ? "restart" : "shuttingdown"));
...
void World::SendAutoBroadcast()
{
    ...
    {
        if (!m_Autobroadcasts.empty())
            id = Firelands::Containers::SelectRandomContainerElement(m_Autobroadcasts).first;
    }
    ...
    LOG_DEBUG("misc", "Sent AutoBroadcast");
}
...
{
    LOG_INFO("misc", "Weekly lfr loot reset for all characters.");
...
void World::ResetGuildCap()
{
    ...
    LOG_INFO("misc", "Guild Daily Cap reset. Week: %u", weekReset);
