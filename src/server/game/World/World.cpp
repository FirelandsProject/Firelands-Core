...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_defaultDbcLocale = LocaleConstant(sConfigMgr->GetOption<int32>("DBC.Locale", 0));
    ...
    SetPlayerAmountLimit(sConfigMgr->GetOption<int32>("PlayerLimit", 100));
    SetMotd(sConfigMgr->GetOption<std::string>("Motd", "Welcome to a Firelands Core Server."));
    ...
    m_bool_configs[CONFIG_ALLOW_BUG_REPORTS_AND_SUGGESTIONS] = sConfigMgr->GetOption<bool>("AllowBugsReportsAndSuggestions", false);
    ...
    rate_values[RATE_HEALTH] = sConfigMgr->GetOption<float>("Rate.Health", 1.0f);
    if (rate_values[RATE_HEALTH] < 0)
    {
        LOG_ERROR("server.loading", "Rate.Health (%f) must be > 0. Using 1 instead.", rate_values[RATE_HEALTH]);
        rate_values[RATE_HEALTH] = 1;
    }
    rate_values[RATE_POWER_MANA] = sConfigMgr->GetOption<float>("Rate.Mana", 1.0f);
    if (rate_values[RATE_POWER_MANA] < 0)
    {
        LOG_ERROR("server.loading", "Rate.Mana (%f) must be > 0. Using 1 instead.", rate_values[RATE_POWER_MANA]);
        rate_values[RATE_POWER_MANA] = 1;
    }
    rate_values[RATE_POWER_RAGE_INCOME] = sConfigMgr->GetOption<float>("Rate.Rage.Income", 1.0f);
    rate_values[RATE_POWER_RAGE_LOSS] = sConfigMgr->GetOption<float>("Rate.Rage.Loss", 1.0f);
    if (rate_values[RATE_POWER_RAGE_LOSS] < 0)
    {
        LOG_ERROR("server.loading", "Rate.Rage.Loss (%f) must be > 0. Using 1 instead.", rate_values[RATE_POWER_RAGE_LOSS]);
        rate_values[RATE_POWER_RAGE_LOSS] = 1;
    }
    rate_values[RATE_POWER_RUNICPOWER_INCOME] = sConfigMgr->GetOption<float>("Rate.RunicPower.Income", 1.0f);
    rate_values[RATE_POWER_RUNICPOWER_LOSS] = sConfigMgr->GetOption<float>("Rate.RunicPower.Loss", 1.0f);
    ...
    rate_values[RATE_MONEY_QUEST] = sConfigMgr->GetOption<float>("Rate.Quest.Money.Reward", 1.0f);
    ...
    rate_values[RATE_MONEY_MAX_LEVEL_QUEST] = sConfigMgr->GetOption<float>("Rate.Quest.Money.Max.Level.Reward", 1.0f);
    ...
    m_int_configs[CONFIG_AUCTION_GETALL_DELAY] = sConfigMgr->GetOption<int32>("Auction.GetAllScanDelay", 900);
    m_int_configs[CONFIG_AUCTION_SEARCH_DELAY] = sConfigMgr->GetOption<int32>("Auction.SearchDelay", 300);
    ...
    m_int_configs[CONFIG_CHAT_EMOTE_LEVEL_REQ] = sConfigMgr->GetOption<int32>("ChatLevelReq.Emote", 1);
    m_int_configs[CONFIG_CHAT_SAY_LEVEL_REQ] = sConfigMgr->GetOption<int32>("ChatLevelReq.Say", 1);
    m_int_configs[CONFIG_CHAT_YELL_LEVEL_REQ] = sConfigMgr->GetOption<int32>("ChatLevelReq.Yell", 1);
    ...
    m_bool_configs[CONFIG_GRID_UNLOAD] = sConfigMgr->GetOption<bool>("GridUnload", true);
    m_bool_configs[CONFIG_BASEMAP_LOAD_GRIDS] = sConfigMgr->GetOption<bool>("BaseMapLoadAllGrids", false);
    ...
    m_bool_configs[CONFIG_INSTANCEMAP_LOAD_GRIDS] = sConfigMgr->GetOption<bool>("InstanceMapLoadAllGrids", false);
    ...
    m_int_configs[CONFIG_INTERVAL_GRIDCLEAN] = sConfigMgr->GetOption<int32>("GridCleanUpDelay", 5 * MINUTE * IN_MILLISECONDS);
    ...
    m_int_configs[CONFIG_INTERVAL_MAPUPDATE] = sConfigMgr->GetOption<int32>("MapUpdateInterval", 10);
    ...
    if (reload)
    {
        ...
        val = sConfigMgr->GetOption<int32>("InstanceServerPort", 8086);
    ...
    else
    {
        ...
        m_int_configs[CONFIG_PORT_INSTANCE] = sConfigMgr->GetOption<int32>("InstanceServerPort", 8086);
    }
    ...
    m_int_configs[CONFIG_MIN_QUEST_SCALED_XP_RATIO] = sConfigMgr->GetOption<int32>("MinQuestScaledXPRatio", 0);
    ...
    m_int_configs[CONFIG_MIN_CREATURE_SCALED_XP_RATIO] = sConfigMgr->GetOption<int32>("MinCreatureScaledXPRatio", 0);
    ...
    m_int_configs[CONFIG_MIN_DISCOVERED_SCALED_XP_RATIO] = sConfigMgr->GetOption<int32>("MinDiscoveredScaledXPRatio", 0);
    ...
    m_float_configs[CONFIG_SIGHT_MONSTER] = sConfigMgr->GetOption<float>("MonsterSight", 50.0f);
    ...
    m_int_configs[CONFIG_DEATH_KNIGHTS_PER_REALM] = sConfigMgr->GetOption<int32>("DeathKnightsPerRealm", 1);
    ...
    m_int_configs[CONFIG_CHARACTER_CREATING_MIN_LEVEL_FOR_DEATH_KNIGHT] = sConfigMgr->GetOption<int32>("CharacterCreating.MinLevelForDeathKnight", 55);
    ...
    m_int_configs[CONFIG_START_DEATH_KNIGHT_PLAYER_LEVEL] = sConfigMgr->GetOption<int32>("StartDeathKnightPlayerLevel", 55);
    ...
    m_int_configs[CONFIG_CURRENCY_RESET_HOUR] = sConfigMgr->GetOption<int32>("Currency.ResetHour", 3);
    ...
    m_int_configs[CONFIG_CURRENCY_RESET_DAY] = sConfigMgr->GetOption<int32>("Currency.ResetDay", 3);
    ...
    m_int_configs[CONFIG_CURRENCY_RESET_INTERVAL] = sConfigMgr->GetOption<int32>("Currency.ResetInterval", 7);
    ...
    m_int_configs[CONFIG_CURRENCY_START_HONOR_POINTS] = sConfigMgr->GetOption<int32>("Currency.StartHonorPoints", 0);
    ...
    m_int_configs[CONFIG_CURRENCY_MAX_HONOR_POINTS] = sConfigMgr->GetOption<int32>("Currency.MaxHonorPoints", 4000);
    ...
    m_int_configs[CONFIG_CURRENCY_START_JUSTICE_POINTS] = sConfigMgr->GetOption<int32>("Currency.StartJusticePoints", 0);
    ...
    m_int_configs[CONFIG_CURRENCY_MAX_JUSTICE_POINTS] = sConfigMgr->GetOption<int32>("Currency.MaxJusticePoints", 4000);
    ...
    m_int_configs[CONFIG_CURRENCY_START_CONQUEST_POINTS] = sConfigMgr->GetOption<int32>("Currency.StartConquestPoints", 0);
    ...
    m_int_configs[CONFIG_CURRENCY_CONQUEST_POINTS_WEEK_CAP] = sConfigMgr->GetOption<int32>("Currency.ConquestPointsWeekCap", 1650);
    ...
    m_int_configs[CONFIG_CURRENCY_CONQUEST_POINTS_ARENA_REWARD] = sConfigMgr->GetOption<int32>("Currency.ConquestPointsArenaReward", 180);
    ...
    m_bool_configs[CONFIG_INSTANT_TAXI] = sConfigMgr->GetOption<bool>("InstantFlightPaths", false);
    ...
    m_bool_configs[CONFIG_CAST_UNSTUCK] = sConfigMgr->GetOption<bool>("CastUnstuck", true);
    ...
    m_int_configs[CONFIG_DAILY_QUEST_RESET_TIME_HOUR] = sConfigMgr->GetOption<int32>("Quests.DailyResetTime", 3);
    ...
    m_int_configs[CONFIG_WEEKLY_QUEST_RESET_TIME_WDAY] = sConfigMgr->GetOption<int32>("Quests.WeeklyResetWDay", 3);
    ...
    m_int_configs[CONFIG_GM_FREEZE_DURATION] = sConfigMgr->GetOption<int32>("GM.FreezeAuraDuration", 0);
    ...
    m_int_configs[CONFIG_CLEAN_OLD_MAIL_TIME] = sConfigMgr->GetOption<int32>("CleanOldMailTime", 4);
    ...
    m_bool_configs[CONFIG_EVENT_ANNOUNCE] = sConfigMgr->GetOption<bool>("Event.Announce", false);
    ...
    m_int_configs[CONFIG_TALENTS_INSPECTING] = sConfigMgr->GetOption<int32>("TalentsInspecting", 1);
    ...
    m_float_configs[CONFIG_THREAT_RADIUS] = sConfigMgr->GetOption<float>("ThreatRadius", 60.0f);
    ...
    m_int_configs[CONFIG_RATED_BATTLEGROUND_ENABLE] = sConfigMgr->GetOption<int32>("RatedBattleground.Enable", 0);
    ...
    m_int_configs[CONFIG_RATED_BATTLEGROUND_REWARD] = sConfigMgr->GetOption<int32>("RatedBattleground.Reward", 40000);
    ...
    m_int_configs[CONFIG_ARENA_RATED_UPDATE_TIMER] = sConfigMgr->GetOption<int32>("Arena.RatedUpdateTimer", 5 * IN_MILLISECONDS);
    ...
    m_bool_configs[CONFIG_ARENA_LOG_EXTENDED_INFO] = sConfigMgr->GetOption<bool>("ArenaLog.ExtendedInfo", false);
    ...
    m_int_configs[CONFIG_CREATURE_PICKPOCKET_REFILL] = sConfigMgr->GetOption<int32>("Creature.PickPocketRefillDelay", 10 * MINUTE);
    ...
    m_int_configs[CONFIG_GUILD_NEWS_LOG_COUNT] = sConfigMgr->GetOption<int32>("Guild.NewsLogRecordsCount", GUILD_NEWSLOG_MAX_RECORDS);
    ...
    m_int_configs[CONFIG_CHARDELETE_DEATH_KNIGHT_MIN_LEVEL] = sConfigMgr->GetOption<int32>("CharDelete.DeathKnight.MinLevel", 0);
    m_int_configs[CONFIG_CHARDELETE_KEEP_DAYS] = sConfigMgr->GetOption<int32>("CharDelete.KeepDays", 30);

    // No aggro from gray mobs
    m_int_configs[CONFIG_NO_GRAY_AGGRO_ABOVE] = sConfigMgr->GetOption<int32>("NoGrayAggro.Above", 0);
    m_int_configs[CONFIG_NO_GRAY_AGGRO_BELOW] = sConfigMgr->GetOption<int32>("NoGrayAggro.Below", 0);
    ...
    m_bool_configs[CONFIG_RESPAWN_DYNAMIC_ESCORTNPC] = sConfigMgr->GetOption<bool>("Respawn.DynamicEscortNPC", false);
    ...
    m_bool_configs[CONFIG_ENABLE_MMAPS] = sConfigMgr->GetOption<bool>("mmap.enablePathFinding", true);
    ...
    m_int_configs[CONFIG_MAX_WHO] = sConfigMgr->GetOption<int32>("MaxWhoListReturns", 49);
    ...
    m_bool_configs[CONFIG_RESET_DUEL_COOLDOWNS] = sConfigMgr->GetOption<bool>("ResetDuelCooldowns", false);
    m_bool_configs[CONFIG_RESET_DUEL_HEALTH_MANA] = sConfigMgr->GetOption<bool>("ResetDuelHealthMana", false);
    ...
    m_bool_configs[CONFIG_ALLOW_TRACK_BOTH_RESOURCES] = sConfigMgr->GetOption<bool>("AllowTrackBothResources", false);
    ...
    // Dungeon finder
    m_int_configs[CONFIG_LFG_OPTIONSMASK] = sConfigMgr->GetOption<int32>("DungeonFinder.OptionsMask", 1);

    // DBC_ItemAttributes
    m_bool_configs[CONFIG_DBC_ENFORCE_ITEM_ATTRIBUTES] = sConfigMgr->GetOption<bool>("DBC.EnforceItemAttributes", true);
    ...
    m_int_configs[CONFIG_BG_REWARD_WINNER_HONOR_FIRST] = sConfigMgr->GetOption<int32>("Battleground.RewardWinnerHonorFirst", 27000);
    m_int_configs[CONFIG_BG_REWARD_WINNER_CONQUEST_FIRST] = sConfigMgr->GetOption<int32>("Battleground.RewardWinnerConquestFirst", 10000);
    m_int_configs[CONFIG_BG_REWARD_WINNER_HONOR_LAST] = sConfigMgr->GetOption<int32>("Battleground.RewardWinnerHonorLast", 13500);
    m_int_configs[CONFIG_BG_REWARD_WINNER_CONQUEST_LAST] = sConfigMgr->GetOption<int32>("Battleground.RewardWinnerConquestLast", 5000);
    m_int_configs[CONFIG_BG_REWARD_LOSER_HONOR_FIRST] = sConfigMgr->GetOption<int32>("Battleground.RewardLoserHonorFirst", 4500);
    m_int_configs[CONFIG_BG_REWARD_LOSER_HONOR_LAST] = sConfigMgr->GetOption<int32>("Battleground.RewardLoserHonorLast", 3500);
    ...
    m_bool_configs[CONFIG_INSTANCES_RESET_ANNOUNCE] = sConfigMgr->GetOption<bool>("InstancesResetAnnounce", false);
    ...
    // Guild configs
    m_bool_configs[CONFIG_GUILD_LEVELING_ENABLED] = sConfigMgr->GetOption<bool>("Guild.LevelingEnabled", true);
    m_int_configs[CONFIG_GUILD_SAVE_INTERVAL] = sConfigMgr->GetOption<int32>("Guild.SaveInterval", 15);
    m_int_configs[CONFIG_GUILD_MAX_LEVEL] = sConfigMgr->GetOption<int32>("Guild.MaxLevel", 25);
    m_int_configs[CONFIG_GUILD_UNDELETABLE_LEVEL] = sConfigMgr->GetOption<int32>("Guild.UndeletableLevel", 4);
    rate_values[RATE_XP_QUEST_GUILD_MODIFIER] = sConfigMgr->GetOption<float>("Guild.XPQuestModifier", 0.25f);
    rate_values[RATE_XP_BASEKILL_GUILD_MODIFIER] = sConfigMgr->GetOption<float>("Guild.XPBaseKillModifier", 4.0f);
    rate_values[RATE_XP_HEROIC_DUNGEON_GUILD_MODIFIER] = sConfigMgr->GetOption<float>("Guild.XPHeroicDungeonModifier", 1.25f);
    rate_values[RATE_XP_HEROIC_RAID_GUILD_MODIFIER] = sConfigMgr->GetOption<float>("Guild.XPHeroicRaidModifier", 1.0f);
    rate_values[RATE_XP_HONOR_EARNED_GUILD_MODIFIER] = sConfigMgr->GetOption<float>("Guild.XPHonorEarnedModifier", 10.0f);
    m_int_configs[CONFIG_GUILD_XP_REWARD_ARENA] = sConfigMgr->GetOption<int32>("Guild.XPRewardInArena", 138800);
    m_int_configs[CONFIG_GUILD_REPUTATION_QUEST_DIVIDER] = sConfigMgr->GetOption<int32>("Guild.ReputationQuestDivider", 450);
    m_int_configs[CONFIG_GUILD_DAILY_XP_CAP] = sConfigMgr->GetOption<int32>("Guild.DailyXPCap", 7807500);
    m_int_configs[CONFIG_GUILD_WEEKLY_REP_CAP] = sConfigMgr->GetOption<int32>("Guild.WeeklyReputationCap", 4375);
    ...
    m_bool_configs[CONFIG_UI_QUESTLEVELS_IN_DIALOGS] = sConfigMgr->GetOption<bool>("UI.ShowQuestLevelsInDialogs", false);
    ...
    m_bool_configs[CONFIG_TOLBARAD_ENABLE] = sConfigMgr->GetOption<bool>("TolBarad.Enable", true);
    m_int_configs[CONFIG_TOLBARAD_PLR_MAX] = sConfigMgr->GetOption<int32>("TolBarad.PlayerMax", 100);
    m_int_configs[CONFIG_TOLBARAD_PLR_MIN] = sConfigMgr->GetOption<int32>("TolBarad.PlayerMin", 0);
    m_int_configs[CONFIG_TOLBARAD_PLR_MIN_LVL] = sConfigMgr->GetOption<int32>("TolBarad.PlayerMinLvl", 85);
    m_int_configs[CONFIG_TOLBARAD_BATTLETIME] = sConfigMgr->GetOption<int32>("TolBarad.BattleTimer", 15);
    m_int_configs[CONFIG_TOLBARAD_BONUSTIME] = sConfigMgr->GetOption<int32>("TolBarad.BonusTime", 5);
    m_int_configs[CONFIG_TOLBARAD_NOBATTLETIME] = sConfigMgr->GetOption<int32>("TolBarad.NoBattleTimer", 150);
    m_int_configs[CONFIG_TOLBARAD_RESTART_AFTER_CRASH] = sConfigMgr->GetOption<int32>("TolBarad.CrashRestartTimer", 10);
    ...
    m_int_configs[CONFIG_PACKET_SPOOF_BANMODE] = sConfigMgr->GetOption<int32>("PacketSpoof.BanMode", (uint32)BAN_ACCOUNT);
    ...
    // AHBot
    m_int_configs[CONFIG_AHBOT_UPDATE_INTERVAL] = sConfigMgr->GetOption<int32>("AuctionHouseBot.Update.Interval", 20);
    ...
    m_bool_configs[CONFIG_PREVENT_RENAME_CUSTOMIZATION] = sConfigMgr->GetOption<bool>("PreventRenameCharacterOnCustomization", false);

    // HotSwap
    m_bool_configs[CONFIG_HOTSWAP_ENABLED] = sConfigMgr->GetOption<bool>("HotSwap.Enabled", true);
    m_bool_configs[CONFIG_HOTSWAP_RECOMPILER_ENABLED] = sConfigMgr->GetOption<bool>("HotSwap.EnableReCompiler", true);
    m_bool_configs[CONFIG_HOTSWAP_EARLY_TERMINATION_ENABLED] = sConfigMgr->GetOption<bool>("HotSwap.EnableEarlyTermination", true);
    m_bool_configs[CONFIG_HOTSWAP_BUILD_FILE_RECREATION_ENABLED] = sConfigMgr->GetOption<bool>("HotSwap.EnableBuildFileRecreation", true);
    m_bool_configs[CONFIG_HOTSWAP_INSTALL_ENABLED] = sConfigMgr->GetOption<bool>("HotSwap.EnableInstall", true);
    m_bool_configs[CONFIG_HOTSWAP_PREFIX_CORRECTION_ENABLED] = sConfigMgr->GetOption<bool>("HotSwap.EnablePrefixCorrection", true);

    // prevent character rename on character customization
    m_bool_configs[CONFIG_PREVENT_RENAME_CUSTOMIZATION] = sConfigMgr->GetOption<bool>("PreventRenameCharacterOnCustomization", false);

    // Allow 5-man parties to use raid warnings
    m_bool_configs[CONFIG_CHAT_PARTY_RAID_WARNINGS] = sConfigMgr->GetOption<bool>("PartyRaidWarnings", false);
    ...
    m_bool_configs[CONFIG_CACHE_DATA_QUERIES] = sConfigMgr->GetOption<bool>("CacheDataQueries", true);

    // Anti movement cheat measure. Time each client have to acknowledge a movement change until they are kicked
    m_int_configs[CONFIG_PENDING_MOVE_CHANGES_TIMEOUT] = sConfigMgr->GetOption<int32>("AntiCheat.PendingMoveChangesTimeoutTime", 0);
...
void World::SetInitialWorldSettings()
{
    ...
    if (uint32 realmId = sConfigMgr->GetOption<int32>("RealmID", 0)) // 0 reserved for auth
...
void World::Update(uint32 diff)
{
    ...
    if (m_timers[WUPDATE_UPTIME].Passed())
    {
        ...
        stmt->SetData(0, tmpDiff);
        stmt->SetData(1, uint16(maxOnlinePlayers));
        stmt->SetData(2, realm.Id.Realm);
        stmt->SetData(3, uint32(GameTime::GetStartTime()));
...
BanReturn World::BanAccount(BanMode mode, std::string const& nameOrIP, uint32 duration_secs, std::string const& reason, std::string const& author)
{
    ...
    case BAN_IP:
    {
        ...
        stmt->SetData(0, nameOrIP);
        resultAccounts = LoginDatabase.Query(stmt);
        stmt = LoginDatabase.GetPreparedStatement(LOGIN_INS_IP_BANNED);
        stmt->SetData(0, nameOrIP);
        stmt->SetData(1, duration_secs);
