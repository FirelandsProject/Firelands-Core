...
void World::SetInitialWorldSettings()
{
    ...
    if (!sObjectMgr->LoadFirelandsString()) {
...
void World::SendWorldTextOptional(uint32 string_id, uint32 flag, ...)
{
    ...
    for (auto const& itr : m_sessions)
    {
        ...
        {
            if (itr.second->GetPlayer()->GetPlayerSetting(FirelandsCataPSSource, SETTING_ANNOUNCER_FLAGS).HasFlag(flag))
            {
                continue;
            }
        }
...
void World::InitWeeklyQuestResetTime()
{
    ...
    m_NextWeeklyQuestReset = wstime > 0s ? wstime : Seconds(Firelands::Time::GetNextTimeWithDayAndHour(4, 6));
...
void World::InitDailyQuestResetTime()
{
    ...
    m_NextDailyQuestReset = wstime > 0s ? wstime : Seconds(Firelands::Time::GetNextTimeWithDayAndHour(-1, 6));
...
void World::InitMonthlyQuestResetTime()
{
    ...
    m_NextMonthlyQuestReset = wstime > 0s ? wstime : Seconds(Firelands::Time::GetNextTimeWithDayAndHour(-1, 6));
...
void World::InitRandomBGResetTime()
{
    ...
    m_NextRandomBGReset = wstime > 0s ? wstime : Seconds(Firelands::Time::GetNextTimeWithDayAndHour(-1, 6));
...
void World::InitCalendarOldEventsDeletionTime()
{
    ...
    Seconds nextDeletionTime = currentDeletionTime > 0s ? currentDeletionTime : Seconds(Firelands::Time::GetNextTimeWithDayAndHour(-1, getIntConfig(CONFIG_CALENDAR_DELETE_OLD_EVENTS_HOUR)));
...
void World::InitGuildResetTime()
{
    ...
    m_NextGuildReset = wstime > 0s ? wstime : Seconds(Firelands::Time::GetNextTimeWithDayAndHour(-1, 6));
...
void World::ResetDailyQuests()
{
    ...
    m_NextDailyQuestReset = Seconds(Firelands::Time::GetNextTimeWithDayAndHour(-1, 6));
    sWorld->setWorldState(WS_DAILY_QUEST_RESET_TIME, m_NextDailyQuestReset.count());
...
void World::ResetWeeklyQuests()
{
    ...
    m_NextWeeklyQuestReset = Seconds(Firelands::Time::GetNextTimeWithDayAndHour(4, 6));
    sWorld->setWorldState(WS_WEEKLY_QUEST_RESET_TIME, m_NextWeeklyQuestReset.count());
...
void World::ResetMonthlyQuests()
{
    ...
    m_NextMonthlyQuestReset = Seconds(Firelands::Time::GetNextTimeWithMonthAndHour(-1, 6));
    sWorld->setWorldState(WS_MONTHLY_QUEST_RESET_TIME, m_NextMonthlyQuestReset.count());
}
...
void World::ResetRandomBG()
{
    ...
    m_NextRandomBGReset = Seconds(Firelands::Time::GetNextTimeWithDayAndHour(-1, 6));
    sWorld->setWorldState(WS_BG_DAILY_RESET_TIME, m_NextRandomBGReset.count());
}
...
void World::ResetGuildCap()
{
    ...
    m_NextGuildReset = Seconds(Firelands::Time::GetNextTimeWithDayAndHour(-1, 6));
    sWorld->setWorldState(WS_GUILD_DAILY_RESET_TIME, m_NextGuildReset.count());
