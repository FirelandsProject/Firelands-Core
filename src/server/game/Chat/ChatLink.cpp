...
// |color|Hglyph:glyph_slot_id:glyph_prop_id|h[{}]|h|r                    -
...
inline bool CheckDelimiter(std::istringstream &iss, char delimiter,
  ...
  if (c != delimiter) {
    LOG_TRACE("chat.system",
              "ChatHandler::isValidChatMessage('{}'): invalid {} link "
...
bool ItemChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!ReadUInt32(iss, itemEntry)) {
    LOG_TRACE("chat.system",
              "ChatHandler::isValidChatMessage('{}'): sequence finished "
  ...
  if (!_item) {
    LOG_TRACE("chat.system",
              "ChatHandler::isValidChatMessage('{}'): got invalid itemEntry {} "
              "in |item command",
  ...
  if (_color != ItemQualityColors[_item->GetQuality()]) {
    LOG_TRACE("chat.system",
              ...
              "{}, but user claims {}",
              iss.str().c_str(), ItemQualityColors[_item->GetQuality()],
              _color);
    return false;
  }
  ...
  for (uint8 index = 0; index < propsCount; ++index) {
    ...
    if (!ReadInt32(iss, id)) {
      LOG_TRACE("chat.system",
                "ChatHandler::isValidChatMessage('{}'): sequence finished "
                "unexpectedly while reading item property ({})",
                iss.str().c_str(), index);
      return false;
    }
    if (id && (index == randomPropertyPosition)) {
      ...
      if (id > 0) {
        ...
        if (!_property) {
          LOG_TRACE("chat.system",
                    "ChatHandler::isValidChatMessage('{}'): got invalid item "
                    "property id {} in |item command",
                    iss.str().c_str(), id);
          return false;
        }
      } else {
...
bool ItemChatLink::ValidateName(char *buffer, char const *context) {
  ...
  char *suffixStrings =
      _suffix ? _suffix->Name : (_property ? _property->Name : nullptr);
  ...
  if (!res)
    LOG_TRACE("chat.system",
              "ChatHandler::isValidChatMessage('{}'): linked item (id: {}) "
...
bool QuestChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!_quest) {
    LOG_TRACE(
        "chat.system",
        "ChatHandler::isValidChatMessage('{}'): quest template {} not found",
  ...
  if (_questLevel >= STRONG_MAX_LEVEL) {
    LOG_TRACE(
        "chat.system",
        "ChatHandler::isValidChatMessage('{}'): quest level {} is too big",
...
bool QuestChatLink::ValidateName(char *buffer, char const *context) {
  ...
  if (!res)
    LOG_TRACE("chat.system",
              "ChatHandler::isValidChatMessage('{}'): linked quest (id: {}) "
...
bool SpellChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!_spell) {
    LOG_TRACE("chat.system",
              "ChatHandler::isValidChatMessage('{}'): got invalid spell id {} "
              "in |spell command",
...
bool SpellChatLink::ValidateName(char *buffer, char const *context) {
  ...
  if (_spell->HasAttribute(SPELL_ATTR0_TRADESPELL)) {
    ...
    if (bounds.first == bounds.second) {
      LOG_TRACE("chat.system",
                "ChatHandler::isValidChatMessage('{}'): skill line not found "
                "for spell {}",
                context, _spell->Id);
      return false;
    }
    ...
    if (!skillInfo) {
      LOG_TRACE("chat.system",
                "ChatHandler::isValidChatMessage('{}'): skill line ability not "
                "found for spell {}",
                context, _spell->Id);
      return false;
    }
    SkillLineEntry const *skillLine =
        sSkillLineStore.LookupEntry(skillInfo->SkillLine);
    if (!skillLine) {
      LOG_TRACE("chat.system",
                "ChatHandler::isValidChatMessage('{}'): skill line not found "
                "for skill {}",
                context, skillInfo->SkillLine);
      return false;
    }

    uint32 skillLineNameLength = strlen(skillLine->DisplayName);
    if (skillLineNameLength > 0 &&
        strncmp(skillLine->DisplayName, buffer, skillLineNameLength) == 0) {
  ...
  LOG_TRACE("chat.system",
            "ChatHandler::isValidChatMessage('{}'): linked spell (id: {}) name "
            "wasn't found in any localization",
            context, _spell->Id);
  return false;
}
...
bool AchievementChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!_achievement) {
    LOG_TRACE("chat.system",
              "ChatHandler::isValidChatMessage('{}'): got invalid achivement "
              "id {} in |achievement command",
  ...
  if (!ReadHex(iss, _guid, 0)) {
    LOG_TRACE("chat.system",
              "ChatHandler::isValidChatMessage('{}'): invalid hexadecimal "
  ...
  for (uint8 index = 0; index < propsCount; ++index) {
    ...
    if (!ReadUInt32(iss, _data[index])) {
      LOG_TRACE("chat.system",
                "ChatHandler::isValidChatMessage('{}'): sequence finished "
                "unexpectedly while reading achievement property ({})",
                iss.str().c_str(), index);
      return false;
    }
  }
...
bool AchievementChatLink::ValidateName(char *buffer, char const *context) {
  ...
  LOG_TRACE("chat.system",
            "ChatHandler::isValidChatMessage('{}'): linked achievement (id: "
            "{}) name wasn't found in any localization",
...
bool TalentChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!talentInfo) {
    LOG_TRACE("chat.system",
              "ChatHandler::isValidChatMessage('{}'): got invalid talent id {} "
              "in |talent command",
...
bool GlyphChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!_glyph) {
    LOG_TRACE("chat.system",
              "ChatHandler::isValidChatMessage('{}'): got invalid glyph id {} "
              "in |glyph command",
...
bool LinkExtractor::IsValidMessage() {
  ...
  while (!_iss.eof()) {
    ...
    case 'H':
      ...
      if (strcmp(buffer, "item") == 0)
        ...
        LOG_TRACE("chat.system",
                  "ChatHandler::isValidChatMessage('{}'): user sent "
    ...
    case 'h':
      ...
      if (*validSequenceIterator == 'h') {
        ...
        if (_iss.get() != '[') {
          LOG_TRACE("chat.system",
                    "ChatHandler::isValidChatMessage('{}'): link caption "
                    "doesn't start with '['",
                    _iss.str().c_str());
          return false;
        }
    ...
    default:
      LOG_TRACE(
          "chat.system",
          "ChatHandler::isValidChatMessage('{}'): got invalid command |{}",
  ...
  if (validSequence != validSequenceIterator) {
    LOG_TRACE("chat.system",
              "ChatHandler::isValidChatMessage('{}'): EOF in active sequence",
              _iss.str().c_str());
    return false;
  }
