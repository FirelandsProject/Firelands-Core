...
bool ItemChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        LOG_TRACE("chat.system", "ChatHandler::isValidChatMessage('%s'): linked item has color %u, but user claims %u", iss.str().c_str(), ItemQualityColors[_item->Quality], _color);
        return false;
    }
...
bool ItemChatLink::ValidateName(char* buffer, const char* context)
{
    ...
    if (!res)
        LOG_TRACE("chat.system", "ChatHandler::isValidChatMessage('%s'): linked item (id: %u) name wasn't found in any localization", context, _item->ItemId);
    return res;
}
...
bool SpellChatLink::ValidateName(char* buffer, const char* context)
{
    ...
    if (_spell->Attributes & SPELL_ATTR0_TRADESPELL)
    {
        ...
        if (!skillLine)
        {
            LOG_TRACE("chat.system", "ChatHandler::isValidChatMessage('%s'): skill line not found for skill %u", context, skillInfo->skillId);
            return false;
        }
...
bool TalentChatLink::Initialize(std::istringstream& iss)
{
    ...
    if (!_spell)
    {
        LOG_TRACE("chat.system", "ChatHandler::isValidChatMessage('%s'): got invalid spell id %u in |trade command", iss.str().c_str(), talentInfo->RankID[0]);
        return false;
    }
    // Delimiter
