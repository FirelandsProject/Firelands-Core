...
static void LogCommandUsage(WorldSession const& session, std::string_view cmdStr)
{
    ...
    std::string logMessage = Firelands::StringFormatFmt("Command: {} [Player: {} ({}) (Account: {}) X: {} Y: {} Z: {} Map: {} ({}) Area: {} ({}) Zone: {} ({}) Selected: {} ({})]",
...
void Firelands::Impl::ChatCommands::ChatCommandNode::SendCommandHelp(ChatHandler& handler) const
{
    ...
    if (hasInvoker)
    {
        if (std::holds_alternative<FirelandsStrings>(_help))
            handler.SendSysMessage(std::get<FirelandsStrings>(_help));
...
/*static*/ std::vector<std::string> Firelands::Impl::ChatCommands::ChatCommandNode::GetAutoCompletionsFor(ChatHandler const& handler, std::string_view cmdStr)
{
    ...
    while (!oldTail.empty())
    {
                ...
                auto possibility = ([prefix = std::string_view(path), suffix = std::string_view(newTail)](std::string_view match)
                    {
                        if (prefix.empty())
                        {
                            return Firelands::StringFormatFmt("{}{}{}", match, COMMAND_DELIMITER, suffix);
                        }
                        else
                        {
                            return Firelands::StringFormatFmt("{}{}{}{}{}", prefix, COMMAND_DELIMITER, match, COMMAND_DELIMITER, suffix);
                        }
                    });
        ...
        {
            path = Firelands::StringFormatFmt("{}{}{}", path, COMMAND_DELIMITER, it1->first);
        }
        cmd = &it1->second;
        map = &cmd->_subCommands;
    ...
    { /* there is some trailing text, leave it as is */
        ...
        { /* if we matched a command at some point, auto-complete it */
            return { Firelands::StringFormatFmt("{}{}{}", path, COMMAND_DELIMITER, oldTail) };
        }
        else
            return {};
    }
        ...
        auto possibility = ([prefix = std::string_view(path)](std::string_view match)
            {
                ...
                {
                    return Firelands::StringFormatFmt("{}{}{}", prefix, COMMAND_DELIMITER, match);
                }
            });
