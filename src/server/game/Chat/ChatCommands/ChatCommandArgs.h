...
namespace Firelands::Impl::ChatCommands
{
    ...
    struct ArgInfo<T, std::enable_if_t<std::is_integral_v<T> || std::is_floating_point_v<T>>>
    {
        static ChatCommandResult TryConsume(T& val, ChatHandler const* handler, std::string_view args)
        {
            ...
            if (Optional<T> v = StringTo<T>(token, 0))
                ...
                return FormatFirelandsString(handler, LANG_CMDPARSER_STRING_VALUE_INVALID, STRING_VIEW_FMT_ARG(token), GetTypeName<T>().c_str());
    ...
    struct ArgInfo<T, std::enable_if_t<std::is_enum_v<T>>>
    {
        ...
        static ChatCommandResult TryConsume(T& val, ChatHandler const* handler, std::string_view args)
        {
            ...
            if (next1)
                return FormatFirelandsString(handler, LANG_CMDPARSER_STRING_VALUE_INVALID, STRING_VIEW_FMT_ARG(strVal), GetTypeName<T>().c_str());
            else
                return next1;
        }
    };
    ...
    struct ArgInfo<Firelands::ChatCommands::Variant<Ts...>>
    {
            ...
            if constexpr (I < N)
            {
                ...
                else
                {
                    ...
                    if (StringStartsWith(nestedResult.GetErrorMessage(), "\""))
                        return Firelands::StringFormat("\"%s\"\n%s %s", thisResult.GetErrorMessage().c_str(), GetFirelandsString(handler, LANG_CMDPARSER_OR), nestedResult.GetErrorMessage().c_str());
                    else
                        return Firelands::StringFormat("\"%s\"\n%s \"%s\"", thisResult.GetErrorMessage().c_str(), GetFirelandsString(handler, LANG_CMDPARSER_OR), nestedResult.GetErrorMessage().c_str());
                }
            }
        ...
        static ChatCommandResult TryConsume(Firelands::ChatCommands::Variant<Ts...>& val, ChatHandler const* handler, std::string_view args)
        {
            ...
            if (result.HasErrorMessage() && (result.GetErrorMessage().find('\n') != std::string::npos))
                return Firelands::StringFormat("%s %s", GetFirelandsString(handler, LANG_CMDPARSER_EITHER), result.GetErrorMessage().c_str());
            return result;
        }
    };
