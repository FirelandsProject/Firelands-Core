...
ChatCommandResult Firelands::ChatCommands::AccountIdentifier::TryConsume(ChatHandler const* handler, std::string_view args)
{
    ...
    if (!id)
        return FormatFirelandsString(handler, LANG_CMDPARSER_ACCOUNT_NAME_NO_EXIST, STRING_VIEW_FMT_ARG(_name));
    ...
    if (AccountMgr::GetName(_id, _name))
        ...
        return FormatFirelandsString(handler, LANG_CMDPARSER_ACCOUNT_ID_NO_EXIST, _id);
}
...
ChatCommandResult Firelands::ChatCommands::PlayerIdentifier::TryConsume(ChatHandler const* handler, std::string_view args)
{
    ...
    if (val.holds_alternative<ObjectGuid::LowType>())
    {
        ...
        {
            return FormatFirelandsString(handler, LANG_CMDPARSER_CHAR_GUID_NO_EXIST, _guid.ToString().c_str());
        }
    ...
    else
    {
        ...
        if (!normalizePlayerName(_name))
            return FormatFirelandsString(handler, LANG_CMDPARSER_CHAR_NAME_INVALID, STRING_VIEW_FMT_ARG(_name));
        ...
        {
            return FormatFirelandsString(handler, LANG_CMDPARSER_CHAR_NAME_NO_EXIST, STRING_VIEW_FMT_ARG(_name));
        }
