...
Channel::Channel(uint32 channelId, uint32 team /*= 0*/,
    ...
    : _announceEnabled(false),  // no join/leave announces
      ...
      _persistentChannel(false), _isOwnerInvisible(false),
      _channelFlags(CHANNEL_FLAG_GENERAL), // for all built-in channels
      _channelId(channelId), _channelTeam(team), _ownerGuid(), _channelName(),
      _channelPassword(), _zoneEntry(zoneEntry) {
...
Channel::Channel(std::string const &name, uint32 team /*= 0*/)
    : _announceEnabled(true), _ownershipEnabled(true),
      _persistentChannel(false), _isOwnerInvisible(false),
      _channelFlags(CHANNEL_FLAG_CUSTOM), _channelId(0), _channelTeam(team),
      _ownerGuid(), _channelName(name), _channelPassword(),
      _zoneEntry(nullptr) {
  ...
  if (sWorld->getBoolConfig(CONFIG_PRESERVE_CUSTOM_CHANNELS)) {
    ...
    if (PreparedQueryResult result = CharacterDatabase.Query(stmt)) // load
    {
      ...
      _channelName =
          fields[0].Get<std::string>(); // re-get channel name. MySQL table
    ...
    } else // save
    {
      ...
      LOG_DEBUG("chat.system", "Channel({}) saved in database", name.c_str());
    }
...
void Channel::UpdateChannelInDB() const {
  if (_persistentChannel) {
    ...
    LOG_DEBUG("chat.system", "Channel({}) updated in database",
              _channelName.c_str());
  }
}
...
void Channel::JoinChannel(Player *player, std::string const &pass) {
  ...
  if (HasFlag(CHANNEL_FLAG_LFG) &&
      ...
      AccountMgr::IsPlayerAccount(
          player->GetSession()->GetSecurity()) && // FIXME: Move to RBAC
      player->GetGroup()) {
  ...
  if (_announceEnabled && !player->GetSession()->HasPermission(
                              rbac::RBAC_PERM_SILENTLY_JOIN_CHANNEL)) {
...
void Channel::KickOrBan(Player const *player, std::string const &badname,
  ...
  if (!info.IsModerator() &&
      !player->GetSession()->HasPermission(
          rbac::RBAC_PERM_CHANGE_CHANNEL_NOT_MODERATOR)) {
  ...
  if (!player->GetSession()->HasPermission(
          rbac::RBAC_PERM_CHANGE_CHANNEL_NOT_MODERATOR) &&
      changeowner && good != _ownerGuid) {
  ...
  } else if (!player->GetSession()->HasPermission(
                 rbac::RBAC_PERM_SILENTLY_JOIN_CHANNEL)) {
...
void Channel::SetMode(Player const *player, std::string const &p2n, bool mod,
  ...
  if (!victim || !IsOn(victim) ||
      (player->GetTeam() != newp->GetTeam() &&
       (!player->GetSession()->HasPermission(
            rbac::RBAC_PERM_TWO_SIDE_INTERACTION_CHANNEL) ||
        !newp->GetSession()->HasPermission(
            rbac::RBAC_PERM_TWO_SIDE_INTERACTION_CHANNEL)))) {
...
void Channel::List(Player const *player) const {
  ...
  for (PlayerContainer::const_iterator i = _playersStore.begin();
    ...
    if (member &&
        (player->GetSession()->HasPermission(
             rbac::RBAC_PERM_WHO_SEE_ALL_SEC_LEVELS) ||
         member->GetSession()->GetSecurity() <=
