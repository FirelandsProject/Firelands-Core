    ...
    if (LoadCommandTable())
    {
        ...
        if (result)
        {
            do
            {
                ...
                SetDataForCommandInTable(commandTableCache, name.c_str(), fields[1].Get<uint8>(), fields[2].Get<std::string>(), name);
            }
...
{
    const char *format = GetFirelandsString(entry);
    char str[1024];
    va_list ap;
    va_start(ap, entry);
...
const char *ChatHandler::GetFirelandsString(int32 entry) const
{
        ...
        if ((table[i].Handler)(this, table[i].Name[0] != '\0' ? text : oldtext))
        {
            if (!AccountMgr::IsPlayerAccount(table[i].SecurityLevel))
            {
                ...
                if (m_session)
                {
                    ...
                    LOG_GM(m_session->GetAccountId(), "Command: {} [Player: {} (Account: {}) X: {} Y: {} Z: {} Map: {} Selected {}: {} (GUID: {})]",
...
bool ChatHandler::SetDataForCommandInTable(std::vector<ChatCommand>& table, char const* text, uint32 security, std::string const& help, std::string const& fullcommand)
{
    ...
    for (uint32 i = 0; i < table.size(); i++)
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table `command` have unexpected subcommand '{}' in command '{}', skip.", text, fullcommand);
            return false;
        }

        if (table[i].SecurityLevel != security)
            LOG_INFO("misc", "Table `command` overwrite for command '{}' default security ({}) by {}", fullcommand, table[i].SecurityLevel, security);
    ...
    if (!cmd.empty())
    {
        if (&table == &getCommandTable())
            LOG_ERROR("sql.sql", "Table `command` have not existed command '{}', skip.", cmd);
        else
            LOG_ERROR("sql.sql", "Table `command` have not existed subcommand '{}' in command '{}', skip.", cmd, fullcommand);
    }
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
        ...
        CellCoord p(Firelands::ComputeCellCoord(pl->GetPositionX(), pl->GetPositionY()));
        Cell cell(p);

        Firelands::GameObjectWithDbGUIDCheck go_check(*pl, lowguid);
        Firelands::GameObjectSearcher<Firelands::GameObjectWithDbGUIDCheck> checker(pl, obj, go_check);

        TypeContainerVisitor<Firelands::GameObjectSearcher<Firelands::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
...
const char *CliHandler::GetFirelandsString(int32 entry) const
{
