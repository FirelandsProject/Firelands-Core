...
bool ChatHandler::HasLowerSecurityAccount(WorldSession *target,
  ...
  if (m_session->HasPermission(rbac::RBAC_PERM_CHECK_FOR_LOWER_SECURITY) &&
      !strong && !sWorld->getBoolConfig(CONFIG_GM_LOWER_SECURITY))
    return false;
  ...
  for (uint32 i = 0; i < table.size(); ++i) {
    ...
    if (!table[i].ChildCommands.empty()) {
      if (!ExecuteCommandInTable(table[i].ChildCommands, text, fullcmd)) {
        if (m_session &&
            !m_session->HasPermission(
                rbac::RBAC_PERM_COMMANDS_NOTIFY_COMMAND_NOT_FOUND_ERROR))
          return false;
...
bool ChatHandler::SetDataForCommandInTable(std::vector<ChatCommand> &table,
  ...
  for (uint32 i = 0; i < table.size(); i++) {
    ...
    // expected subcommand by full name DB content
      ...
      LOG_ERROR("sql.sql",
                "Table `command` contains an unexpected subcommand '{}' in "
                "command '{}', skipped.",
    ...
    if (table[i].Permission != permission)
      LOG_INFO("misc",
               "Table `command` overwrite for command '{}' default permission "
               "({}) by {}",
  ...
  if (!cmd.empty()) {
    if (&table == &getCommandTable())
      LOG_ERROR(
          "sql.sql",
          "Table `command` contains a non-existing command '{}', skipped.",
          cmd.c_str());
    else
      LOG_ERROR("sql.sql",
                "Table `command` contains a non-existing subcommand '{}' in "
                "command '{}', skipped.",
...
bool ChatHandler::_ParseCommands(char const *text) {
  ...
  if (m_session && !m_session->HasPermission(
                       rbac::RBAC_PERM_COMMANDS_NOTIFY_COMMAND_NOT_FOUND_ERROR))
    return false;
...
size_t ChatHandler::BuildChatPacket(WorldPacket &data, ChatMsg chatType,
  ...
  if (sender) {
    ...
    if (Player const *playerSender = sender->ToPlayer()) {
      ...
      gmMessage = playerSender->GetSession()->HasPermission(
          rbac::RBAC_PERM_COMMAND_GM_CHAT);
    }
  }
...
uint32 ChatHandler::extractSpellIdFromLink(char *text) {
  ...
  // [name] Shift-click form |color|Hglyph:glyph_slot_id:glyph_prop_id|h[{}]|h|r
