...
void OpcodeTable::ValidateAndSetOpcode<true, true>(uint16 opcode, char const* name, SessionStatus status, PacketProcessing processing, pOpcodeHandler handler)
{
    ...
    {
        LOG_ERROR("network.opcode", "Tried to override handler of %s with %s (opcode %u)", opcodeTable[opcode]->Name, name, opcode);
        return;
    }
...
{
    LOG_ERROR("network.opcode", "Tried to set handler for an invalid opcode %d", opcode);
}
...
{
    LOG_ERROR("network.opcode", "Opcode %s got value 0", name);
}
