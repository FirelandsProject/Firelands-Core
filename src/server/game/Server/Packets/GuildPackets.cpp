...
WorldPacket const *WorldPackets::Guild::GuildBankQueryResults::Write() {
  ...
  for (GuildBankItemInfo const &item : ItemInfo) {
    for (Guild::GuildBankItemInfo::GuildBankSocketEnchant const &socketEnchant :
         item.SocketEnchant) {
...
WorldPacket const *WorldPackets::Guild::GuildRosterUpdate::Write() {
  ...
  for (GuildRosterMemberData const &rosterMemberData : MemberData) {
    ...
    _worldPacket << rosterMemberData.Note;
    _worldPacket << rosterMemberData.OfficerNote;
    ...
    _worldPacket << rosterMemberData.Name;
    _worldPacket << int32(
        rosterMemberData.VirtualRealmAddress); // Todo: Validate
  }
...
ByteBuffer &operator<<(ByteBuffer &data,
                       WorldPackets::Guild::GuildRosterProfessionData const
                           &rosterProfessionData) {
