...
WorldPacket const *WorldPackets::Misc::PhaseShiftChange::Write() {
  ...
  for (uint16 uiMapPhaseId : UiMapPhaseIDs)
    _worldPacket << uint16(uiMapPhaseId); // phase id, controls only map display
                                          // (visible on all maps)
...
WorldPackets::Misc::Weather::Weather()
...
WorldPacket const *WorldPackets::Misc::LoadCUFProfiles::Write() {
  ...
  for (CUFProfile const *cufProfile : CUFProfiles) {
    ...
    _worldPacket.WriteBit(
        cufProfile->BoolOptions[CUF_AUTO_ACTIVATE_10_PLAYERS]);
    _worldPacket.WriteBit(cufProfile->BoolOptions[CUF_AUTO_ACTIVATE_5_PLAYERS]);
    _worldPacket.WriteBit(
        cufProfile->BoolOptions[CUF_AUTO_ACTIVATE_25_PLAYERS]);
    ...
    _worldPacket.WriteBit(
        cufProfile->BoolOptions[CUF_DISPLAY_HORIZONTAL_GROUPS]);
    _worldPacket.WriteBit(
        cufProfile->BoolOptions[CUF_AUTO_ACTIVATE_40_PLAYERS]);
    ...
    _worldPacket.WriteBit(
        cufProfile->BoolOptions[CUF_DISPLAY_NON_BOSS_DEBUFFS]);
    _worldPacket.WriteBit(
        cufProfile->BoolOptions[CUF_DISPLAY_MAIN_TANK_AND_ASSIST]);
    ...
    _worldPacket.WriteBit(
        cufProfile->BoolOptions[CUF_DISPLAY_ONLY_DISPELLABLE_DEBUFFS]);
    ...
    _worldPacket.WriteBit(
        cufProfile->BoolOptions[CUF_AUTO_ACTIVATE_15_PLAYERS]);
  ...
  for (CUFProfile const *cufProfile : CUFProfiles) {
    ...
    _worldPacket << cufProfile->ProfileName;
  }
