...
void WorldSession::SendPacket(WorldPacket const *packet,
  ...
  if (packet->GetConnection() != CONNECTION_TYPE_DEFAULT) {
    if (packet->GetConnection() != CONNECTION_TYPE_INSTANCE &&
      ...
      LOG_ERROR("network.opcode",
                "Prevented sending of instance only opcode %u with connection "
                "type %u to %s",
...
void WorldSession::HandleAddonRegisteredPrefixesOpcode(
  ...
  for (uint32 i = 0; i < count; ++i) {
    ...
    recvPacket >> readedData;
    _registeredAddonPrefixes.push_back(readedData);
  }
...
QueryCallback WorldSession::LoadPermissionsAsync() {
  ...
  LOG_DEBUG("rbac",
            "WorldSession::LoadPermissions [AccountId: %u, Name: %s, realmId: "
            "%d, secLevel: %u]",
...
void WorldSession::InvalidateRBACData() {
  LOG_DEBUG("rbac",
            "WorldSession::Invalidaterbac::RBACData [AccountId: %u, Name: %s, "
            "realmId: %d]",
            _RBACData->GetId(), _RBACData->GetName().c_str(), realm.Id.Realm);
  delete _RBACData;
  _RBACData = nullptr;
...
bool WorldSession::DosProtection::EvaluateOpcode(WorldPacket &p,
  ...
  case POLICY_BAN: {
    ...
    sWorld->BanAccount(bm, nameOrIp, duration, "DOS (Packet Flooding/Spoofing",
                       "Server: AutoDOS");
    LOG_WARN("network", "AntiDOS: Player automatically banned for %u seconds.",
             duration);
    Session->KickPlayer();
    return false;
  }
...
bool WorldSession::IsRightUnitBeingMoved(ObjectGuid guid) {
  ...
  // edit: this wouldn't happen in retail but it does in FC, even with a
  // legitimate client.
