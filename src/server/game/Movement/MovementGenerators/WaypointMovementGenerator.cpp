...
void WaypointMovementGenerator<Creature>::OnArrived(Creature* creature)
{
    ...
    {
        LOG_DEBUG("maps.script", "Creature movement start script %u at point %u for {}.", i_path->at(i_currentNode)->event_id, i_currentNode, creature->GetGUID());
...
void FlightPathMovementGenerator::PreloadEndGrid()
{
    ...
    if (endMap)
    {
        LOG_INFO("misc", "Preloading rid (%f, %f) for map %u at node index %u/%u", _endGridX, _endGridY, _endMapId, _preloadTargetNode, (uint32)(i_path->size()-1));
        endMap->LoadGrid(_endGridX, _endGridY);
    }
    else
        LOG_INFO("misc", "Unable to determine map to preload flightmaster grid");
}
