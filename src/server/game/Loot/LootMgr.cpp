...
uint32 LootStore::LoadLootTable()
{
    ...
    do
    {
        ...
        float  chanceOrQuestChance = fields[2].Get<float>();
        uint16 lootmode = fields[3].Get<uint16>();
        uint8  group = fields[4].Get<uint8>();
        int32  mincountOrRef = fields[5].Get<int32>();
        int32  maxcount = fields[6].Get<uint8>();
...
void LootStore::ReportUnusedIds(LootIdSet const& lootIdSet) const
{
    ...
    for (LootIdSet::const_iterator itr = lootIdSet.begin(); itr != lootIdSet.end(); ++itr)
        LOG_ERROR("sql.sql", "Table '{}' entry {} isn't {} and not referenced from loot, and then useless.", GetName(), *itr, GetEntryName());
}

void LootStore::ReportNotExistedId(uint32 id) const
{
    LOG_ERROR("sql.sql", "Table '{}' entry {} ({}) does not exist but used as loot id in DB.", GetName(), id, GetEntryName());
}
...
bool LootStoreItem::IsValid(LootStore const& store, uint32 entry) const
{
    ...
    {
        LOG_ERROR("sql.sql", "Table '{}' entry {} item {}: group ({}) must be less {} - skipped", store.GetName(), entry, itemid, group, 1 << 7);
        return false;
    }

    if (mincountOrRef == 0)
    {
        LOG_ERROR("sql.sql", "Table '{}' entry {} item {}: wrong mincountOrRef ({}) - skipped", store.GetName(), entry, itemid, mincountOrRef);
        return false;
    }
    ...
    if (mincountOrRef > 0)                                  // item (quest or non-quest) entry, maybe grouped
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table '{}' entry {} item {}: max count ({}) less that min count ({}) - skipped", store.GetName(), entry, itemid, int32(maxcount), mincountOrRef);
            return false;
        }
    }
    ...
    {
        if (needs_quest)
            LOG_ERROR("sql.sql", "Table '{}' entry {} item {}: quest chance will be treated as non-quest chance", store.GetName(), entry, itemid);
