...
void CharacterCache::LoadCharacterCacheStorage() {
  ...
  do {
    ...
    AddCharacterCacheEntry(
        ObjectGuid::Create<HighGuid::Player>(fields[0].Get<uint32>()) /*guid*/,
  ...
  LOG_INFO("server.loading",
           ...
           _characterCacheStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
ObjectGuid
CharacterCache::GetCharacterGuidByName(std::string const &name) const {
  auto itr = _characterCacheByNameStore.find(name);
  if (itr != _characterCacheByNameStore.end())
...
bool CharacterCache::GetPlayerGuildIdByGUID(ObjectGuid guid,
                                            uint32 &guildId) const {
  auto itr = _characterCacheStore.find(guid);
  if (itr == _characterCacheStore.end())
    return false;
