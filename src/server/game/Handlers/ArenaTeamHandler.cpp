...
void WorldSession::HandleArenaTeamCreateOpcode(WorldPacket &recvData) {
  ...
  if (_player->GetArenaTeamId(slot)) {
    SendArenaTeamCommandResult(ERR_ARENA_TEAM_INVITE_SS, "", _player->GetName(),
...
void WorldSession::HandleArenaTeamInviteOpcode(WorldPacket &recvData) {
  ...
  if (!player->IsMaxLevel()) {
    SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S, "", player->GetName(),
  ...
  if (player->GetArenaTeamId(arenaTeam->GetSlot())) {
    SendArenaTeamCommandResult(ERR_ARENA_TEAM_INVITE_SS, "", player->GetName(),
                               ERR_ALREADY_IN_ARENA_TEAM_S);
    return;
  }
...
void WorldSession::HandleArenaTeamRemoveOpcode(WorldPacket &recvData) {
  ...
  // Broadcast event
  arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_REMOVE_SSS, ObjectGuid::Empty, 3,
                            name, arenaTeam->GetName(), _player->GetName());
}
...
void WorldSession::HandleArenaTeamLeaderOpcode(WorldPacket &recvData) {
  ...
  arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_LEADER_CHANGED_SSS,
                            ObjectGuid::Empty, 3, _player->GetName(), name,
                            arenaTeam->GetName());
}
...
void WorldSession::SendArenaTeamCommandResult(uint32 teamAction,
  ...
  WorldPacket data(SMSG_ARENA_TEAM_COMMAND_RESULT,
                   2 + team.length() + player.length() + 4 + 4);
...
void WorldSession::SendNotInArenaTeamPacket(uint8 type) {
  ...
  uint32 error = 0;
  data << uint32(
      error); // 0 = ERR_ARENA_NO_TEAM_II, 1 = ERR_ARENA_EXPIRED_CAIS, 2 =
