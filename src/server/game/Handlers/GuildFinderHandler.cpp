...
void WorldSession::HandleGuildFinderAddRecruit(WorldPacket &recvPacket) {
  ...
  if (sGuildFinderMgr
          ->GetAllMembershipRequestsForPlayer(
  ...
  if (!(classRoles & GUILDFINDER_ALL_ROLES) ||
    ...
    return;
  if (!(availability & AVAILABILITY_ALWAYS) ||
  ...
  MembershipRequest request = MembershipRequest(
      GetPlayer()->GetGUID(), guildLowGuid, availability, classRoles,
      guildInterests, comment, GameTime::GetGameTime());
...
void WorldSession::HandleGuildFinderBrowse(WorldPacket &recvPacket) {
  ...
  uint32 availability = 0;
  uint32 guildInterests = 0;
  uint32 playerLevel = 0; // Raw player level (1-85), do they use
                          // MAX_FINDER_LEVEL when on level 85 ?
  ...
  LFGuildPlayer settings(player->GetGUID().GetCounter(), classRoles,
                         availability, guildInterests, ANY_FINDER_LEVEL);
  LFGuildStore guildList =
      sGuildFinderMgr->GetGuildsMatchingSetting(settings, player->GetTeamId());
  ...
  WorldPacket data(SMSG_LF_GUILD_BROWSE_UPDATED,
                   3 + guildCount * 65); // Estimated size
  data.WriteBits(guildCount, 19);

  for (LFGuildStore::const_iterator itr = guildList.begin();
    ...
    bufferData << guildSettings.GetComment();
    ...
    bufferData << guild->GetName();

    bufferData << uint32(guild->GetAchievementMgr().GetAchievementPoints());

    bufferData.WriteByteSeq(guildGUID[7]);

    bufferData << uint8(sGuildFinderMgr->HasRequest(
        player->GetGUID().GetCounter(), guild->GetGUID())); // Request pending
...
void WorldSession::HandleGuildFinderDeclineRecruit(WorldPacket &recvPacket) {
  ...
  sGuildFinderMgr->RemoveMembershipRequest(playerGuid.GetCounter(),
                                           GetPlayer()->GetGuildId());
}

void WorldSession::HandleGuildFinderGetApplications(
    WorldPacket & /*recvPacket*/) {
  LOG_DEBUG("network",
            "WORLD: Received CMSG_LF_GUILD_GET_APPLICATIONS"); // Empty opcode

  std::list<MembershipRequest> applicatedGuilds =
      sGuildFinderMgr->GetAllMembershipRequestsForPlayer(
          GetPlayer()->GetGUID().GetCounter());
  uint32 applicationsCount = applicatedGuilds.size();
  WorldPacket data(SMSG_LF_GUILD_MEMBERSHIP_LIST_UPDATED,
                   7 + 54 * applicationsCount);
  data.WriteBits(applicationsCount, 20);

  if (applicationsCount > 0) {
    ByteBuffer bufferData(54 * applicationsCount);
    for (std::list<MembershipRequest>::const_iterator itr =
             applicatedGuilds.begin();
         itr != applicatedGuilds.end(); ++itr) {
      Guild *guild = sGuildMgr->GetGuildById(itr->GetGuildId());
      LFGuildSettings guildSettings =
          sGuildFinderMgr->GetGuildSettings(itr->GetGuildId());
      ...
      bufferData << uint32(
          ...
          GameTime::GetGameTime()); // Time left to application expiry (seconds)
      ...
      bufferData << uint32(
          ...
          request.GetSubmitTime()); // Time since application (seconds)
      bufferData << uint32(guildSettings.GetInterests());
    }
  ...
  data << uint32(
      10 - sGuildFinderMgr->CountRequestsFromPlayer(
               GetPlayer()->GetGUID().GetCounter())); // Applications count left
...
void WorldSession::HandleGuildFinderGetRecruits(WorldPacket &recvPacket) {
  ...
  std::vector<MembershipRequest> recruitsList =
      sGuildFinderMgr->GetAllMembershipRequestsForGuild(player->GetGuildId());
  ...
  WorldPacket data(SMSG_LF_GUILD_RECRUIT_LIST_UPDATED,
                   7 + 26 * recruitCount + 53 * recruitCount);
  data.WriteBits(recruitCount, 20);

  for (std::vector<MembershipRequest>::const_iterator itr =
           recruitsList.begin();
       itr != recruitsList.end(); ++itr) {
    MembershipRequest request = *itr;
    ...
    dataBuffer << int32(
        GameTime::GetGameTime() -
        request
            .GetSubmitTime()); // Time in seconds since application submitted.
    dataBuffer << int32(request.GetAvailability());
    dataBuffer << int32(request.GetClassRoles());
    dataBuffer << int32(request.GetInterests());
    dataBuffer << int32(
        request.GetExpiryTime() -
        GameTime::GetGameTime()); // TIme in seconds until application expires.

    dataBuffer << request.GetName();
    dataBuffer << request.GetComment();
...
void WorldSession::HandleGuildFinderPostRequest(WorldPacket & /*recvPacket*/) {
  LOG_DEBUG("network",
            "WORLD: Received CMSG_LF_GUILD_POST_REQUEST"); // Empty opcode
  ...
  LFGuildSettings settings =
      sGuildFinderMgr->GetGuildSettings(player->GetGuildId());
...
void WorldSession::HandleGuildFinderRemoveRecruit(WorldPacket &recvPacket) {
  ...
  sGuildFinderMgr->RemoveMembershipRequest(GetPlayer()->GetGUID().GetCounter(),
                                           guildGuid.GetCounter());
}

// Sent any time a guild master sets an option in the interface and when listing
// / unlisting his guild
void WorldSession::HandleGuildFinderSetGuildPost(WorldPacket &recvPacket) {
  ...
  if (Guild *guild = sGuildMgr->GetGuildById(
          player->GetGuildId())) // Player must be guild master
    if (guild->GetLeaderGUID() != player->GetGUID())
      return;

  LFGuildSettings settings(listed, player->GetTeamId(), player->GetGuildId(),
                           classRoles, availability, guildInterests, level,
                           comment);
