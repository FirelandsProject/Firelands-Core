...
void WorldSession::HandleSendMail(WorldPacket& recvData)
{
    ...
    if (!receiverGuid)
    {
        LOG_INFO("network", "Player %u is sending mail to %s (GUID: non-existing!) with subject %s "
    ...
    LOG_INFO("network", "Player %u is sending mail to %s (%s) with subject %s and body %s "
    ...
    else
    {
        ...
        stmt->SetData(0, receiverGuid.GetCounter());
