...
void WorldSession::HandleCalendarGetCalendar(WorldPacket& /*recvData*/)
{
    ...
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_GET_CALENDAR [{}]", guid);
...
void WorldSession::HandleCalendarGetEvent(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_GET_EVENT. Player ["
        UI64FMTD "] Event [{}]", _player->GetGUID(), eventId);
...
{
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_GUILD_FILTER [{}]", _player->GetGUID());
    ...
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_GUILD_FILTER: Min level [%d], Max level [%d], Min rank [%d]", minLevel, maxLevel, minRank);
}

void WorldSession::HandleCalendarArenaTeam(WorldPacket& recvData)
{
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_ARENA_TEAM [{}]", _player->GetGUID());
...
void WorldSession::HandleCalendarUpdateEvent(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_UPDATE_EVENT [{}] EventId [{}"
                   "], InviteId [{}] Title %s, Description %s, type %u "
...
void WorldSession::HandleCalendarCopyEvent(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_COPY_EVENT [{}], EventId [{}"
        "] inviteId [{}] Time: %u", guid, eventId, inviteId, eventTime);
...
{
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_EVENT_INVITE");
    ...
    else
    {
        ...
        if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
        {
            ...
            inviteeGuid = MAKE_NEW_GUID(fields[0].Get<uint32>(), 0, HIGHGUID_PLAYER);
            inviteeTeam = Player::TeamForRace(fields[1].Get<uint8>());
    ...
    if (QueryResult result = CharacterDatabase.PQuery("SELECT flags FROM character_social WHERE guid = {} AND friend = {}", inviteeGuid, playerGuid))
    {
        Field* fields = result->Fetch();
...
void WorldSession::HandleCalendarEventSignup(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_EVENT_SIGNUP [{}] EventId [{}] Tentative %u", guid, eventId, tentative);
...
void WorldSession::HandleCalendarEventRsvp(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_EVENT_RSVP [{}] EventId ["
        UI64FMTD "], InviteId [{}], status %u", guid, eventId,
        inviteId, status);
...
void WorldSession::HandleCalendarEventRemoveInvite(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_EVENT_REMOVE_INVITE ["
        UI64FMTD "] EventId [{}], ownerInviteId ["
        UI64FMTD "], Invitee ([{}] id: [{}])",
...
void WorldSession::HandleCalendarEventStatus(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_EVENT_STATUS [{}] EventId ["
...
void WorldSession::HandleCalendarEventModeratorStatus(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_EVENT_MODERATOR_STATUS [{}] EventId ["
        UI64FMTD "] ownerInviteId [{}], Invitee ([{}] id: ["
...
void WorldSession::HandleCalendarComplain(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_COMPLAIN [{}] EventId ["
        UI64FMTD "] guid [{}] InviteId [{}]", guid, eventId, complainGUID, inviteId);
...
void WorldSession::HandleCalendarGetNumPending(WorldPacket& /*recvData*/)
{
    ...
    LOG_DEBUG("network.opcode", "CMSG_CALENDAR_GET_NUM_PENDING: [{}"
...
void WorldSession::HandleSetSavedInstanceExtend(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network.opcode", "CMSG_SET_SAVED_INSTANCE_EXTEND - MapId: %u, Difficulty: %u, ToggleExtend: %s", mapId, difficulty, toggleExtend ? "On" : "Off");
...
{
    LOG_DEBUG("network.opcode", "%s", add ? "SMSG_CALENDAR_RAID_LOCKOUT_ADDED" : "SMSG_CALENDAR_RAID_LOCKOUT_REMOVED");
    time_t currTime = time(NULL);
...
void WorldSession::SendCalendarRaidLockoutUpdated(InstanceSave const* save)
{
    ...
    uint64 guid = _player->GetGUID();
    LOG_DEBUG("network.opcode", "SMSG_CALENDAR_RAID_LOCKOUT_UPDATED [{}"
