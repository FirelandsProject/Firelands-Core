        ...
        do
        {
            ...
            if (!Player::ValidateAppearance(charInfo.RaceID, charInfo.ClassID, charInfo.SexID, charInfo.HairStyle, charInfo.HairColor, charInfo.FaceID, charInfo.FacialHair, charInfo.SkinID))
            {
                ...
                if (!(charInfo.Flags2 == CHAR_CUSTOMIZE_FLAG_CUSTOMIZE))
                {
                    ...
                    stmt->SetData(1, charInfo.Guid.GetCounter());
...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    {
        LOG_ERROR("entities.player.cheat", "Expansion %u account:[%d] tried to Create character with expansion %u race (%u)", GetAccountExpansion(), GetAccountId(), raceEntry->Race_related, createInfo->Race);
    ...
    {
        LOG_ERROR("entities.player.cheat", "Expansion %u account:[%d] tried to Create character with expansion %u class (%u)", GetAccountExpansion(), GetAccountId(), classEntry->Required_expansion, createInfo->Class);
                            ...
                            AddTransactionCallback(CharacterDatabase.AsyncCommitTransaction(characterTransaction)).AfterComplete([this, newChar = std::move(newChar)](bool success)
                                {
                                    if (success)
                                    {
                                        LOG_INFO("entities.player.character", "Account: %d (IP: %s) Create Character:[%s] (GUID: %u)", GetAccountId(), GetRemoteAddress().c_str(), newChar->GetName().c_str(), newChar->GetGUID().GetCounter());
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder const& holder)
{
    ...
    if (pCurrChar->GetGuildId() != 0)
    {
        ...
        else
        {
            ...
            LOG_ERROR("misc", "Player %s (GUID: %u) marked as member of not existing guild (id: %u), removing guild membership for player.", pCurrChar->GetName().c_str(), pCurrChar->GetGUID().GetCounter(), pCurrChar->GetGuildId());
            pCurrChar->SetInGuild(0);
        }
    }
...
void WorldSession::HandleCharCustomize(WorldPacket& recvData)
{
    ...
    {
        LOG_INFO("entities.player.cheat", "Account %u, IP: %s tried to customise %s, but it does not belong to their account!",
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
    ...
    {
        LOG_INFO("entities.player.cheat", "Account %u, IP: %s tried to factionchange character %s, but it does not belong to their account!",
    ...
    if (oldRace != factionChangeInfo->Race)
    {
        ...
        if (factionChangeInfo->FactionChange)
        {
            ...
            for (std::map<uint32, uint32>::const_iterator it = sObjectMgr->FactionChangeAchievements.begin(); it != sObjectMgr->FactionChangeAchievements.end(); ++it)
            {
                ...
                stmt->SetData(0, uint16(newTeam == ALLIANCE ? achiev_alliance : achiev_horde));
                stmt->SetData(1, lowGuid);
                trans->Append(stmt);

                stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHAR_ACHIEVEMENT);
                stmt->SetData(0, uint16(newTeam == ALLIANCE ? achiev_alliance : achiev_horde));
                stmt->SetData(1, uint16(newTeam == ALLIANCE ? achiev_horde : achiev_alliance));
                stmt->SetData(2, lowGuid);
                trans->Append(stmt);
            }
            ...
            for (std::map<uint32, uint32>::const_iterator it = sObjectMgr->FactionChangeItems.begin(); it != sObjectMgr->FactionChangeItems.end(); ++it)
            {
                ...
                stmt->SetData(0, (newTeam == ALLIANCE ? item_alliance : item_horde));
                stmt->SetData(1, (newTeam == ALLIANCE ? item_horde : item_alliance));
                stmt->SetData(2, lowGuid);
                trans->Append(stmt);
            }
            ...
            for (std::map<uint32, uint32>::const_iterator it = sObjectMgr->FactionChangeQuests.begin(); it != sObjectMgr->FactionChangeQuests.end(); ++it)
            {
                ...
                stmt->SetData(0, lowGuid);
                stmt->SetData(1, (newTeam == ALLIANCE ? quest_alliance : quest_horde));
                trans->Append(stmt);

                stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHAR_QUESTSTATUS_REWARDED_FACTION_CHANGE);
                stmt->SetData(0, (newTeam == ALLIANCE ? quest_alliance : quest_horde));
                stmt->SetData(1, (newTeam == ALLIANCE ? quest_horde : quest_alliance));
                stmt->SetData(2, lowGuid);
                trans->Append(stmt);
            }
            ...
            // Disable all old-faction specific quests
            {
                ...
                for (auto const& questTemplatePair : questTemplates)
                {
                    ...
                    if (!(questTemplatePair.second.GetAllowableRaces() & newRaceMask))
                    {
                        ...
                        stmt->SetData(0, lowGuid);
                        stmt->SetData(1, questTemplatePair.first);
                        trans->Append(stmt);
                    }
                }
            }
            ...
            for (std::map<uint32, uint32>::const_iterator it = sObjectMgr->FactionChangeSpells.begin(); it != sObjectMgr->FactionChangeSpells.end(); ++it)
            {
                ...
                stmt->SetData(0, (newTeam == ALLIANCE ? spell_alliance : spell_horde));
                stmt->SetData(1, lowGuid);
                trans->Append(stmt);

                stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHAR_SPELL_FACTION_CHANGE);
                stmt->SetData(0, (newTeam == ALLIANCE ? spell_alliance : spell_horde));
                stmt->SetData(1, (newTeam == ALLIANCE ? spell_horde : spell_alliance));
                stmt->SetData(2, lowGuid);
                trans->Append(stmt);
            }
...
void WorldSession::HandleReorderCharacters(WorldPacket& recvData)
{
    ...
    for (uint8 i = 0; i < charactersCount; ++i)
    {
        ...
        stmt->SetData(1, guids[i].GetCounter());
        trans->Append(stmt);
    }
