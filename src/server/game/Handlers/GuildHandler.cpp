...
void WorldSession::HandleGuildRemoveOpcode(WorldPacket &recvPacket) {
  ...
  LOG_DEBUG("guild", "CMSG_GUILD_REMOVE [%s]: Target: %s",
            GetPlayerInfo().c_str(), playerGuid.ToString().c_str());
...
void WorldSession::HandleGuildGetRosterOpcode(
...
void WorldSession::HandleGuildPromoteOpcode(WorldPacket &recvPacket) {
  ...
  LOG_DEBUG("guild", "CMSG_GUILD_PROMOTE [%s]: Target: %s",
            GetPlayerInfo().c_str(), targetGuid.ToString().c_str());
...
void WorldSession::HandleGuildAssignRankOpcode(WorldPacket &recvPacket) {
  ...
  LOG_DEBUG(
      "guild",
      "CMSG_GUILD_ASSIGN_MEMBER_RANK [%s]: Target: %s Rank: %u, Issuer: %s",
      GetPlayerInfo().c_str(), targetGuid.ToString().c_str(), rankId,
      setterGuid.ToString().c_str());
...
void WorldSession::HandleGuildSetNoteOpcode(WorldPacket &recvPacket) {
  ...
  LOG_DEBUG("guild",
            "CMSG_GUILD_SET_NOTE [%s]: Target: %s, Note: %s, Public: %u",
            GetPlayerInfo().c_str(), playerGuid.ToString().c_str(),
            note.c_str(), ispublic);
...
void WorldSession::HandleGuildDelRankOpcode(WorldPacket &recvPacket) {
  ...
  LOG_DEBUG("guild", "CMSG_GUILD_DEL_RANK [%s]: Rank: %u",
            GetPlayerInfo().c_str(), rankId);
...
void WorldSession::HandleSaveGuildEmblemOpcode(WorldPacket &recvPacket) {
  ...
  LOG_DEBUG("guild",
            "MSG_SAVE_GUILD_EMBLEM [%s]: Guid: [%s] Style: %d, Color: %d, "
            "BorderStyle: %d, BorderColor: %d, BackgroundColor: %d",
            GetPlayerInfo().c_str(), vendorGuid.ToString().c_str(),
            emblemInfo.GetStyle(), emblemInfo.GetColor(),
            emblemInfo.GetBorderStyle(), emblemInfo.GetBorderColor(),
  ...
  if (GetPlayer()->GetNPCIfCanInteractWith(vendorGuid,
    ...
    if (Guild *guild = GetPlayer()->GetGuild()) {
      guild->UpdateAchievementCriteria(
          ACHIEVEMENT_CRITERIA_TYPE_BUY_GUILD_TABARD, 0, 0, 0, nullptr,
          GetPlayer());
...
void WorldSession::HandleGuildBankMoneyWithdrawn(
    WorldPackets::Guild::GuildBankRemainingWithdrawMoneyQuery
        & /*recvPacket */) {
  LOG_DEBUG("guild", "CMSG_GUILD_BANK_REMAINING_WITHDRAW_MONEY_QUERY [%s]",
            GetPlayerInfo().c_str());
...
void WorldSession::HandleGuildPermissionsQuery(
    WorldPackets::Guild::GuildPermissionsQuery & /*packet*/) {
  LOG_DEBUG("guild", "CMSG_GUILD_PERMISSIONS_QUERY [%s]",
            GetPlayerInfo().c_str());
...
void WorldSession::HandleGuildBankQueryTab(
  ...
  LOG_DEBUG("guild",
            "CMSG_GUILD_BANK_QUERY_TAB [%s]: %s, TabId: %u, ShowTabs: %u",
            GetPlayerInfo().c_str(), packet.Banker.ToString().c_str(),
            packet.Tab, packet.FullUpdate);
...
void WorldSession::HandleGuildBankDepositMoney(
  ...
  LOG_DEBUG(
      "guild", "CMSG_GUILD_BANK_DEPOSIT_MONEY [%s]: [%s], money: " UI64FMTD,
...
void WorldSession::HandleGuildBankWithdrawMoney(
  ...
  LOG_DEBUG(
      "guild", "CMSG_GUILD_BANK_WITHDRAW_MONEY [%s]: [%s], money: " UI64FMTD,
      GetPlayerInfo().c_str(), packet.Banker.ToString().c_str(), packet.Money);
...
void WorldSession::HandleGuildBankSwapItems(
  ...
  if (packet.BankOnly) {
    guild->SwapItems(GetPlayer(), packet.BankTab1, packet.BankSlot1,
                     packet.BankTab, packet.BankSlot, packet.StackCount);
  } else {
    // Player <-> Bank
    // Allow to work with inventory only
    if (!Player::IsInventoryPos(packet.ContainerSlot,
                                packet.ContainerItemSlot) &&
        !packet.AutoStore)
      GetPlayer()->SendEquipError(EQUIP_ERR_INTERNAL_BAG_ERROR, nullptr);
    else
      guild->SwapItemsWithInventory(
          GetPlayer(), packet.ToSlot != 0, packet.BankTab, packet.BankSlot,
          packet.ContainerSlot, packet.ContainerItemSlot, packet.StackCount);
  }
}

void WorldSession::HandleGuildBankBuyTab(
    WorldPackets::Guild::GuildBankBuyTab &packet) {
  LOG_DEBUG("guild", "CMSG_GUILD_BANK_BUY_TAB [%s]: [%s[, TabId: %u",
            GetPlayerInfo().c_str(), packet.Banker.ToString().c_str(),
            packet.BankTab);

  if (!packet.Banker || GetPlayer()->GetGameObjectIfCanInteractWith(
                            packet.Banker, GAMEOBJECT_TYPE_GUILD_BANK))
    if (Guild *guild = GetPlayer()->GetGuild())
...
void WorldSession::HandleGuildBankUpdateTab(
  ...
  LOG_DEBUG(
      "guild",
      "CMSG_GUILD_BANK_UPDATE_TAB [%s]: [%s], TabId: %u, Name: %s, Icon: %s",
      GetPlayerInfo().c_str(), packet.Banker.ToString().c_str(), packet.BankTab,
      packet.Name.c_str(), packet.Icon.c_str());
  if (!packet.Name.empty() && !packet.Icon.empty())
    if (GetPlayer()->GetGameObjectIfCanInteractWith(packet.Banker,
                                                    GAMEOBJECT_TYPE_GUILD_BANK))
      if (Guild *guild = GetPlayer()->GetGuild())
        guild->HandleSetBankTabInfo(this, packet.BankTab, packet.Name,
                                    packet.Icon);
}

void WorldSession::HandleGuildBankLogQuery(
    WorldPackets::Guild::GuildBankLogQuery &packet) {
  LOG_DEBUG("guild", "CMSG_GUILD_BANK_LOG_QUERY [%s]: TabId: %u",
            GetPlayerInfo().c_str(), packet.Tab);

  if (Guild *guild = GetPlayer()->GetGuild())
    guild->SendBankLog(this, packet.Tab);
}

void WorldSession::HandleGuildBankTextQuery(
    WorldPackets::Guild::GuildBankTextQuery &packet) {
  LOG_DEBUG("guild", "CMSG_GUILD_BANK_TEXT_QUERY [%s]: TabId: %u",
            GetPlayerInfo().c_str(), packet.Tab);
...
void WorldSession::HandleGuildBankSetTabText(
    WorldPackets::Guild::GuildBankSetTabText &packet) {
  LOG_DEBUG("guild", "CMSG_SET_GUILD_BANK_TEXT [%s]: TabId: %u, Text: %s",
            GetPlayerInfo().c_str(), packet.Tab, packet.TabText.c_str());

  if (Guild *guild = GetPlayer()->GetGuild())
    guild->SetBankTabText(packet.Tab, packet.TabText);
}

void WorldSession::HandleGuildXPQueryOpcode(
    WorldPackets::Guild::GuildXPQuery &packet) {
  LOG_DEBUG("guild", "CMSG_QUERY_GUILD_XP [%s]: Guild: %s",
...
void WorldSession::HandleGuildSetRankPermissionsOpcode(
    WorldPacket &recvPacket) {
  Guild *guild = GetPlayer()->GetGuild();
  if (!guild) {
    recvPacket.rfinish();
  ...
  for (uint8 tabId = 0; tabId < GUILD_BANK_MAX_TABS; ++tabId) {
    ...
    rightsAndSlots[tabId] =
        GuildBankRightsAndSlots(tabId, uint8(bankRights), slots);
  }
  ...
  LOG_DEBUG("guild", "CMSG_GUILD_SET_RANK_PERMISSIONS [%s]: Rank: %s (%u)",
            GetPlayerInfo().c_str(), rankName.c_str(), newRankId);

  guild->HandleSetRankInfo(this, newRankId, rankName, newRights, moneyPerDay,
                           rightsAndSlots);
}
...
void WorldSession::HandleGuildRequestChallengeUpdate(
...
void WorldSession::HandleAutoDeclineGuildInvites(WorldPacket &recvPacket) {
  ...
  GetPlayer()->ApplyModFlag(PLAYER_FLAGS, PLAYER_FLAGS_AUTO_DECLINE_GUILD,
                            enable != 0);
}

void WorldSession::HandleRequestGuildRewardsList(
...
void WorldSession::HandleGuildReplaceGuildMaster(
    WorldPackets::Guild::ReplaceGuildMaster & /*packet */) {
  if (Guild *guild = GetPlayer()->GetGuild())
    guild->HandleSetNewGuildMaster(this, "", true);
}

void WorldSession::HandleGuildSetGuildMaster(
    WorldPackets::Guild::SetGuildMaster &packet) {
  if (Guild *guild = GetPlayer()->GetGuild())
...
void WorldSession::HandleGuildSwitchRank(WorldPacket &recvPacket) {
  ...
  uint32 rank;
  bool direction;                 // if its true, then the rank rises, if no, it
  goes down
...
void WorldSession::HandleGuildAchievementMembers(WorldPacket &recvPacket) {
  ...
  if (Guild *pGuild = sGuildMgr->GetGuildByGuid(guildGuid)) {
    if (pGuild->GetAchievementMgr().HasAchieved(achievementId)) {
      ...
      CompletedAchievementData *achievement =
          pGuild->GetAchievementMgr().GetCompletedDataForAchievement(
              achievementId);
      ...
      for (std::set<ObjectGuid>::iterator itr = achievement->guids.begin();
           itr != achievement->guids.end(); ++itr) {
  ...
  if (pGuild) {
    ...
    _queryProcessor.AddCallback(
        CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback(
            std::bind(&WorldSession::HandleGuildRenameCallback, this, newName,
                      std::placeholders::_1)));

    WorldPacket data(SMSG_GUILD_FLAGGED_FOR_RENAME, 1);

    data.WriteBit(true); // it may send false also, but we don't know how to
                         // handle exeptions by the DB layer
    data.FlushBits();
...
void WorldSession::HandleGuildQueryRecipesOpcode(
...
void WorldSession::HandleGuildQueryMembersForRecipe(
    WorldPackets::Guild::GuildQueryMembersForRecipe &packet) {
  if (Guild *guild = sGuildMgr->GetGuildByGuid(packet.GuildGUID))
    if (guild->IsMember(_player->GetGUID()))
      guild->SendMembersForRecipe(_player, packet.SkillLineID, packet.SpellID,
                                  packet.UniqueBit);
}

void WorldSession::HandleGuildQueryMemberRecipe(
    WorldPackets::Guild::GuildQueryMemberRecipes &packet) {
  if (Guild *guild = sGuildMgr->GetGuildByGuid(packet.GuildGUID))
    if (guild->IsMember(_player->GetGUID()) &&
        guild->IsMember(packet.GuildMember))
      guild->SendRecipesOfMember(_player, packet.SkillLineID,
                                 packet.GuildMember);
}
