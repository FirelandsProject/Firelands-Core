...
void WorldSession::HandleWhoOpcode(
  ...
  for (WhoListPlayerInfo const &target : whoList) {
    ...
    if (target.GetSecurity() > AccountTypes(gmLevelInWhoList) &&
        !HasPermission(rbac::RBAC_PERM_WHO_SEE_ALL_SEC_LEVELS))
      continue;
  ...
  if (player->GetMapId() != at->target_mapId) {
    if (Map::EnterState denyReason =
      ...
      case Map::CANNOT_ENTER_DIFFICULTY_UNAVAILABLE:
        LOG_DEBUG("maps",
                  "MAP: Player '%s' attempted to enter instance map %d but the "
                  "requested difficulty was not found",
                  player->GetName().c_str(), at->target_mapId);
...
void WorldSession::HandleSaveCUFProfiles(WorldPacket &recvPacket) {
  ...
  for (uint8 i = 0; i < count; ++i) {
    ...
    recvPacket >> profiles[i]->ProfileName;
