...
bool Group::Create(Player* leader)
{
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        ...
        stmt->SetData(index++, GUID_LOPART(m_looterGuid));
        stmt->SetData(index++, uint8(m_lootThreshold));
        stmt->SetData(index++, uint32(m_targetIcons[0]));
        stmt->SetData(index++, uint32(m_targetIcons[1]));
        stmt->SetData(index++, uint32(m_targetIcons[2]));
        stmt->SetData(index++, uint32(m_targetIcons[3]));
        stmt->SetData(index++, uint32(m_targetIcons[4]));
        stmt->SetData(index++, uint32(m_targetIcons[5]));
        stmt->SetData(index++, uint32(m_targetIcons[6]));
        stmt->SetData(index++, uint32(m_targetIcons[7]));
...
{
    m_dbStoreId = fields[15].Get<uint32>();
    m_guid = MAKE_NEW_GUID(sGroupMgr->GenerateGroupId(), 0, HIGHGUID_GROUP);
    m_leaderGuid = MAKE_NEW_GUID(fields[0].Get<uint32>(), 0, HIGHGUID_PLAYER);

    // group leader not exist
    if (!sObjectMgr->GetPlayerNameByGUID(fields[0].Get<uint32>(), m_leaderName))
        return;

    m_lootMethod = LootMethod(fields[1].Get<uint8>());
    m_looterGuid = MAKE_NEW_GUID(fields[2].Get<uint32>(), 0, HIGHGUID_PLAYER);
    m_lootThreshold = ItemQualities(fields[3].Get<uint8>());

    for (uint8 i = 0; i < TARGETICONCOUNT; ++i)
        m_targetIcons[i] = fields[4+i].Get<uint32>();
...
bool Group::AddMember(Player* player)
{
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        ...
        stmt->SetData(1, GUID_LOPART(member.guid));
...
void Group::ChangeLeader(uint64 newLeaderGuid)
{
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        ...
        stmt->SetData(0, newLeader->GetGUIDLow());
        stmt->SetData(1, m_dbStoreId);
...
{
    LOG_DEBUG("network.opcode", "Group::MasterLoot (SMSG_LOOT_MASTER_LIST)");
...
InstanceGroupBind* Group::BindToInstance(InstanceSave* save, bool permanent, bool load)
{
    ...
    bind.perm = permanent;
    if (!load)
        LOG_DEBUG("maps", "Group::BindToInstance: Group (guid: {}, storage id: {}) is now bound to map {}, instance {}, difficulty {}, isLfg {}",
...
void Group::BroadcastGroupUpdate(void)
{
    ...
    for (member_citerator citr = m_memberSlots.begin(); citr != m_memberSlots.end(); ++citr)
    {
        ...
        if (pp && pp->IsInWorld())
        {
            ...
            LOG_DEBUG("misc", "-- Forced group value update for '{}'", pp->GetName());
        }
    }
}
...
{
    LOG_DEBUG("network.opcode", "Group::MarkerChanged (SMSG_RAID_MARKERS_CHANGED)");
...
{
    LOG_DEBUG("network.opcode", "Group::RemoveMarker");
