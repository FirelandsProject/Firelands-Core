...
void LoadDisables() {
  ...
  do {
    ...
    case DISABLE_TYPE_SPELL:
      if (!(sSpellMgr->GetSpellInfo(entry) ||
        ...
        LOG_ERROR(
            "sql.sql",
            "Spell entry {} from `disables` doesn't exist in dbc, skipped.",
            entry);
        continue;
      }

      if (!flags || flags > MAX_SPELL_DISABLE_TYPE) {
        LOG_ERROR("sql.sql", "Disable flags for spell {} are invalid, skipped.",
                  entry);
        continue;
      }
      ...
      if (flags & SPELL_DISABLE_MAP) {
        std::vector<std::string_view> tokens =
            Firelands::Tokenize(params_0, ',', true);
      ...
      if (flags & SPELL_DISABLE_AREA) {
        std::vector<std::string_view> tokens =
            Firelands::Tokenize(params_1, ',', true);
        for (uint8 i = 0; i < tokens.size();)
    ...
    case DISABLE_TYPE_LFG_MAP: {
      ...
      if (!mapEntry) {
        LOG_ERROR("sql.sql",
                  "Map entry {} from `disables` doesn't exist in dbc, skipped.",
                  entry);
        continue;
      }
      ...
      case MAP_RAID:
        if (flags & DUNGEON_STATUSFLAG_HEROIC &&
            !sDBCManager.GetMapDifficultyData(entry, DUNGEON_DIFFICULTY_HEROIC))
          flags -= DUNGEON_STATUSFLAG_HEROIC;
        if (flags & RAID_STATUSFLAG_10MAN_HEROIC &&
            !sDBCManager.GetMapDifficultyData(entry,
                                              RAID_DIFFICULTY_10MAN_HEROIC))
          flags -= RAID_STATUSFLAG_10MAN_HEROIC;
        if (flags & RAID_STATUSFLAG_25MAN_HEROIC &&
            !sDBCManager.GetMapDifficultyData(entry,
      ...
      if (isFlagInvalid) {
        LOG_ERROR("sql.sql", "Disable flags for map {} are invalid, skipped.",
                  entry);
        continue;
      }
      break;
    ...
    case DISABLE_TYPE_BATTLEGROUND:
      if (!sBattlemasterListStore.LookupEntry(entry)) {
        LOG_ERROR("sql.sql",
                  "Battleground entry {} from `disables` doesn't exist in dbc, "
                  "skipped.",
                  entry);
        continue;
      }
    ...
    case DISABLE_TYPE_OUTDOORPVP:
      if (entry > MAX_OUTDOORPVP_TYPES) {
        LOG_ERROR(
            "sql.sql",
            "OutdoorPvPTypes value {} from `disables` is invalid, skipped.",
            entry);
        continue;
      }
      if (flags)
    ...
    case DISABLE_TYPE_ACHIEVEMENT_CRITERIA:
      if (!sAchievementMgr->GetAchievementCriteria(entry)) {
        LOG_ERROR("sql.sql",
                  "Achievement Criteria entry {} from `disables` doesn't exist "
                  "in dbc, skipped.",
                  entry);
        continue;
      }
    ...
    case DISABLE_TYPE_VMAP: {
      ...
      case MAP_COMMON:
        if (flags & VMAP::VMAP_DISABLE_AREAFLAG)
          LOG_INFO("misc", "Areaflag disabled for world map {}.", entry);
        if (flags & VMAP::VMAP_DISABLE_LIQUIDSTATUS)
          LOG_INFO("misc", "Liquid status disabled for world map {}.", entry);
        break;
      case MAP_INSTANCE:
      case MAP_RAID:
        if (flags & VMAP::VMAP_DISABLE_HEIGHT)
          LOG_INFO("misc", "Height disabled for instance map {}.", entry);
        if (flags & VMAP::VMAP_DISABLE_LOS)
          LOG_INFO("misc", "LoS disabled for instance map {}.", entry);
        break;
      case MAP_BATTLEGROUND:
        if (flags & VMAP::VMAP_DISABLE_HEIGHT)
          LOG_INFO("misc", "Height disabled for battleground map {}.", entry);
        if (flags & VMAP::VMAP_DISABLE_LOS)
          LOG_INFO("misc", "LoS disabled for battleground map {}.", entry);
        break;
      case MAP_ARENA:
        if (flags & VMAP::VMAP_DISABLE_HEIGHT)
          LOG_INFO("misc", "Height disabled for arena map {}.", entry);
        if (flags & VMAP::VMAP_DISABLE_LOS)
          LOG_INFO("misc", "LoS disabled for arena map {}.", entry);
        break;
      default:
        break;
      }
    ...
    case DISABLE_TYPE_MMAP: {
      ...
      switch (mapEntry->MapType) {
      case MAP_COMMON:
        LOG_INFO("misc", "Pathfinding disabled for world map {}.", entry);
        break;
      case MAP_INSTANCE:
      case MAP_RAID:
        LOG_INFO("misc", "Pathfinding disabled for instance map {}.", entry);
        break;
      case MAP_BATTLEGROUND:
        LOG_INFO("misc", "Pathfinding disabled for battleground map {}.",
                 entry);
        break;
      case MAP_ARENA:
        LOG_INFO("misc", "Pathfinding disabled for arena map {}.", entry);
        break;
      default:
        break;
      }
  ...
  LOG_INFO("server.loading", ">> Loaded {} disables in {} ms", total_count,
...
void CheckQuestDisables() {
  ...
  LOG_INFO("server.loading", ">> Checked {} quest disables in {} ms", count,
           GetMSTimeDiffToNow(oldMSTime));
}
...
bool IsPathfindingEnabled(uint32 mapId) {
  return sWorld->getBoolConfig(CONFIG_ENABLE_MMAPS) &&
         !IsDisabledFor(DISABLE_TYPE_MMAP, mapId, nullptr,
