...
char const
    *const ConditionMgr::StaticSourceTypeData[CONDITION_SOURCE_TYPE_MAX] = {
        "None",
        "Creature Loot",
        "Disenchant Loot",
        "Fishing Loot",
...
ConditionMgr::ConditionTypeInfo const
    ConditionMgr::StaticConditionTypeData[CONDITION_MAX] = {
...
bool Condition::Meets(ConditionSourceInfo &sourceInfo) const {
  ...
  if (!object) {
    LOG_DEBUG("condition", "Condition object not found for {}",
              ToString().c_str());
    return false;
  }
    ...
    if (Player *player = object->ToPlayer()) {
      if (FactionEntry const *faction =
        ...
        condMeets = (ConditionValue2 &
                     (1 << player->GetReputationMgr().GetRank(faction))) != 0;
    }
    break;
  }
  ...
  case CONDITION_NEAR_CREATURE: {
    condMeets =
        object->FindNearestCreature(ConditionValue1, (float)ConditionValue2,
                                    bool(!ConditionValue3)) != nullptr;
    break;
  }
  case CONDITION_NEAR_GAMEOBJECT: {
    condMeets = object->FindNearestGameObject(
                    ConditionValue1, (float)ConditionValue2) != nullptr;
    break;
  }
  ...
  case CONDITION_SPAWNMASK: {
    condMeets =
        ((1 << object->GetMap()->GetSpawnMode()) & ConditionValue1) != 0;
    break;
  }
  ...
  case CONDITION_REALM_ACHIEVEMENT: {
    AchievementEntry const *achievement =
        sAchievementMgr->GetAchievement(ConditionValue1);
  ...
  case CONDITION_QUESTSTATE: {
    if (Player *player = object->ToPlayer()) {
      if (((ConditionValue2 & (1 << QUEST_STATUS_NONE)) &&
           (player->GetQuestStatus(ConditionValue1) == QUEST_STATUS_NONE)) ||
          ...
          ((ConditionValue2 & (1 << QUEST_STATUS_FAILED)) &&
           (player->GetQuestStatus(ConditionValue1) == QUEST_STATUS_FAILED)) ||
  ...
  bool script = sScriptMgr->OnConditionCheck(
...
uint32 ConditionMgr::GetSearcherTypeMaskForConditionList(
    ConditionContainer const &conditions) const {
  if (conditions.empty())
    return GRID_MAP_TYPE_MASK_ALL;
  //     groupId, typeMask
  std::map<uint32, uint32> elseGroupSearcherTypeMasks;
  for (ConditionContainer::const_iterator i = conditions.begin();
       i != conditions.end(); ++i) {
    ...
    std::map<uint32, uint32>::const_iterator itr =
        elseGroupSearcherTypeMasks.find((*i)->ElseGroup);
    ...
    if ((*i)->ReferenceId) // handle reference
    {
      ...
      ASSERT(ref != ConditionReferenceStore.end() &&
             ...
             "reference");
      elseGroupSearcherTypeMasks[(*i)->ElseGroup] &=
  ...
  uint32 mask = 0;
  for (std::map<uint32, uint32>::const_iterator i =
           elseGroupSearcherTypeMasks.begin();
       i != elseGroupSearcherTypeMasks.end(); ++i)
    mask |= i->second;
...
bool ConditionMgr::IsObjectMeetToConditionList(
  ...
  for (Condition const *condition : conditions) {
    LOG_DEBUG("condition",
              "ConditionMgr::IsPlayerMeetToConditionList {} val1: {}",
              condition->ToString().c_str(), condition->ConditionValue1);
    if (condition->isLoaded()) {
      ...
      else if (!(*itr).second) //! If another condition in this group was
                               //! unmatched before this, don't bother checking
                               //! (the group is false anyway)
        continue;
      ...
      if (condition->ReferenceId) // handle reference
      {
        ...
        if (ref != ConditionReferenceStore.end()) {
          ...
          LOG_DEBUG(
              "condition",
              "ConditionMgr::IsPlayerMeetToConditionList {} Reference template "
              "-{} not found",
              condition->ToString().c_str(),
              condition
                  ->ReferenceId); // checked at loading, should never happen
        }
...
bool ConditionMgr::IsObjectMeetToConditions(
    WorldObject *object, ConditionContainer const &conditions) const {
  ConditionSourceInfo srcInfo = ConditionSourceInfo(object);
  return IsObjectMeetToConditions(srcInfo, conditions);
}

bool ConditionMgr::IsObjectMeetToConditions(
    WorldObject *object1, WorldObject *object2,
    ConditionContainer const &conditions) const {
...
bool ConditionMgr::IsObjectMeetingNotGroupedConditions(
    ConditionSourceType sourceType, uint32 entry,
    ConditionSourceInfo &sourceInfo) const {
  if (sourceType > CONDITION_SOURCE_TYPE_NONE &&
      sourceType < CONDITION_SOURCE_TYPE_MAX) {
    ConditionsByEntryMap::const_iterator i =
...
bool ConditionMgr::IsObjectMeetingNotGroupedConditions(
    ConditionSourceType sourceType, uint32 entry, WorldObject *target0,
    WorldObject *target1 /*= nullptr*/,
    WorldObject *target2 /*= nullptr*/) const {
  ConditionSourceInfo conditionSource(target0, target1, target2);
  return IsObjectMeetingNotGroupedConditions(sourceType, entry,
                                             conditionSource);
}

bool ConditionMgr::HasConditionsForNotGroupedEntry(
    ConditionSourceType sourceType, uint32 entry) const {
  if (sourceType > CONDITION_SOURCE_TYPE_NONE &&
      sourceType < CONDITION_SOURCE_TYPE_MAX)
    if (ConditionStore[sourceType].find(entry) !=
        ConditionStore[sourceType].end())
      return true;

  return false;
}

bool ConditionMgr::IsObjectMeetingSpellClickConditions(
    uint32 creatureId, uint32 spellId, WorldObject *clicker,
    WorldObject *target) const {
  ConditionEntriesByCreatureIdMap::const_iterator itr =
...
bool ConditionMgr::IsObjectMeetingSmartEventConditions(
    int32 entryOrGuid, uint32 eventId, uint32 sourceType, Unit *unit,
    WorldObject *baseObject) const {
...
bool ConditionMgr::IsSpellUsedInSpellClickConditions(uint32 spellId) const {
  return SpellsUsedInSpellClickConditions.find(spellId) !=
         SpellsUsedInSpellClickConditions.end();
}
...
void ConditionMgr::LoadConditions(bool isReload) {
  ...
  do {
    ...
    cond->NegativeCondition = fields[10].Get<bool>();
    ...
    if (iConditionTypeOrReference < 0) // it has a reference
    {
      ...
      if (cond->ConditionValue1)
        LOG_ERROR("sql.sql", "Condition {} {} has useless data in value1 ({})!",
                  rowType, iSourceTypeOrReferenceId, cond->ConditionValue1);
      if (cond->ConditionValue2)
        LOG_ERROR("sql.sql", "Condition {} {} has useless data in value2 ({})!",
                  rowType, iSourceTypeOrReferenceId, cond->ConditionValue2);
      if (cond->ConditionValue3)
        LOG_ERROR("sql.sql", "Condition {} {} has useless data in value3 ({})!",
    ...
    {
      ConditionReferenceStore[std::abs(iSourceTypeOrReferenceId)].push_back(
    ...
    if (cond->SourceGroup && !CanHaveSourceGroupSet(cond->SourceType)) {
      LOG_ERROR("sql.sql", "{} has not allowed value of SourceGroup = {}!",
                cond->ToString().c_str(), cond->SourceGroup);
      delete cond;
      continue;
    }
    if (cond->SourceId && !CanHaveSourceIdSet(cond->SourceType)) {
      LOG_ERROR("sql.sql", "{} has not allowed value of SourceId = {}!",
                cond->ToString().c_str(), cond->SourceId);
      delete cond;
      continue;
    }

    if (cond->ErrorType && cond->SourceType != CONDITION_SOURCE_TYPE_SPELL) {
      LOG_ERROR("sql.sql", "{} can't have ErrorType ({}), set to 0!",
                cond->ToString().c_str(), cond->ErrorType);
      cond->ErrorType = 0;
    }

    if (cond->ErrorTextId && !cond->ErrorType) {
      LOG_ERROR("sql.sql",
                "{} has any ErrorType, ErrorTextId ({}) is set, set to 0!",
                cond->ToString().c_str(), cond->ErrorTextId);
      cond->ErrorTextId = 0;
    }
    ...
    if (cond->SourceGroup) {
      ...
      if (!valid) {
        LOG_ERROR("sql.sql", "{} Not handled grouped condition.",
                  cond->ToString().c_str());
        delete cond;
      } else {
    ...
    if (cond->SourceType == CONDITION_SOURCE_TYPE_SPELL_CLICK_EVENT &&
        cond->ConditionType == CONDITION_AURA)
      SpellsUsedInSpellClickConditions.insert(cond->ConditionValue1);
    ConditionStore[cond->SourceType][cond->SourceEntry].push_back(cond);
    ++count;
  } while (result->NextRow());

  LOG_INFO("server.loading", ">> Loaded {} conditions in {} ms", count,
...
bool ConditionMgr::addToLootTemplate(Condition *cond,
  ...
  if (!loot) {
    LOG_ERROR("sql.sql", "{} LootTemplate {} not found.",
              cond->ToString().c_str(), cond->SourceGroup);
    return false;
  }
  ...
  LOG_ERROR("sql.sql", "{} Item {} not found in LootTemplate {}.",
            cond->ToString().c_str(), cond->SourceEntry, cond->SourceGroup);
  return false;
}
...
bool ConditionMgr::addToGossipMenus(Condition *cond) const {
  ...
  LOG_ERROR("sql.sql", "{} GossipMenu {} not found.", cond->ToString().c_str(),
            cond->SourceGroup);
  return false;
}
...
bool ConditionMgr::addToGossipMenuItems(Condition *cond) const {
  ...
  LOG_ERROR("sql.sql", "{} GossipMenuId {} Item {} not found.",
            cond->ToString().c_str(), cond->SourceGroup, cond->SourceEntry);
  return false;
}
...
bool ConditionMgr::addToSpellImplicitTargetConditions(Condition *cond) const {
  ...
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    ...
    if (conditionEffMask & (1 << i)) {
      ...
      case CONDITION_OBJECT_ENTRY_GUID: {
        uint32 implicitTargetMask =
            GetTargetFlagMask(spellInfo->Effects[i].TargetA.GetObjectType()) |
            GetTargetFlagMask(spellInfo->Effects[i].TargetB.GetObjectType());
        if ((implicitTargetMask & TARGET_FLAG_UNIT_MASK) &&
            cond->ConditionValue1 != TYPEID_UNIT &&
            cond->ConditionValue1 != TYPEID_PLAYER) {
          LOG_ERROR("sql.sql",
                    "{} in `condition` table - spell {} EFFECT_{} - "
                    "target requires ConditionValue1 to be either TYPEID_UNIT "
                    "({}) or TYPEID_PLAYER ({})",
                    cond->ToString().c_str(), spellInfo->Id, uint32(i),
                    uint32(TYPEID_UNIT), uint32(TYPEID_PLAYER));
          return false;
        }

        if ((implicitTargetMask & TARGET_FLAG_GAMEOBJECT_MASK) &&
            cond->ConditionValue1 != TYPEID_GAMEOBJECT) {
          LOG_ERROR(
              "sql.sql",
              "{} in `condition` table - spell {} EFFECT_{} - "
              "target requires ConditionValue1 to be TYPEID_GAMEOBJECT ({})",
              cond->ToString().c_str(), spellInfo->Id, uint32(i),
              uint32(TYPEID_GAMEOBJECT));
          return false;
        }

        if ((implicitTargetMask & TARGET_FLAG_CORPSE_MASK) &&
            cond->ConditionValue1 != TYPEID_CORPSE) {
          LOG_ERROR("sql.sql",
                    "{} in `condition` table - spell {} EFFECT_{} - "
                    "target requires ConditionValue1 to be TYPEID_CORPSE ({})",
                    cond->ToString().c_str(), spellInfo->Id, uint32(i),
                    uint32(TYPEID_CORPSE));
          return false;
        }
        break;
      }
    ...
    auto itr = std::find_if(sharedMasks.begin(), sharedMasks.end(),
                            [i](uint32 mask) { return !!(mask & (1 << i)); });
    ...
    if (uint32 commonMask = effectMask & conditionEffMask) {
      ...
      if (sharedList) {
        ...
        if (conditionEffMask != effectMask) {
          LOG_ERROR("sql.sql",
                    "{} in `condition` table, has incorrect SourceGroup {} "
                    "(spell effectMask) set - "
...
bool ConditionMgr::addToPhases(Condition *cond) const {
  if (!cond->SourceEntry) {
    if (PhaseInfoStruct const *phaseInfo =
            sObjectMgr->GetPhaseInfo(cond->SourceGroup)) {
      bool found = false;
      for (uint32 areaId : phaseInfo->Areas) {
        if (std::vector<PhaseAreaInfo> *phases =
  ...
  } else if (std::vector<PhaseAreaInfo> *phases =
                 const_cast<std::vector<PhaseAreaInfo> *>(
                     sObjectMgr->GetPhasesForArea(cond->SourceEntry))) {
    for (PhaseAreaInfo &phase : *phases) {
      if (phase.PhaseInfo->Id == cond->SourceGroup) {
        phase.Conditions.push_back(cond);
        return true;
  ...
  LOG_ERROR("sql.sql", "{} Area {} does not have phase {}.",
            cond->ToString().c_str(), cond->SourceGroup, cond->SourceEntry);
  return false;
}
...
bool ConditionMgr::addToSpellArea(Condition *cond) const {
  if (cond->SourceGroup) {
    bool found = false;
    SpellAreaForAreaMapBounds bounds =
        sSpellMgr->GetSpellAreaForAreaMapBounds(cond->SourceEntry);
    for (SpellAreaForAreaMap::const_iterator &itr = bounds.first;
    ...
    LOG_ERROR("sql.sql",
              "{} invalid spell ID {} specified for area/zone ID {}.",
              cond->ToString().c_str(), cond->SourceGroup, cond->SourceEntry);
    return false;
  }

  LOG_ERROR("sql.sql",
            "{} SpellID {} does not have a entry for area/zone ID {}.",
            cond->ToString().c_str(), cond->SourceGroup, cond->SourceEntry);
  return false;
}
...
bool ConditionMgr::isSourceTypeValid(Condition *cond) const {
  ...
  case CONDITION_SOURCE_TYPE_CREATURE_LOOT_TEMPLATE: {
    ...
    if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
      LOG_ERROR("sql.sql",
                "{} SourceType, SourceEntry in `condition` table, does not "
  ...
  case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET: {
    ...
    if ((cond->SourceGroup > MAX_EFFECT_MASK) || !cond->SourceGroup) {
      LOG_ERROR("sql.sql",
                "{} in `condition` table, has incorrect SourceGroup (spell "
                "effectMask) set, ignoring.",
                cond->ToString().c_str());
      return false;
    }
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
      ...
      LOG_ERROR(
          "sql.sql",
          "SourceEntry {} SourceGroup {} in `condition` table - spell {} does "
          "not have implicit targets of types: _AREA_, _CONE_, _NEARBY_, "
          "__CHAIN__ for effect {}, SourceGroup needs correction, ignoring.",
  ...
  case CONDITION_SOURCE_TYPE_QUEST_AVAILABLE:
    ...
    if (!sObjectMgr->GetQuestTemplate(cond->SourceEntry)) {
      LOG_ERROR("sql.sql",
                "{} SourceEntry specifies non-existing quest, skipped.",
                cond->ToString().c_str());
      return false;
    }
    break;
...
bool ConditionMgr::isConditionTypeValid(Condition *cond) const {
  if (cond->ConditionType == CONDITION_NONE ||
      cond->ConditionType >= CONDITION_MAX) {
    LOG_ERROR("sql.sql",
              "{} Invalid ConditionType in `condition` table, ignoring.",
              cond->ToString(true).c_str());
    return false;
  }
  ...
  if (cond->ConditionTarget >= cond->GetMaxAvailableConditionTargets()) {
    LOG_ERROR(
        "sql.sql",
        "{} in `condition` table, has incorrect ConditionTarget set, ignoring.",
        cond->ToString(true).c_str());
    return false;
  }
  ...
  case CONDITION_AURA: {
    if (!sSpellMgr->GetSpellInfo(cond->ConditionValue1)) {
      LOG_ERROR("sql.sql", "{} has non existing spell (Id: {}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    ...
    if (cond->ConditionValue2 > EFFECT_2) {
      LOG_ERROR(
          "sql.sql",
          "{} has non existing effect index ({}) (must be 0..2), skipped.",
          cond->ToString(true).c_str(), cond->ConditionValue2);
      return false;
    }
    break;
  }
  case CONDITION_ITEM: {
    ...
    if (!proto) {
      LOG_ERROR("sql.sql", "{} Item ({}) does not exist, skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }

    if (!cond->ConditionValue2) {
      LOG_ERROR("sql.sql", "{} Zero item count in ConditionValue2, skipped.",
                cond->ToString(true).c_str());
      return false;
    }
    break;
  ...
  case CONDITION_ZONEID: {
    ...
    if (!areaEntry) {
      LOG_ERROR("sql.sql", "{} Area ({}) does not exist, skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    ...
    if (areaEntry->ParentAreaID != 0) {
      LOG_ERROR("sql.sql",
                "{} requires to be in area ({}) which is a subzone but zone "
                "expected, skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
  ...
  case CONDITION_REPUTATION_RANK: {
    ...
    if (!factionEntry) {
      LOG_ERROR("sql.sql", "{} has non existing faction ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_TEAM: {
    if (cond->ConditionValue1 != ALLIANCE && cond->ConditionValue1 != HORDE) {
      LOG_ERROR("sql.sql", "{} specifies unknown team ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_SKILL: {
    ...
    if (!pSkill) {
      LOG_ERROR("sql.sql", "{} specifies non-existing skill ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    ...
    if (cond->ConditionValue2 < 1 ||
      ...
      LOG_ERROR("sql.sql",
                "{} specifies skill ({}) with invalid value ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1,
  ...
  case CONDITION_QUESTSTATE:
    if (cond->ConditionValue2 >= (1 << MAX_QUEST_STATUS)) {
      LOG_ERROR("sql.sql", "{} has invalid state mask ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue2);
      return false;
    }
    [[fallthrough]];
  ...
  case CONDITION_DAILY_QUEST_DONE: {
    if (!sObjectMgr->GetQuestTemplate(cond->ConditionValue1)) {
      LOG_ERROR("sql.sql", "{} points to non-existing quest ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_ACTIVE_EVENT: {
    ...
    if (cond->ConditionValue1 >= events.size() ||
      ...
      LOG_ERROR("sql.sql", "{} has non existing event id ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_ACHIEVEMENT: {
    AchievementEntry const *achievement =
        sAchievementMgr->GetAchievement(cond->ConditionValue1);
    if (!achievement) {
      LOG_ERROR("sql.sql", "{} has non existing achivement id ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_CLASS: {
    if (!(cond->ConditionValue1 & CLASSMASK_ALL_PLAYABLE)) {
      LOG_ERROR("sql.sql", "{} has non existing classmask ({}), skipped.",
                cond->ToString(true).c_str(),
  ...
  case CONDITION_RACE: {
    if (!(cond->ConditionValue1 & RACEMASK_ALL_PLAYABLE)) {
      LOG_ERROR("sql.sql", "{} has non existing racemask ({}), skipped.",
                cond->ToString(true).c_str(),
  ...
  case CONDITION_GENDER: {
    if (!Player::IsValidGender(uint8(cond->ConditionValue1))) {
      LOG_ERROR("sql.sql", "{} has invalid gender ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_MAPID: {
    ...
    if (!me) {
      LOG_ERROR("sql.sql", "{} has non existing map ({}), skipped",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_LEVEL: {
    if (cond->ConditionValue2 >= COMP_TYPE_MAX) {
      LOG_ERROR("sql.sql", "{} has invalid ComparisionType ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue2);
      return false;
    }
    break;
  ...
  case CONDITION_DRUNKENSTATE: {
    if (cond->ConditionValue1 > DRUNKEN_SMASHED) {
      LOG_ERROR("sql.sql", "{} has invalid state ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_NEAR_CREATURE: {
    if (!sObjectMgr->GetCreatureTemplate(cond->ConditionValue1)) {
      LOG_ERROR("sql.sql",
                "{} has non existing creature template entry ({}), skipped",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_NEAR_GAMEOBJECT: {
    if (!sObjectMgr->GetGameObjectTemplate(cond->ConditionValue1)) {
      LOG_ERROR("sql.sql",
                "{} has non existing gameobject template entry ({}), skipped.",
                cond->ToString().c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  }
  case CONDITION_OBJECT_ENTRY_GUID: {
      ...
      if (cond->ConditionValue3) {
        if (CreatureData const *creatureData =
          ...
          if (cond->ConditionValue2 &&
            ...
            LOG_ERROR("sql.sql",
                      "{} has guid {} set but does not match creature entry "
                      "({}), skipped.",
                      cond->ToString(true).c_str(), cond->ConditionValue3,
                      cond->ConditionValue2);
            return false;
          }
        } else {
          LOG_ERROR("sql.sql",
                    "{} has non existing creature guid ({}), skipped.",
                    cond->ToString(true).c_str(), cond->ConditionValue3);
          return false;
        }
      }
      ...
      if (cond->ConditionValue3) {
        if (GameObjectData const *goData =
          ...
          if (cond->ConditionValue2 && goData->id != cond->ConditionValue2) {
            LOG_ERROR("sql.sql",
                      "{} has guid {} set but does not match gameobject entry "
                      "({}), skipped.",
                      cond->ToString(true).c_str(), cond->ConditionValue3,
                      cond->ConditionValue2);
            return false;
          ...
          LOG_ERROR("sql.sql",
                    "{} has non existing gameobject guid ({}), skipped.",
                    cond->ToString(true).c_str(), cond->ConditionValue3);
          return false;
        }
      }
    ...
    case TYPEID_CORPSE:
      ...
      break;
    default:
      LOG_ERROR("sql.sql", "{} has wrong typeid set ({}), skipped",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_TYPE_MASK: {
    if (!cond->ConditionValue1 ||
      ...
      LOG_ERROR("sql.sql", "{} has invalid typemask set ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue2);
      return false;
    }
    break;
  ...
  case CONDITION_RELATION_TO: {
    if (cond->ConditionValue1 >= cond->GetMaxAvailableConditionTargets()) {
      LOG_ERROR("sql.sql",
                "{} has invalid ConditionValue1(ConditionTarget selection) "
                "({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    if (cond->ConditionValue1 == cond->ConditionTarget) {
      LOG_ERROR("sql.sql",
                "{} has ConditionValue1(ConditionTarget selection) set to self "
                "({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    if (cond->ConditionValue2 >= RELATION_MAX) {
      LOG_ERROR("sql.sql",
                "{} has invalid ConditionValue2(RelationType) ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue2);
      return false;
    }
    break;
  ...
  case CONDITION_REACTION_TO: {
    ...
    if (!cond->ConditionValue2) {
      LOG_ERROR("sql.sql",
                "{} has invalid ConditionValue2(rankMask) ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue2);
      return false;
    }
    break;
  ...
  case CONDITION_HP_VAL: {
    ...
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "{} has useless data in value3 ({})!",
                cond->ToString(true).c_str(), cond->ConditionValue3);
    break;
  }
  case CONDITION_HP_PCT: {
    if (cond->ConditionValue1 > 100) {
      LOG_ERROR("sql.sql", "{} has too big percent value ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    if (cond->ConditionValue2 >= COMP_TYPE_MAX) {
    ...
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "{} has useless data in value3 ({})!",
                cond->ToString().c_str(), cond->ConditionValue3);
    break;
  }
  case CONDITION_WORLD_STATE: {
    if (!sWorld->getWorldState(cond->ConditionValue1)) {
      LOG_ERROR("sql.sql",
                "{} has non existing world state in value1 ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_PHASEID: {
    if (!sPhaseStore.LookupEntry(cond->ConditionValue1)) {
      LOG_ERROR(
          "sql.sql",
          "Phase condition has nonexistent phaseid in value1 ({}), skipped",
  ...
  case CONDITION_TERRAIN_SWAP: {
    if (cond->ConditionValue2)
      LOG_ERROR("sql.sql",
                "Terrain swap condition has useless data in value2 ({})!",
                cond->ConditionValue2);
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql",
                "Terrain swap condition has useless data in value3 ({})!",
  ...
  case CONDITION_TITLE: {
    ...
    if (!titleEntry) {
      LOG_ERROR("sql.sql", "{} has non existing title in value1 ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_SPAWNMASK: {
    if (cond->ConditionValue1 > SPAWNMASK_RAID_ALL) {
      LOG_ERROR("sql.sql",
                "{} has non existing SpawnMask in value1 ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_UNIT_STATE: {
    if (!(cond->ConditionValue1 & UNIT_STATE_ALL_STATE_SUPPORTED)) {
      LOG_ERROR("sql.sql",
                "{} has non existing UnitState in value1 ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_CREATURE_TYPE: {
    if (!cond->ConditionValue1 ||
      ...
      LOG_ERROR("sql.sql",
                "{} has non existing CreatureType in value1 ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_REALM_ACHIEVEMENT: {
    ...
    if (!achievement) {
      LOG_ERROR("sql.sql",
                "{} has non existing realm first achivement id ({}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_STAND_STATE: {
    ...
    if (!valid) {
      LOG_ERROR("sql.sql", "{} has non-existing stand state ({},{}), skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1,
                cond->ConditionValue2);
      return false;
    }
  ...
  case CONDITION_PET_TYPE:
    if (cond->ConditionValue1 >= (1 << MAX_PET_TYPE)) {
      LOG_ERROR("sql.sql", "{} has non-existing pet type {}, skipped.",
                cond->ToString(true).c_str(), cond->ConditionValue1);
      return false;
    }
    break;
  ...
  if (cond->ConditionValue1 &&
      !StaticConditionTypeData[cond->ConditionType].HasConditionValue1)
    LogUselessConditionValue(cond, 1, cond->ConditionValue1);
  if (cond->ConditionValue2 &&
      !StaticConditionTypeData[cond->ConditionType].HasConditionValue2)
    LogUselessConditionValue(cond, 2, cond->ConditionValue2);
  if (cond->ConditionValue3 &&
      !StaticConditionTypeData[cond->ConditionType].HasConditionValue3)
    LogUselessConditionValue(cond, 3, cond->ConditionValue3);

  return true;
}

void ConditionMgr::LogUselessConditionValue(Condition *cond, uint8 index,
                                            uint32 value) {
  LOG_ERROR("sql.sql", "{} has useless data in ConditionValue{} ({})!",
            cond->ToString(true).c_str(), index, value);
}
...
void ConditionMgr::Clean() {
  for (ConditionReferenceContainer::iterator itr =
    ...
    for (ConditionContainer::const_iterator it = itr->second.begin();
  ...
  for (uint32 i = 0; i < CONDITION_SOURCE_TYPE_MAX; ++i) {
    for (ConditionsByEntryMap::iterator it = ConditionStore[i].begin();
         it != ConditionStore[i].end(); ++it)
      for (ConditionContainer::const_iterator itr = it->second.begin();
  ...
  for (ConditionEntriesByCreatureIdMap::iterator itr =
           VehicleSpellConditionStore.begin();
       itr != VehicleSpellConditionStore.end(); ++itr)
    for (ConditionsByEntryMap::iterator it = itr->second.begin();
         it != itr->second.end(); ++it)
      for (ConditionContainer::const_iterator i = it->second.begin();
           i != it->second.end(); ++i)
        delete *i;
  ...
  // ConditionMgr ;)
  for (std::vector<Condition *>::const_iterator itr =
