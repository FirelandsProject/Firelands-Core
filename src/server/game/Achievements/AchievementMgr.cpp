...
bool AchievementCriteriaData::IsValid(AchievementCriteriaEntry const* criteria)
{
    ...
    switch (criteria->type)
    {
        ...
        default:
            ...
            {
                LOG_ERROR("sql.sql", "Table `achievement_criteria_data` has data for non-supported criteria type (Entry: %u Type: %u), ignored.", criteria->ID, criteria->type);
                return false;
            }
            break;
    }
    ...
    switch (dataType)
    {
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_TEAM:
            ...
            {
                LOG_ERROR("sql.sql", "Table `achievement_criteria_data` (Entry: %u Type: %u) for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_S_TEAM (%u) has unknown team in value1 (%u), ignored.",
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_PLAYER_CLASS_RACE:
            ...
            return true;
        default:
            LOG_ERROR("sql.sql", "Table `achievement_criteria_data` (Entry: %u Type: %u) has data for non-supported data type (%u), ignored.", criteria->ID, criteria->type, dataType);
            return false;
    }
}
...
bool AchievementCriteriaData::Meets(uint32 criteria_id, Player const* source, Unit const* target, uint32 miscValue1 /*= 0*/) const
{
    switch (dataType)
    {
        ...
        case ACHIEVEMENT_CRITERIA_DATA_INSTANCE_SCRIPT:
        {
            ...
            if (!map->IsDungeon())
            {
                LOG_ERROR("achievement", "Achievement system call ACHIEVEMENT_CRITERIA_DATA_INSTANCE_SCRIPT (%u) for achievement criteria %u for non-dungeon/non-raid map %u",
            ...
            if (!instance)
            {
                LOG_ERROR("achievement", "Achievement system call ACHIEVEMENT_CRITERIA_DATA_INSTANCE_SCRIPT (%u) for achievement criteria %u for map %u but map does not have a instance script",
                    ACHIEVEMENT_CRITERIA_DATA_INSTANCE_SCRIPT, criteria_id, map->GetId());
                return false;
            }
...
{
    LOG_DEBUG("achievement", "ResetAchievementCriteria(%u, " UI64FMTD ", " UI64FMTD ")", type, miscValue1, miscValue2);
...
void AchievementMgr<T>::UpdateAchievementCriteria(AchievementCriteriaTypes type, uint64 miscValue1 /*= 0*/, uint64 miscValue2 /*= 0*/, uint64 miscValue3 /*= 0*/, Unit const* unit /*= NULL*/, Player* referencePlayer /*= NULL*/, GameObject* go /*= NULL*/)
{
    ...
    for (AchievementCriteriaEntryList::const_iterator i = achievementCriteriaList.begin(); i != achievementCriteriaList.end(); ++i)
    {
        ...
        if (!achievement)
        {
            LOG_ERROR("achievement", "UpdateAchievementCriteria: Achievement %u not found!", achievementCriteria->achievement);
            continue;
        }
...
void AchievementMgr<T>::SetCriteriaProgress(AchievementCriteriaEntry const* entry, uint64 changeValue, Player* referencePlayer, ProgressType ptype)
{
    ...
    LOG_DEBUG("achievement", "SetCriteriaProgress(%u, " UI64FMTD ") for (%s GUID: %u)",
...
bool AchievementMgr<T>::CanUpdateCriteria(AchievementCriteriaEntry const* criteria, AchievementEntry const* achievement, uint64 miscValue1, uint64 miscValue2, uint64 miscValue3, Unit const* unit, Player* referencePlayer, GameObject* go /*= NULL*/)
{
    ...
    {
        LOG_TRACE("achievement", "CanUpdateCriteria: %s (Id: %u Type %s) Disabled",
    ...
    {
        LOG_TRACE("achievement", "CanUpdateCriteria: %s (Id: %u Type %s) Wrong map",
            criteria->name, criteria->ID, AchievementGlobalMgr::GetCriteriaTypeString(criteria->type));
        return false;
    }
    ...
    {
        LOG_TRACE("achievement", "CanUpdateCriteria: %s (Id: %u Type %s) Wrong faction",
            criteria->name, criteria->ID, AchievementGlobalMgr::GetCriteriaTypeString(criteria->type));
        return false;
    }
    ...
    {
        LOG_TRACE("achievement", "CanUpdateCriteria: %s (Id: %u Type %s) Conditions not satisfied",
            criteria->name, criteria->ID, AchievementGlobalMgr::GetCriteriaTypeString(criteria->type));
        return false;
    }
...
void AchievementGlobalMgr::LoadAchievementCriteriaList()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %u achievement criteria and %u guild achievement criteria in %u ms", criterias, guildCriterias, GetMSTimeDiffToNow(oldMSTime));
}
...
void AchievementGlobalMgr::LoadCompletedAchievements()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %lu realm first completed achievements in %u ms", (unsigned long)m_allCompletedAchievements.size(), GetMSTimeDiffToNow(oldMSTime));
}
    ...
    do
    {
        ...
        else
        {
            ...
            if (!reward.text.empty())
                LOG_ERROR("sql.sql", "Table `achievement_reward` (ID: %u) does not have sender data but has mail text.", id);
        }
        ...
        if (reward.itemId)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Table `achievement_reward` (ID: %u) has invalid item id %u, reward mail will not contain item.", id, reward.itemId);
                reward.itemId = 0;
            }
        }
