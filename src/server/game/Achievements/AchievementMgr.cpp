...
bool AchievementCriteriaData::IsValid(AchievementCriteriaEntry const* criteria)
{
    ...
    default:
        ...
        {
            LOG_ERROR("sql.sql", "Table `achievement_criteria_data` has data for non-supported criteria type (Entry: {} Type: {}), ignored.", criteria->ID, criteria->type);
            return false;
        }
        break;
    }
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_CLASS_RACE:
        ...
        {
            LOG_ERROR("sql.sql", "Table `achievement_criteria_data` (Entry: {} Type: {}) for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_CLASS_RACE ({}) must not have 0 in either value field, ignored.",
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_TEAM:
        ...
        {
            LOG_ERROR("sql.sql", "Table `achievement_criteria_data` (Entry: {} Type: {}) for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_S_TEAM ({}) has unknown team in value1 ({}), ignored.",
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_PLAYER_CLASS_RACE:
        ...
        return true;
    default:
        LOG_ERROR("sql.sql", "Table `achievement_criteria_data` (Entry: {} Type: {}) has data for non-supported data type ({}), ignored.", criteria->ID, criteria->type, dataType);
        return false;
    }
}
...
bool AchievementCriteriaData::Meets(uint32 criteria_id, Player const* source, Unit const* target, uint32 miscValue1 /*= 0*/) const
{
    ...
    case ACHIEVEMENT_CRITERIA_DATA_INSTANCE_SCRIPT:
    {
        ...
        if (!map->IsDungeon())
        {
            LOG_ERROR("achievement", "Achievement system call ACHIEVEMENT_CRITERIA_DATA_INSTANCE_SCRIPT ({}) for achievement criteria {} for non-dungeon/non-raid map {}",
        ...
        if (!instance)
        {
            LOG_ERROR("achievement", "Achievement system call ACHIEVEMENT_CRITERIA_DATA_INSTANCE_SCRIPT ({}) for achievement criteria {} for map {} but map does not have a instance script",
                ACHIEVEMENT_CRITERIA_DATA_INSTANCE_SCRIPT, criteria_id, map->GetId());
            return false;
        }
...
void AchievementMgr<Player>::SaveToDB(SQLTransaction& trans)
{
    if (!m_completedAchievements.empty())
    {
        ...
        for (CompletedAchievementMap::iterator iter = m_completedAchievements.begin(); iter != m_completedAchievements.end(); ++iter)
        {
            ...
            if (!need_execute)
            {
                ssdel << "DELETE FROM character_achievement WHERE guid = " << m_player->GetGUIDLow() << " AND achievement IN (";
            ...
            ssdel << iter->first;
            ssins << '(' << m_player->GetGUIDLow() << ',' << iter->first << ',' << uint64(iter->second.date) << ')';
    ...
    if (!m_criteriaProgress.empty())
    {
        ...
        for (CriteriaProgressMap::iterator iter = m_criteriaProgress.begin(); iter != m_criteriaProgress.end(); ++iter)
        {
            ...
            // deleted data (including 0 progress state)
            {
                ...
                if (!need_execute_del)
                {
                    ssdel << "DELETE FROM character_achievement_progress WHERE guid = " << m_player->GetGUIDLow() << " AND criteria IN (";
            ...
            if (iter->second.counter != 0)
            {
                ...
                // new/changed record data
                ssins << '(' << m_player->GetGUIDLow() << ',' << iter->first << ',' << iter->second.counter << ',' << iter->second.date << ')';
            }
...
void AchievementMgr<Guild>::SaveToDB(SQLTransaction& trans)
{
    ...
    for (CriteriaProgressMap::iterator itr = m_criteriaProgress.begin(); itr != m_criteriaProgress.end(); ++itr)
    {
        ...
        stmt->SetData(4, GUID_LOPART(itr->second.CompletedGUID));
        trans->Append(stmt);
...
void AchievementMgr<Player>::CompletedCriteria(AchievementCriteriaEntry const* achievementCriteria, Player* referencePlayer)
{
    ...
    {
        SetCriteriaProgress(achievementCriteria, 1, m_player, PROGRESS_ACCUMULATE);
        CompletedCriteriaFor(achievement, m_player);
    }
}
    ...
    if (achievementResult)
    {
        do
        {
            ...
            if (AchievementReward const* reward = sAchievementMgr->GetAchievementReward(achievement))
                if (uint32 titleId = reward->titleId[achievement->ID == 1793 ? m_player->getGender() : (m_player->GetOTeam() == ALLIANCE ? 0 : 1)])
                    if (CharTitlesEntry const* titleEntry = sCharTitlesStore.LookupEntry(titleId))
                        m_player->SetTitle(titleEntry);
    ...
    if (criteriaResult)
    {
        ...
        do
        {
            ...
            if (!criteria)
            {
                ...
                stmt->SetData(1, GUID_LOPART(m_player->GetGUID()));
...
void AchievementMgr<Guild>::LoadFromDB(PreparedQueryResult achievementResult, PreparedQueryResult criteriaResult)
{
    if (achievementResult)
    {
        do
        {
            ...
            std::vector<std::string_view>guids = Firelands::Tokenize(fields[2].Get<std::string>(), ' ');
    ...
    if (criteriaResult)
    {
        ...
        do
        {
            ...
            time_t date = time_t(fields[2].Get<uint32>());
            uint64 guid = fields[3].Get<uint32>();

            AchievementCriteriaEntry const* criteria = sAchievementMgr->GetAchievementCriteria(id);
            if (!criteria)
            {
                // we will remove not existed criteria for all guilds
                LOG_ERROR("achievement", "Non-existing achievement criteria {} data removed from table `guild_achievement_progress`.", id);
...
void AchievementMgr<Player>::Reset()
{
    ...
    // re-fill data
    CheckAllAchievementCriteria(m_player);
}
...
void AchievementMgr<T>::SendAchievementEarned(AchievementEntry const* achievement) const
{
    ...
    {
        Firelands::AchievementChatBuilder _builder(m_player, CHAT_MSG_GUILD_ACHIEVEMENT, BROADCAST_TEXT_ACHIEVEMENT_EARNED, achievement->ID);
        Firelands::LocalizedPacketDo<Firelands::AchievementChatBuilder> _localizer(_builder);
        guild->BroadcastWorker(_localizer, m_player);
    }
    ...
    if (achievement->flags & (ACHIEVEMENT_FLAG_REALM_FIRST_KILL | ACHIEVEMENT_FLAG_REALM_FIRST_REACH | ACHIEVEMENT_FLAG_REALM_FIRST_GUILD))
    {
        ...
        WorldPacket data(SMSG_SERVER_FIRST_ACHIEVEMENT, m_player->GetName().size() + 1 + 8 + 4 + 4);
    ...
    {
        Firelands::AchievementChatBuilder _builder(m_player, CHAT_MSG_ACHIEVEMENT, BROADCAST_TEXT_ACHIEVEMENT_EARNED, achievement->ID);
        Firelands::LocalizedPacketDo<Firelands::AchievementChatBuilder> _localizer(_builder);
        Firelands::PlayerDistWorker<Firelands::LocalizedPacketDo<Firelands::AchievementChatBuilder> > _worker(m_player, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY), _localizer);
        m_player->VisitNearbyWorldObject(sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY), _worker);
    }

    WorldPacket data(SMSG_ACHIEVEMENT_EARNED, 8 + 4 + 8);
    data.append(m_player->GetPackGUID());
    data << uint32(achievement->ID);
    data.AppendPackedTime(time(NULL));
    data << uint32(0);  // does not notify player ingame
    m_player->SendMessageToSetInRange(&data, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY), true);
}
...
void AchievementMgr<Guild>::SendCriteriaUpdate(AchievementCriteriaEntry const* entry, CriteriaProgress const* progress, uint32 /*timeElapsed*/, bool /*timedCompleted*/) const
{
    ...
    m_player->BroadcastPacketIfTrackingAchievement(&data, entry->ID);
}
...
void AchievementMgr<T>::UpdateAchievementCriteria(AchievementCriteriaTypes type, uint64 miscValue1 /*= 0*/, uint64 miscValue2 /*= 0*/, uint64 miscValue3 /*= 0*/, Unit const* unit /*= NULL*/, Player* referencePlayer /*= NULL*/, GameObject* go /*= NULL*/)
{
    ...
    if (!referencePlayer)
    {
        LOG_DEBUG("achievement", "UpdateAchievementCriteria: Player is NULL! Cant update criteria");
        return;
    }
    ...
    {
        LOG_DEBUG("achievement", "UpdateAchievementCriteria: [Player {} GM mode on] {}, {} ({}), {}, {}, {} "
            , referencePlayer->GetName().c_str(), GetLogNameForGuid(m_player->GetGUID()), AchievementGlobalMgr::GetCriteriaTypeString(type), type, miscValue1, miscValue2, miscValue3);
        return;
    }

    LOG_DEBUG("achievement", "UpdateAchievementCriteria: {}, {} ({}), {}, {}, {} "
        , GetLogNameForGuid(m_player->GetGUID()), AchievementGlobalMgr::GetCriteriaTypeString(type), type, miscValue1, miscValue2, miscValue3);
    ...
    for (AchievementCriteriaEntryList::const_iterator i = achievementCriteriaList.begin(); i != achievementCriteriaList.end(); ++i)
    {
        ...
        if (!achievement)
        {
            LOG_ERROR("achievement", "UpdateAchievementCriteria: Achievement {} not found!", achievementCriteria->achievement);
            continue;
        }
...
void AchievementMgr<T>::SetCriteriaProgress(AchievementCriteriaEntry const* entry, uint64 changeValue, Player* referencePlayer, ProgressType ptype)
{
    ...
    LOG_DEBUG("achievement", "SetCriteriaProgress({}, {}) for ({} GUID: {})",
        entry->ID, changeValue, GetLogNameForGuid(m_player->GetGUID()), GUID_LOPART(m_player->GetGUID()));
...
void AchievementMgr<Player>::CompletedAchievement(AchievementEntry const* achievement, Player* referencePlayer)
{
    ...
    LOG_INFO("achievement", "AchievementMgr::CompletedAchievement({}). Player: {} ({})",
    ...
    if (reward->sender)
    {
        Item* item = reward->itemId ? Item::CreateItem(reward->itemId, 1, m_player) : NULL;

        int loc_idx = m_player->GetSession()->GetSessionDbLocaleIndex();
        ...
        draft.SendMailTo(trans, m_player, MailSender(MAIL_CREATURE, reward->sender));
        CharacterDatabase.CommitTransaction(trans);
    }
}
...
{
    LOG_DEBUG("achievement", "AchievementMgr<Guild>::CompletedAchievement({})", achievement->ID);
    ...
    {
        if (referencePlayer->GetGuildId() == m_player->GetId())
        ...
        if (Group const* group = referencePlayer->GetGroup())
            for (GroupReference const* ref = group->GetFirstMember(); ref != NULL; ref = ref->next())
                if (Player const* groupMember = ref->getSource())
                    if (groupMember->GetGuildId() == m_player->GetId())
...
bool AchievementMgr<T>::CanUpdateCriteria(AchievementCriteriaEntry const* criteria, AchievementEntry const* achievement, uint64 miscValue1, uint64 miscValue2, uint64 miscValue3, Unit const* unit, Player* referencePlayer, GameObject* go /*= NULL*/)
{
    ...
    {
        LOG_TRACE("achievement", "CanUpdateCriteria: {} (Id: {} Type {}) Wrong map",
            criteria->name, criteria->ID, AchievementGlobalMgr::GetCriteriaTypeString(criteria->type));
        return false;
    }
    ...
    {
        LOG_TRACE("achievement", "CanUpdateCriteria: {} (Id: {} Type {}) Wrong faction",
            criteria->name, criteria->ID, AchievementGlobalMgr::GetCriteriaTypeString(criteria->type));
        return false;
    }
    ...
    {
        LOG_TRACE("achievement", "CanUpdateCriteria: {} (Id: {} Type {}) Is Completed",
            criteria->name, criteria->ID, AchievementGlobalMgr::GetCriteriaTypeString(criteria->type));
        return false;
    }
    ...
    {
        LOG_TRACE("achievement", "CanUpdateCriteria: {} (Id: {} Type {}) Requirements not satisfied",
            criteria->name, criteria->ID, AchievementGlobalMgr::GetCriteriaTypeString(criteria->type));
        return false;
    }
    ...
    {
        LOG_TRACE("achievement", "CanUpdateCriteria: {} (Id: {} Type {}) Additional requirements not satisfied",
            criteria->name, criteria->ID, AchievementGlobalMgr::GetCriteriaTypeString(criteria->type));
        return false;
    }
    ...
    {
        LOG_TRACE("achievement", "CanUpdateCriteria: {} (Id: {} Type {}) Conditions not satisfied",
            criteria->name, criteria->ID, AchievementGlobalMgr::GetCriteriaTypeString(criteria->type));
        return false;
    }
...
void AchievementGlobalMgr::LoadAchievementCriteriaList()
{
    ...
    LOG_INFO("server.loading", ">> Loaded {} achievement criteria and {} guild achievement criteria in {} ms", criterias, guildCriterias, GetMSTimeDiffToNow(oldMSTime));
}
...
void AchievementGlobalMgr::LoadAchievementCriteriaData()
{
    ...
    do
    {
        ...
        const char* scriptName = fields[4].Get<std::string>().c_str();
        uint32 scriptId = 0;
...
void AchievementGlobalMgr::LoadCompletedAchievements()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %lu realm first completed achievements in {} ms", (unsigned long)m_allCompletedAchievements.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void AchievementGlobalMgr::LoadRewards()
{
    ...
    do
    {
        ...
        if (!achievement)
        {
            LOG_ERROR("sql.sql", "Table `achievement_reward` contains a wrong achievement ID ({}), ignored.", id);
            continue;
        }
        ...
        {
            LOG_ERROR("sql.sql", "Table `achievement_reward` (ID: {}) does not contain title or item reward data. Ignored.", id);
            continue;
        }

        if (achievement->requiredFaction == ACHIEVEMENT_FACTION_ANY && ((reward.titleId[0] == 0) != (reward.titleId[1] == 0)))
            LOG_ERROR("sql.sql", "Table `achievement_reward` (ID: {}) contains the title (A: {} H: {}) for only one team.", id, reward.titleId[0], reward.titleId[1]);
        ...
        if (reward.titleId[0])
        {
            ...
            if (!titleEntry)
            {
                LOG_ERROR("sql.sql", "Table `achievement_reward` (Entry: {}) contains an invalid title id ({}) in `title_A`, set to 0", id, reward.titleId[0]);
                reward.titleId[0] = 0;
            }
        }
        ...
        if (reward.titleId[1])
        {
            ...
            if (!titleEntry)
            {
                LOG_ERROR("sql.sql", "Table `achievement_reward` (Entry: {}) contains an invalid title id ({}) in `title_H`, set to 0", id, reward.titleId[1]);
                reward.titleId[1] = 0;
            }
        }
        ...
        if (reward.sender)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Table `achievement_reward` (ID: {}) contains an invalid creature ID {} as sender, mail reward skipped.", id, reward.sender);
                reward.sender = 0;
            }
        }
        else
        {
            if (reward.itemId)
                LOG_ERROR("sql.sql", "Table `achievement_reward` (ID: {}) does not have sender data, but contains an item reward. Item will not be rewarded.", id);

            if (!reward.subject.empty())
                LOG_ERROR("sql.sql", "Table `achievement_reward` (ID: {}) does not have sender data, but contains a mail subject.", id);

            if (!reward.text.empty())
                LOG_ERROR("sql.sql", "Table `achievement_reward` (ID: {}) does not have sender data but has mail text.", id);
        }
        ...
        if (reward.itemId)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Table `achievement_reward` (ID: {}) has invalid item id {}, reward mail will not contain item.", id, reward.itemId);
                reward.itemId = 0;
            }
        }
    ...
    LOG_INFO("server.loading", ">> Loaded {} achievement rewards in {} ms.", uint32(m_achievementRewards.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void AchievementGlobalMgr::LoadRewardLocales()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table `achievement_reward_locale` (ID: {}) contains locale strings for a non-existing achievement reward.", id);
            continue;
        }
    ...
    LOG_INFO("server.loading", ">> Loaded {} achievement reward locale strings in {} ms.", uint32(m_achievementRewardLocales.size()), GetMSTimeDiffToNow(oldMSTime));
}
