...
CalendarInvite::~CalendarInvite() { sCalendarMgr->FreeInviteId(_inviteId); }
...
void CalendarMgr::LoadFromDB() {
  ...
  if (QueryResult result = CharacterDatabase.Query(
    ...
    do {
      ...
      ObjectGuid creatorGUID =
          ObjectGuid(HighGuid::Player, fields[1].Get<uint32>());
  ...
  LOG_INFO("server.loading", ">> Loaded {} calendar events in {} ms", count,
  ...
  if (QueryResult result = CharacterDatabase.Query(
    ...
    do {
      ...
      ObjectGuid invitee =
          ObjectGuid(HighGuid::Player, fields[2].Get<uint32>());
      ObjectGuid senderGUID =
          ObjectGuid(HighGuid::Player, fields[3].Get<uint32>());
  ...
  LOG_INFO("server.loading", ">> Loaded {} calendar invites in {} ms", count,
           GetMSTimeDiffToNow(oldMSTime));
...
void CalendarMgr::UpdateEvent(CalendarEvent *calendarEvent) {
  ...
  stmt->SetData(8, uint32(calendarEvent->GetTimeZoneTime())); // correct?
...
void CalendarMgr::RemoveAllPlayerEventsAndInvites(ObjectGuid guid) {
  for (CalendarEventStore::const_iterator itr = _events.begin();
    ...
    if ((*itr)->GetCreatorGUID() == guid)
      RemoveEvent((*itr)->GetEventId(),
                  ObjectGuid::Empty); // don't send mail if removing a character
...
void CalendarMgr::RemovePlayerGuildEventsAndSignups(
    ObjectGuid guid, ObjectGuid::LowType guildId) {
  for (CalendarEventStore::const_iterator itr = _events.begin();
       itr != _events.end(); ++itr)
    if ((*itr)->GetCreatorGUID() == guid &&
...
CalendarEvent *CalendarMgr::GetEvent(uint64 eventId) const {
  ...
  LOG_DEBUG("calendar", "CalendarMgr::GetEvent: [" UI64FMTD "] not found!",
            eventId);
  return nullptr;
}
...
CalendarInvite *CalendarMgr::GetInvite(uint64 inviteId) const {
  ...
  LOG_DEBUG("calendar", "CalendarMgr::GetInvite: [" UI64FMTD "] not found!",
            inviteId);
  return nullptr;
}
...
void CalendarMgr::SendCalendarEventInviteRemoveAlert(
    ObjectGuid guid, CalendarEvent const &calendarEvent,
    CalendarInviteStatus status) {
  if (Player *player = ObjectAccessor::FindConnectedPlayer(guid)) {
...
void CalendarMgr::SendCalendarCommandResult(ObjectGuid guid, CalendarError err,
