...
bool Pet::LoadPetData(Player *owner, uint32 petEntry, uint32 petnumber,
                      bool current) {
  m_loading = true;
  ...
  if (IsCritter()) {
    ...
    owner->MovePositionToFirstCollision(pos, DEFAULT_FOLLOW_DISTANCE_PET,
                                        DEFAULT_FOLLOW_ANGLE);
    Relocate(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(),
    ...
    if (!IsPositionValid()) {
      LOG_ERROR("entities.pet",
                ...
                "isn't valid (X: %f Y: %f)",
                GetGUID().GetCounter(), GetEntry(), GetPositionX(),
                GetPositionY());
      return false;
    }
  ...
  case HUNTER_PET:
    ...
    SetByteFlag(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_PET_FLAGS,
                playerPetData->Renamed
  ...
  default:
    if (!IsPetGhoul())
      LOG_ERROR("entities.pet", "Pet have incorrect type (%u) for pet loading.",
                getPetType());
    break;
  }

  SetUInt32Value(UNIT_FIELD_PET_NAME_TIMESTAMP,
                 uint32(GameTime::GetGameTime())); // cast can't be helped here
  ...
  if (getPetType() == SUMMON_PET &&
    ...
    if (!savedhealth && getPetType() == HUNTER_PET)
      ...
      SetPower(POWER_MANA, savedmana > uint32(GetMaxPower(POWER_MANA))
  ...
  UpdateAllStats();
  SetFullHealth(); // Set full health and mana after pet scaling auras has been
                   // applied
  ...
  if (getPetType() == HUNTER_PET) {
    ...
    stmt->SetData(0, owner->GetGUID().GetCounter());
    stmt->SetData(1, GetCharmInfo()->GetPetNumber());
...
void Pet::SavePetToDB(PetSaveMode mode) {
  ...
  PlayerPetData *playerPetData =
      GetOwner()->GetPlayerPetDataById(m_charmInfo->GetPetNumber());
  ...
  if (mode == PET_SAVE_NEW_PET) {
    Optional<uint8> slot = IsHunterPet()
                               ? GetOwner()->GetFirstUnusedActivePetSlot()
                               : GetOwner()->GetFirstUnusedPetSlot();
  ...
  if (mode >= PET_SAVE_CURRENT_STATE) {
    ...
    stmt->SetData(0, petId);
    stmt->SetData(1, GetEntry());
    stmt->SetData(2, ownerLowGUID);
    stmt->SetData(3, GetNativeDisplayId());
    stmt->SetData(4, getLevel());
    stmt->SetData(5, GetUInt32Value(UNIT_FIELD_PETEXPERIENCE));
    stmt->SetData(6, GetReactState());
    stmt->SetData(7, m_petSlot);
    stmt->SetData(8, m_name);
    stmt->SetData(9,
                  HasByteFlag(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_PET_FLAGS,
                              UNIT_CAN_BE_RENAMED)
                      ? 0
                      : 1);
    stmt->SetData(10, mode == PET_SAVE_DISMISS ? 0 : 1);
    stmt->SetData(11, curhealth);
    stmt->SetData(12, curmana);

    stmt->SetData(13, GenerateActionBarData());

    stmt->SetData(14, uint32(GameTime::GetGameTime())); // unsure about this
    stmt->SetData(15, GetUInt32Value(UNIT_CREATED_BY_SPELL));
    stmt->SetData(16, getPetType());
    trans->Append(stmt);

    CharacterDatabase.CommitTransaction(trans);

    if (m_petSlot > PET_SLOT_LAST)
      LOG_ERROR("sql.sql", "Pet::SavePetToDB: bad slot %u for pet %u!",
                m_petSlot, petId);
...
void Pet::Update(uint32 diff) {
  ...
  case CORPSE: {
    if (!IsHunterPet() || m_corpseRemoveTime <= GameTime::GetGameTime()) {
      Remove(PET_SAVE_DISMISS); // hunters' pets never get removed because of
                                // death, NEVER!
      return;
    }
    break;
  ...
  case ALIVE: {
    ...
    if ((!IsWithinDistInMap(owner, GetMap()->GetVisibilityRange()) &&
         !isPossessed()) ||
    ...
    if (isControlled()) {
      if (owner->GetPetGUID() != GetGUID()) {
        LOG_ERROR("entities.pet", "Pet %u is not pet of owner %s, removed",
                  GetEntry(), GetOwner()->GetName().c_str());
...
bool Guardian::InitStatsForLevel(uint8 petlevel) {
  ...
  // Resistance
  // Hunters pets should not inherit resistances from creature_template, they
  // have separate auras for that
  if (!IsHunterPet())
    for (uint8 i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; ++i)
      SetStatFlatModifier(UnitMods(UNIT_MOD_RESISTANCE_START + i), BASE_VALUE,
  ...
  case HUNTER_PET: {
    ...
    // these formula may not be correct; however, it is designed to be close to
    // what it should be this makes dps 0.5 of pets level
    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE,
                        float(petlevel - (petlevel / 4)));
    // damage range is then petlevel / 2
    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE,
                        float(petlevel + (petlevel / 4)));
    // damage is increased afterwards as strength and pet scaling modify attack
    // power
    break;
  }
  default: {
    ...
    case ENTRY_TREANT: {
      float bonusDmg =
          GetOwner()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_NATURE) *
          0.15f;
      SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE,
    ...
    case ENTRY_FIRE_ELEMENTAL: {
      if (Unit *owner = m_owner->GetOwner()) {
        SetCreateHealth(owner->CountPctFromMaxHealth(75));
        SetBonusDamage(int32(
            owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FIRE) * 0.5f));
      }
    ...
    case ENTRY_SHADOWFIEND: {
      ...
      int32 bonus_dmg =
          int32(GetOwner()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SHADOW) *
                0.375f);
      SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE,
    ...
    case ENTRY_SPIRIT_WOLF: {
      ...
      SetStatFlatModifier(UNIT_MOD_ARMOR, BASE_VALUE,
                          float(GetOwner()->GetArmor()) *
                              0.35f); // Bonus Armor (35% of player armor)
      SetStatFlatModifier(UNIT_MOD_STAT_STAMINA, BASE_VALUE,
                          float(GetOwner()->GetStat(STAT_STAMINA)) *
                              0.3f); // Bonus Stamina (30% of player stamina)
    ...
    case ENTRY_GARGOYLE: {
      ...
      SetBonusDamage(
          int32(GetOwner()->GetTotalAttackPowerValue(BASE_ATTACK) * 0.5f));
      SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE,
                          float(petlevel - (petlevel / 4)));
      SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE,
                          float(petlevel + (petlevel / 4)));
    ...
    case ENTRY_BLOODWORM: {
      ...
      SetBonusDamage(
          int32(m_owner->GetTotalAttackPowerValue(BASE_ATTACK) * 0.006f));
    ...
    case ENTRY_INFERNAL: {
      if (m_owner->GetTypeId() ==
          TYPEID_PLAYER) // Infernal get 100% of owners spell, Immolation has
                         // his own coef.
        SetBonusDamage(
            int32(m_owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SPELL)));
    ...
    case ENTRY_GHOUL:
      ...
      break;
    default: {
      /* ToDo: Check what 5f5d2028 broke/fixed and how much of
       * Creature::UpdateLevelDependantStats() should be copied here (or moved
       * to another method or if that function should be called here or not just
...
uint32 Pet::GetCurrentFoodBenefitLevel(uint32 itemlevel) const {
  ...
  if (getLevel() <= itemlevel + 10) // possible to feed level 85 pet with ilevel
                                    // 75 level food for full effect
    return 50;
  // -10 to -20
...
void Pet::_LoadSpells() {
  CharacterDatabasePreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_SEL_PET_SPELL);
  stmt->SetData(0, m_charmInfo->GetPetNumber());
  PreparedQueryResult result = CharacterDatabase.Query(stmt);
  ...
  if (result) {
    do {
      ...
      addSpell(fields[0].Get<uint32>(), ActiveStates(fields[1].Get<uint8>()),
               PETSPELL_UNCHANGED);
...
void Pet::_LoadAuras(uint32 timediff) {
  ...
  CharacterDatabasePreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_SEL_PET_AURA);
  stmt->SetData(0, m_charmInfo->GetPetNumber());
  PreparedQueryResult result = CharacterDatabase.Query(stmt);
  ...
  if (result) {
    do {
      ...
      if (!spellInfo) {
        LOG_ERROR("entities.pet", "Unknown aura (spellid %u), ignore.",
                  spellid);
        continue;
      }
      ...
      if (remaintime != -1 &&
          (!spellInfo->IsPositive() ||
           spellInfo->HasAttribute(SPELL_ATTR4_FADES_WHILE_LOGGED_OUT))) {
      ...
      if (Aura *aura = Aura::TryCreate(spellInfo, effmask, this, nullptr,
        ...
        LOG_DEBUG("entities.pet", "Added aura spellid %u, effectmask %u",
...
bool Pet::addSpell(uint32 spellId, ActiveStates active /*= ACT_DECIDE*/,
  ...
  // Store pet scaling auras in a own vector to handle the updating more
  // efficient
  ...
  if (spellInfo->HasAttribute(SPELL_ATTR4_HIDDEN_IN_SPELLBOOK) &&
      !IsWarlockMinion())
    return true;
...
bool Pet::removeSpell(uint32 spell_id, bool learn_prev, bool clear_ab) {
  ...
  if (clear_ab && !learn_prev &&
    ...
    if (!m_loading)
      GetOwner()->PetSpellInitialize(); // need update action bar for last
                                        // removed rank
  }
  ...
  for (uint32 i = 0; i < sTalentStore.GetNumRows(); ++i) {
    ...
    for (uint8 j = 0; j < MAX_TALENT_RANK; ++j) {
      for (PetSpellMap::const_iterator itr = m_spells.begin();
        ...
        if (itrFirstId == talentInfo->SpellRank[j] ||
            sSpellMgr->IsSpellLearnToSpell(talentInfo->SpellRank[j],
                                           itrFirstId)) {
...
void Pet::resetTalentsForAllPetsOf(Player * /*owner*/,
  ...
  uint32 exceptPetNumber = onlinePet ? onlinePet->GetCharmInfo()->GetPetNumber()
  : 0;

  PreparedStatement* stmt =
  CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_PET); stmt->SetData(0,
  owner->GetGUID().GetCounter()); stmt->SetData(1, exceptPetNumber);
  ...
  stmt->SetData(0, owner->GetGUID().GetCounter());
  stmt->SetData(1, exceptPetNumber);
  PreparedQueryResult result = CharacterDatabase.Query(stmt);
...
Player *Pet::GetOwner() const { return Minion::GetOwner()->ToPlayer(); }

float Pet::GetNativeObjectScale() const {
  CreatureFamilyEntry const *creatureFamily =
      sCreatureFamilyStore.LookupEntry(GetCreatureTemplate()->family);
  CreatureDisplayInfoEntry const *displayInfo =
      sCreatureDisplayInfoStore.LookupEntry(GetNativeDisplayId());

  if (creatureFamily && displayInfo && creatureFamily->MinScale > 0.0f &&
      getPetType() == HUNTER_PET) {
    if (CreatureModelDataEntry const *modelData =
            sCreatureModelDataStore.LookupEntry(displayInfo->ModelID)) {
      ...
      float minLevelScaleMod =
          level >= minScaleLevel ? level - minScaleLevel : 0.f;
      ...
      scale = (creatureFamily->MaxScale - creatureFamily->MinScale) * scaleMod +
