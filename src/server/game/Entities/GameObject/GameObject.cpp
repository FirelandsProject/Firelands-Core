...
void GameObject::RemoveFromOwner()
{
    ...
    LOG_FATAL("misc", "Removed GameObject (GUID: %u Entry: %u SpellId: %u LinkedGO: %u) that just lost any reference to the owner (GUID: %u Type: '%s') GO list",
...
bool GameObject::Create(uint32 guidlow, uint32 name_id, Map* map, uint32 phaseMask, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 animprogress, GOState go_state, uint32 artKit)
{
    ...
    if (!IsPositionValid())
    {
        LOG_ERROR("misc", "Gameobject (GUID: %u Entry: %u) not created. Suggested coordinates isn't valid (X: %f Y: %f)", guidlow, name_id, x, y);
        return false;
    }
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    if (owner)                    // hunter trap
                    {
                        ...
                        Firelands::AnyUnfriendlyNoTotemUnitInObjectRangeCheck checker(this, owner, radius);
                        Firelands::UnitListSearcher<Firelands::AnyUnfriendlyNoTotemUnitInObjectRangeCheck> searcher(this, possibleTargets, checker);
...
void GameObject::TriggeringLinkedGameObject(uint32 trapEntry, Unit* target)
{
    ...
    GameObject* trapGO = NULL;
    {
        ...
        Firelands::NearestGameObjectEntryInObjectRangeCheck go_check(*target, trapEntry, range);
        Firelands::GameObjectLastSearcher<Firelands::NearestGameObjectEntryInObjectRangeCheck> checker(this, trapGO, go_check);

        TypeContainerVisitor<Firelands::GameObjectLastSearcher<Firelands::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer > object_checker(checker);
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    TypeContainerVisitor<Firelands::GameObjectSearcher<Firelands::NearestGameObjectFishingHole>, GridTypeMapContainer > grid_object_checker(checker);
