...
void FormationMgr::AddCreatureToGroup(uint32 leaderGuid, Creature *creature) {
  ...
  CreatureGroupHolderType::iterator itr =
      map->CreatureGroupHolder.find(leaderGuid);

  // Add member to an existing group
  if (itr != map->CreatureGroupHolder.end()) {
    LOG_DEBUG(
        "entities.unit",
        "Group found: {}, inserting creature GUID: {}, Group InstanceID {}",
        leaderGuid, creature->GetGUID().GetCounter(),
        creature->GetInstanceId());
    itr->second->AddMember(creature);
  }
  // Create new group
  else {
    LOG_DEBUG("entities.unit", "Group not found: {}. Creating new group.",
              leaderGuid);
...
void FormationMgr::RemoveCreatureFromGroup(CreatureGroup *group,
  ...
  LOG_DEBUG("entities.unit",
            "Deleting member pointer to GUID: {} from group {}", group->GetId(),
            member->GetSpawnId());
  ...
  if (group->isEmpty()) {
    ...
    LOG_DEBUG("entities.unit", "Deleting group with InstanceID {}",
...
void FormationMgr::LoadCreatureFormations() {
  ...
  QueryResult result = WorldDatabase.Query(
      "SELECT LeaderGUID, MemberGUID, FollowDistance, FollowAngle, GroupAI, "
      "InversionPoint1, InversionPoint2 FROM creature_formations ORDER BY "
      "LeaderGUID");

  if (!result) {
    LOG_ERROR("server.loading", ">>  Loaded 0 creatures in formations. DB "
                                "table `creature_formations` is empty!");
    return;
  }
  ...
  do {
    ...
    // check data correctness
    {
      if (!sObjectMgr->GetCreatureData(group_member.LeaderGUID)) {
        LOG_ERROR(
            "sql.sql",
            "creature_formations table leader guid {} incorrect (not exist)",
      ...
      if (!sObjectMgr->GetCreatureData(memberGUID)) {
        LOG_ERROR(
            "sql.sql",
            "creature_formations table member guid {} incorrect (not exist)",
            memberGUID);
        continue;
      }
    }
  ...
  LOG_INFO("server.loading", ">> Loaded {} creatures in formations in {} ms",
           count, GetMSTimeDiffToNow(oldMSTime));
}

void CreatureGroup::AddMember(Creature *member) {
  LOG_DEBUG("entities.unit", "CreatureGroup::AddMember: Adding unit GUID: {}.",
            member->GetGUID().GetCounter());

  // Check if it is a leader
  if (member->GetSpawnId() == m_groupID) {
    LOG_DEBUG("entities.unit",
              "Unit GUID: {} is formation leader. Adding group.",
              member->GetGUID().GetCounter());
    m_leader = member;
  }
...
void CreatureGroup::MemberEngagingTarget(Creature *member, Unit *target) {
  ...
  for (CreatureGroupMemberType::iterator itr = m_members.begin();
    ...
    if (((other != m_leader && (groupAI & FLAG_MEMBERS_ASSIST_LEADER)) ||
         (other == m_leader && (groupAI & FLAG_LEADER_ASSISTS_MEMBER))) &&
...
void CreatureGroup::FormationReset(bool dismiss) {
  for (CreatureGroupMemberType::iterator itr = m_members.begin();
    ...
    if (itr->first != m_leader && itr->first->IsAlive()) {
      ...
      LOG_DEBUG("entities.unit", "Set {} movement for member GUID: {}",
                dismiss ? "default" : "idle",
                itr->first->GetGUID().GetCounter());
    }
  }
  m_Formed = !dismiss;
}
...
void CreatureGroup::LeaderStartedMoving() {
  ...
  for (CreatureGroupMemberType::iterator itr = m_members.begin();
    ...
    if (member == m_leader || !member->IsAlive() || member->GetVictim() ||
        !(itr->second.GroupAI & FLAG_IDLE_IN_FORMATION))
      continue;

    float angle = itr->second.FollowAngle + float(M_PI);
    float dist = itr->second.FollowDistance;

    MovementGenerator const *moveGen =
        member->GetMotionMaster()->GetMotionSlot(MOTION_SLOT_IDLE);
    if (!moveGen ||
        moveGen->GetMovementGeneratorType() != FORMATION_MOTION_TYPE)
      member->GetMotionMaster()->MoveFormation(m_leader, dist, angle,
                                               itr->second.InversionPoint1,
                                               itr->second.InversionPoint2);
  }
}
