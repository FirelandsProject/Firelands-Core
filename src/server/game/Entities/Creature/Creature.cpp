...
bool VendorItem::IsGoldRequired(ItemTemplate const *pProto) const {
  return (pProto->GetFlags2() & ITEM_FLAG2_DONT_IGNORE_BUY_PRICE) ||
         !ExtendedCost;
}

bool VendorItemData::RemoveItem(uint32 item_id, uint8 type) {
  auto newEnd = std::remove_if(
      m_items.begin(), m_items.end(), [=](VendorItem const &vendorItem) {
...
uint32 CreatureTemplate::GetFirstInvisibleModel() const {
  CreatureModelInfo const *modelInfo =
      sObjectMgr->GetCreatureModelInfo(Modelid1);
...
WorldPacket CreatureTemplate::BuildQueryData(LocaleConstant loc) const {
  ...
  if (std::vector<uint32> const *items =
...
bool ForcedDespawnDelayEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/) {
  m_owner.DespawnOrUnsummon(
      0, m_respawnTimer); // since we are here, we are not TempSummon as object
                          // type cannot change during runtime
  return true;
}

Creature::Creature(bool isWorldObject)
    : Unit(isWorldObject), MapObject(), m_groupLootTimer(0),
      lootingGroupLowGUID(0), m_PlayerDamageReq(0), m_lootRecipient(),
      m_lootRecipientGroup(0), _pickpocketLootRestore(0), m_corpseRemoveTime(0),
      m_respawnTime(0), m_respawnDelay(300), m_corpseDelay(60),
      m_respawnradius(0.0f), m_boundaryCheckTime(2500), m_combatPulseTime(0),
      m_combatPulseDelay(0), m_reactState(REACT_AGGRESSIVE),
      m_defaultMovementType(IDLE_MOTION_TYPE), m_spawnId(0), m_equipmentId(0),
      m_originalEquipmentId(0), m_AlreadyCallAssistance(false),
      m_AlreadySearchedAssistance(false), m_regenHealth(true),
      m_cannotReachTarget(false), m_cannotReachTimer(0),
      m_meleeDamageSchoolMask(SPELL_SCHOOL_MASK_NORMAL), m_originalEntry(0),
      m_homePosition(), m_transportHomePosition(), m_creatureInfo(nullptr),
      m_creatureData(nullptr), _waypointPathId(0),
      _currentWaypointNodeInfo(0, 0), _cyclicSplinePathId(0),
      m_formation(nullptr), m_triggerJustAppeared(true),
      m_respawnCompatibilityMode(false), _lastDamagedTime(0),
...
void Creature::AddToWorld() {
  ...
  if (!IsInWorld()) {
    ...
    LOG_DEBUG(
        "entities.unit",
        "Adding creature {} with entry {} and DBGUID {} to world in map {}",
        GetGUID().GetCounter(), GetEntry(), m_spawnId, GetMap()->GetId());
...
void Creature::RemoveFromWorld() {
  if (IsInWorld()) {
    ...
    if (m_spawnId)
      Firelands::Containers::MultimapErasePair(
          GetMap()->GetCreatureBySpawnIdStore(), m_spawnId, this);

    LOG_DEBUG(
        "entities.unit",
        "Removing creature {} with entry {} and DBGUID {} to world in map {}",
        GetGUID().GetCounter(), GetEntry(), m_spawnId, GetMap()->GetId());
    GetMap()->GetObjectsStore().Remove<Creature>(GetGUID());
  }
}

void Creature::DisappearAndDie() { ForcedDespawn(0); }

bool Creature::IsReturningHome() const {
  if (GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_ACTIVE) ==
      HOME_MOTION_TYPE)
    return true;
...
void Creature::RemoveCorpse(bool setSpawnTime, bool destroyForNearbyPlayers) {
  ...
  if (m_respawnCompatibilityMode) {
    ...
    if (setSpawnTime)
      m_respawnTime = std::max<time_t>(GameTime::GetGameTime() + respawnDelay,
                                       m_respawnTime);

    // if corpse was removed during falling, the falling will continue and
    // override relocation to respawn position
    if (IsFalling())
      StopMoving();
...
bool Creature::InitEntry(uint32 entry, CreatureData const *data /*= nullptr*/) {
  ...
  {
    LOG_ERROR("sql.sql",
              "Creature (Entry: {}) has invalid model {} defined in table "
              "`creature_template`, can't load.",
              entry, displayID);
    return false;
  }
  ...
  m_defaultMovementType =
      MovementGeneratorType(data ? data->movementType : cinfo->MovementType);
  ...
  SetNoNpcDamageBelowPctHealthValue(
      sObjectMgr->GetSparringHealthLimitFor(GetEntry()));
...
bool Creature::UpdateEntry(uint32 entry, CreatureData const *data /*= nullptr*/,
  ...
  // Do not update guardian stats here - they are handled in
  ...
  if (!IsGuardian()) {
    ...
    SetStatFlatModifier(UNIT_MOD_RESISTANCE_HOLY, BASE_VALUE,
                        float(cInfo->resistance[SPELL_SCHOOL_HOLY]));
    SetStatFlatModifier(UNIT_MOD_RESISTANCE_FIRE, BASE_VALUE,
                        float(cInfo->resistance[SPELL_SCHOOL_FIRE]));
    SetStatFlatModifier(UNIT_MOD_RESISTANCE_NATURE, BASE_VALUE,
                        float(cInfo->resistance[SPELL_SCHOOL_NATURE]));
    SetStatFlatModifier(UNIT_MOD_RESISTANCE_FROST, BASE_VALUE,
                        float(cInfo->resistance[SPELL_SCHOOL_FROST]));
    SetStatFlatModifier(UNIT_MOD_RESISTANCE_SHADOW, BASE_VALUE,
                        float(cInfo->resistance[SPELL_SCHOOL_SHADOW]));
    SetStatFlatModifier(UNIT_MOD_RESISTANCE_ARCANE, BASE_VALUE,
...
void Creature::Update(uint32 diff) {
  ...
  case JUST_RESPAWNED:
    ...
    LOG_ERROR(
        "entities.unit",
        "Creature (GUID: {} Entry: {}) in wrong state: JUST_RESPAWNED (4)",
  ...
  case JUST_DIED:
    ...
    LOG_ERROR("entities.unit",
              "Creature (GUID: {} Entry: {}) in wrong state: JUST_DIED (1)",
              GetGUID().GetCounter(), GetEntry());
    break;
  case DEAD: {
    if (!m_respawnCompatibilityMode) {
      LOG_ERROR("entities.unit",
                "Creature (GUID: {} Entry: {}) in wrong state: DEAD (3)",
                GetGUID().GetCounter(), GetEntry());
      break;
    }
    ...
    if (m_respawnTime <= now) {
      ...
      if (m_creatureData && !GetMap()->IsSpawnGroupActive(
                                m_creatureData->spawnGroupData->groupId)) {
      ...
      else // the master is dead
      {
        ...
        if (targetGuid == dbtableHighGuid) // if linking self, never respawn
          ...
          // linked guid can be a boss, uses std::numeric_limits<time_t>::max to
          // never respawn in that instance we shall inherit it instead of
          // adding and causing an overflow
  ...
  case CORPSE: {
    ...
    if (m_groupLootTimer && lootingGroupLowGUID) {
      ...
      RemoveCorpse(false);
      LOG_DEBUG("entities.unit", "Removing corpse... {} ",
  ...
  case ALIVE: {
    ...
    // if periodic combat pulse is enabled and we are both in combat and in a
    // dungeon, do this now
...
void Creature::RegenerateHealth() {
  ...
  if (IsInEvadeMode() || (IsEngaged() && !IsPolymorphed() &&
                          !CanNotReachTarget())) // regenerate health if not in
                                                 // combat or if polymorphed
    return;
  ...
  addvalue += GetTotalAuraModifier(SPELL_AURA_MOD_REGEN) *
              UNIT_HEALTH_REGENERATION_INTERVAL / (5 * IN_MILLISECONDS);
...
void Creature::DoFleeToGetAssistance() {
  ...
  if (radius > 0) {
    ...
    Firelands::NearestAssistCreatureInCreatureRangeCheck u_check(
        this, GetVictim(), radius);
    Firelands::CreatureLastSearcher<
        Firelands::NearestAssistCreatureInCreatureRangeCheck>
    ...
    if (!creature)
      // SetFeared(true, EnsureVictim()->GetGUID(), 0,
...
bool Creature::Create(ObjectGuid::LowType guidlow, Map *map, uint32 entry,
                      Position const &pos,
                      CreatureData const *data /*= nullptr*/,
                      uint32 vehId /*= 0*/, bool dynamic) {
  ASSERT(map);
  SetMap(map);

  if (data) {
    PhasingHandler::InitDbPhaseShift(GetPhaseShift(), data->phaseUseFlags,
                                     data->phaseId, data->phaseGroup);
  ...
  // Check if the position is valid before calling CreateFromProto(), otherwise
  // we might add Auras to Creatures at invalid position, triggering a crash
  // about Auras not removed in the destructor
  if (!IsPositionValid()) {
    LOG_ERROR("entities.unit",
              "Creature::Create(): given coordinates for creature (guidlow {}, "
              "entry {}) are not valid (X: {}, Y: {}, Z: {}, O: {})",
              guidlow, entry, pos.GetPositionX(), pos.GetPositionY(),
...

  {
    ...
    GetMap()->GetFullTerrainStatusForPosition(
        GetPhaseShift(), GetPositionX(), GetPositionY(), GetPositionZ(), data,
        MAP_ALL_LIQUIDS, DEFAULT_COLLISION_HEIGHT);
    ProcessPositionDataChanged(data);
  }

  // Allow players to see those units while dead, do it here (mayby altered by
  // addon auras)
  ...
  if (GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_DUNGEON_BOSS &&
      map->IsDungeon())
    m_respawnDelay = 0; // special value, prevents respawn for dungeon bosses
                        // unless overridden
  ...
  if (CanHaveThreatList())
    ...
    if (!target && IsSummon()) {
      if (Unit *owner = ToTempSummon()->GetOwner()) {
        ...
        if (!target) {
          for (ControlList::const_iterator itr = owner->m_Controlled.begin();
...
void Creature::SaveToDB(uint32 mapid, uint8 spawnMask) {
  ...
  if (!GetTransport()) {
    ...
    data.mapId = mapid;
    data.spawnPoint.Relocate(GetTransOffsetX(), GetTransOffsetY(),
                             GetTransOffsetZ(), GetTransOffsetO());
  }
  data.spawntimesecs = m_respawnDelay;
  // prevent add data integrity problems
  data.spawndist =
      GetDefaultMovementType() == IDLE_MOTION_TYPE ? 0.0f : m_respawnradius;
  ...
  stmt->SetData(index++, data.phaseId);
  stmt->SetData(index++, data.phaseGroup);
  ...
  stmt->SetData(index++, m_respawnradius);
...
void Creature::UpdateLevelDependantStats() {
  ...
  SetStatFlatModifier(UNIT_MOD_ATTACK_POWER_RANGED, BASE_VALUE,
...
bool Creature::CreateFromProto(ObjectGuid::LowType guidlow, uint32 entry,
                               CreatureData const *data /*= nullptr*/,
                               uint32 vehId /*= 0*/) {
  SetZoneScript();
  ...
  Object::_Create(guidlow, entry,
                  (vehId || cinfo->VehicleId) ? HighGuid::Vehicle
                                              : HighGuid::Unit);
...
bool Creature::LoadFromDB(ObjectGuid::LowType spawnId, Map *map, bool addToMap,
                          bool allowDuplicate) {
  if (!allowDuplicate) {
    // If an alive instance of this spawnId is already found, skip creation
    // If only dead instance(s) exist, despawn them and spawn a new (maybe also
    // dead) version
    ...
    if (creatureBounds.first != creatureBounds.second) {
      for (auto itr = creatureBounds.first; itr != creatureBounds.second;
        ...
        if (itr->second->IsAlive()) {
          LOG_DEBUG("maps", "Would have spawned {} but {} already exists",
                    spawnId,
                    creatureBounds.first->second->GetGUID().ToString().c_str());
          return false;
        } else {
          despawnList.push_back(itr->second);
          LOG_DEBUG("maps", "Despawned dead instance of spawn {} ({})", spawnId,
  ...
  if (!data) {
    LOG_ERROR(
        "sql.sql",
        "Creature (SpawnID {}) not found in table `creature`, can't load. ",
        spawnId);
    return false;
  }
  ...
  m_respawnCompatibilityMode =
      ((data->spawnGroupData->flags & SPAWNGROUP_FLAG_COMPATIBILITY_MODE) != 0);
  m_creatureData = data;
  m_respawnradius = data->spawndist;
  m_respawnDelay = data->spawntimesecs;

  if (!Create(map->GenerateLowGuid<HighGuid::Unit>(), map, data->id,
              data->spawnPoint, data, 0U, !m_respawnCompatibilityMode))
    return false;
  ...
  if (!m_respawnTime &&
    ...
    if (!m_respawnCompatibilityMode) {
      // @todo pools need fixing! this is just a temporary thing, but they
      // violate dynspawn principles
      if (!sPoolMgr->IsPartOfAPool<Creature>(spawnId)) {
        LOG_ERROR("entities.unit",
                  "Creature (SpawnID {}) trying to load in inactive spawn "
                  "group '{}':\n",
  ...
  if (m_respawnTime) {
    if (!m_respawnCompatibilityMode) {
      ...
      if (!sPoolMgr->IsPartOfAPool<Creature>(spawnId)) {
        LOG_ERROR("entities.unit",
                  "Creature (SpawnID {}) trying to load despite a respawn "
                  "timer in progress:\n",
                  spawnId);
        return false;
      }
    ...
    if (CanFly()) {
      float tz = map->GetHeight(GetPhaseShift(), data->spawnPoint, true,
                                MAX_FALL_DISTANCE);
      if (data->spawnPoint.GetPositionZ() - tz > 0.1f &&
          Firelands::IsValidMapCoord(tz))
        Relocate(data->spawnPoint.GetPositionX(),
                 data->spawnPoint.GetPositionY(), tz);
    }
  }
...
void Creature::SetSpawnHealth() {
  ...
  SetHealth((m_deathState == ALIVE || m_deathState == JUST_RESPAWNED)
                ? curhealth
                : 0);
}
...
bool Creature::hasInvolvedQuest(uint32 quest_id) const {
  return sObjectMgr->GetCreatureQuestInvolvedRelations(GetEntry())
      .HasQuest(quest_id);
}
...
/*static*/ bool Creature::DeleteFromDB(ObjectGuid::LowType spawnId) {
  ...
  sMapMgr->DoForAllMapsWithMapId(
      data->mapId, [spawnId, trans](Map *map) -> void {
        // despawn all active creatures, and remove their respawns
        std::vector<Creature *> toUnload;
        for (auto const &pair : Firelands::Containers::MapEqualRange(
                 map->GetCreatureBySpawnIdStore(), spawnId))
...
bool Creature::CanStartAttack(Unit const *who, bool force) const {
  ...
  if ((IsImmuneToNPC() &&
       !who->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED)) ||
      (IsImmuneToPC() &&
       who->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED)))
    return false;
  ...
  if (CheckNoGrayAggroConfig(who->getLevelForTarget(this),
                             getLevelForTarget(who)))
    return false;
...
float Creature::GetAttackDistance(Unit const *player) const {
  // WoW Wiki: the minimum radius seems to be 5 yards, while the maximum range
  // is 45 yards
  ...
  uint8 expansionMaxLevel = uint8(
      DBCManager::GetMaxLevelForExpansion(GetCreatureTemplate()->expansion));
  ...
  // The combatreach should not get taken into account for the distance so we
  // drop it from the range (see Supremus as expample)
  ...
  // The aggro range of creatures with higher levels than the total player level
  // for the expansion should get the maxlevel treatment This makes sure that
  // creatures such as bosses wont have a bigger aggro range than the rest of
  // the npc's The following code is used for blizzlike behavior such as
  // skipable bosses (e.g. Commander Springvale at level 85)
...
void Creature::setDeathState(DeathState s) {
  ...
  if (s == JUST_DIED) {
    ...
    if (uint32 scalingMode = sWorld->getIntConfig(CONFIG_RESPAWN_DYNAMICMODE))
      GetMap()->ApplyDynamicModeRespawnScaling(this, m_spawnId, respawnDelay,
                                               scalingMode);

    // @todo remove the boss respawn time hack in a dynspawn follow-up once we
    // have creature groups in instances
    if (m_respawnCompatibilityMode) {
      if (IsDungeonBoss() && !m_respawnDelay)
        m_respawnTime = std::numeric_limits<time_t>::max(); // never respawn in
                                                            // this instance
      else
    ...
    _spellFocusInfo.Reset(); // cancel pending reacquisition of targets post
                             // spell focusing.
    SetTarget(ObjectGuid::Empty); // drop target - dead mobs shouldn't ever
                                  // target things
    ...
    SetUInt32Value(
        ...
        0); // if creature is mounted on a virtual mount, remove it at death
...
void Creature::Respawn(bool force) {
  ...
  if (m_respawnCompatibilityMode) {
    ...
    if (getDeathState() == DEAD) {
      LOG_DEBUG("entities.unit", "Respawning creature {} ({})",
      ...
      if (UnitAI *ai = AI()) // reset the AI to be sure no dirty or
                             // uninitialized values will be used till next tick
        ai->Reset();

      m_triggerJustAppeared = true;

      uint32 poolid =
          GetSpawnId() ? sPoolMgr->IsPartOfAPool<Creature>(GetSpawnId()) : 0;
      if (poolid)
  ...
  LOG_DEBUG("entities.unit", "Respawning creature {} ({})", GetName().c_str(),
...
void Creature::ForcedDespawn(uint32 timeMSToDespawn,
  ...
  if (timeMSToDespawn) {
    m_Events.AddEvent(new ForcedDespawnDelayEvent(*this, forceRespawnTimer),
...
bool Creature::IsImmunedToSpell(SpellInfo const *spellInfo, Unit *caster,
                                Optional<uint8> effectMask /*= {}*/) const {
  if (!spellInfo)
    return false;
...
bool Creature::IsImmunedToSpellEffect(SpellInfo const *spellInfo, uint32 index,
                                      Unit *caster) const {
...
Unit *Creature::SelectNearestTarget(float dist,
  ...
  Firelands::UnitLastSearcher<Firelands::NearestHostileUnitCheck> searcher(
      this, target, u_check);
...
Unit *Creature::SelectNearestTargetInAttackDistance(float dist) const {
  if (dist > MAX_VISIBILITY_DISTANCE) {
    LOG_ERROR("entities.unit",
              "Creature (GUID: {} Entry: {}) "
              "SelectNearestTargetInAttackDistance called with dist > "
              "MAX_VISIBILITY_DISTANCE. Distance set to ATTACK_DISTANCE.",
              GetGUID().GetCounter(), GetEntry());
  ...
  Firelands::UnitLastSearcher<
      Firelands::NearestHostileUnitInAttackDistanceCheck>
...
void Creature::CallAssistance() {
  if (!m_AlreadyCallAssistance && GetVictim() && !IsPet() && !IsCharmed()) {
    ...
    if (radius > 0) {
      ...
      Firelands::AnyAssistCreatureInRangeCheck u_check(this, GetVictim(),
                                                       radius);
      Firelands::CreatureListSearcher<Firelands::AnyAssistCreatureInRangeCheck>
          searcher(this, assistList, u_check);
      Cell::VisitGridObjects(this, searcher, radius);

      if (!assistList.empty()) {
        AssistDelayEvent *e =
            new AssistDelayEvent(EnsureVictim()->GetGUID(), *this);
...
void Creature::CallForHelp(float radius) {
  ...
  if (!target) {
    LOG_ERROR(
        "entities.unit",
        "Creature {} ({}) trying to call for help without being in combat.",
        GetEntry(), GetName().c_str());
    return;
  }

  Firelands::CallOfHelpCreatureInRangeDo u_do(this, target, radius);
  Firelands::CreatureWorker<Firelands::CallOfHelpCreatureInRangeDo> worker(
      this, u_do);
...
void Creature::SaveRespawnTime(uint32 forceDelay) {
  ...
  time_t thisRespawnTime =
      forceDelay ? GameTime::GetGameTime() + forceDelay : m_respawnTime;
  GetMap()->SaveRespawnTime(
      SPAWN_TYPE_CREATURE, m_spawnId, GetOriginalEntry(), thisRespawnTime,
      Firelands::ComputeGridCoord(GetHomePosition().GetPositionX(),
                                  GetHomePosition().GetPositionY())
          .GetId());
}
...
bool Creature::CanCreatureAttack(Unit const *victim, bool /*force*/) const {
  ...
  if (!GetCharmerOrOwnerGUID().IsPlayer()) {
    ...
    // don't check distance to home position if recently damaged, this should
    // include taunt auras
    if (!isWorldBoss() && (GetLastDamagedTime() > GameTime::GetGameTime() ||
  ...
  float dist =
      std::min<float>(GetMap()->GetVisibilityRange(), SIZE_OF_GRID_CELL * 2);
...
bool Creature::LoadCreaturesAddon() {
  ...
  if (cainfo->bytes2 != 0) {
    ...
    SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_SHEATH_STATE,
                 uint8(cainfo->bytes2 & 0xFF));
    // SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_PVP_FLAG,
    // uint8((cainfo->bytes2 >> 8) & 0xFF)); SetByteValue(UNIT_FIELD_BYTES_2,
    // UNIT_BYTES_2_OFFSET_PET_FLAGS, uint8((cainfo->bytes2 >> 16) & 0xFF));
    SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_PET_FLAGS, 0);
    // SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_SHAPESHIFT_FORM,
    // uint8((cainfo->bytes2 >> 24) & 0xFF));
  ...
  if (!cainfo->auras.empty()) {
    for (std::vector<uint32>::const_iterator itr = cainfo->auras.begin();
      ...
      if (!AdditionalSpellInfo) {
        LOG_ERROR("sql.sql",
                  "Creature (GUID: {} Entry: {}) has wrong spell {} defined in "
                  "`auras` field.",
                  GetGUID().GetCounter(), GetEntry(), *itr);
        continue;
      }
      ...
      LOG_DEBUG("entities.unit",
                "Spell: {} added to creature (GUID: {} Entry: {})", *itr,
                GetGUID().GetCounter(), GetEntry());
    }
  }
...
void Creature::SendZoneUnderAttackMessage(Player *attacker) {
  ...
  sWorld->SendGlobalMessage(packet.Write(), nullptr,
...
void Creature::SetInCombatWithZone() {
  if (!CanHaveThreatList()) {
    LOG_ERROR("entities.unit",
              "Creature entry {} call SetInCombatWithZone but creature cannot "
              "have threat list.",
              GetEntry());
    return;
  }
  ...
  if (!map->IsDungeon()) {
    LOG_ERROR("entities.unit",
              "Creature entry {} call SetInCombatWithZone for map (id: {}) "
...
bool Creature::HasSpell(uint32 spellID) const {
  return std::find(std::begin(m_spells), std::end(m_spells), spellID) !=
         std::end(m_spells);
}
...
void Creature::InitializeCreatureMovementInfo(uint32 movementId) {
  if (CreatureMovementInfoOverride const *movementInfoOverride =
          sObjectMgr->GetCreatureMovementInfoOverride(movementId)) {
...
void Creature::InitializeMovementSpeeds() {
  // This segment needs to be removed once model based speeds are implemented
  // and movementIds fully added.
...
void Creature::AllLootRemovedFromCorpse() {
  ...
  // corpse skinnable, but without skinning flag, and then skinned, corpse will
  // despawn next update
  ...
  m_respawnTime =
      std::max<time_t>(m_corpseRemoveTime + m_respawnDelay, m_respawnTime);
}
...
uint32 Creature::GetPetAutoSpellOnPos(uint8 pos) const {
  if (pos >= MAX_SPELL_CHARM || !m_charmInfo ||
...
void Creature::SetPosition(float x, float y, float z, float o) {
  ...
  if (!Firelands::IsValidMapCoord(x, y, z, o)) {
    LOG_DEBUG("entities.unit",
              "Creature::SetPosition({}, {}, {}) .. bad coordinates!", x, y, z);
    return;
  }
...
float Creature::GetAggroRange(Unit const *target) const {
  // Determines the aggro range for creatures (usually pets), used mainly for
  // aggressive pet target selection. Based on data from wowwiki due to lack
  // of 3.3.5a data
...
Unit *Creature::SelectNearestHostileUnitInAggroRange(bool useLOS) const {
  // Selects nearest hostile target within creature's aggro range. Used
  // primarily by
  ...
  Firelands::UnitSearcher<Firelands::NearestHostileUnitInAggroRangeCheck>
      searcher(this, target, u_check);
...
void Creature::UpdateMovementFlags(bool initializeDBStates) {
  ...
  // These movement states are static and should only get applied when
  // initializing a creature's entry (when spawning or transforming)
  if (initializeDBStates) {
    ...
    if (GetMovementTemplate().IsGroundAllowed()) {
      // Hovering always requires the creature to be alive to avoid visual
      // issues. The IsAlive() check can be removed when respawn compatability
      // has been removed.
      // @todo: sniffed spawn locations have applied hover offsets to the
      // position z value so either substract them when parsing sniffs or skip
      // the initial relocation from Unit::SetHover
      if (IsAlive() && GetMovementTemplate().IsHoverEnabled())
        SetHover(true);
    }
  } else {
    // These are realtime movement flag updates which are being updated per tick
    // Set the movement flags if the creature is in that mode. (Only fly if
    // actually in air, only swim if in water, etc)
    float ground = GetFloorZ();
    bool isInAir =
        (G3D::fuzzyGt(
             GetPositionZ(),
             ground +
                 (IsHovering() ? GetFloatValue(UNIT_FIELD_HOVERHEIGHT) : 0.0f) +
...
void Creature::SetObjectScale(float scale) {
  ...
  if (CreatureModelInfo const *minfo =
    ...
    SetFloatValue(
        ...
        (IsPet() ? DEFAULT_PLAYER_COMBAT_REACH : minfo->combat_reach) * scale);
  }
}
...
void Creature::SetDisplayId(uint32 modelId) {
  ...
  if (CreatureModelInfo const *minfo =
    ...
    SetFloatValue(UNIT_FIELD_COMBATREACH, (IsPet() ? DEFAULT_PLAYER_COMBAT_REACH
                                                   : minfo->combat_reach) *
                                              GetObjectScale());
  }
}
...
void Creature::SetSpellFocus(Spell const *focusSpell,
  ...
  if (!IsAlive() || HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_FEIGN_DEATH) ||
      HasAuraType(SPELL_AURA_FEIGN_DEATH))
    return;
  ...
  // Store original orientation and target values to restore them when releasing
  // the spell focus
  if (!_spellFocusInfo
           .ReacquiringTargetDelay) { // only overwrite these fields if we
                                      // aren't transitioning from one spell
                                      // focus to another
  ...
  // Focusing behaves different based on the given spell attributes. Tracking
  // channel spells only face the target's direction and stay there. Normal
  // spell casts fixate on their target instead
  bool const noTurnDuringCast =
      spellInfo->HasAttribute(SPELL_ATTR5_DONT_TURN_DURING_CAST);
  bool const turnDisabled =
      HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_DISABLE_TURN);

  ObjectGuid newTarget = [&]() {
    // Spells that do not allow turning or channeled spells that will track
    // their channel target do not set a unit field target
    if (turnDisabled || noTurnDuringCast ||
        spellInfo->HasAttribute(SPELL_ATTR1_CHANNEL_TRACK_TARGET) ||
        target == this)
  ...
  if (GetTarget() != newTarget) {
    ...
    // Sniffs indicate that UNIT_FIELD_TARGET is being updated outside of the
    // regular update object interval so we squeeze the packet out as well
    std::vector<Player *> playersNearby;
    GetPlayerListInGrid(playersNearby, GetVisibilityRange());
    for (Player *player : playersNearby) {
      // only update players that are known to the client (have already been
      // created)
  ...
  // Spells that do not face via UNIT_FIELD_TARGET and can turn, will face their
  // target one last time
  if (newTarget.IsEmpty() && !turnDisabled && target != this) {
    SetFacingTo(GetAngle(target));
    // We to manually update the orientation as the spline does so on the next
    // update tick while we need a updated position now for further spell target
    // selection
...
bool Creature::HasSpellFocus(Spell const *focusSpell) const {
  if (isDead()) // dead creatures cannot focus
  {
    if (_spellFocusInfo.FocusSpell || _spellFocusInfo.ReacquiringTargetDelay) {
      LOG_WARN("entities.unit",
               "Creature '{}' (entry {}) has spell focus (spell id {}, delay "
               "{}ms) despite being dead.",
               GetName().c_str(), GetEntry(),
               _spellFocusInfo.FocusSpell
                   ? _spellFocusInfo.FocusSpell->GetSpellInfo()->Id
                   : 0,
               _spellFocusInfo.ReacquiringTargetDelay);
    }
    return false;
  }
  ...
  if (focusSpell)
    ...
    return (_spellFocusInfo.FocusSpell ||
            _spellFocusInfo.ReacquiringTargetDelay);
}
...
void Creature::ReleaseSpellFocus(Spell const *focusSpell, bool withDelay) {
  ...
  if (_spellFocusInfo.FocusSpell->GetSpellInfo()->HasAttribute(
...
void Creature::ReacquireSpellFocusTarget() {
  if (!HasSpellFocus()) {
    LOG_ERROR("entities.unit", "Creature::ReacquireSpellFocusTarget() being "
                               "alled with HasSpellFocus() return false.");
    return;
  }
  ...
  if (!HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_DISABLE_TURN)) {
    if (_spellFocusInfo.OriginalUnitTarget) {
      if (WorldObject const *objTarget = ObjectAccessor::GetWorldObject(
              *this, _spellFocusInfo.OriginalUnitTarget))
...
void Creature::StartPickPocketRefillTimer() {
  _pickpocketLootRestore =
      ...
      sWorld->getIntConfig(CONFIG_CREATURE_PICKPOCKET_REFILL);
}
...
void Creature::SetTextRepeatId(uint8 textGroup, uint8 id) {
  ...
  if (std::find(repeats.begin(), repeats.end(), id) == repeats.end())
    ...
    LOG_ERROR("sql.sql",
              "CreatureTextMgr: TextGroup {} for Creature({}) GuidLow {} Entry "
              "{}, id {} already added",
              uint32(textGroup), GetName().c_str(), GetGUID().GetCounter(),
              GetEntry(), uint32(id));
}
...
bool Creature::CanGiveExperience() const {
  return !IsCritter() && !IsPet() && !IsTotem() &&
         !(GetCreatureTemplate()->flags_extra &
...
void Creature::AtEngage(Unit *target) {
  ...
  if (!(GetCreatureTemplate()->type_flags &
        CREATURE_TYPE_FLAG_MOUNTED_COMBAT_ALLOWED))
    Dismount();

  RefreshSwimmingFlag();

  MovementGeneratorType const movetype =
      GetMotionMaster()->GetCurrentMovementGeneratorType();
  if (movetype == WAYPOINT_MOTION_TYPE ||
      movetype == CYCLIC_SPLINE_MOTION_TYPE || movetype == POINT_MOTION_TYPE ||
      (IsAIEnabled() && AI()->IsEscorted()))
