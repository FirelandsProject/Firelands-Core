...
void Creature::Update(uint32 diff)
{
    ...
    switch (m_deathState)
    {
        case JUST_RESPAWNED:
            ...
            LOG_ERROR("entities.unit", "Creature (GUID: %u Entry: %u) in wrong state: JUST_RESPAWNED (4)", GetGUIDLow(), GetEntry());
            break;
        case JUST_DIED:
            // Must not be called, see Creature::setDeathState JUST_DIED -> CORPSE promoting.
            LOG_ERROR("entities.unit", "Creature (GUID: %u Entry: %u) in wrong state: JUST_DEAD (1)", GetGUIDLow(), GetEntry());
            break;
        case DEAD:
        {
...
void Creature::DoFleeToGetAssistance()
{
    ...
    if (radius >0)
    {
        ...
        TypeContainerVisitor<Firelands::CreatureLastSearcher<Firelands::NearestAssistCreatureInCreatureRangeCheck>, GridTypeMapContainer > grid_creature_searcher(searcher);
...
bool Creature::isCanTrainingOf(Player* player, bool msg) const
{
    ...
    {
        LOG_ERROR("sql.sql", "Creature %u (Entry: %u) have UNIT_NPC_FLAG_TRAINER but have empty trainer spell list.",
            GetGUIDLow(), GetEntry());
        return false;
    }
...
void Creature::SaveToDB(uint32 mapid, uint8 spawnMask, uint32 phaseMask)
{
    ...
    stmt->SetData(index++, int32(GetEquipmentId()));
...
void Creature::DeleteFromDB()
{
    if (!m_DBTableGuid)
    {
        LOG_ERROR("entities.unit", "Trying to delete not saved creature! LowGUID: %u, Entry: %u", GetGUIDLow(), GetEntry());
        return;
    }
...
void Creature::Respawn(bool force)
{
    ...
    if (getDeathState() == DEAD)
    {
        ...
        LOG_DEBUG("entities.unit", "Respawning creature %s (GuidLow: %u, Full GUID: " UI64FMTD " Entry: %u)",
...
Unit* Creature::SelectNearestTarget(float dist) const
{
...

    {
        ...
        Firelands::NearestHostileUnitCheck u_check(this, dist);
        Firelands::UnitLastSearcher<Firelands::NearestHostileUnitCheck> searcher(this, target, u_check);

        TypeContainerVisitor<Firelands::UnitLastSearcher<Firelands::NearestHostileUnitCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
        TypeContainerVisitor<Firelands::UnitLastSearcher<Firelands::NearestHostileUnitCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
Unit* Creature::SelectNearestTargetInAttackDistance(float dist) const
{
    ...
    {
        LOG_ERROR("entities.unit", "Creature (GUID: %u Entry: %u) SelectNearestTargetInAttackDistance called with dist > MAX_VISIBILITY_DISTANCE. Distance set to ATTACK_DISTANCE.", GetGUIDLow(), GetEntry());
...

    {
        ...
        TypeContainerVisitor<Firelands::UnitLastSearcher<Firelands::NearestHostileUnitInAttackDistanceCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
        TypeContainerVisitor<Firelands::UnitLastSearcher<Firelands::NearestHostileUnitInAttackDistanceCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
Player* Creature::SelectNearestPlayer(float distance, int32 aura) const
{
    ...
    Firelands::NearestPlayerInObjectRangeCheck checker(this, distance, aura);
...
void Creature::SendAIReaction(AiReaction reactionType)
{
    ...
    LOG_DEBUG("network.opcode", "WORLD: Sent SMSG_AI_REACTION, type %u.", reactionType);
}
    ...
    if (!m_AlreadyCallAssistance && getVictim() && !isPet() && !isCharmed())
    {
        ...
        if (radius > 0)
        {
...

            {
                ...
                TypeContainerVisitor<Firelands::CreatureListSearcher<Firelands::AnyAssistCreatureInRangeCheck>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
void Creature::CallForHelp(float radius)
{
    ...
    Firelands::CallOfHelpCreatureInRangeDo u_do(this, getVictim(), radius);
    Firelands::CreatureWorker<Firelands::CallOfHelpCreatureInRangeDo> worker(this, u_do);

    TypeContainerVisitor<Firelands::CreatureWorker<Firelands::CallOfHelpCreatureInRangeDo>, GridTypeMapContainer >  grid_creature_searcher(worker);
...
bool Creature::LoadCreaturesAddon(bool reload)
{
    ...
    if (!cainfo->auras.empty())
    {
        for (std::vector<uint32>::const_iterator itr = cainfo->auras.begin(); itr != cainfo->auras.end(); ++itr)
        {
            ...
            {
                if (!reload)
                    LOG_ERROR("sql.sql", "Creature (GUID: %u Entry: %u) has duplicate aura (spell %u) in `auras` field.", GetDBTableGUIDLow(), GetEntry(), *itr);
