...
uint32 GossipMenu::AddMenuItem(int32 optionIndex, uint8 icon,
...
void GossipMenu::AddMenuItem(uint32 menuId, uint32 menuItemId, uint32 sender,
  ...
  for (GossipMenuItemsContainer::const_iterator itr = bounds.first;
    ...
    if (GetLocale() != DEFAULT_LOCALE) {
      if (!optionBroadcastText) {
        ...
        if (GossipMenuItemsLocale const *gossipMenuLocale =
                sObjectMgr->GetGossipMenuItemsLocale(menuId, menuItemId))
    ...
    AddGossipMenuItemData(optionIndex, itr->second.ActionMenuID,
                          itr->second.ActionPoiID);
  }
}

void GossipMenu::AddGossipMenuItemData(uint32 optionIndex,
...
void PlayerMenu::SendGossipMenu(uint32 titleTextId, ObjectGuid objectGUID) {
  ...
  bool questLevelInTitle =
      sWorld->getBoolConfig(CONFIG_UI_QUESTLEVELS_IN_DIALOGS);
  ...
  for (uint8 i = 0; i < _questMenu.GetMenuItemCount(); ++i) {
    ...
    if (Quest const *quest = sObjectMgr->GetQuestTemplate(questID)) {
      ...
      data << uint8(
          quest->IsAutoComplete() && quest->IsRepeatable() &&
          !quest->IsDailyOrWeekly() &&
          !quest->IsMonthly()); // 3.3.3 icon changes - 0: yellow exclapamtion
                                // mark, 1: blue question mark
...
void PlayerMenu::SendPointOfInterest(uint32 id) const {
  ...
  if (!poi) {
    LOG_ERROR("sql.sql", "Request to send non-existing POI (Id: {}), ignored.",
              id);
    return;
  }
  ...
  if (localeConstant != LOCALE_enUS)
    if (PointOfInterestLocale const *localeData =
            sObjectMgr->GetPointOfInterestLocale(id))
...
void PlayerMenu::SendQuestGiverQuestList(QEmote const &eEmote,
  ...
  if (QuestGreeting const *questGreeting = sObjectMgr->GetQuestGreeting(guid)) {
    ...
    if (localeConstant != LOCALE_enUS)
      if (QuestGreetingLocale const *questGreetingLocale =
              sObjectMgr->GetQuestGreetingLocale(
                  MAKE_PAIR32(guid.GetEntry(), guid.GetTypeId())))
        ObjectMgr::GetLocaleString(questGreetingLocale->greeting,
                                   localeConstant, strGreeting);
  ...
  for (uint32 i = 0; i < _questMenu.GetMenuItemCount(); ++i) {
    ...
    if (Quest const *quest = sObjectMgr->GetQuestTemplate(questID)) {
      ...
      LocaleConstant localeConstant = _session->GetSessionDbLocaleIndex();
      if (localeConstant != LOCALE_enUS)
        if (QuestLocale const *questTemplateLocaleData =
                sObjectMgr->GetQuestLocale(questID))
          ObjectMgr::GetLocaleString(questTemplateLocaleData->Title,
                                     localeConstant, title);

      if (questLevelInTitle)
        Quest::AddQuestLevelToTitle(title, quest->GetQuestLevel());

      bool repeatable = quest->IsAutoComplete() && quest->IsRepeatable() &&
                        !quest->IsDailyOrWeekly() && !quest->IsMonthly();
      questList.QuestDataText.emplace_back(
          questID, questMenuItem.QuestIcon, quest->GetQuestLevel(),
          quest->GetFlags(), repeatable, title);
    }
  }
  ...
  LOG_DEBUG(
      "network",
      "WORLD: Sent SMSG_QUEST_GIVER_QUEST_LIST_MESSAGE (QuestGiverGUID: {})",
      guid.ToString().c_str());
}
...
void PlayerMenu::SendQuestGiverStatus(uint32 questStatus,
  ...
  LOG_DEBUG("network",
            "WORLD: Sent SMSG_QUEST_GIVER_STATUS Guid = {}, Status = {}",
            npcGUID.ToString().c_str(), questStatus);
}

void PlayerMenu::SendQuestGiverQuestDetails(Quest const *quest,
                                            ObjectGuid npcGUID,
                                            bool autoLaunched,
                                            bool displayPopup) const {
  ...
  if (localeConstant != LOCALE_enUS) {
    if (QuestLocale const *localeData =
      ...
      ObjectMgr::GetLocaleString(localeData->Title, localeConstant,
                                 packet.QuestTitle);
      ObjectMgr::GetLocaleString(localeData->Details, localeConstant,
                                 packet.LogDescription);
      ObjectMgr::GetLocaleString(localeData->Objectives, localeConstant,
                                 packet.Objectives);
      ObjectMgr::GetLocaleString(localeData->AreaDescription, localeConstant,
                                 packet.DescriptionText);
      ObjectMgr::GetLocaleString(localeData->QuestGiverTextWindow,
                                 localeConstant, packet.PortraitGiverText);
      ObjectMgr::GetLocaleString(localeData->QuestGiverTargetName,
                                 localeConstant, packet.PortraitGiverName);
      ObjectMgr::GetLocaleString(localeData->QuestTurnTextWindow,
                                 localeConstant, packet.PortraitTurnInText);
      ObjectMgr::GetLocaleString(localeData->QuestTurnTargetName,
                                 localeConstant, packet.PortraitTurnInName);
    }
  }
  ...
  LOG_DEBUG("network",
            "WORLD: Sent SMSG_QUESTGIVER_QUEST_DETAILS NPC={}, questid={}",
            npcGUID.ToString().c_str(), quest->GetQuestId());
}

void PlayerMenu::SendQuestQueryResponse(Quest const *quest) const {
  if (sWorld->getBoolConfig(CONFIG_CACHE_DATA_QUERIES))
    _session->SendPacket(&quest->QueryData[static_cast<uint32>(
        _session->GetSessionDbLocaleIndex())]);
  else {
    WorldPacket queryPacket =
        quest->BuildQueryData(_session->GetSessionDbLocaleIndex());
    _session->SendPacket(&queryPacket);
  }

  LOG_DEBUG("network", "WORLD: Sent SMSG_QUEST_QUERY_RESPONSE questid={}",
            quest->GetQuestId());
}

void PlayerMenu::SendQuestGiverOfferReward(Quest const *quest,
                                           ObjectGuid npcGUID,
                                           bool autoLaunched) const {
  ...
  if (localeConstant != LOCALE_enUS) {
    if (QuestLocale const *localeData =
      ...
      ObjectMgr::GetLocaleString(localeData->Title, localeConstant,
                                 packet.QuestTitle);
      ObjectMgr::GetLocaleString(localeData->OfferRewardText, localeConstant,
                                 packet.RewardText);
      ObjectMgr::GetLocaleString(localeData->QuestGiverTextWindow,
                                 localeConstant, packet.PortraitGiverText);
      ObjectMgr::GetLocaleString(localeData->QuestGiverTargetName,
  ...
  for (uint32 i = 0; i < QUEST_EMOTE_COUNT && quest->OfferRewardEmote[i]; ++i)
    offer.Emotes.emplace_back(quest->OfferRewardEmote[i],
                              quest->OfferRewardEmoteDelay[i]);
...
void PlayerMenu::SendQuestGiverRequestItems(Quest const *quest,
  ...
  if (localeConstant != LOCALE_enUS) {
    if (QuestLocale const *localeData =
      ...
      ObjectMgr::GetLocaleString(localeData->RequestItemsText, localeConstant,
                                 requestItemsText);
    }
  }
  ...
  packet.QuestID = quest->GetQuestId();
  packet.QuestGiverGUID = npcGUID;
  packet.CompEmoteType =
      canComplete ? quest->GetCompleteEmote() : quest->GetIncompleteEmote();
  packet.AutoLaunched = autoLaunched;
  packet.QuestFlags = quest->GetFlags();
  packet.SuggestPartyMembers = quest->GetSuggestedPlayers();
  packet.MoneyToGet =
      quest->GetRewOrReqMoney() < 0 ? -quest->GetRewOrReqMoney() : 0;
  ...
  for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; ++i) {
    ...
    if (data.ItemCount[i] < quest->RequiredItemCount[i] &&
        !_session->GetPlayer()->HasItemCount(quest->RequiredItemId[i],
                                             quest->RequiredItemCount[i]))
      collectObjectiveComplete = false;

    packet.Collect.emplace_back(quest->RequiredItemId[i],
                                quest->RequiredItemCount[i], displayId);
  }
  ...
  for (uint8 i = 0; i < QUEST_REQUIRED_CURRENCY_COUNT; ++i) {
    ...
    packet.Currency.emplace_back(quest->RequiredCurrencyId[i],
  ...
  packet.StatusFlags[0] = killCreditObjectiveComplete
                              ? QUEST_STATUS_FLAG_KILL_OBJECTIVE_COMPLETE
                              : QUEST_STATUS_FLAG_NONE;
  packet.StatusFlags[1] = collectObjectiveComplete
                              ? QUEST_STATUS_FLAG_COLLECT_OBJECTIVE_COMPLETE
                              : QUEST_STATUS_FLAG_NONE;
  ...
  _session->SendPacket(packet.Write());
  LOG_DEBUG("network",
            "WORLD: Sent SMSG_QUEST_GIVER_REQUEST_ITEMS NPC={}, questid={}",
            npcGUID.ToString().c_str(), quest->GetQuestId());
}
