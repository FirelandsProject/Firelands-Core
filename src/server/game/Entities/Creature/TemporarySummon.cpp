  ...
  if (owner) {
    ...
    if (m_Properties->Control != SUMMON_CATEGORY_WILD) {
      ...
      if (owner && (owner->IsPlayer() || owner->IsTotem())) {
        ...
        if (guildGUID) {
          SetGuidValue(OBJECT_FIELD_DATA,
                       owner->GetGuidValue(OBJECT_FIELD_DATA));
...
void Minion::InitStats(uint32 duration) {
  ...
  if ((IsMinion() || IsControlableGuardian()) && !IsTotem() && !IsVehicle())
    ...
    // Store the totem elementals in players controlled list as well to trigger
    // aggro mechanics
    if (GetOwner()->IsTotem())
  ...
  if (m_Properties && m_Properties->Slot == SUMMON_SLOT_MINIPET) {
    SelectLevel(); // some summoned creaters have different from 1 DB data for
                   // level/hp
  ...
  if ((IsMinion() || IsControlableGuardian()) && !IsTotem() && !IsVehicle())
    ...
    if (owner->m_Controlled.find(this) != owner->m_Controlled.end())
      ...
      LOG_FATAL("entities.unit",
                "Minion::RemoveFromWorld: Owner {} tried to remove a "
                "non-existing controlled unit {} from controlled unit set.",
                owner->GetGUID().ToString().c_str(),
                GetGUID().ToString().c_str());

    if (owner->IsTotem())
      if (Unit *totemOwner = owner->GetOwner())
        if (totemOwner->m_Controlled.find(this) !=
            totemOwner->m_Controlled.end())
...
bool Minion::IsGuardianPet() const {
  return IsPet() ||
         (m_Properties && m_Properties->Control == SUMMON_CATEGORY_PET);
}
...
Guardian::Guardian(SummonPropertiesEntry const *properties, Unit *owner,
                   bool isWorldObject)
    : Minion(properties, owner, isWorldObject), m_bonusSpellDamage(0) {
