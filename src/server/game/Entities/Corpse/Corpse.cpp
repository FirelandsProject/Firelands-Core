...
void Corpse::SaveToDB() {
  ...
  stmt->SetData(index++, _ConcatFields(CORPSE_FIELD_ITEM,
                                       EQUIPMENT_SLOT_END));    // itemCache
  ...
  stmt->SetData(index++,
                GetUInt32Value(CORPSE_FIELD_DYNAMIC_FLAGS)); // dynFlags
...
void Corpse::ResetGhostTime() { m_time = GameTime::GetGameTime(); }
...
bool Corpse::LoadCorpseFromDB(ObjectGuid::LowType guid, Field *fields) {
  ...
  // bytes2, flags, dynFlags, time, corpseType, instanceId, guid FROM corpse
  // WHERE mapId = ? AND instanceId = ?
  ...
  _LoadIntoDataField(fields[6].Get<std::string>(), CORPSE_FIELD_ITEM,
                     EQUIPMENT_SLOT_END);
  ...
  if (!IsPositionValid()) {
    LOG_ERROR("entities.player",
              "Corpse ({}, owner: {}) is not created, given coordinates are "
              "not valid (X: {}, Y: {}, Z: {})",
              GetGUID().ToString().c_str(), GetOwnerGUID().ToString().c_str(),
              posX, posY, posZ);
    return false;
  }
