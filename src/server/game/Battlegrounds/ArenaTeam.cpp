...
ArenaTeam::ArenaTeam()
    : TeamId(0), Type(0), TeamName(), CaptainGuid(), BackgroundColor(0),
...
bool ArenaTeam::Create(ObjectGuid captainGuid, uint8 type,
  ...
  LOG_DEBUG("bg.arena",
            "New ArenaTeam created [Id: {}, Name: {}] [Type: {}] [Captain low "
            "GUID: {}]",
            GetId(), GetName().c_str(), GetType(), captainLowGuid);
  return true;
}
...
bool ArenaTeam::AddMember(ObjectGuid playerGuid) {
  ...
  if (player) {
    ...
    CharacterCacheEntry const *cInfo =
  ...
  if ((player && player->GetArenaTeamId(GetSlot())) ||
      sCharacterCache->GetCharacterArenaTeamIdByGuid(playerGuid, GetType()) !=
          0) {
    LOG_DEBUG("bg.arena", "Arena: {} {} already has an arena team of type {}",
              playerGuid.ToString().c_str(), playerName.c_str(), GetType());
    return false;
  }
  ...
  Player::RemovePetitionsAndSigns(playerGuid,
                                  static_cast<CharterTypes>(GetType()));
  ...
  LOG_DEBUG("bg.arena",
            "Player: {} [{}] joined arena team type: {} [Id: {}, Name: {}].",
            playerName.c_str(), playerGuid.ToString().c_str(), GetType(),
            GetId(), GetName().c_str());
...
bool ArenaTeam::LoadMembersFromDB(QueryResult result) {
  ...
  do {
    ...
    if (newMember.Name.empty()) {
      LOG_ERROR("sql.sql",
                "ArenaTeam {} has member with empty name - probably {} doesn't "
                "exist, deleting him from memberlist!",
                arenaTeamId, newMember.Guid.ToString().c_str());
    ...
    Members.push_back(newMember);
    sCharacterCache->UpdateCharacterArenaTeamId(newMember.Guid, GetSlot(),
                                                GetId());
  ...
  if (Empty() || !captainPresentInTeam) {
    ...
    LOG_DEBUG("bg.arena",
              "ArenaTeam {} does not have any members or its captain is not in "
              "team, disbanding it...",
              TeamId);
    return false;
  }
...
void ArenaTeam::SetCaptain(ObjectGuid guid) {
  ...
  if (Player *newCaptain = ObjectAccessor::FindPlayer(guid)) {
    ...
    if (oldCaptain) {
      LOG_DEBUG(
          "bg.arena",
          "Player: {} [GUID: {}] promoted player: {} [GUID: {}] to leader of "
          "arena team [Id: {}, Name: {}] [Type: {}].",
          oldCaptain->GetName().c_str(), oldCaptain->GetGUID().GetCounter(),
          newCaptain->GetName().c_str(), newCaptain->GetGUID().GetCounter(),
          GetId(), GetName().c_str(), GetType());
    }
  }
}
...
void ArenaTeam::DelMember(ObjectGuid guid, bool cleanDb) {
  ...
  if (player) {
    ...
    LOG_DEBUG(
        "bg.arena",
        "Player: {} [GUID: {}] left arena team type: {} [Id: {}, Name: {}].",
        player->GetName().c_str(), player->GetGUID().GetCounter(), GetType(),
        GetId(), GetName().c_str());
  }
...
void ArenaTeam::Disband(WorldSession *session) {
  // Broadcast update
  if (session) {
    BroadcastEvent(ERR_ARENA_TEAM_DISBANDED_S, ObjectGuid::Empty, 2,
                   session->GetPlayerName(), GetName(), "");
    if (Player *player = session->GetPlayer())
      LOG_DEBUG("bg.arena",
                "Player: {} [GUID: {}] disbanded arena team type: {} [Id: {}, "
                "Name: {}].",
                player->GetName().c_str(), player->GetGUID().GetCounter(),
                GetType(), GetId(), GetName().c_str());
  }
...
void ArenaTeam::BroadcastEvent(ArenaTeamEvents event, ObjectGuid guid,
  ...
  case 3:
    ...
    break;
  default:
    LOG_ERROR("bg.arena", "Unhandled strCount {} in ArenaTeam::BroadcastEvent",
              strCount);
    return;
  }
...
uint8 ArenaTeam::GetSlotByType(uint32 type) {
  ...
  LOG_ERROR("bg.arena", "FATAL: Unknown arena team type {} for some arena team",
            type);
  return 0xFF;
}
...
uint8 ArenaTeam::GetTypeBySlot(uint8 slot) {
  ...
  LOG_ERROR("bg.arena", "FATAL: Unknown arena team slot {} for some arena team",
            slot);
  return 0xFF;
}
...
float ArenaTeam::GetChanceAgainst(uint32 ownRating, uint32 opponentRating) {
  ...
  return 1.0f /
         (1.0f + std::exp(std::log(10.0f) *
                          (float(opponentRating) - float(ownRating)) / 650.0f));
}
...
int32 ArenaTeam::WonAgainst(uint32 ownMMRating, uint32 opponentMMRating,
                            int32 &ratingChange) {
...
int32 ArenaTeam::LostAgainst(uint32 ownMMRating, uint32 opponentMMRating,
                             int32 &ratingChange) {
