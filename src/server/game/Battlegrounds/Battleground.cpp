...
void Battleground::Update(uint32 diff) {
  ...
  if (!GetPlayersSize()) {
    ...
    //  [[   but if you use battleground object again (more battles possible to
    //  be played on 1 instance)
    //       then this condition should be removed and code:
    //       if (!GetInvitedCount(HORDE) && !GetInvitedCount(ALLIANCE))
    //           this->AddToFreeBGObjectsQueue(); // not yet implemented
    //       should be used instead of current
    //  ]]
    //  Battleground Template instance cannot be updated, because it would be
    //  deleted
    if (!GetInvitedCount(HORDE) && !GetInvitedCount(ALLIANCE))
  ...
  case STATUS_IN_PROGRESS:
    ...
    // after 47 minutes without one team losing, the arena closes with no winner
    ...
    for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin();
      ...
      if (Player *player = ObjectAccessor::FindPlayer(itr->first)) {
        ...
        Position const *startPos = GetTeamStartPosition(
            Battleground::GetTeamIndexByTeamId(player->GetBGTeam()));
        if (pos.GetExactDistSq(startPos) > maxDist) {
          LOG_DEBUG("bg.battleground",
                    "BATTLEGROUND: Sending {} back to start location (map: {}) "
                    "(possible exploit)",
                    player->GetName().c_str(), GetMapId());
          player->TeleportTo(GetMapId(), startPos->GetPositionX(),
...
inline void Battleground::_ProcessOfflineQueue() {
  ...
  if (!m_OfflineQueue.empty()) {
    BattlegroundPlayerMap::iterator itr =
        m_Players.find(*(m_OfflineQueue.begin()));
  ...
  if (m_LastResurrectTime >= RESURRECTION_INTERVAL) {
    if (GetReviveQueueSize()) {
      for (std::map<ObjectGuid, GuidVector>::iterator itr =
        ...
        Creature *sh = nullptr;
        for (GuidVector::const_iterator itr2 = (itr->second).begin();
      ...
      m_LastResurrectTime = 0;
             ...
             500) // Resurrect players only half a second later, to see spirit
                  // heal effect on NPC
  {
    for (GuidVector::const_iterator itr = m_ResurrectQueue.begin();
...
inline void Battleground::_ProcessJoin(uint32 diff) {
  ...
  if (m_CountdownTimer >= 10000) {
    ...
    packet.Type = 0;
    packet.TimeLeft =
        countdownMaxForBGType - (GetElapsedTime() / IN_MILLISECONDS);
  ...
  if (!(m_Events & BG_STARTING_EVENT_1)) {
    ...
    if (!FindBgMap()) {
      LOG_ERROR("bg.battleground",
                "Battleground::_ProcessJoin: map (map id: {}, instance id: {}) "
                "is not created!",
  ...
  // Delay expired (after 2 or 1 minute)
    ...
    if (isArena()) {
      ...
      for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin();
        ...
        if (Player *player = ObjectAccessor::FindPlayer(itr->first)) {
          ...
          BattlegroundQueueTypeId bgQueueTypeId =
              sBattlegroundMgr->BGQueueTypeId(m_TypeID, GetArenaType());
          uint32 queueSlot = player->GetBattlegroundQueueIndex(bgQueueTypeId);

          sBattlegroundMgr->BuildBattlegroundStatusPacket(
              &status, this, player, queueSlot, STATUS_IN_PROGRESS,
              player->GetBattlegroundQueueJoinTime(m_TypeID), GetElapsedTime(),
              GetArenaType());
      ...
      if (sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_ENABLE))
        sWorld->SendWorldText(LANG_BG_STARTED_ANNOUNCE_WORLD, GetName().c_str(),
...
Player *Battleground::_GetPlayer(ObjectGuid guid, bool offlineRemove,
                                 char const *context) const {
  Player *player = nullptr;
  if (!offlineRemove) {
    // should this be ObjectAccessor::FindConnectedPlayer() to return players
    // teleporting ?
    player = ObjectAccessor::FindPlayer(guid);
    if (!player)
      LOG_ERROR("bg.battleground",
                "Battleground::{}: player ({}) not found for BG (map: {}, "
                "instance id: {})!",
...
Player *
Battleground::_GetPlayerForTeam(uint32 teamId,
...
void Battleground::SendChatMessage(Creature *source, uint8 textId,
                                   WorldObject *target /*= nullptr*/) {
  sCreatureTextMgr->SendChat(source, textId, target);
}

void Battleground::SendBroadcastText(uint32 id, ChatMsg msgType,
                                     WorldObject const *target) {
  if (!sObjectMgr->GetBroadcastText(id)) {
    LOG_ERROR("bg.battleground",
              "Battleground::SendBroadcastText: `broadcast_text` (ID: {}) was "
              "not found",
              id);
    return;
  }

  Firelands::BroadcastTextBuilder builder(nullptr, msgType, id, GENDER_MALE,
                                          target);
  Firelands::LocalizedPacketDo<Firelands::BroadcastTextBuilder> localizer(
      builder);
...
void Battleground::PlaySoundToAll(uint32 soundID) {
  SendPacketToAll(
      WorldPackets::Misc::PlaySound(ObjectGuid::Empty, soundID).Write());
}
...
void Battleground::RewardReputationToTeam(uint32 faction_id, uint32 Reputation,
                                          uint32 TeamID) {
  ...
  for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
    ...
    AddPct(repGain,
           player->GetTotalAuraModifier(SPELL_AURA_MOD_REPUTATION_GAIN));
    AddPct(repGain, player->GetTotalAuraModifierByMiscValue(
                        SPELL_AURA_MOD_FACTION_REPUTATION_GAIN, faction_id));
...
void Battleground::EndBattleground(uint32 winner) {
  ...
  for (BattlegroundPlayerMap::iterator itr = m_Players.begin();
    ...
    } else // needed cause else in av some creatures will kill the players at
           // the end
      player->CombatStop();
      ...
      if (IsRandom() || BattlegroundMgr::IsBGWeekend(GetTypeID())) {
        ...
        if (!player->GetRandomWinner()) {
          ...
          player->ModifyCurrency(
              ...
              sWorld->getIntConfig(CONFIG_BG_REWARD_WINNER_CONQUEST_FIRST));
          player->SetRandomWinner(true);
        }
      } else // 50cp awarded for each non-rated battleground won
        player->ModifyCurrency(
            CURRENCY_TYPE_CONQUEST_META_ARENA,
            sWorld->getIntConfig(CONFIG_BG_REWARD_WINNER_CONQUEST_LAST));
      ...
      if (!guildAwarded) {
        ...
        if (uint32 guildId = GetBgMap()->GetOwnerGuildId(player->GetBGTeam())) {
          if (Guild *guild = sGuildMgr->GetGuildById(guildId))
            guild->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_WIN_BG,
                                             player->GetMapId(), 0, 0, nullptr,
                                             player);
        }
      }
    } else {
    ...
    WorldPacket data;
    sBattlegroundMgr->BuildBattlegroundStatusPacket(
        &data, this, player, player->GetBattlegroundQueueIndex(bgQueueTypeId),
        STATUS_IN_PROGRESS, player->GetBattlegroundQueueJoinTime(GetTypeID()),
        GetElapsedTime(), GetArenaType());
    player->SendDirectMessage(&data);
...
void Battleground::RemovePlayerAtLeave(ObjectGuid guid, bool Transport,
  ...
  if (participant) // if the player was a match participant, remove auras, calc
                   // rating, update queue
  {
    if (player) {
      ...
      if (isArena()) {
        bgTypeId = BATTLEGROUND_AA; // set the bg type to all arenas (it will be
                                    // used for queue refreshing)

        // unsummon current and summon old pet if there was one and there isn't
        // a current pet
      ...
      if (SendPacket) {
        ...
        sBattlegroundMgr->BuildBattlegroundStatusPacket(
            &data, this, player,
            player->GetBattlegroundQueueIndex(bgQueueTypeId), STATUS_NONE,
            player->GetBattlegroundQueueJoinTime(bgTypeId), 0, m_ArenaType);
        player->SendDirectMessage(&data);
      }
    ...
    if (isBattleground() && GetStatus() < STATUS_WAIT_LEAVE) {
      // a player has left the battleground, so there are free slots -> add to
      // queue
  ...
  if (player) {
    ...
    LOG_DEBUG("bg.battleground", "Removed player {} from Battleground.",
...
void Battleground::StartBattleground() {
  ...
  // players when first BG::Update() method is executed and it doesn't matter if
  // we call StartBattleground() more times, because m_Battlegrounds is a map
  // and instance id never changes
  sBattlegroundMgr->AddBattleground(this);

  if (m_IsRated)
    LOG_DEBUG("bg.arena",
              "Arena match type: {} for Team1Id: {} - Team2Id: {} started.",
              m_ArenaType, m_ArenaTeamIds[TEAM_ALLIANCE],
...
void Battleground::AddPlayer(Player *player) {
  ...
  WorldPacket data;
  sBattlegroundMgr->BuildPlayerJoinedBattlegroundPacket(&data,
                                                        player->GetGUID());
  SendPacketToTeam(team, &data, player, false);
  ...
  sBattlegroundMgr->BuildBattlegroundStatusPacket(
      &data, this, player, queueSlot, STATUS_IN_PROGRESS,
      player->GetBattlegroundQueueJoinTime(m_TypeID), GetElapsedTime(),
      GetArenaType());
  player->SendDirectMessage(&data);
...
void Battleground::EventPlayerLoggedIn(Player *player) {
  ...
  for (GuidDeque::iterator itr = m_OfflineQueue.begin();
       itr != m_OfflineQueue.end(); ++itr) {
    if (*itr == guid) {
...
void Battleground::EventPlayerLoggedOut(Player *player) {
  ObjectGuid guid = player->GetGUID();
  if (!IsPlayerInBattleground(
          guid)) // Check if this player really is in battleground (might be a
                 // GM who teleported inside)
    return;

  // player is correct pointer, it is checked in WorldSession::LogoutPlayer()
  m_OfflineQueue.push_back(player->GetGUID());
  m_Players[guid].OfflineRemoveTime =
      GameTime::GetGameTime() + MAX_OFFLINE_TIME;
  if (GetStatus() == STATUS_IN_PROGRESS) {
    ...
    if (isArena())
      if (GetAlivePlayersCountByTeam(player->GetBGTeam()) <= 1 &&
          GetPlayersCountByTeam(GetOtherTeam(player->GetBGTeam())))
...
uint32 Battleground::GetFreeSlotsForTeam(uint32 Team) const {
  // if BG is starting and CONFIG_BATTLEGROUND_INVITATION_TYPE ==
  // BG_QUEUE_INVITATION_TYPE_NO_BALANCE, invite anyone
  if (GetStatus() == STATUS_WAIT_JOIN &&
      sWorld->getIntConfig(CONFIG_BATTLEGROUND_INVITATION_TYPE) ==
  ...
  // BG_QUEUE_INVITATION_TYPE_NO_BALANCE, do not allow to join too much players
  // of one faction
  ...
  if (GetStatus() == STATUS_IN_PROGRESS || GetStatus() == STATUS_WAIT_JOIN) {
    ...
    uint32 diff2 = (thisTeamInvitedCount < GetMaxPlayersPerTeam())
                       ? GetMaxPlayersPerTeam() - thisTeamInvitedCount
                       : 0;
...
bool Battleground::UpdatePlayerScore(Player *player, uint32 type, uint32 value,
                                     bool doAddHonor) {
  ...
  if (type == SCORE_BONUS_HONOR && doAddHonor && isBattleground())
    player->RewardHonor(
        nullptr, 1,
        value); // RewardHonor calls UpdatePlayerScore with doAddHonor = false
  else
...
void Battleground::RelocateDeadPlayers(ObjectGuid guideGuid) {
  ...
  if (!ghostList.empty()) {
    ...
    for (GuidVector::const_iterator itr = ghostList.begin();
      ...
      if (closestGrave)
        player->TeleportTo(GetMapId(), closestGrave->Loc.X, closestGrave->Loc.Y,
                           closestGrave->Loc.Z, player->GetOrientation());
    }
    ghostList.clear();
  }
}
...
bool Battleground::AddObject(uint32 type, uint32 entry, float x, float y,
  ...
  // Temporally add safety check for bad spawns and send log (object rotations
  // need to be rechecked in sniff)
  if (!rotation0 && !rotation1 && !rotation2 && !rotation3) {
    LOG_DEBUG("bg.battleground",
              "Battleground::AddObject: gameoobject [entry: {}, object type: "
              "{}] for BG (map: {}) has zeroed rotation fields, "
  ...
  if (!go->Create(GetBgMap()->GenerateLowGuid<HighGuid::GameObject>(), entry,
                  GetBgMap(), Position(x, y, z, o), rot, 255, goState)) {
    LOG_ERROR("bg.battleground",
...
bool Battleground::AddObject(uint32 type, uint32 entry, Position const &pos,
                             float rotation0, float rotation1, float rotation2,
                             float rotation3, uint32 respawnTime /*= 0*/,
                             GOState goState /*= GO_STATE_READY*/) {
  return AddObject(type, entry, pos.GetPositionX(), pos.GetPositionY(),
                   pos.GetPositionZ(), pos.GetOrientation(), rotation0,
  ...
  if (!obj) {
    if (logError)
      LOG_ERROR("bg.battleground",
                ...
                "found for BG (map: {}, instance id: {})!",
                type, BgObjects[type].ToString().c_str(), m_MapId,
                m_InstanceID);
    else
      LOG_INFO("bg.battleground",
               "Battleground::GetBGObject: gameobject (type: {}, {}) not found "
               "for BG (map: {}, instance id: {})!",
               type, BgObjects[type].ToString().c_str(), m_MapId, m_InstanceID);
  }
  return obj;
  ...
  if (!creature) {
    if (logError)
      LOG_ERROR("bg.battleground",
                ...
                "found for BG (map: {}, instance id: {})!",
                type, BgCreatures[type].ToString().c_str(), m_MapId,
                m_InstanceID);
    else
      LOG_INFO("bg.battleground",
               "Battleground::GetBGCreature: creature (type: {}, {}) not found "
               "for BG (map: {}, instance id: {})!",
               type, BgCreatures[type].ToString().c_str(), m_MapId,
               m_InstanceID);
  }
...
Creature *Battleground::AddCreature(uint32 entry, uint32 type, float x, float y,
  ...
  if (transport) {
    if (Creature *creature = transport->SummonPassenger(
  ...
  if (!creature->Create(map->GenerateLowGuid<HighGuid::Unit>(), map, entry,
                        {x, y, z, o})) {
    LOG_ERROR("bg.battleground",
  ...
  if (!cinfo) {
    LOG_ERROR("bg.battleground",
              "Battleground::AddCreature: creature template (entry: {}) does "
              "not exist for BG (map: {}, instance id: {})!",
              entry, m_MapId, m_InstanceID);
    delete creature;
    return nullptr;
  }
...
Creature *Battleground::AddCreature(uint32 entry, uint32 type,
                                    Position const &pos,
                                    TeamId teamId /*= TEAM_NEUTRAL*/,
                                    uint32 respawntime /*= 0*/,
                                    Transport *transport) {
  return AddCreature(entry, type, pos.GetPositionX(), pos.GetPositionY(),
                     pos.GetPositionZ(), pos.GetOrientation(), teamId,
...
bool Battleground::DelCreature(uint32 type) {
  ...
  LOG_ERROR("bg.battleground",
            "Battleground::DelCreature: creature (type: {}, {}) not found for "
            "BG (map: {}, instance id: {})!",
...
bool Battleground::DelObject(uint32 type) {
  ...
  LOG_ERROR("bg.battleground",
            "Battleground::DelObject: gameobject (type: {}, {}) not found for "
            "BG (map: {}, instance id: {})!",
            type, BgObjects[type].ToString().c_str(), m_MapId, m_InstanceID);
  BgObjects[type].Clear();
  return false;
...
bool Battleground::AddSpiritGuide(uint32 type, float x, float y, float z,
                                  float o, TeamId teamId /*= TEAM_NEUTRAL*/) {
...
bool Battleground::AddSpiritGuide(uint32 type, Position const &pos,
                                  TeamId teamId /*= TEAM_NEUTRAL*/) {
  return AddSpiritGuide(type, pos.GetPositionX(), pos.GetPositionY(),
                        pos.GetPositionZ(), pos.GetOrientation(), teamId);
}

void Battleground::SendMessageToAll(uint32 entry, ChatMsg msgType,
                                    Player const *source) {
  if (!entry)
    return;

  Firelands::FirelandsStringChatBuilder builder(nullptr, msgType, entry,
                                                source);
  Firelands::LocalizedPacketDo<Firelands::FirelandsStringChatBuilder> localizer(
      builder);
  BroadcastWorker(localizer);
}

void Battleground::PSendMessageToAll(uint32 entry, ChatMsg msgType,
                                     Player const *source, ...) {
  if (!entry)
    return;
...
void Battleground::HandleTriggerBuff(ObjectGuid go_guid) {
  ...
  if (index < 0) {
    LOG_ERROR("bg.battleground",
              "Battleground::HandleTriggerBuff: cannot find buff gameobject "
              "({}, entry: {}, type: {}) in internal data for BG (map: {}, "
              "instance id: {})!",
              go_guid.ToString().c_str(), obj->GetEntry(), obj->GetGoType(),
              m_MapId, m_InstanceID);
    return;
  }
...
void Battleground::HandleKillPlayer(Player *victim, Player *killer) {
  ...
  if (killer) {
    ...
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
      ...
      if (creditedPlayer->GetTeam() == killer->GetTeam() &&
  ...
  for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
    ...
    if (itr->second.Team == Team) {
      ...
      if (player && player->IsAlive() &&
          player->GetShapeshiftForm() != FORM_SPIRITOFREDEMPTION)
        ++count;
    }
  }
  return count;
...
int32 Battleground::GetObjectType(ObjectGuid guid) {
  for (uint32 i = 0; i < BgObjects.size(); ++i)
    if (BgObjects[i] == guid)
      return i;
  LOG_ERROR("bg.battleground",
            "Battleground::GetObjectType: player used gameobject ({}) which is "
            "not in internal data for BG (map: {}, instance id: {}), cheating?",
...
void Battleground::SetBgRaid(uint32 TeamID, Group *bg_raid) {
  Group *&old_raid =
      TeamID == ALLIANCE ? m_BgRaids[TEAM_ALLIANCE] : m_BgRaids[TEAM_HORDE];
  if (old_raid)
...
uint32 Battleground::GetTeamScore(uint32 teamId) const {
  ...
  LOG_ERROR("bg.battleground", "GetTeamScore with wrong Team {} for BG {}",
            teamId, GetTypeID());
  return 0;
}
...
void Battleground::HandleAreaTrigger(Player *player, uint32 trigger) {
  LOG_DEBUG("bg.battleground",
            "Unhandled AreaTrigger {} in Battleground {}. Player coords (x: "
            "{}, y: {}, z: {})",
            trigger, player->GetMapId(), player->GetPositionX(),
...
bool Battleground::CheckAchievementCriteriaMeet(uint32 criteriaId,
                                                ...
                                                uint32 /*miscvalue1*/) {
  LOG_ERROR("bg.battleground",
            "Battleground::CheckAchievementCriteriaMeet: No implementation for "
            "criteria {}",
            criteriaId);
  return false;
}
