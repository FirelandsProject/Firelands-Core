...
bool BattlegroundSA::ResetObjs() {
  ...
  if (!AddCreature(BG_SA_NpcEntries[BG_SA_NPC_KANRETHAD], BG_SA_NPC_KANRETHAD,
                   BG_SA_NpcSpawnlocs[BG_SA_NPC_KANRETHAD])) {
    LOG_ERROR("bg.battleground",
              "SOTA: couldn't spawn Kanrethad, aborted. Entry: {}",
              BG_SA_NpcEntries[BG_SA_NPC_KANRETHAD]);
    return false;
  }

  for (uint8 i = 0; i <= BG_SA_PORTAL_DEFFENDER_RED; i++) {
    if (!AddObject(i, BG_SA_ObjEntries[i], BG_SA_ObjSpawnlocs[i], 0, 0, 0, 0,
                   RESPAWN_ONE_DAY)) {
      LOG_ERROR("bg.battleground",
                "SOTA: couldn't spawn BG_SA_PORTAL_DEFFENDER_RED, Entry: {}",
                BG_SA_ObjEntries[i]);
      continue;
    }
  }
  ...
  for (uint8 i = BG_SA_BOAT_ONE; i <= BG_SA_BOAT_TWO; i++) {
    ...
    if (!AddObject(i, boatid, BG_SA_ObjSpawnlocs[i].GetPositionX(),
                   ...
                   BG_SA_ObjSpawnlocs[i].GetPositionZ() +
                       (Attackers ? -3.750f : 0),
                   BG_SA_ObjSpawnlocs[i].GetOrientation(), 0, 0, 0, 0,
                   RESPAWN_ONE_DAY)) {
      LOG_ERROR("bg.battleground",
                "SOTA: couldn't spawn one of the BG_SA_BOAT, Entry: {}",
                boatid);
      continue;
    }
  }
  ...
  for (uint8 i = BG_SA_SIGIL_1; i <= BG_SA_LEFT_FLAGPOLE; i++) {
    if (!AddObject(i, BG_SA_ObjEntries[i], BG_SA_ObjSpawnlocs[i], 0, 0, 0, 0,
                   RESPAWN_ONE_DAY)) {
      LOG_ERROR("bg.battleground", "SOTA: couldn't spawn Sigil, Entry: {}",
                BG_SA_ObjEntries[i]);
      continue;
    }
  }
  ...
  GetBGObject(BG_SA_BOAT_ONE)
      ->SetParentRotation(QuaternionData(0.f, 0.f, 1.0f, 0.0002f));
  GetBGObject(BG_SA_BOAT_TWO)
      ->SetParentRotation(QuaternionData(0.f, 0.f, 1.0f, 0.00001f));
  ...
  for (uint8 i = 0; i < BG_SA_DEMOLISHER_5; i++) {
    if (!AddCreature(BG_SA_NpcEntries[i], i, BG_SA_NpcSpawnlocs[i],
                     Attackers == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE,
                     600)) {
      LOG_ERROR(
          "bg.battleground",
          "SOTA: couldn't spawn Cannon or demolisher, Entry: {}, Attackers: {}",
          BG_SA_NpcEntries[i],
          Attackers == TEAM_ALLIANCE ? "Horde(1)" : "Alliance(0)");
      continue;
    }
  }
  ...
  for (uint8 i = 0; i < BG_SA_MAX_GY; i++) {
    WorldSafeLocsEntry const *sg =
        sWorldSafeLocsStore.LookupEntry(BG_SA_GYEntries[i]);

    if (!sg) {
      LOG_ERROR("bg.battleground", "SOTA: Can't find GY entry {}",
                BG_SA_GYEntries[i]);
      return false;
    }
    ...
    if (i == BG_SA_BEACH_GY) {
      ...
      AddSpiritGuide(i + BG_SA_MAXNPC, sg->Loc.X, sg->Loc.Y, sg->Loc.Z,
                     BG_SA_GYOrientation[i], Attackers);
    } else {
      GraveyardStatus[i] =
          ((Attackers == TEAM_HORDE) ? TEAM_ALLIANCE : TEAM_HORDE);
      if (!AddSpiritGuide(i + BG_SA_MAXNPC, sg->Loc.X, sg->Loc.Y, sg->Loc.Z,
  ...
  for (uint8 i = BG_SA_CENTRAL_FLAG; i <= BG_SA_LEFT_FLAG; i++) {
    if (!AddObject(i,
                   ...
                   BG_SA_ObjSpawnlocs[i], 0, 0, 0, 0, RESPAWN_ONE_DAY)) {
      LOG_ERROR("bg.battleground",
  ...
  for (uint8 i = BG_SA_BOMB; i < BG_SA_MAXOBJ; i++) {
    if (!AddObject(i, BG_SA_ObjEntries[BG_SA_BOMB], BG_SA_ObjSpawnlocs[i], 0, 0,
                   0, 0, RESPAWN_ONE_DAY)) {
      LOG_ERROR("bg.battleground", "SOTA: couldn't spawn SA Bomb Entry: {}",
                BG_SA_ObjEntries[BG_SA_BOMB] + i);
      continue;
    }
    GetBGObject(i)->SetFaction(atF);
  }
  ...
  if (Status == BG_SA_WARMUP) {
    ...
    if (Status == BG_SA_ROUND_ONE) {
      ...
      if (TotalTime >= EndRoundTimer) {
        ...
        RoundScores[1].winner =
            (Attackers == TEAM_ALLIANCE) ? TEAM_HORDE : TEAM_ALLIANCE;
        if (RoundScores[0].time == RoundScores[1].time)
          EndBattleground(0);
        else if (RoundScores[0].time < RoundScores[1].time)
          EndBattleground(RoundScores[0].winner == TEAM_ALLIANCE ? ALLIANCE
                                                                 : HORDE);
        else
          EndBattleground(RoundScores[1].winner == TEAM_ALLIANCE ? ALLIANCE
                                                                 : HORDE);
        return;
      }
    }
...
void BattlegroundSA::FillInitialWorldStates(
  ...
  data.Worldstates.emplace_back(uint32(BG_SA_ANCIENT_GATEWS),
                                uint32(GateStatus[BG_SA_ANCIENT_GATE]));
  data.Worldstates.emplace_back(uint32(BG_SA_YELLOW_GATEWS),
                                uint32(GateStatus[BG_SA_YELLOW_GATE]));
  data.Worldstates.emplace_back(uint32(BG_SA_GREEN_GATEWS),
                                uint32(GateStatus[BG_SA_GREEN_GATE]));
  data.Worldstates.emplace_back(uint32(BG_SA_BLUE_GATEWS),
                                uint32(GateStatus[BG_SA_BLUE_GATE]));
  data.Worldstates.emplace_back(uint32(BG_SA_RED_GATEWS),
                                uint32(GateStatus[BG_SA_RED_GATE]));
  data.Worldstates.emplace_back(uint32(BG_SA_PURPLE_GATEWS),
                                uint32(GateStatus[BG_SA_PURPLE_GATE]));

  data.Worldstates.emplace_back(uint32(BG_SA_BONUS_TIMER), uint32(0));

  data.Worldstates.emplace_back(uint32(BG_SA_HORDE_ATTACKS),
                                uint32(hordeAttacks));
  data.Worldstates.emplace_back(uint32(BG_SA_ALLY_ATTACKS),
                                uint32(allyAttacks));

  // Time will be sent on first update...
  data.Worldstates.emplace_back(uint32(BG_SA_ENABLE_TIMER),
                                uint32(TimerEnabled));
  ...
  data.Worldstates.emplace_back(
      uint32(BG_SA_RIGHT_GY_HORDE),
      uint32(GraveyardStatus[BG_SA_RIGHT_CAPTURABLE_GY] == TEAM_HORDE));
  data.Worldstates.emplace_back(
      uint32(BG_SA_LEFT_GY_HORDE),
      uint32(GraveyardStatus[BG_SA_LEFT_CAPTURABLE_GY] == TEAM_HORDE));
  data.Worldstates.emplace_back(
      uint32(BG_SA_CENTER_GY_HORDE),
      uint32(GraveyardStatus[BG_SA_CENTRAL_CAPTURABLE_GY] == TEAM_HORDE));

  data.Worldstates.emplace_back(
      uint32(BG_SA_RIGHT_GY_ALLIANCE),
      uint32(GraveyardStatus[BG_SA_RIGHT_CAPTURABLE_GY] == TEAM_ALLIANCE));
  data.Worldstates.emplace_back(
      uint32(BG_SA_LEFT_GY_ALLIANCE),
      uint32(GraveyardStatus[BG_SA_LEFT_CAPTURABLE_GY] == TEAM_ALLIANCE));
  data.Worldstates.emplace_back(
      uint32(BG_SA_CENTER_GY_ALLIANCE),
      uint32(GraveyardStatus[BG_SA_CENTRAL_CAPTURABLE_GY] == TEAM_ALLIANCE));

  data.Worldstates.emplace_back(uint32(BG_SA_HORDE_DEFENCE_TOKEN),
                                uint32(allyAttacks));
  data.Worldstates.emplace_back(uint32(BG_SA_ALLIANCE_DEFENCE_TOKEN),
                                uint32(hordeAttacks));

  data.Worldstates.emplace_back(uint32(BG_SA_LEFT_ATT_TOKEN_HRD),
                                uint32(hordeAttacks));
  data.Worldstates.emplace_back(uint32(BG_SA_RIGHT_ATT_TOKEN_HRD),
                                uint32(hordeAttacks));
  data.Worldstates.emplace_back(uint32(BG_SA_RIGHT_ATT_TOKEN_ALL),
                                uint32(allyAttacks));
  data.Worldstates.emplace_back(uint32(BG_SA_LEFT_ATT_TOKEN_ALL),
                                uint32(allyAttacks));
}
...
void BattlegroundSA::AddPlayer(Player *player) {
  ...
  PlayerScores[player->GetGUID().GetCounter()] =
      new BattlegroundSAScore(player->GetGUID(), player->GetBGTeam());
...
void BattlegroundSA::RemovePlayer(Player * /*player*/, ObjectGuid /*guid*/,
                                  uint32 /*team*/) {}
...
void BattlegroundSA::ProcessEvent(WorldObject *obj, uint32 eventId,
    ...
    case GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING: {
      if (GateInfo const *gate = GetGate(obj->GetEntry())) {
        ...
        if (eventId == go->GetGOInfo()->building.damagedEvent) {
          ...
          PlaySoundToAll(Attackers == TEAM_ALLIANCE
...
WorldSafeLocsEntry const *BattlegroundSA::GetClosestGraveyard(Player *player) {
  ...
  nearest =
      player->GetExactDistSq(closest->Loc.X, closest->Loc.Y, closest->Loc.Z);
...
void BattlegroundSA::CaptureGraveyard(BG_SA_Graveyards i, Player *Source) {
  ...
  AddSpiritGuide(i + BG_SA_MAXNPC, sg->Loc.X, sg->Loc.Y, sg->Loc.Z,
                 BG_SA_GYOrientation[i], GraveyardStatus[i]);
  uint32 npc = 0;
  uint32 flag = 0;
  ...
  case BG_SA_LEFT_CAPTURABLE_GY:
    ...
    if (Creature *rigspark = AddCreature(BG_SA_NpcEntries[npc], npc,
                                         BG_SA_NpcSpawnlocs[npc], Attackers))
      rigspark->AI()->Talk(TEXT_SPARKLIGHT_RIGSPARK_SPAWN);

    for (uint8 j = BG_SA_DEMOLISHER_7; j <= BG_SA_DEMOLISHER_8; j++) {
      AddCreature(BG_SA_NpcEntries[j], j, BG_SA_NpcSpawnlocs[j],
                  (Attackers == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE),
                  600);
    ...
    if (Creature *c = Source->FindNearestCreature(NPC_WORLD_TRIGGER, 500.0f))
      SendChatMessage(c,
                      Source->GetTeamId() == TEAM_ALLIANCE
  ...
  case BG_SA_RIGHT_CAPTURABLE_GY:
    ...
    if (Creature *sparklight = AddCreature(BG_SA_NpcEntries[npc], npc,
                                           BG_SA_NpcSpawnlocs[npc], Attackers))
      sparklight->AI()->Talk(TEXT_SPARKLIGHT_RIGSPARK_SPAWN);

    for (uint8 j = BG_SA_DEMOLISHER_5; j <= BG_SA_DEMOLISHER_6; j++) {
      AddCreature(BG_SA_NpcEntries[j], j, BG_SA_NpcSpawnlocs[j],
                  Attackers == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE, 600);
  ...
  if (CanInteractWithObject(BG_SA_TITAN_RELIC)) {
    if (clicker->GetTeamId() == Attackers) {
      if (clicker->GetTeamId() == TEAM_ALLIANCE)
        SendBroadcastText(BG_SA_TEXT_ALLIANCE_CAPTURED_TITAN_PORTAL,
