...
void BattlegroundAV::HandleKillUnit(Creature *unit, Player *killer) {
  ...
  */
  if (entry == BG_AV_CreatureInfo[AV_NPC_A_BOSS]) {
    CastSpellOnTeam(23658, HORDE); // this is a spell which finishes a quest
    ...
    CastSpellOnTeam(23658, ALLIANCE); // this is a spell which finishes a quest
                                      // where a player has to kill the boss
    ...
    if (!m_CaptainAlive[0]) {
      LOG_ERROR("bg.battleground", "Killed a Captain twice, please report this "
...
void BattlegroundAV::HandleQuestComplete(uint32 questid, Player *player) {
  ...
  /// quest available, go/npc de/spawning)and maybe honor
  ...
  case AV_QUEST_H_NEAR_MINE:
    ...
    if (m_Team_QuestStatus[team][5] == 28) {
      ...
      if (m_Team_QuestStatus[team][6] == 7)
        LOG_DEBUG("bg.battleground",
                  "BG_AV Quest {} completed (need to implement some events "
...
void BattlegroundAV::UpdateScore(
    uint16 team,
    int16
        points) { // note: to remove reinforcementpoints points must be
                  // negative, for adding reinforcements points must be positive
  ASSERT(team == ALLIANCE || team == HORDE);
  uint8 teamindex = GetTeamIndexByTeamId(team); // 0=ally 1=horde
  m_Team_Scores[teamindex] += points;

  UpdateWorldState(
      ((teamindex == TEAM_HORDE) ? AV_Horde_Score : AV_Alliance_Score),
...
Creature *BattlegroundAV::AddAVCreature(uint16 cinfoid, uint16 type) {
  ...
  if (type >= AV_CPLACE_MAX) // static
  {
    ...
    creature =
        AddCreature(BG_AV_CreatureInfo[cinfoid], type, BG_AV_CreaturePos[type]);
  }
  if (!creature)
    return nullptr;
...
void BattlegroundAV::AddPlayer(Player *player) {
  ...
  PlayerScores[player->GetGUID().GetCounter()] =
      new BattlegroundAVScore(player->GetGUID(), player->GetBGTeam());
}
  ...
  for (uint8 i = TEAM_ALLIANCE; i <= TEAM_HORDE; ++i) {
    ...
    if (rep[i] != 0)
      RewardReputationToTeam(i == 0 ? 730 : 729, rep[i],
                             i == 0 ? ALLIANCE : HORDE);
    if (kills[i] != 0)
      RewardHonorToTeam(GetBonusHonorFromKill(kills[i]),
                        i == 0 ? ALLIANCE : HORDE);
  }
...
bool BattlegroundAV::UpdatePlayerScore(Player *player, uint32 type,
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node) {
  ...
  LOG_DEBUG("bg.battleground",
            "bg_av: player destroyed point node {} object {}", node, object);
  ...
  if (IsTower(node)) {
    ...
    SpawnBGObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION + 3 * node,
                  RESPAWN_ONE_DAY);
    SpawnBGObject(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION +
    ...
    {
      for (uint8 i = 0; i < 4; i++) {
        SpawnBGObject(((owner == ALLIANCE) ? BG_AV_OBJECT_SNOW_EYECANDY_PA
                                           : BG_AV_OBJECT_SNOW_EYECANDY_PH) +
                          i,
                      RESPAWN_ONE_DAY);
        SpawnBGObject(((owner == ALLIANCE) ? BG_AV_OBJECT_SNOW_EYECANDY_A
  ...
  if (StaticNodeInfo const *nodeInfo = GetStaticNodeInfo(node))
    if (Creature *herold = GetBGCreature(AV_CPLACE_HERALD))
      herold->AI()->Talk(owner == ALLIANCE ? nodeInfo->TextIds.AllianceCapture
                                           : nodeInfo->TextIds.HordeCapture);
}
...
void BattlegroundAV::ChangeMineOwner(uint8 mine, uint32 team, bool initial) {
  ...
  LOG_DEBUG("bg.battleground", "bg_av populating mine {} (0=north, 1=south)",
            mine);
  uint16 miner;
  ...
  if (team == ALLIANCE || team == HORDE) {
    ...
    if (Creature *herold = GetBGCreature(AV_CPLACE_HERALD)) {
      if (mine == AV_NORTH_MINE)
        herold->AI()->Talk(team == ALLIANCE ? TEXT_IRONDEEP_MINE_ALLIANCE_TAKEN
                                            : TEXT_IRONDEEP_MINE_HORDE_TAKEN);
      else if (mine == AV_SOUTH_MINE)
        herold->AI()->Talk(team == ALLIANCE ? TEXT_COLDTOOTH_MINE_ALLIANCE_TAKEN
...
void BattlegroundAV::PopulateNode(BG_AV_Nodes node) {
  ...
  if (IsTower(node))
    creatureid =
        (owner == ALLIANCE) ? AV_NPC_A_TOWERDEFENSE : AV_NPC_H_TOWERDEFENSE;
  else {
    uint8 team2 = GetTeamIndexByTeamId(owner);
    if (m_Team_QuestStatus[team2][0] < 500)
      creatureid =
          (owner == ALLIANCE) ? AV_NPC_A_GRAVEDEFENSE0 : AV_NPC_H_GRAVEDEFENSE0;
    else if (m_Team_QuestStatus[team2][0] < 1000)
      creatureid =
          (owner == ALLIANCE) ? AV_NPC_A_GRAVEDEFENSE1 : AV_NPC_H_GRAVEDEFENSE1;
    else if (m_Team_QuestStatus[team2][0] < 1500)
      creatureid =
          (owner == ALLIANCE) ? AV_NPC_A_GRAVEDEFENSE2 : AV_NPC_H_GRAVEDEFENSE2;
    else
      creatureid =
          (owner == ALLIANCE) ? AV_NPC_A_GRAVEDEFENSE3 : AV_NPC_H_GRAVEDEFENSE3;
    // spiritguide
    if (BgCreatures[node])
      DelCreature(node);
    if (!AddSpiritGuide(node, BG_AV_CreaturePos[node],
                        GetTeamIndexByTeamId(owner)))
      LOG_ERROR("bg.battleground", "AV: couldn't spawn spiritguide at node {}",
                node);
  }
  for (uint8 i = 0; i < 4; i++)
  ...
  if (!trigger) {
    trigger =
        AddCreature(WORLD_TRIGGER, node + 302, BG_AV_CreaturePos[node + 302],
                    GetTeamIndexByTeamId(owner));
  }
  ...
  if (trigger) {
    if (owner != ALLIANCE &&
        owner != HORDE) // node can be neutral, remove trigger
    {
      DelCreature(node + 302);
      return;
    }
    trigger->SetFaction(owner == ALLIANCE ? FACTION_ALLIANCE_GENERIC
...
uint32 BattlegroundAV::GetObjectThroughNode(
    BG_AV_Nodes
        node) { // this function is the counterpart to GetNodeThroughObject()
...
void BattlegroundAV::EventPlayerClickedOnFlag(Player *source,
  ...
  LOG_DEBUG("bg.battleground", "BG_AV using gameobject {} with type {}",
            target_obj->GetEntry(), object);
  if (object < 0)
    return;
...
void BattlegroundAV::EventPlayerDefendsPoint(Player *player, uint32 object) {
  ...
  if (m_Nodes[node].TotalOwner ==
      AV_NEUTRAL_TEAM) { // until snowfall doesn't belong to anyone it is better
                         // handled in assault-code
    ASSERT(node ==
           BG_AV_NODES_SNOWFALL_GRAVE); // currently the only neutral grave
    EventPlayerAssaultsPoint(player, object);
    return;
  }
  LOG_DEBUG("bg.battleground", "player defends point object: {} node: {}",
            object, node);
  ...
  } else if (node == BG_AV_NODES_SNOWFALL_GRAVE) // snowfall eyecandy
  {
    for (uint8 i = 0; i < 4; i++) {
      SpawnBGObject(((owner == ALLIANCE) ? BG_AV_OBJECT_SNOW_EYECANDY_PA
                        ...
                        i,
                    RESPAWN_ONE_DAY);
      SpawnBGObject(((team == ALLIANCE) ? BG_AV_OBJECT_SNOW_EYECANDY_A
                                        : BG_AV_OBJECT_SNOW_EYECANDY_H) +
                        i,
                    RESPAWN_IMMEDIATELY);
    }
  ...
  if (StaticNodeInfo const *nodeInfo = GetStaticNodeInfo(node))
    if (Creature *herold = GetBGCreature(AV_CPLACE_HERALD))
      herold->AI()->Talk(team == ALLIANCE ? nodeInfo->TextIds.AllianceCapture
                                          : nodeInfo->TextIds.HordeCapture);

  // update the statistic for the defending player
  UpdatePlayerScore(
      player, IsTower(node) ? SCORE_TOWERS_DEFENDED : SCORE_GRAVEYARDS_DEFENDED,
      1);
}
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player *player, uint32 object) {
  ...
  uint32 team = player->GetTeam();
  LOG_DEBUG("bg.battleground", "bg_av: player assaults point object {} node {}",
            object, node);
                                          ...
                                          // capping + it gets eyecandy stuff
  {
    ...
    {
      if (!(owner == AV_NEUTRAL_TEAM &&
            m_Nodes[node].TotalOwner == AV_NEUTRAL_TEAM))
        return;
    ...
    } else if (m_Nodes[node].TotalOwner ==
               AV_NEUTRAL_TEAM) // recapping, when no team owns this node realy
    {
  ...
  if (StaticNodeInfo const *nodeInfo = GetStaticNodeInfo(node))
    if (Creature *herold = GetBGCreature(AV_CPLACE_HERALD))
      herold->AI()->Talk(team == ALLIANCE ? nodeInfo->TextIds.AllianceAttack
                                          : nodeInfo->TextIds.HordeAttack);
...
void BattlegroundAV::FillInitialWorldStates(
  ...
  for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i) {
    ...
    data.Worldstates.emplace_back(
        uint32(BGAVNodeInfo[i].WorldStateIds.AllianceAssault),
        uint32(owner == ALLIANCE && state == POINT_ASSAULTED));
    data.Worldstates.emplace_back(
        uint32(BGAVNodeInfo[i].WorldStateIds.AllianceControl),
        uint32(owner == ALLIANCE && state >= POINT_DESTROYED));
    data.Worldstates.emplace_back(
        uint32(BGAVNodeInfo[i].WorldStateIds.HordeAssault),
        uint32(owner == HORDE && state == POINT_ASSAULTED));
    data.Worldstates.emplace_back(
        uint32(BGAVNodeInfo[i].WorldStateIds.HordeControl),
        uint32(owner == HORDE && state >= POINT_DESTROYED));
  }

  data.Worldstates.emplace_back(
      uint32(AV_SNOWFALL_N),
      uint32(m_Nodes[BG_AV_NODES_SNOWFALL_GRAVE].Owner == AV_NEUTRAL_TEAM));

  data.Worldstates.emplace_back(uint32(AV_Alliance_Score),
                                uint32(m_Team_Scores[0]));
  data.Worldstates.emplace_back(uint32(AV_Horde_Score),
                                uint32(m_Team_Scores[1]));
...
void BattlegroundAV::UpdateNodeWorldState(BG_AV_Nodes node) {
  if (StaticNodeInfo const *nodeInfo = GetStaticNodeInfo(node)) {
    ...
    UpdateWorldState(nodeInfo->WorldStateIds.AllianceAssault,
                     owner == ALLIANCE && state == POINT_ASSAULTED);
    UpdateWorldState(nodeInfo->WorldStateIds.AllianceControl,
                     owner == ALLIANCE && state >= POINT_DESTROYED);
    UpdateWorldState(nodeInfo->WorldStateIds.HordeAssault,
                     owner == HORDE && state == POINT_ASSAULTED);
    UpdateWorldState(nodeInfo->WorldStateIds.HordeControl,
                     owner == HORDE && state >= POINT_DESTROYED);
  }
...
void BattlegroundAV::SendMineWorldStates(uint32 mine) {
  ...
  //    ASSERT(m_Mine_PrevOwner[mine] == ALLIANCE || m_Mine_PrevOwner[mine] ==
  //    HORDE || m_Mine_PrevOwner[mine] == AV_NEUTRAL_TEAM);
  //    ASSERT(m_Mine_Owner[mine] == ALLIANCE || m_Mine_Owner[mine] == HORDE ||
  //    m_Mine_Owner[mine] == AV_NEUTRAL_TEAM);

  uint8 owner, prevowner,
      mine2; // those variables are needed to access the right worldstate in the
             // BG_AV_MineWorldStates array
...
WorldSafeLocsEntry const *BattlegroundAV::GetClosestGraveyard(Player *player) {
  ...
  minDist = (pGraveyard->Loc.X - x) * (pGraveyard->Loc.X - x) +
            (pGraveyard->Loc.Y - y) * (pGraveyard->Loc.Y - y);
  ...
  for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i <= BG_AV_NODES_FROSTWOLF_HUT;
    ...
    if (m_Nodes[i].Owner == player->GetTeam() &&
      ...
      if (entry) {
        dist = (entry->Loc.X - x) * (entry->Loc.X - x) +
               (entry->Loc.Y - y) * (entry->Loc.Y - y);
        if (dist < minDist) {
          minDist = dist;
          pGraveyard = entry;
        }
...
bool BattlegroundAV::SetupBattleground() {
  ...
  if ( // alliance gates
      !AddObject(BG_AV_OBJECT_DOOR_A, BG_AV_OBJECTID_GATE_A,
                 BG_AV_DoorPositons[0], 0, 0,
                 std::sin(BG_AV_DoorPositons[0].GetOrientation() / 2),
                 std::cos(BG_AV_DoorPositons[0].GetOrientation() / 2),
                 RESPAWN_IMMEDIATELY)
      // horde gates
      || !AddObject(BG_AV_OBJECT_DOOR_H, BG_AV_OBJECTID_GATE_H,
                    BG_AV_DoorPositons[1], 0, 0,
                    std::sin(BG_AV_DoorPositons[1].GetOrientation() / 2),
                    std::cos(BG_AV_DoorPositons[1].GetOrientation() / 2),
                    RESPAWN_IMMEDIATELY)) {
  ...
  for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i) {
    if (i <= BG_AV_NODES_FROSTWOLF_HUT) {
      if (!AddObject(i, BG_AV_OBJECTID_BANNER_A_B, BG_AV_ObjectPos[i], 0, 0,
                     std::sin(BG_AV_ObjectPos[i].GetOrientation() / 2),
                     std::cos(BG_AV_ObjectPos[i].GetOrientation() / 2),
                     RESPAWN_ONE_DAY) ||
          !AddObject(i + 11, BG_AV_OBJECTID_BANNER_CONT_A_B, BG_AV_ObjectPos[i],
                     0, 0, std::sin(BG_AV_ObjectPos[i].GetOrientation() / 2),
                     std::cos(BG_AV_ObjectPos[i].GetOrientation() / 2),
                     RESPAWN_ONE_DAY) ||
          !AddObject(i + 33, BG_AV_OBJECTID_BANNER_H_B, BG_AV_ObjectPos[i], 0,
                     0, std::sin(BG_AV_ObjectPos[i].GetOrientation() / 2),
                     std::cos(BG_AV_ObjectPos[i].GetOrientation() / 2),
                     RESPAWN_ONE_DAY) ||
          !AddObject(i + 22, BG_AV_OBJECTID_BANNER_CONT_H_B, BG_AV_ObjectPos[i],
                     0, 0, std::sin(BG_AV_ObjectPos[i].GetOrientation() / 2),
                     std::cos(BG_AV_ObjectPos[i].GetOrientation() / 2),
                     RESPAWN_ONE_DAY)
          // aura
          || !AddObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION + i * 3,
                        BG_AV_OBJECTID_AURA_N, BG_AV_ObjectPos[i], 0, 0,
                        std::sin(BG_AV_ObjectPos[i].GetOrientation() / 2),
                        std::cos(BG_AV_ObjectPos[i].GetOrientation() / 2),
                        RESPAWN_ONE_DAY) ||
          !AddObject(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION + i * 3,
                     BG_AV_OBJECTID_AURA_A, BG_AV_ObjectPos[i], 0, 0,
                     std::sin(BG_AV_ObjectPos[i].GetOrientation() / 2),
                     std::cos(BG_AV_ObjectPos[i].GetOrientation() / 2),
                     RESPAWN_ONE_DAY) ||
          !AddObject(BG_AV_OBJECT_AURA_H_FIRSTAID_STATION + i * 3,
                     BG_AV_OBJECTID_AURA_H, BG_AV_ObjectPos[i], 0, 0,
                     std::sin(BG_AV_ObjectPos[i].GetOrientation() / 2),
                     std::cos(BG_AV_ObjectPos[i].GetOrientation() / 2),
                     RESPAWN_ONE_DAY)) {
        LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some "
                                     "object Battleground not created!2");
        return false;
      }
    } else // towers
    {
      ...
      {
        if (!AddObject(i, BG_AV_OBJECTID_BANNER_A, BG_AV_ObjectPos[i], 0, 0,
                       std::sin(BG_AV_ObjectPos[i].GetOrientation() / 2),
                       std::cos(BG_AV_ObjectPos[i].GetOrientation() / 2),
                       RESPAWN_ONE_DAY) ||
            !AddObject(i + 22, BG_AV_OBJECTID_BANNER_CONT_H, BG_AV_ObjectPos[i],
                       0, 0, std::sin(BG_AV_ObjectPos[i].GetOrientation() / 2),
                       std::cos(BG_AV_ObjectPos[i].GetOrientation() / 2),
                       RESPAWN_ONE_DAY) ||
            !AddObject(BG_AV_OBJECT_TAURA_A_DUNBALDAR_SOUTH +
                           (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
                       BG_AV_OBJECTID_AURA_A, BG_AV_ObjectPos[i + 8], 0, 0,
                       std::sin(BG_AV_ObjectPos[i + 8].GetOrientation() / 2),
                       std::cos(BG_AV_ObjectPos[i + 8].GetOrientation() / 2),
                       RESPAWN_ONE_DAY) ||
            !AddObject(BG_AV_OBJECT_TAURA_H_DUNBALDAR_SOUTH +
                           (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
                       BG_AV_OBJECTID_AURA_N, BG_AV_ObjectPos[i + 8], 0, 0,
                       std::sin(BG_AV_ObjectPos[i + 8].GetOrientation() / 2),
                       std::cos(BG_AV_ObjectPos[i + 8].GetOrientation() / 2),
                       RESPAWN_ONE_DAY) ||
            !AddObject(BG_AV_OBJECT_TFLAG_A_DUNBALDAR_SOUTH +
                       ...
                       0, std::sin(BG_AV_ObjectPos[i + 8].GetOrientation() / 2),
                       std::cos(BG_AV_ObjectPos[i + 8].GetOrientation() / 2),
                       RESPAWN_ONE_DAY) ||
            !AddObject(BG_AV_OBJECT_TFLAG_H_DUNBALDAR_SOUTH +
                           (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
                       BG_AV_OBJECTID_TOWER_BANNER_PH, BG_AV_ObjectPos[i + 8],
                       0, 0,
                       std::sin(BG_AV_ObjectPos[i + 8].GetOrientation() / 2),
                       std::cos(BG_AV_ObjectPos[i + 8].GetOrientation() / 2),
                       RESPAWN_ONE_DAY)) {
          LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some "
                                       "object Battleground not created!3");
          return false;
        }
      } else // horde towers
      {
        if (!AddObject(i + 7, BG_AV_OBJECTID_BANNER_CONT_A, BG_AV_ObjectPos[i],
                       0, 0, std::sin(BG_AV_ObjectPos[i].GetOrientation() / 2),
                       std::cos(BG_AV_ObjectPos[i].GetOrientation() / 2),
                       RESPAWN_ONE_DAY) ||
            ...
            !AddObject(BG_AV_OBJECT_TAURA_H_DUNBALDAR_SOUTH +
                           (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
                       BG_AV_OBJECTID_AURA_H, BG_AV_ObjectPos[i + 8], 0, 0,
                       std::sin(BG_AV_ObjectPos[i + 8].GetOrientation() / 2),
                       std::cos(BG_AV_ObjectPos[i + 8].GetOrientation() / 2),
                       RESPAWN_ONE_DAY) ||
            !AddObject(BG_AV_OBJECT_TFLAG_A_DUNBALDAR_SOUTH +
                           (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
                       BG_AV_OBJECTID_TOWER_BANNER_PA, BG_AV_ObjectPos[i + 8],
                       0, 0,
                       std::sin(BG_AV_ObjectPos[i + 8].GetOrientation() / 2),
                       std::cos(BG_AV_ObjectPos[i + 8].GetOrientation() / 2),
                       RESPAWN_ONE_DAY) ||
            !AddObject(BG_AV_OBJECT_TFLAG_H_DUNBALDAR_SOUTH +
                       ...
                       RESPAWN_ONE_DAY)) {
          LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some "
                                       "object Battleground not created!4");
          return false;
        }
      }
    ...
    for (uint8 j = 0; j <= 9; j++) {
      if (j < 5) {
        if (!AddObject(
                ...
                BG_AV_ObjectPos[AV_OPLACE_BURN_BUILDING_A + (i * 10) + j], 0, 0,
                std::sin(
  ...
  for (uint16 i = 0;
    ...
    if (!AddObject(BG_AV_OBJECT_MINE_SUPPLY_N_MIN + i, BG_AV_OBJECTID_MINE_N,
                   BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_N_MIN + i], 0, 0,
                   std::sin(BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_N_MIN + i]
                                .GetOrientation() /
                            2),
                   std::cos(BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_N_MIN + i]
                                .GetOrientation() /
                            2),
                   RESPAWN_ONE_DAY)) {
      LOG_ERROR("bg.battleground",
  ...
  for (uint16 i = 0;
    ...
    if (!AddObject(BG_AV_OBJECT_MINE_SUPPLY_S_MIN + i, BG_AV_OBJECTID_MINE_S,
                   BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_S_MIN + i], 0, 0,
                   std::sin(BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_S_MIN + i]
                                .GetOrientation() /
                            2),
                   std::cos(BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_S_MIN + i]
                                .GetOrientation() /
                            2),
                   RESPAWN_ONE_DAY)) {
      LOG_ERROR("bg.battleground",
  ...
  if (!AddObject(
          BG_AV_OBJECT_FLAG_N_SNOWFALL_GRAVE, BG_AV_OBJECTID_BANNER_SNOWFALL_N,
          BG_AV_ObjectPos[BG_AV_NODES_SNOWFALL_GRAVE], 0, 0,
          std::sin(
              BG_AV_ObjectPos[BG_AV_NODES_SNOWFALL_GRAVE].GetOrientation() / 2),
          std::cos(
              BG_AV_ObjectPos[BG_AV_NODES_SNOWFALL_GRAVE].GetOrientation() / 2),
          RESPAWN_ONE_DAY)) {
    LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some object "
                                 "Battleground not created!8");
    return false;
  }
  for (uint8 i = 0; i < 4; i++) {
    if (!AddObject(
            BG_AV_OBJECT_SNOW_EYECANDY_A + i, BG_AV_OBJECTID_SNOWFALL_CANDY_A,
            BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i], 0, 0,
            std::sin(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i].GetOrientation() /
                     2),
            std::cos(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i].GetOrientation() /
                     2),
            RESPAWN_ONE_DAY) ||
        !AddObject(
            BG_AV_OBJECT_SNOW_EYECANDY_PA + i, BG_AV_OBJECTID_SNOWFALL_CANDY_PA,
            BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i], 0, 0,
            std::sin(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i].GetOrientation() /
                     2),
            std::cos(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i].GetOrientation() /
            ...
            RESPAWN_ONE_DAY) ||
        !AddObject(
            BG_AV_OBJECT_SNOW_EYECANDY_H + i, BG_AV_OBJECTID_SNOWFALL_CANDY_H,
            BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i], 0, 0,
            std::sin(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i].GetOrientation() /
                     2),
            std::cos(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i].GetOrientation() /
            ...
            RESPAWN_ONE_DAY) ||
        !AddObject(
            BG_AV_OBJECT_SNOW_EYECANDY_PH + i, BG_AV_OBJECTID_SNOWFALL_CANDY_PH,
            BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i], 0, 0,
            std::sin(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i].GetOrientation() /
                     2),
            std::cos(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i].GetOrientation() /
...
void BattlegroundAV::AssaultNode(BG_AV_Nodes node, uint16 team) {
  ...
  if (m_Nodes[node].State == POINT_ASSAULTED &&
      m_Nodes[node]
          .TotalOwner) // only assault an assaulted node if no totalowner exists
  {
...
void BattlegroundAV::ResetBGSubclass() {
  ...
  InitNode(BG_AV_NODES_SNOWFALL_GRAVE, AV_NEUTRAL_TEAM,
...
bool BattlegroundAV::CheckAchievementCriteriaMeet(uint32 criteriaId,
  ...
  return Battleground::CheckAchievementCriteriaMeet(criteriaId, source, target,
                                                    miscValue);
}
