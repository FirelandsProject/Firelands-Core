...
void BattlegroundTP::PostUpdateImpl(uint32 diff) {
  if (GetStatus() == STATUS_IN_PROGRESS) {
    ...
    while (uint32 eventId = events.ExecuteEvent()) {
      switch (eventId) {
      case TP_EVENT_SPAWN_FLAGS: // Spawn flags in 2,5 seconds after battle
                                 // started
        SpawnBGObject(BG_TP_OBJECT_A_FLAG, RESPAWN_IMMEDIATELY);
        SpawnBGObject(BG_TP_OBJECT_H_FLAG, RESPAWN_IMMEDIATELY);
        SendBroadcastText(BG_TP_TEXT_FLAG_PLACED_ALLIANCE,
                          CHAT_MSG_BG_SYSTEM_NEUTRAL);
        SendBroadcastText(BG_TP_TEXT_FLAG_PLACED_HORDE,
                          CHAT_MSG_BG_SYSTEM_NEUTRAL);
        PlaySoundToAll(BG_TP_SOUND_FLAGS_RESPAWNED);
        break;
      case TP_EVENT_CLOSE_BATTLEGROUND: // Close battleground after 27 minutes
                                        // of its creation
        if (GetTeamScore(TEAM_ALLIANCE) == 0) {
          ...
          EndBattleground(
              ALLIANCE); // Alliance has > 0, Horde has 0, alliance wins
        else if (GetTeamScore(TEAM_HORDE) ==
                 GetTeamScore(TEAM_ALLIANCE)) // Team score equal, winner is
                                              // team that scored the last flag
          EndBattleground(_lastFlagCaptureTeam);
        else if (GetTeamScore(TEAM_HORDE) >
                 GetTeamScore(TEAM_ALLIANCE)) // Last but not least, check who
    ...
    if (_bothFlagsKept) {
      ...
      if (_flagDebuffState == 0 &&
          _flagSpellForceTimer >= 10 * MINUTE * IN_MILLISECONDS) // 10 minutes
      {
...
void BattlegroundTP::AddPlayer(Player *player) {
  ...
  PlayerScores[player->GetGUID().GetCounter()] =
      new BattlegroundTPScore(player->GetGUID(), player->GetBGTeam());
}
...
void BattlegroundTP::StartingEventOpenDoors() {
  ...
  for (uint32 i = BG_TP_OBJECT_SPEEDBUFF_1; i <= BG_TP_OBJECT_BERSERKBUFF_2;
       ++i)
    SpawnBGObject(i, RESPAWN_IMMEDIATELY);
...
bool BattlegroundTP::SetupBattleground() {
  if (!AddObject(BG_TP_OBJECT_A_FLAG, BG_OBJECT_A_FLAG_TP_ENTRY, 2118.210f,
                 191.621f, 44.052f, 5.741259f, 0, 0, 0.9996573f, 0.02617699f,
                 BG_TP_FLAG_RESPAWN_TIME / 1000) ||
      !AddObject(BG_TP_OBJECT_H_FLAG, BG_OBJECT_H_FLAG_TP_ENTRY, 1578.337f,
                 344.0451f, 2.418409f, 2.792518f, 0, 0, 0.984807f, 0.1736523f,
                 BG_TP_FLAG_RESPAWN_TIME / 1000)
      /// Buffs
      || !AddObject(BG_TP_OBJECT_SPEEDBUFF_1, GB_OBJECT_SPEED_BUFF_1_TP_ENTRY,
                    1544.55f, 303.852f, 0.692371f, 6.265733f, 0, 0, 0, 0,
                    BUFF_RESPAWN_TIME) ||
      !AddObject(BG_TP_OBJECT_SPEEDBUFF_2, BG_OBJECTID_SPEEDBUFF_ENTRY,
                 2175.87f, 226.622f, 43.7629f, 2.60053f, 0, 0, 0, 0,
                 BUFF_RESPAWN_TIME) ||
      !AddObject(BG_TP_OBJECT_REGENBUFF_1, GB_OBJECT_FOOD_BUFF_1_TP_ENTRY,
                 1754.163f, 242.125f, -14.13157f, 1.151916f, 0, 0, 0, 0,
                 BUFF_RESPAWN_TIME) ||
      !AddObject(BG_TP_OBJECT_REGENBUFF_2, BG_OBJECTID_REGENBUFF_ENTRY,
                 1951.18f, 383.795f, -10.5257f, 4.06662f, 0, 0, 0, 0,
                 BUFF_RESPAWN_TIME) ||
      !AddObject(BG_TP_OBJECT_BERSERKBUFF_1, BG_OBJECTID_BERSERKERBUFF_ENTRY,
                 1932.832f, 226.7917f, -17.05979f, 2.44346f, 0, 0, 0, 0,
                 BUFF_RESPAWN_TIME) ||
      !AddObject(BG_TP_OBJECT_BERSERKBUFF_2,
                 GB_OBJECT_BERSERKER_BUFF_1_TP_ENTRY, 1737.569f, 435.845f,
                 -8.0863f, 5.51524f, 0, 0, 0, 0, BUFF_RESPAWN_TIME)
      /// Alliance gates
      || !AddObject(BG_TP_OBJECT_DOOR_A_1, BG_OBJECT_DOOR_A_1_TP_ENTRY,
                    2118.088f, 154.6754f, 43.57089f, 2.609261f, 0, 0, 0, 0,
                    RESPAWN_IMMEDIATELY) ||
      !AddObject(BG_TP_OBJECT_DOOR_A_2, BG_OBJECT_DOOR_A_2_TP_ENTRY, 2156.0f,
                 219.2059f, 43.6256f, 2.609261f, 0, 0, 0, 0,
                 RESPAWN_IMMEDIATELY) ||
      !AddObject(BG_TP_OBJECT_DOOR_A_3, BG_OBJECT_DOOR_A_3_TP_ENTRY, 2135.525f,
                 218.926f, 43.60946f, 5.750861f, 0, 0, 0, 0,
                 RESPAWN_IMMEDIATELY)
      /// Horde gates
      || !AddObject(BG_TP_OBJECT_DOOR_H_1, BG_OBJECT_DOOR_H_1_TP_ENTRY,
                    1556.656f, 314.7127f, 1.589001f, 6.178466f, 0, 0, 0, 0,
                    RESPAWN_IMMEDIATELY) ||
      !AddObject(BG_TP_OBJECT_DOOR_H_2, BG_OBJECT_DOOR_H_2_TP_ENTRY, 1574.605f,
                 321.2421f, 1.58989f, 6.178466f, 0, 0, 0, 0,
                 RESPAWN_IMMEDIATELY) ||
      !AddObject(BG_TP_OBJECT_DOOR_H_3, BG_OBJECT_DOOR_H_3_TP_ENTRY, 1558.088f,
                 372.7654f, 1.723727f, 6.178466f, 0, 0, 0, 0,
                 RESPAWN_IMMEDIATELY) ||
      !AddObject(BG_TP_OBJECT_DOOR_H_4, BG_OBJECT_DOOR_H_4_TP_ENTRY, 1558.623f,
                 379.1595f, -6.409669f, 4.607672f, 0, 0, 0, 0,
                 RESPAWN_IMMEDIATELY)) {
    LOG_ERROR("misc", "BatteGroundTP: Failed to spawn some objects. "
  ...
  for (uint32 i = TP_GRAVEYARD_START_ALLIANCE; i < TP_MAX_GRAVEYARDS; ++i) {
    WorldSafeLocsEntry const *grave =
        sWorldSafeLocsStore.LookupEntry(BG_TP_GraveyardIds[i]);

    if (grave) {
      uint8 team = i % 2; ///< If 0 team == TEAM_ALLIANCE else TEAM_HORDE
      uint32 creatureType =
          team == TEAM_ALLIANCE ? TP_SPIRIT_ALLIANCE : TP_SPIRIT_HORDE;
      float orientation = float(team == TEAM_ALLIANCE ? M_PI : 0);

      if (!AddSpiritGuide(creatureType, grave->Loc.X, grave->Loc.Y,
                          grave->Loc.Z, orientation, TeamId(team))) {
        LOG_ERROR("misc",
                  "BatteGroundTP: Failed to spawn spirit guide id: {}. "
                  "Battleground not created!",
                  grave->ID);
        return false;
      }
      ...
      LOG_ERROR(
          "misc",
          "BatteGroundTP: Failed to find grave {}. Battleground not created!",
          BG_TP_GraveyardIds[i]);
      return false;
    }
  }
...
void BattlegroundTP::FillInitialWorldStates(
    WorldPackets::WorldState::InitWorldStates &data) {
  /// Show how many flags had been captured
  data.Worldstates.emplace_back(uint32(BG_TP_FLAG_CAPTURES_ALLIANCE),
                                uint32(GetTeamScore(TEAM_ALLIANCE)));
  data.Worldstates.emplace_back(uint32(BG_TP_FLAG_CAPTURES_HORDE),
                                uint32(GetTeamScore(TEAM_HORDE)));

  /// Show MAX number of flags (x/3)
  data.Worldstates.emplace_back(uint32(BG_TP_FLAG_CAPTURES_MAX),
                                uint32(BG_TP_MAX_TEAM_SCORE));
  ...
  if (GetStatus() == STATUS_IN_PROGRESS) {
    ...
    for (uint8 team = TEAM_ALLIANCE; team <= TEAM_HORDE; ++team) {
      ...
      case BG_TP_FLAG_STATE_ON_GROUND:
        data.Worldstates.emplace_back(uint32(BG_TP_FLAG_UNK_ALLIANCE + team),
                                      uint32(-1));
        data.Worldstates.emplace_back(
            uint32(BG_TP_FLAG_STATE_HORDE + team),
            uint32(3)); ///< Show if team's flag is carried
        break;
      case BG_TP_FLAG_STATE_ON_PLAYER:
        ...
        data.Worldstates.emplace_back(
            uint32(BG_TP_FLAG_STATE_HORDE + team),
            uint32(2)); ///< Show if team's flag is carried
        break;
      default: ///< In Base
        data.Worldstates.emplace_back(uint32(BG_TP_FLAG_UNK_ALLIANCE + team),
                                      uint32(0));
        data.Worldstates.emplace_back(
            uint32(BG_TP_FLAG_STATE_HORDE + team),
            uint32(1)); ///< Show if team's flag is carried
        break;
      }
    }
    ...
    data.Worldstates.emplace_back(uint32(BG_TP_STATE_TIMER),
                                  uint32(25 - _minutesElapsed));
  } else {
...
bool BattlegroundTP::UpdatePlayerScore(Player *player, uint32 type,
...
WorldSafeLocsEntry const *BattlegroundTP::GetClosestGraveyard(Player *player) {
  ...
  if (GetStatus() !=
      STATUS_IN_PROGRESS) ///< If battle didn't start yet and player is death
                          ///< (unprobably) revive in flagroom
    return sWorldSafeLocsStore.LookupEntry(
        BG_TP_GraveyardIds[TP_GRAVEYARD_FLAGROOM_ALLIANCE + team]);

  if (isRated()) {
    if ((player->GetAreaId() == 5680 && team == TEAM_HORDE) ||
        (player->GetAreaId() == 5681 && team == TEAM_ALLIANCE))
      return sWorldSafeLocsStore.LookupEntry(
          BG_TP_GraveyardIds[TP_GRAVEYARD_START_ALLIANCE + team]);
    else
      return sWorldSafeLocsStore.LookupEntry(
          BG_TP_GraveyardIds[TP_GRAVEYARD_MIDDLE_ALLIANCE + team]);
  } else {
    /// Check if player if is closer to the enemy base than the center
    WorldSafeLocsEntry const *grave_enemy_base =
        sWorldSafeLocsStore.LookupEntry(
            BG_TP_GraveyardIds[TP_GRAVEYARD_FLAGROOM_ALLIANCE + (team ^ 1)]);
    WorldSafeLocsEntry const *grave_enemy_middle =
        sWorldSafeLocsStore.LookupEntry(
            BG_TP_GraveyardIds[TP_GRAVEYARD_MIDDLE_ALLIANCE + (team ^ 1)]);

    if (player->GetDistance2d(grave_enemy_base->Loc.X,
                              grave_enemy_base->Loc.Y) <
        player->GetDistance2d(grave_enemy_middle->Loc.X,
                              grave_enemy_middle->Loc.Y))
      return sWorldSafeLocsStore.LookupEntry(
          BG_TP_GraveyardIds[TP_GRAVEYARD_START_ALLIANCE + team]);
    else
      return sWorldSafeLocsStore.LookupEntry(
...
void BattlegroundTP::EventPlayerDroppedFlag(Player *player) {
  if (GetStatus() != STATUS_IN_PROGRESS) {
    ...
    // spawning the "dropped" flag), neither send unnecessary messages just take
    // off the aura
  ...
  if (set) {
    ...
    if (player->GetTeam() == ALLIANCE) {
      SendBroadcastText(BG_TP_TEXT_HORDE_FLAG_DROPPED, CHAT_MSG_BG_SYSTEM_HORDE,
                        player);
      UpdateWorldState(BG_TP_FLAG_UNK_HORDE, uint32(-1));
    } else {
      SendBroadcastText(BG_TP_TEXT_ALLIANCE_FLAG_DROPPED,
                        CHAT_MSG_BG_SYSTEM_ALLIANCE, player);
      UpdateWorldState(BG_TP_FLAG_UNK_ALLIANCE, uint32(-1));
    }

    _flagsDropTimer[GetTeamIndexByTeamId(player->GetTeam()) ? 0 : 1] =
        BG_TP_FLAG_DROP_TIME;
  }
}

void BattlegroundTP::EventPlayerClickedOnFlag(Player *player,
  ...
  if (player->GetTeam() == HORDE &&
      GetFlagState(ALLIANCE) == BG_TP_FLAG_STATE_ON_BASE &&
      BgObjects[BG_TP_OBJECT_A_FLAG] == target_obj->GetGUID()) {
    SendBroadcastText(BG_TP_TEXT_ALLIANCE_FLAG_PICKED_UP,
  ...
  if (player->GetTeam() == ALLIANCE &&
      GetFlagState(HORDE) == BG_TP_FLAG_STATE_ON_BASE &&
      BgObjects[BG_TP_OBJECT_H_FLAG] == target_obj->GetGUID()) {
    SendBroadcastText(BG_TP_TEXT_HORDE_FLAG_PICKED_UP,
                      CHAT_MSG_BG_SYSTEM_ALLIANCE, player);
  ...
  // ground!)
  if (GetFlagState(ALLIANCE) == BG_TP_FLAG_STATE_ON_GROUND &&
      player->IsWithinDistInMap(target_obj, 10) &&
      target_obj->GetGOInfo()->entry == BG_OBJECT_A_FLAG_GROUND_TP_ENTRY) {
    if (player->GetTeam() == ALLIANCE) {
      SendBroadcastText(BG_TP_TEXT_ALLIANCE_FLAG_RETURNED,
                        CHAT_MSG_BG_SYSTEM_ALLIANCE, player);
  ...
  if (GetFlagState(HORDE) == BG_TP_FLAG_STATE_ON_GROUND &&
      player->IsWithinDistInMap(target_obj, 10) &&
      target_obj->GetGOInfo()->entry == BG_OBJECT_H_FLAG_GROUND_TP_ENTRY) {
    if (player->GetTeam() == HORDE) {
      SendBroadcastText(BG_TP_TEXT_HORDE_FLAG_RETURNED,
                        CHAT_MSG_BG_SYSTEM_HORDE, player);
...
void BattlegroundTP::EventPlayerCapturedFlag(Player *player) {
  ...
  if (player->GetTeam() == ALLIANCE) {
    if (!IsHordeFlagPickedup())
      return;
    SetHordeFlagPicker(
        ObjectGuid::Empty); // must be before aura remove to prevent 2 events
                            // (drop+capture) at the same time
  ...
  if (player->GetTeam() == ALLIANCE)
    SendBroadcastText(BG_TP_TEXT_CAPTURED_HORDE_FLAG,
                      CHAT_MSG_BG_SYSTEM_ALLIANCE, player);
  else
    SendBroadcastText(BG_TP_TEXT_CAPTURED_ALLIANCE_FLAG,
                      CHAT_MSG_BG_SYSTEM_HORDE, player);
  ...
  if (winner) {
    ...
    _flagsTimer[GetTeamIndexByTeamId(player->GetTeam()) ? 0 : 1] =
        BG_TP_FLAG_RESPAWN_TIME;
  }
}

void BattlegroundTP::RemovePlayer(Player *player, ObjectGuid guid,
                                  uint32 /* team */) {
  // sometimes flag aura not removed :(
  if (IsAllianceFlagPickedup() && m_FlagKeepers[TEAM_ALLIANCE] == guid) {
    if (!player) {
      LOG_ERROR("bg.battleground",
                "BattlegroundTP: Removing offline player who has the FLAG!!");
      SetAllianceFlagPicker(ObjectGuid::Empty);
      RespawnFlag(ALLIANCE, false);
    } else
      EventPlayerDroppedFlag(player);
...
void BattlegroundTP::UpdateFlagState(uint32 team, uint32 value) {
  ...
  case BG_TP_FLAG_STATE_WAIT_RESPAWN:
    UpdateWorldState(team == TEAM_ALLIANCE ? BG_TP_FLAG_UNK_ALLIANCE
                                           : BG_TP_FLAG_UNK_HORDE,
                     0);
    UpdateWorldState(team == TEAM_ALLIANCE ? BG_TP_FLAG_STATE_HORDE
...
void BattlegroundTP::RespawnFlagAfterDrop(uint32 team) {
  ...
  if (GameObject *obj = GetBgMap()->GetGameObject(GetDroppedFlagGUID(team)))
    ...
    LOG_ERROR("bg.battleground", "unknown dropped flag ({})",
              GetDroppedFlagGUID(team).ToString().c_str());
...
bool BattlegroundTP::CheckAchievementCriteriaMeet(uint32 criteriaId,
                                                  Player const *player,
                                                  Unit const *target,
                                                  uint32 miscValue) {
  switch (criteriaId) {
  case BG_CRITERIA_CHECK_SAVE_THE_DAY:
    if (target)
      if (Player const *playerTarget = target->ToPlayer())
        return GetFlagState(playerTarget->GetTeam()) ==
               BG_TP_FLAG_STATE_ON_BASE;
    return false;
  }

  return Battleground::CheckAchievementCriteriaMeet(criteriaId, player, target,
                                                    miscValue);
}
