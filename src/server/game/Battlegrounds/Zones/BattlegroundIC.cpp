    ...
    if (nodePoint[i].nodeType == NODE_TYPE_DOCKS) {
      if (nodePoint[i].nodeState == NODE_STATE_CONTROLLED_A ||
        ...
        if (docksTimer <= diff) {
          ...
          for (uint8 u = (nodePoint[i].faction == TEAM_ALLIANCE
                              ? BG_IC_NPC_CATAPULT_1_A
                              : BG_IC_NPC_CATAPULT_1_H);
               u < (nodePoint[i].faction == TEAM_ALLIANCE
    ...
    if (nodePoint[i].needChange) {
      if (nodePoint[i].timer <= diff) {
        ...
        GetBGObject(nodePoint[i].gameobject_type)
            ->SetFaction(nodePoint[i].faction == TEAM_ALLIANCE
                             ? BG_IC_Factions[1]
                             : BG_IC_Factions[0]);
  ...
  if (resourceTimer <= diff) {
    for (uint8 i = 0; i < NODE_TYPE_DOCKS; ++i) {
      if (nodePoint[i].nodeState == NODE_STATE_CONTROLLED_A ||
        ...
        RewardHonorToTeam(RESOURCE_HONOR_AMOUNT,
                          nodePoint[i].faction == TEAM_ALLIANCE ? ALLIANCE
                                                                : HORDE);
...
void BattlegroundIC::AddPlayer(Player *player) {
  ...
  PlayerScores[player->GetGUID().GetCounter()] =
      new BattlegroundICScore(player->GetGUID(), player->GetBGTeam());
...
void BattlegroundIC::RemovePlayer(Player *player, ObjectGuid /*guid*/,
                                  uint32 /*team*/) {
  if (player) {
...
void BattlegroundIC::HandleAreaTrigger(Player *player, uint32 trigger) {
  ...
  /// @hack: this spell should be cast by npc 22515 (World Trigger) and not by
  /// the player
...
void BattlegroundIC::FillInitialWorldStates(
  ...
  data.Worldstates.emplace_back(uint32(BG_IC_ALLIANCE_RENFORT),
                                uint32(factionReinforcements[TEAM_ALLIANCE]));
  data.Worldstates.emplace_back(uint32(BG_IC_HORDE_RENFORT),
  ...
  for (uint8 i = 0; i < MAX_NODE_TYPES; ++i)
    data.Worldstates.emplace_back(
        uint32(nodePoint[i].worldStates[nodePoint[i].nodeState]), uint32(1));
}
...
bool BattlegroundIC::SetupBattleground() {
  ...
  if (!AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 3, BG_IC_SpiritGuidePos[5],
                      TEAM_ALLIANCE) ||
      !AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 4, BG_IC_SpiritGuidePos[6],
                      TEAM_HORDE) ||
      !AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 5, BG_IC_SpiritGuidePos[7],
                      TEAM_ALLIANCE) ||
      !AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 6, BG_IC_SpiritGuidePos[8],
                      TEAM_HORDE)) {
    LOG_ERROR("bg.battleground",
  ...
  gunshipAlliance =
      sTransportMgr->CreateTransport(GO_ALLIANCE_GUNSHIP, GetBgMap());
    ...
    if (nodePoint[i].gameobject_entry == target_obj->GetEntry()) {
      ...
      if (nodePoint[i].banners[BANNER_A_CONTESTED] == nextBanner ||
        ...
        if (nodePoint[i].faction == TEAM_ALLIANCE)
          ...
          SendBroadcastText(ICNodes[i].TextAssaulted, CHAT_MSG_BG_SYSTEM_HORDE,
                            player);
        ...
        HandleContestedNodes(&nodePoint[i]);
                 ...
                 nextBanner ==
                     nodePoint[i].banners
                         [BANNER_H_CONTROLLED]) // if we are going to spawn the
                                                // definitve faction banner, we
                                                // dont need the timer anymore
      {
        nodePoint[i].timer = BANNER_STATE_CHANGE_TIME;
        nodePoint[i].needChange = false;
        ...
        if (nodePoint[i].faction == TEAM_ALLIANCE)
          ...
          SendBroadcastText(ICNodes[i].TextDefended, CHAT_MSG_BG_SYSTEM_HORDE,
                            player);
      ...
      DelObject(nodePoint[i].gameobject_type);
      if (!AddObject(nodePoint[i].gameobject_type,
                     nodePoint[i].gameobject_entry, cords[0], cords[1],
                     cords[2], cords[3], 0, 0, 0, 0, RESPAWN_ONE_DAY)) {
        LOG_ERROR("bg.battleground",
                  "Isle of Conquest: There was an error spawning a banner "
                  "(type: {}, entry: {}). Isle of Conquest BG cancelled.",
                  nodePoint[i].gameobject_type, nodePoint[i].gameobject_entry);
        EndBattleground(0);
      }
...
uint32 BattlegroundIC::GetNextBanner(ICNodePoint *node, uint32 team,
  ...
  if (returnDefinitve)
    ...
    return node->banners[(team == TEAM_ALLIANCE ? BANNER_A_CONTROLLED
                                                : BANNER_H_CONTROLLED)];
  ...
  if (node->last_entry == 0)
    ...
    return node->banners[(team == TEAM_ALLIANCE ? BANNER_A_CONTESTED
                                                : BANNER_H_CONTESTED)];

  // If the actual banner is the definitive faction banner, we must return the
  // grey banner of the player's faction
  if (node->gameobject_entry == node->banners[BANNER_A_CONTROLLED] ||
      node->gameobject_entry == node->banners[BANNER_H_CONTROLLED])
    return node->banners[(team == TEAM_ALLIANCE ? BANNER_A_CONTESTED
                                                : BANNER_H_CONTESTED)];

  // If the actual banner is the grey faction banner, we must return the
  // previous banner
  if (node->gameobject_entry == node->banners[BANNER_A_CONTESTED] ||
      node->banners[BANNER_H_CONTESTED])
...
void BattlegroundIC::HandleContestedNodes(ICNodePoint *node) {
  if (node->nodeType == NODE_TYPE_HANGAR) {
    if (gunshipAlliance && gunshipHorde)
      (node->faction == TEAM_ALLIANCE ? gunshipHorde : gunshipAlliance)
...
void BattlegroundIC::HandleCapturedNodes(ICNodePoint *node, bool recapture) {
  if (node->nodeType != NODE_TYPE_REFINERY &&
      node->nodeType != NODE_TYPE_QUARRY) {
    if (!AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + node->nodeType - 2,
                        BG_IC_SpiritGuidePos[node->nodeType], node->faction))
      LOG_ERROR("bg.battleground",
                "Isle of Conquest: Failed to spawn spirit guide! point: {}, "
                "team: {}, ",
                node->nodeType, node->faction);
  }
  ...
  case BG_IC_GO_HANGAR_BANNER: {
    ...
    for (uint8 u = 0; u < MAX_HANGAR_TELEPORTER_EFFECTS_SPAWNS; ++u) {
      ...
      if (!AddObject(type,
                     ...
                     BG_IC_HangarTeleporterEffects[u], 0, 0, 0, 0,
                     RESPAWN_ONE_DAY, GO_STATE_ACTIVE))
        LOG_ERROR("bg.battleground",
                  "Isle of Conquest: There was an error spawning a gunship "
                  "portal effects. Type: {}",
                  BG_IC_GO_HANGAR_TELEPORTER_1 + u);
    ...
    for (uint8 u = 0; u < MAX_TRIGGER_SPAWNS_PER_FACTION; ++u) {
      if (!AddCreature(NPC_WORLD_TRIGGER_NOT_FLOATING,
                       ...
                       BG_IC_HangarTrigger[node->faction], node->faction,
                       RESPAWN_ONE_DAY,
                       node->faction == TEAM_ALLIANCE ? gunshipAlliance
                                                      : gunshipHorde))
        LOG_ERROR("bg.battleground",
    ...
    for (uint8 u = 0; u < MAX_CAPTAIN_SPAWNS_PER_FACTION; ++u) {
      ...
      if (type == BG_IC_NPC_GUNSHIP_CAPTAIN_1)
        if (AddCreature(
                node->faction == TEAM_ALLIANCE ? NPC_ALLIANCE_GUNSHIP_CAPTAIN
                                               : NPC_HORDE_GUNSHIP_CAPTAIN,
                type,
                BG_IC_HangarCaptains[node->faction == TEAM_ALLIANCE ? 2 : 0],
                node->faction, RESPAWN_ONE_DAY))
      ...
      if (type == BG_IC_NPC_GUNSHIP_CAPTAIN_2)
        if (!AddCreature(
                node->faction == TEAM_ALLIANCE ? NPC_ALLIANCE_GUNSHIP_CAPTAIN
                                               : NPC_HORDE_GUNSHIP_CAPTAIN,
                type,
                BG_IC_HangarCaptains[node->faction == TEAM_ALLIANCE ? 3 : 1],
                node->faction, RESPAWN_ONE_DAY,
                node->faction == TEAM_ALLIANCE ? gunshipAlliance
                                               : gunshipHorde))
          LOG_ERROR("bg.battleground",
    ...
    (node->faction == TEAM_ALLIANCE ? gunshipAlliance : gunshipHorde)
        ->EnableMovement(true);
    break;
  }
  case BG_IC_GO_QUARRY_BANNER:
    RemoveAuraOnTeam(SPELL_QUARRY,
                     (node->faction == TEAM_ALLIANCE ? HORDE : ALLIANCE));
    CastSpellOnTeam(SPELL_QUARRY,
                    (node->faction == TEAM_ALLIANCE ? ALLIANCE : HORDE));
    break;
  case BG_IC_GO_REFINERY_BANNER:
    RemoveAuraOnTeam(SPELL_OIL_REFINERY,
                     (node->faction == TEAM_ALLIANCE ? HORDE : ALLIANCE));
    CastSpellOnTeam(SPELL_OIL_REFINERY,
                    (node->faction == TEAM_ALLIANCE ? ALLIANCE : HORDE));
    break;
  case BG_IC_GO_DOCKS_BANNER:
    ...
    // we must del opposing faction vehicles when the node is captured (unused
    // ones)
    for (uint8 i =
             (node->faction == TEAM_ALLIANCE ? BG_IC_NPC_GLAIVE_THROWER_1_H
                                             : BG_IC_NPC_GLAIVE_THROWER_1_A);
         i < (node->faction == TEAM_ALLIANCE ? BG_IC_NPC_GLAIVE_THROWER_2_H
                                             : BG_IC_NPC_GLAIVE_THROWER_2_A);
         ++i) {
    ...
    for (uint8 i = (node->faction == TEAM_ALLIANCE ? BG_IC_NPC_CATAPULT_1_H
                                                   : BG_IC_NPC_CATAPULT_1_A);
         i < (node->faction == TEAM_ALLIANCE ? BG_IC_NPC_CATAPULT_4_H
                                             : BG_IC_NPC_CATAPULT_4_A);
         ++i) {
    ...
    for (uint8 i = 0; i < MAX_GLAIVE_THROWERS_SPAWNS_PER_FACTION; ++i) {
      uint8 type =
          (node->faction == TEAM_ALLIANCE ? BG_IC_NPC_GLAIVE_THROWER_1_A
                                          : BG_IC_NPC_GLAIVE_THROWER_1_H) +
          i;
      ...
      if (AddCreature(node->faction == TEAM_ALLIANCE ? NPC_GLAIVE_THROWER_A
                                                     : NPC_GLAIVE_THROWER_H,
                      type, BG_IC_DocksVehiclesGlaives[i], node->faction,
                      RESPAWN_ONE_DAY))
        GetBGCreature(type)->SetFaction(
            BG_IC_Factions[(node->faction == TEAM_ALLIANCE ? 0 : 1)]);
    }

    // spawning catapults
    for (uint8 i = 0; i < MAX_CATAPULTS_SPAWNS_PER_FACTION; ++i) {
      uint8 type = (node->faction == TEAM_ALLIANCE ? BG_IC_NPC_CATAPULT_1_A
                                                   : BG_IC_NPC_CATAPULT_1_H) +
                   i;
      ...
      if (AddCreature(NPC_CATAPULT, type, BG_IC_DocksVehiclesCatapults[i],
                      node->faction, RESPAWN_ONE_DAY))
        GetBGCreature(type)->SetFaction(
            BG_IC_Factions[(node->faction == TEAM_ALLIANCE ? 0 : 1)]);
    }
  ...
  case BG_IC_GO_WORKSHOP_BANNER: {
    ...
    if (!recapture) {
      ...
      // ones)
      for (uint8 i =
               (node->faction == TEAM_ALLIANCE ? BG_IC_NPC_DEMOLISHER_1_H
                                               : BG_IC_NPC_DEMOLISHER_1_A);
           i < (node->faction == TEAM_ALLIANCE ? BG_IC_NPC_DEMOLISHER_4_H
                                               : BG_IC_NPC_DEMOLISHER_4_A);
           ++i) {
      ...
      for (uint8 i = 0; i < MAX_DEMOLISHERS_SPAWNS_PER_FACTION; ++i) {
        uint8 type =
            (node->faction == TEAM_ALLIANCE ? BG_IC_NPC_DEMOLISHER_1_A
                                            : BG_IC_NPC_DEMOLISHER_1_H) +
            i;
        ...
        if (AddCreature(NPC_DEMOLISHER, type, BG_IC_WorkshopVehicles[i],
                        node->faction, RESPAWN_ONE_DAY))
          GetBGCreature(type)->SetFaction(
              BG_IC_Factions[(node->faction == TEAM_ALLIANCE ? 0 : 1)]);
      }
      ...
      int8 enemySiege =
          (node->faction == TEAM_ALLIANCE ? BG_IC_NPC_SIEGE_ENGINE_H
                                          : BG_IC_NPC_SIEGE_ENGINE_A);
      ...
      uint8 siegeType =
          (node->faction == TEAM_ALLIANCE ? BG_IC_NPC_SIEGE_ENGINE_A
                                          : BG_IC_NPC_SIEGE_ENGINE_H);
      if (!GetBGCreature(siegeType, false) ||
          !GetBGCreature(siegeType)->IsAlive()) {
        AddCreature((node->faction == TEAM_ALLIANCE ? NPC_SIEGE_ENGINE_A
                                                    : NPC_SIEGE_ENGINE_H),
                    siegeType, BG_IC_WorkshopVehicles[4], node->faction,
                    RESPAWN_ONE_DAY);

        if (Creature *siegeEngine = GetBGCreature(siegeType)) {
          siegeEngine->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE |
                                                     UNIT_FLAG_CANNOT_SWIM);
...
void BattlegroundIC::DestroyGate(Player *player, GameObject *go) {
  ...
  if (go->GetEntry() == GO_HORDE_GATE_1 || go->GetEntry() == GO_HORDE_GATE_2 ||
      go->GetEntry() == GO_HORDE_GATE_3) {
    if (!GetBgMap()->GetCreature(
            BgCreatures[BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].type]) &&
        !AddCreature(BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].entry,
                     BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].type,
                     BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].x,
                     BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].y,
                     BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].z,
                     BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].o,
                     BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].team,
                     RESPAWN_ONE_DAY))
      LOG_ERROR("bg.battleground",
                "Isle of Conquest: There was an error spawning creature {}",
                BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].entry);
  } else if (go->GetEntry() == GO_ALLIANCE_GATE_1 ||
             go->GetEntry() == GO_ALLIANCE_GATE_2 ||
             go->GetEntry() == GO_ALLIANCE_GATE_3) {
    if (!GetBgMap()->GetCreature(
            BgCreatures
                [BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE]
                     .type]) &&
        !AddCreature(
            BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE].entry,
            BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE].type,
            BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE].x,
            BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE].y,
            BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE].z,
            BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE].o,
            BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE].team,
            RESPAWN_ONE_DAY))
      LOG_ERROR(
          "bg.battleground",
          "Isle of Conquest: There was an error spawning creature {}",
...
WorldSafeLocsEntry const *BattlegroundIC::GetClosestGraveyard(Player *player) {
  ...
  if (!nodes.empty()) {
    ...
    for (uint8 i = 0; i < nodes.size(); ++i) {
      WorldSafeLocsEntry const *entry =
          sWorldSafeLocsStore.LookupEntry(BG_IC_GraveyardIds[nodes[i]]);
      if (!entry)
        continue;
      float dist = (entry->Loc.X - player_x) * (entry->Loc.X - player_x) +
                   (entry->Loc.Y - player_y) * (entry->Loc.Y - player_y);
      if (mindist > dist) {
        mindist = dist;
        good_entry = entry;
      }
  ...
  if (!good_entry)
    good_entry = sWorldSafeLocsStore.LookupEntry(
        BG_IC_GraveyardIds[teamIndex + MAX_NODE_TYPES]);
...
bool BattlegroundIC::IsAllNodesControlledByTeam(uint32 team) const {
  uint32 count = 0;
  ICNodeState controlledState =
      team == ALLIANCE ? NODE_STATE_CONTROLLED_A : NODE_STATE_CONTROLLED_H;
...
bool BattlegroundIC::IsSpellAllowed(uint32 spellId,
  ...
  case SPELL_QUARRY: {
    ...
    uint8 nodeType =
        spellId == SPELL_OIL_REFINERY ? NODE_TYPE_REFINERY : NODE_TYPE_QUARRY;
    uint8 nodeState = team == TEAM_ALLIANCE ? NODE_STATE_CONTROLLED_A
                                            : NODE_STATE_CONTROLLED_H;
