    ...
    for (int team = 0; team < BG_TEAMS_COUNT; ++team) {
      ...
      if (m_lastTick[team] > BG_AB_TickIntervals[points]) {
        ...
        if (m_ReputationScoreTics[team] >= m_ReputationTics) {
          (team == TEAM_ALLIANCE) ? RewardReputationToTeam(509, 10, ALLIANCE)
...
void BattlegroundAB::AddPlayer(Player *player) {
  ...
  PlayerScores[player->GetGUID().GetCounter()] =
      new BattlegroundABScore(player->GetGUID(), player->GetBGTeam());
}
...
void BattlegroundAB::FillInitialWorldStates(
  ...
  for (uint8 node = 0; node < BG_AB_DYNAMIC_NODES_COUNT; ++node)
    data.Worldstates.emplace_back(BG_AB_OP_NODEICONS[node],
  ...
  for (uint8 node = 0; node < BG_AB_DYNAMIC_NODES_COUNT; ++node)
    for (uint8 i = 1; i < BG_AB_DYNAMIC_NODES_COUNT; ++i)
      data.Worldstates.emplace_back(
          uint32(BG_AB_OP_NODESTATES[node] + plusArray[i]),
  ...
  data.Worldstates.emplace_back(uint32(BG_AB_OP_OCCUPIED_BASES_ALLY),
                                uint32(ally));
  data.Worldstates.emplace_back(uint32(BG_AB_OP_OCCUPIED_BASES_HORDE),
                                uint32(horde));

  // Team scores
  data.Worldstates.emplace_back(uint32(BG_AB_OP_RESOURCES_MAX),
                                uint32(BG_AB_MAX_TEAM_SCORE));
  data.Worldstates.emplace_back(uint32(BG_AB_OP_RESOURCES_WARNING),
                                uint32(BG_AB_WARNING_NEAR_VICTORY_SCORE));
  data.Worldstates.emplace_back(uint32(BG_AB_OP_RESOURCES_ALLY),
                                uint32(m_TeamScores[TEAM_ALLIANCE]));
  data.Worldstates.emplace_back(uint32(BG_AB_OP_RESOURCES_HORDE),
...
void BattlegroundAB::_SendNodeUpdate(uint8 node) {
  ...
  if (m_prevNodes[node])
    UpdateWorldState(BG_AB_OP_NODESTATES[node] + plusArray[m_prevNodes[node]],
                     0);
  else
...
void BattlegroundAB::_NodeOccupied(uint8 node, Team team) {
  if (!AddSpiritGuide(node, BG_AB_SpiritGuidePos[node],
  ...
  for (uint8 i = 0; i < BG_AB_DYNAMIC_NODES_COUNT; ++i)
    if (m_Nodes[i] == GetTeamIndexByTeamId(team) + BG_AB_NODE_TYPE_OCCUPIED &&
        !m_NodeTimers[i])
      ++capturedNodes;
  ...
  Creature *trigger = !BgCreatures[node + 7] ? GetBGCreature(node + 7)
                                             : nullptr; // 0-6 spirit guides
  if (!trigger)
    trigger = AddCreature(WORLD_TRIGGER, node + 7, BG_AB_NodePositions[node],
                          GetTeamIndexByTeamId(team));
...
void BattlegroundAB::EventPlayerClickedOnFlag(Player *source,
  ...
  while ((node < BG_AB_DYNAMIC_NODES_COUNT) &&
    ...
    obj = GetBgMap()->GetGameObject(
        BgObjects[node * 8 + BG_AB_OBJECT_AURA_CONTESTED]);
  }
  ...
  if (m_Nodes[node] == BG_AB_NODE_TYPE_NEUTRAL) {
    ...
    if (teamIndex == TEAM_ALLIANCE)
      ...
      SendBroadcastText(ABNodes[node].TextHordeClaims, CHAT_MSG_BG_SYSTEM_HORDE,
                        source);
  ...
  // If node is contested
    ...
    sound = (teamIndex == TEAM_ALLIANCE) ? BG_AB_SOUND_NODE_ASSAULTED_ALLIANCE
...
bool BattlegroundAB::SetupBattleground() {
  for (int i = 0; i < BG_AB_DYNAMIC_NODES_COUNT; ++i) {
    if (!AddObject(BG_AB_OBJECT_BANNER_NEUTRAL + 8 * i,
                   ...
                   0, std::sin(BG_AB_NodePositions[i].GetOrientation() / 2),
                   std::cos(BG_AB_NodePositions[i].GetOrientation() / 2),
                   RESPAWN_ONE_DAY) ||
        !AddObject(BG_AB_OBJECT_BANNER_CONT_A + 8 * i,
                   BG_AB_OBJECTID_BANNER_CONT_A, BG_AB_NodePositions[i], 0, 0,
                   std::sin(BG_AB_NodePositions[i].GetOrientation() / 2),
                   std::cos(BG_AB_NodePositions[i].GetOrientation() / 2),
                   RESPAWN_ONE_DAY) ||
        !AddObject(BG_AB_OBJECT_BANNER_CONT_H + 8 * i,
                   BG_AB_OBJECTID_BANNER_CONT_H, BG_AB_NodePositions[i], 0, 0,
                   std::sin(BG_AB_NodePositions[i].GetOrientation() / 2),
                   std::cos(BG_AB_NodePositions[i].GetOrientation() / 2),
                   RESPAWN_ONE_DAY) ||
        !AddObject(BG_AB_OBJECT_BANNER_ALLY + 8 * i, BG_AB_OBJECTID_BANNER_A,
                   BG_AB_NodePositions[i], 0, 0,
                   std::sin(BG_AB_NodePositions[i].GetOrientation() / 2),
                   std::cos(BG_AB_NodePositions[i].GetOrientation() / 2),
                   RESPAWN_ONE_DAY) ||
        !AddObject(BG_AB_OBJECT_BANNER_HORDE + 8 * i, BG_AB_OBJECTID_BANNER_H,
                   BG_AB_NodePositions[i], 0, 0,
                   std::sin(BG_AB_NodePositions[i].GetOrientation() / 2),
                   std::cos(BG_AB_NodePositions[i].GetOrientation() / 2),
                   RESPAWN_ONE_DAY) ||
        !AddObject(BG_AB_OBJECT_AURA_ALLY + 8 * i, BG_AB_OBJECTID_AURA_A,
                   BG_AB_NodePositions[i], 0, 0,
                   std::sin(BG_AB_NodePositions[i].GetOrientation() / 2),
                   std::cos(BG_AB_NodePositions[i].GetOrientation() / 2),
                   RESPAWN_ONE_DAY) ||
        !AddObject(BG_AB_OBJECT_AURA_HORDE + 8 * i, BG_AB_OBJECTID_AURA_H,
                   BG_AB_NodePositions[i], 0, 0,
                   std::sin(BG_AB_NodePositions[i].GetOrientation() / 2),
                   std::cos(BG_AB_NodePositions[i].GetOrientation() / 2),
                   RESPAWN_ONE_DAY) ||
        !AddObject(BG_AB_OBJECT_AURA_CONTESTED + 8 * i, BG_AB_OBJECTID_AURA_C,
                   BG_AB_NodePositions[i], 0, 0,
                   std::sin(BG_AB_NodePositions[i].GetOrientation() / 2),
                   std::cos(BG_AB_NodePositions[i].GetOrientation() / 2),
                   RESPAWN_ONE_DAY)) {
  ...
  for (int i = 0; i < BG_AB_DYNAMIC_NODES_COUNT; ++i) {
    if (!AddObject(BG_AB_OBJECT_SPEEDBUFF_STABLES + 3 * i, Buff_Entries[0],
                   ...
                   std::cos(BG_AB_BuffPositions[i][3] / 2), RESPAWN_ONE_DAY) ||
...
bool BattlegroundAB::UpdatePlayerScore(Player *player, uint32 type,
