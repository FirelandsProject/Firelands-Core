...
void BattlegroundWS::PostUpdateImpl(uint32 diff) {
  if (GetStatus() == STATUS_IN_PROGRESS) {
      ...
      if (GetTeamScore(TEAM_ALLIANCE) == 0) {
        ...
        EndBattleground(
            ALLIANCE); // Alliance has > 0, Horde has 0, alliance wins
               ...
               GetTeamScore(TEAM_ALLIANCE)) // Team score equal, winner is team
                                            // that scored the last flag
        EndBattleground(_lastFlagCaptureTeam);
      else if (GetTeamScore(TEAM_HORDE) >
               GetTeamScore(TEAM_ALLIANCE)) // Last but not least, check who has
                                            // the higher score
        EndBattleground(HORDE);
      else
    ...
    else if (GetElapsedTime() >
             uint32(_minutesElapsed * MINUTE * IN_MILLISECONDS) +
...
void BattlegroundWS::AddPlayer(Player *player) {
  ...
  PlayerScores[player->GetGUID().GetCounter()] =
      new BattlegroundWGScore(player->GetGUID(), player->GetBGTeam());
}
...
void BattlegroundWS::EventPlayerDroppedFlag(Player *player) {
  ...
  if (set) {
    ...
    if (player->GetTeam() == ALLIANCE) {
      SendBroadcastText(BG_WS_TEXT_HORDE_FLAG_DROPPED, CHAT_MSG_BG_SYSTEM_HORDE,
                        player);
...
bool BattlegroundWS::SetupBattleground() {
  // flags
  if (!AddObject(BG_WS_OBJECT_A_FLAG, BG_OBJECT_A_FLAG_WS_ENTRY, 1540.423f,
      ...
      !AddObject(BG_WS_OBJECT_H_FLAG, BG_OBJECT_H_FLAG_WS_ENTRY, 916.0226f,
                 1434.405f, 345.413f, 0.01745329f, 0, 0, 0.008726535f,
                 0.9999619f, BG_WS_FLAG_RESPAWN_TIME / 1000)
      // buffs
      || !AddObject(BG_WS_OBJECT_SPEEDBUFF_1, BG_OBJECTID_SPEEDBUFF_ENTRY,
                    1449.93f, 1470.71f, 342.6346f, -1.64061f, 0, 0, 0.7313537f,
                    -0.6819983f, BUFF_RESPAWN_TIME) ||
      !AddObject(BG_WS_OBJECT_SPEEDBUFF_2, BG_OBJECTID_SPEEDBUFF_ENTRY,
                 1005.171f, 1447.946f, 335.9032f, 1.64061f, 0, 0, 0.7313537f,
                 0.6819984f, BUFF_RESPAWN_TIME) ||
      !AddObject(BG_WS_OBJECT_REGENBUFF_1, BG_OBJECTID_REGENBUFF_ENTRY,
                 1317.506f, 1550.851f, 313.2344f, -0.2617996f, 0, 0, 0.1305263f,
                 -0.9914448f, BUFF_RESPAWN_TIME) ||
      !AddObject(BG_WS_OBJECT_REGENBUFF_2, BG_OBJECTID_REGENBUFF_ENTRY,
                 1110.451f, 1353.656f, 316.5181f, -0.6806787f, 0, 0, 0.333807f,
                 -0.9426414f, BUFF_RESPAWN_TIME) ||
      ...
      // alliance gates
      || !AddObject(BG_WS_OBJECT_DOOR_A_1, BG_OBJECT_DOOR_A_1_WS_ENTRY,
                    1503.335f, 1493.466f, 352.1888f, 3.115414f, 0, 0,
      ...
      !AddObject(BG_WS_OBJECT_DOOR_A_5, BG_OBJECT_DOOR_A_5_WS_ENTRY, 1492.347f,
                 1458.34f, 342.3712f, -0.03490669f, 0, 0, 0.01745246f,
                 -0.9998477f, RESPAWN_IMMEDIATELY) ||
      !AddObject(BG_WS_OBJECT_DOOR_A_6, BG_OBJECT_DOOR_A_6_WS_ENTRY, 1503.466f,
                 1493.367f, 351.7352f, -0.03490669f, 0, 0, 0.01745246f,
                 -0.9998477f, RESPAWN_IMMEDIATELY)
      // horde gates
      ||
      ...
      !AddObject(BG_WS_OBJECT_DOOR_H_4, BG_OBJECT_DOOR_H_4_WS_ENTRY, 950.7952f,
                 1459.583f, 342.1523f, 0.05235988f, 0, 0, 0.02617695f,
                 0.9996573f, RESPAWN_IMMEDIATELY)) {
    LOG_ERROR(
        "sql.sql",
  ...
  WorldSafeLocsEntry const *sg =
      sWorldSafeLocsStore.LookupEntry(WS_GRAVEYARD_MAIN_ALLIANCE);
  if (!sg || !AddSpiritGuide(WS_SPIRIT_MAIN_ALLIANCE, sg->Loc.X, sg->Loc.Y,
                             sg->Loc.Z, 3.124139f, TEAM_ALLIANCE)) {
    LOG_ERROR("sql.sql", "BatteGroundWS: Failed to spawn Alliance spirit "
                         "guide! Battleground not created!");
    return false;
  }

  sg = sWorldSafeLocsStore.LookupEntry(WS_GRAVEYARD_MAIN_HORDE);
  if (!sg || !AddSpiritGuide(WS_SPIRIT_MAIN_HORDE, sg->Loc.X, sg->Loc.Y,
                             sg->Loc.Z, 3.193953f, TEAM_HORDE)) {
    LOG_ERROR("sql.sql", "BatteGroundWS: Failed to spawn Horde spirit guide! "
...
bool BattlegroundWS::UpdatePlayerScore(Player *player, uint32 type,
...
void BattlegroundWS::FillInitialWorldStates(
  ...
  data.Worldstates.emplace_back(uint32(BG_WS_FLAG_CAPTURES_ALLIANCE),
                                uint32(GetTeamScore(TEAM_ALLIANCE)));
  data.Worldstates.emplace_back(uint32(BG_WS_FLAG_CAPTURES_HORDE),
  ...
  data.Worldstates.emplace_back(uint32(BG_WS_FLAG_CAPTURES_MAX),
                                uint32(BG_WS_MAX_TEAM_SCORE));

  if (GetStatus() == STATUS_IN_PROGRESS) {
    data.Worldstates.emplace_back(uint32(BG_WS_STATE_TIMER_ACTIVE), uint32(1));
    data.Worldstates.emplace_back(uint32(BG_WS_STATE_TIMER),
