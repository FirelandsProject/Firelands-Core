  ...
  if (GetStatus() == STATUS_IN_PROGRESS) {
    ...
    if (m_PointAddingTimer <= 0) {
      ...
      if (m_TeamPointsCount[TEAM_ALLIANCE] > 0)
        AddPoints(ALLIANCE,
                  BG_EY_TickPoints[m_TeamPointsCount[TEAM_ALLIANCE] - 1]);
    ...
    if (m_TowerCapCheckTimer <= 0) {
      ...
      /*I used this order of calls, because although we will check if one player
        is in gameobject's distance 2 times but we can count of players on
        current point in CheckSomeoneLeftPoint
      */
...
void BattlegroundEY::StartingEventOpenDoors() {
  ...
  for (uint32 i = 0; i < EY_POINTS_MAX; ++i) {
    ...
    SpawnBGObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + buff + i * 3,
                  RESPAWN_IMMEDIATELY);
  }
...
void BattlegroundEY::CheckSomeoneJoinedPoint() {
  ...
  for (uint8 i = 0; i < EY_POINTS_MAX; ++i) {
    obj = GetBgMap()->GetGameObject(
        BgObjects[BG_EY_OBJECT_TOWER_CAP_FEL_REAVER + i]);

    if (obj) {
      uint8 j = 0;
      while (j < m_PlayersNearPoint[EY_POINTS_MAX].size()) {
        Player *player =
            ObjectAccessor::FindPlayer(m_PlayersNearPoint[EY_POINTS_MAX][j]);
        if (!player) {
          LOG_ERROR("bg.battleground",
                    "BattlegroundEY:CheckSomeoneJoinedPoint: Player ({}) could "
                    "not be found!",
                    m_PlayersNearPoint[EY_POINTS_MAX][j].ToString().c_str());
          ++j;
          continue;
        }
        if (player->CanCaptureTowerPoint() &&
            player->IsWithinDistInMap(obj, BG_EY_POINT_RADIUS)) {
          // player joined point!
          // show progress bar
          UpdateWorldStateForPlayer(PROGRESS_BAR_PERCENT_GREY,
                                    BG_EY_PROGRESS_BAR_PERCENT_GREY, player);
          UpdateWorldStateForPlayer(PROGRESS_BAR_STATUS, m_PointBarStatus[i],
                                    player);
          ...
          m_PlayersNearPoint[EY_POINTS_MAX].erase(
              m_PlayersNearPoint[EY_POINTS_MAX].begin() + j);
        } else
          ++j;
      }
    }
  ...
  for (uint8 i = 0; i < EY_POINTS_MAX; ++i) {
    ...
    if (obj) {
      ...
      while (j < m_PlayersNearPoint[i].size()) {
        ...
        if (!player) {
          LOG_ERROR("bg.battleground",
                    "BattlegroundEY:CheckSomeoneLeftPoint Player ({}) could "
                    "not be found!",
                    m_PlayersNearPoint[i][j].ToString().c_str());
          // move non-existing players to "free space" - this will cause many
          // errors showing in log, but it is a very important bug
        ...
        // move player out of point (add him to players that are out of points
        {
          m_PlayersNearPoint[EY_POINTS_MAX].push_back(m_PlayersNearPoint[i][j]);
          m_PlayersNearPoint[i].erase(m_PlayersNearPoint[i].begin() + j);
          this->UpdateWorldStateForPlayer(PROGRESS_BAR_SHOW,
                                          BG_EY_PROGRESS_BAR_DONT_SHOW, player);
        } else {
          // player is neat flag, so update count:
          m_CurrentPointPlayersCount[2 * i +
                                     GetTeamIndexByTeamId(player->GetTeam())]++;
          ++j;
        }
      }
    }
...
void BattlegroundEY::UpdatePointStatuses() {
  for (uint8 point = 0; point < EY_POINTS_MAX; ++point) {
    ...
    for (uint8 i = 0; i < m_PlayersNearPoint[point].size(); ++i) {
      ...
      if (player) {
        this->UpdateWorldStateForPlayer(PROGRESS_BAR_STATUS,
                                        m_PointBarStatus[point], player);
        // if point owner changed we must evoke event!
        if (pointOwnerTeamId != m_PointOwnedByTeam[point]) {
          // point was uncontrolled and player is from team which captured point
          if (m_PointState[point] == EY_POINT_STATE_UNCONTROLLED &&
              player->GetTeam() == pointOwnerTeamId)
            this->EventTeamCapturedPoint(player, point);

          // point was under control and player isn't from team which controlled
          // it
          if (m_PointState[point] == EY_POINT_UNDER_CONTROL &&
              player->GetTeam() != m_PointOwnedByTeam[point])
            this->EventTeamLostPoint(player, point);
        }

        /// @workaround The original AreaTrigger is covered by a bigger one and
        /// not triggered on client side.
        if (point == FEL_REAVER &&
            m_PointOwnedByTeam[point] == player->GetTeam())
...
void BattlegroundEY::UpdatePointsIcons(uint32 Team, uint32 Point) {
  ...
  if (m_PointState[Point] == EY_POINT_UNDER_CONTROL) {
    ...
    if (Team == ALLIANCE)
      UpdateWorldState(
          m_PointsIconStruct[Point].WorldStateAllianceControlledIndex, 1);
    else
...
void BattlegroundEY::AddPlayer(Player *player) {
  ...
  PlayerScores[player->GetGUID().GetCounter()] =
      new BattlegroundEYScore(player->GetGUID(), player->GetBGTeam());
...
void BattlegroundEY::HandleAreaTrigger(Player *player, uint32 trigger) {
  ...
  case TR_BLOOD_ELF_POINT:
    if (m_PointState[BLOOD_ELF] == EY_POINT_UNDER_CONTROL &&
        m_PointOwnedByTeam[BLOOD_ELF] == player->GetTeam())
      if (m_FlagState && GetFlagPickerGUID() == player->GetGUID())
...
bool BattlegroundEY::SetupBattleground() {
  // doors
  if (!AddObject(BG_EY_OBJECT_DOOR_A, BG_OBJECT_A_DOOR_EY_ENTRY, 2527.6f,
                 1596.91f, 1262.13f, -3.12414f, -0.173642f, -0.001515f,
                 0.98477f, -0.008594f, RESPAWN_IMMEDIATELY) ||
      !AddObject(BG_EY_OBJECT_DOOR_H, BG_OBJECT_H_DOOR_EY_ENTRY, 1803.21f,
                 1539.49f, 1261.09f, 3.14159f, 0.173648f, 0, 0.984808f, 0,
                 RESPAWN_IMMEDIATELY)
      // banners (alliance)
      ...
      !AddObject(BG_EY_OBJECT_A_BANNER_FEL_REAVER_LEFT,
                 ...
                 1.8675f, 0, 0, 0.803857f, 0.594823f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_A_BANNER_FEL_REAVER_RIGHT,
                 BG_OBJECT_A_BANNER_EY_ENTRY, 2092.35f, 1775.46f, 1187.08f,
                 -0.401426f, 0, 0, 0.199368f, -0.979925f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_A_BANNER_BLOOD_ELF_CENTER,
                 BG_OBJECT_A_BANNER_EY_ENTRY, 2047.19f, 1349.19f, 1189.0f,
                 -1.62316f, 0, 0, 0.725374f, -0.688354f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_A_BANNER_BLOOD_ELF_LEFT,
                 BG_OBJECT_A_BANNER_EY_ENTRY, 2074.32f, 1385.78f, 1194.72f,
                 0.488692f, 0, 0, 0.241922f, 0.970296f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_A_BANNER_BLOOD_ELF_RIGHT,
                 BG_OBJECT_A_BANNER_EY_ENTRY, 2025.13f, 1386.12f, 1192.74f,
                 2.3911f, 0, 0, 0.930418f, 0.366501f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_A_BANNER_DRAENEI_RUINS_CENTER,
                 BG_OBJECT_A_BANNER_EY_ENTRY, 2276.8f, 1400.41f, 1196.33f,
                 2.44346f, 0, 0, 0.939693f, 0.34202f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_A_BANNER_DRAENEI_RUINS_LEFT,
                 BG_OBJECT_A_BANNER_EY_ENTRY, 2305.78f, 1404.56f, 1199.38f,
                 1.74533f, 0, 0, 0.766044f, 0.642788f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_A_BANNER_DRAENEI_RUINS_RIGHT,
                 BG_OBJECT_A_BANNER_EY_ENTRY, 2245.4f, 1366.41f, 1195.28f,
      ...
      !AddObject(BG_EY_OBJECT_A_BANNER_MAGE_TOWER_LEFT,
                 BG_OBJECT_A_BANNER_EY_ENTRY, 2269.13f, 1737.7f, 1186.66f,
                 0.994838f, 0, 0, 0.477159f, 0.878817f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_A_BANNER_MAGE_TOWER_RIGHT,
                 BG_OBJECT_A_BANNER_EY_ENTRY, 2300.86f, 1741.25f, 1187.7f,
      ...
      !AddObject(BG_EY_OBJECT_H_BANNER_FEL_REAVER_RIGHT,
                 BG_OBJECT_H_BANNER_EY_ENTRY, 2092.35f, 1775.46f, 1187.08f,
                 -0.401426f, 0, 0, 0.199368f, -0.979925f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_H_BANNER_BLOOD_ELF_CENTER,
                 BG_OBJECT_H_BANNER_EY_ENTRY, 2047.19f, 1349.19f, 1189.0f,
                 -1.62316f, 0, 0, 0.725374f, -0.688354f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_H_BANNER_BLOOD_ELF_LEFT,
                 BG_OBJECT_H_BANNER_EY_ENTRY, 2074.32f, 1385.78f, 1194.72f,
                 0.488692f, 0, 0, 0.241922f, 0.970296f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_H_BANNER_BLOOD_ELF_RIGHT,
                 BG_OBJECT_H_BANNER_EY_ENTRY, 2025.13f, 1386.12f, 1192.74f,
                 2.3911f, 0, 0, 0.930418f, 0.366501f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_H_BANNER_DRAENEI_RUINS_CENTER,
                 BG_OBJECT_H_BANNER_EY_ENTRY, 2276.8f, 1400.41f, 1196.33f,
                 2.44346f, 0, 0, 0.939693f, 0.34202f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_H_BANNER_DRAENEI_RUINS_LEFT,
                 BG_OBJECT_H_BANNER_EY_ENTRY, 2305.78f, 1404.56f, 1199.38f,
                 1.74533f, 0, 0, 0.766044f, 0.642788f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_H_BANNER_DRAENEI_RUINS_RIGHT,
                 BG_OBJECT_H_BANNER_EY_ENTRY, 2245.4f, 1366.41f, 1195.28f,
                 2.21657f, 0, 0, 0.894934f, 0.446198f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_H_BANNER_MAGE_TOWER_CENTER,
                 BG_OBJECT_H_BANNER_EY_ENTRY, 2270.84f, 1784.08f, 1186.76f,
                 2.42601f, 0, 0, 0.936672f, 0.350207f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_H_BANNER_MAGE_TOWER_LEFT,
                 BG_OBJECT_H_BANNER_EY_ENTRY, 2269.13f, 1737.7f, 1186.66f,
                 0.994838f, 0, 0, 0.477159f, 0.878817f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_H_BANNER_MAGE_TOWER_RIGHT,
                 BG_OBJECT_H_BANNER_EY_ENTRY, 2300.86f, 1741.25f, 1187.7f,
                 -0.785398f, 0, 0, 0.382683f, -0.92388f, RESPAWN_ONE_DAY)
      // banners (natural)
      ...
      !AddObject(BG_EY_OBJECT_N_BANNER_FEL_REAVER_RIGHT,
                 BG_OBJECT_N_BANNER_EY_ENTRY, 2092.35f, 1775.46f, 1187.08f,
                 -0.401426f, 0, 0, 0.199368f, -0.979925f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_N_BANNER_BLOOD_ELF_CENTER,
                 BG_OBJECT_N_BANNER_EY_ENTRY, 2047.19f, 1349.19f, 1189.0f,
                 -1.62316f, 0, 0, 0.725374f, -0.688354f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_N_BANNER_BLOOD_ELF_LEFT,
                 BG_OBJECT_N_BANNER_EY_ENTRY, 2074.32f, 1385.78f, 1194.72f,
                 0.488692f, 0, 0, 0.241922f, 0.970296f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_N_BANNER_BLOOD_ELF_RIGHT,
                 BG_OBJECT_N_BANNER_EY_ENTRY, 2025.13f, 1386.12f, 1192.74f,
                 2.3911f, 0, 0, 0.930418f, 0.366501f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_N_BANNER_DRAENEI_RUINS_CENTER,
                 BG_OBJECT_N_BANNER_EY_ENTRY, 2276.8f, 1400.41f, 1196.33f,
                 2.44346f, 0, 0, 0.939693f, 0.34202f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_N_BANNER_DRAENEI_RUINS_LEFT,
                 BG_OBJECT_N_BANNER_EY_ENTRY, 2305.78f, 1404.56f, 1199.38f,
                 1.74533f, 0, 0, 0.766044f, 0.642788f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_N_BANNER_DRAENEI_RUINS_RIGHT,
                 BG_OBJECT_N_BANNER_EY_ENTRY, 2245.4f, 1366.41f, 1195.28f,
                 2.21657f, 0, 0, 0.894934f, 0.446198f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_N_BANNER_MAGE_TOWER_CENTER,
                 BG_OBJECT_N_BANNER_EY_ENTRY, 2270.84f, 1784.08f, 1186.76f,
                 2.42601f, 0, 0, 0.936672f, 0.350207f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_N_BANNER_MAGE_TOWER_LEFT,
                 BG_OBJECT_N_BANNER_EY_ENTRY, 2269.13f, 1737.7f, 1186.66f,
                 0.994838f, 0, 0, 0.477159f, 0.878817f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_N_BANNER_MAGE_TOWER_RIGHT,
                 BG_OBJECT_N_BANNER_EY_ENTRY, 2300.86f, 1741.25f, 1187.7f,
                 -0.785398f, 0, 0, 0.382683f, -0.92388f, RESPAWN_ONE_DAY)
      // flags
      ...
      !AddObject(BG_EY_OBJECT_FLAG_FEL_REAVER, BG_OBJECT_FLAG1_EY_ENTRY,
                 2044.28f, 1729.68f, 1189.96f, -0.017453f, 0, 0, 0.008727f,
                 -0.999962f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_FLAG_BLOOD_ELF, BG_OBJECT_FLAG1_EY_ENTRY,
                 2048.83f, 1393.65f, 1194.49f, 0.20944f, 0, 0, 0.104528f,
                 0.994522f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_FLAG_DRAENEI_RUINS, BG_OBJECT_FLAG1_EY_ENTRY,
                 2286.56f, 1402.36f, 1197.11f, 3.72381f, 0, 0, 0.957926f,
                 -0.287016f, RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_FLAG_MAGE_TOWER, BG_OBJECT_FLAG1_EY_ENTRY,
                 2284.48f, 1731.23f, 1189.99f, 2.89725f, 0, 0, 0.992546f,
                 0.121869f, RESPAWN_ONE_DAY)
      // tower cap
      || !AddObject(BG_EY_OBJECT_TOWER_CAP_FEL_REAVER,
                    BG_OBJECT_FR_TOWER_CAP_EY_ENTRY, 2024.600708f, 1742.819580f,
                    1195.157715f, 2.443461f, 0, 0, 0.939693f, 0.342020f,
                    RESPAWN_ONE_DAY) ||
      !AddObject(BG_EY_OBJECT_TOWER_CAP_BLOOD_ELF,
                 BG_OBJECT_BE_TOWER_CAP_EY_ENTRY, 2050.493164f, 1372.235962f,
                 1194.563477f, 1.710423f, 0, 0, 0.754710f, 0.656059f,
                 RESPAWN_ONE_DAY) ||
      ...
      !AddObject(BG_EY_OBJECT_TOWER_CAP_MAGE_TOWER,
                 BG_OBJECT_HU_TOWER_CAP_EY_ENTRY, 2282.121582f, 1760.006958f,
                 1189.707153f, 1.919862f, 0, 0, 0.819152f, 0.573576f,
                 RESPAWN_ONE_DAY)) {
    LOG_ERROR("sql.sql", "BatteGroundEY: Failed to spawn some objects. The "
                         "battleground was not created.");
    return false;
  }
  ...
  for (int i = 0; i < EY_POINTS_MAX; ++i) {
    ...
    if (!at) {
      LOG_ERROR("bg.battleground", "BattlegroundEY: Unknown trigger: {}",
                m_Points_Trigger[i]);
      continue;
    }
    if (!AddObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + i * 3, Buff_Entries[0],
                   at->Pos.X, at->Pos.Y, at->Pos.Z, 0.907571f, 0, 0, 0.438371f,
                   0.898794f, RESPAWN_ONE_DAY) ||
        !AddObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + i * 3 + 1,
                   Buff_Entries[1], at->Pos.X, at->Pos.Y, at->Pos.Z, 0.907571f,
                   0, 0, 0.438371f, 0.898794f, RESPAWN_ONE_DAY) ||
        !AddObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + i * 3 + 2,
                   Buff_Entries[2], at->Pos.X, at->Pos.Y, at->Pos.Z, 0.907571f,
                   0, 0, 0.438371f, 0.898794f, RESPAWN_ONE_DAY))
      LOG_ERROR("bg.battleground",
                "BattlegroundEY: Could not spawn Speedbuff Fel Reaver.");
  }

  WorldSafeLocsEntry const *sg =
      sWorldSafeLocsStore.LookupEntry(EY_GRAVEYARD_MAIN_ALLIANCE);
  if (!sg || !AddSpiritGuide(EY_SPIRIT_MAIN_ALLIANCE, sg->Loc.X, sg->Loc.Y,
                             sg->Loc.Z, 3.124139f, TEAM_ALLIANCE)) {
    LOG_ERROR("sql.sql", "BatteGroundEY: Failed to spawn spirit guide. The "
                         "battleground was not created.");
    return false;
  }

  sg = sWorldSafeLocsStore.LookupEntry(EY_GRAVEYARD_MAIN_HORDE);
  if (!sg || !AddSpiritGuide(EY_SPIRIT_MAIN_HORDE, sg->Loc.X, sg->Loc.Y,
                             sg->Loc.Z, 3.193953f, TEAM_HORDE)) {
    LOG_ERROR("sql.sql", "BatteGroundEY: Failed to spawn spirit guide. The "
                         "battleground was not created.");
    return false;
  }
...
void BattlegroundEY::Reset() {
  ...
  m_HonorTics =
      (isBGWeekend) ? BG_EY_EYWeekendHonorTicks : BG_EY_NotEYWeekendHonorTicks;
...
void BattlegroundEY::RespawnFlagAfterDrop() {
  ...
  if (obj)
    ...
    LOG_ERROR("bg.battleground", "BattlegroundEY: Unknown dropped flag ({}).",
              GetDroppedFlagGUID().ToString().c_str());
...
void BattlegroundEY::EventPlayerDroppedFlag(Player *player) {
  if (GetStatus() != STATUS_IN_PROGRESS) {
    // if not running, do not cast things at the dropper player, neither send
    // unnecessary messages just take off the aura
...
void BattlegroundEY::EventPlayerClickedOnFlag(Player *player,
  ...
  if (player->GetTeam() == ALLIANCE)
    SendBroadcastText(BG_EY_TEXT_TAKEN_FLAG, CHAT_MSG_BG_SYSTEM_ALLIANCE,
                      player);
  else
...
void BattlegroundEY::EventTeamLostPoint(Player *player, uint32 Point) {
  ...
  if (Team == ALLIANCE)
    SendBroadcastText(m_LosingPointTypes[Point].MessageIdAlliance,
                      CHAT_MSG_BG_SYSTEM_ALLIANCE, player);
  else
    SendBroadcastText(m_LosingPointTypes[Point].MessageIdHorde,
                      CHAT_MSG_BG_SYSTEM_HORDE, player);
  UpdatePointsIcons(Team, Point);
  UpdatePointsCount(Team);

  // remove bonus honor aura trigger creature when node is lost
  DelCreature(Point + 6); // nullptr checks are in DelCreature! 0-5 spirit
                          // guides
}
...
void BattlegroundEY::EventTeamCapturedPoint(Player *player, uint32 Point) {
  ...
  WorldSafeLocsEntry const *sg =
      sWorldSafeLocsStore.LookupEntry(m_CapturingPointTypes[Point].GraveyardId);
  if (!sg || !AddSpiritGuide(Point, sg->Loc.X, sg->Loc.Y, sg->Loc.Z, 3.124139f,
                             GetTeamIndexByTeamId(Team)))
    LOG_ERROR("bg.battleground",
              "BatteGroundEY: Failed to spawn spirit guide. point: {}, team: "
              "{}, graveyard_id: {}",
  ...
  if (!trigger)
    trigger =
        AddCreature(WORLD_TRIGGER, Point + 6, BG_EY_TriggerPositions[Point],
                    GetTeamIndexByTeamId(Team));
  ...
  // faction for trigger
  if (trigger) {
    trigger->SetFaction(Team == ALLIANCE ? FACTION_ALLIANCE_GENERIC
...
void BattlegroundEY::EventPlayerCapturedFlag(Player *player,
  ...
  if (player->GetTeam() == ALLIANCE) {
    ...
    SendBroadcastText(BG_EY_TEXT_HORDE_CAPTURED_FLAG, CHAT_MSG_BG_SYSTEM_HORDE,
                      player);
...
void BattlegroundEY::FillInitialWorldStates(
  ...
  data.Worldstates.emplace_back(uint32(EY_HORDE_BASE),
                                uint32(m_TeamPointsCount[TEAM_HORDE]));
  data.Worldstates.emplace_back(uint32(EY_ALLIANCE_BASE),
                                uint32(m_TeamPointsCount[TEAM_ALLIANCE]));
  ...
  data.Worldstates.emplace_back(
      int32(DRAENEI_RUINS_HORDE_CONTROL),
      uint32(m_PointOwnedByTeam[DRAENEI_RUINS] == HORDE &&
             m_PointState[DRAENEI_RUINS] == EY_POINT_UNDER_CONTROL));

  data.Worldstates.emplace_back(
      uint32(DRAENEI_RUINS_ALLIANCE_CONTROL),
      uint32(m_PointOwnedByTeam[DRAENEI_RUINS] == ALLIANCE &&
             m_PointState[DRAENEI_RUINS] == EY_POINT_UNDER_CONTROL));

  data.Worldstates.emplace_back(
      uint32(DRAENEI_RUINS_UNCONTROL),
      uint32(m_PointState[DRAENEI_RUINS] != EY_POINT_UNDER_CONTROL));

  data.Worldstates.emplace_back(
      uint32(MAGE_TOWER_ALLIANCE_CONTROL),
      uint32(m_PointOwnedByTeam[MAGE_TOWER] == ALLIANCE &&
             m_PointState[MAGE_TOWER] == EY_POINT_UNDER_CONTROL));

  data.Worldstates.emplace_back(
      uint32(MAGE_TOWER_HORDE_CONTROL),
      uint32(m_PointOwnedByTeam[MAGE_TOWER] == HORDE &&
             m_PointState[MAGE_TOWER] == EY_POINT_UNDER_CONTROL));

  data.Worldstates.emplace_back(
      uint32(MAGE_TOWER_UNCONTROL),
      uint32(m_PointState[MAGE_TOWER] != EY_POINT_UNDER_CONTROL));

  data.Worldstates.emplace_back(
      uint32(FEL_REAVER_HORDE_CONTROL),
      uint32(m_PointOwnedByTeam[FEL_REAVER] == HORDE &&
             m_PointState[FEL_REAVER] == EY_POINT_UNDER_CONTROL));

  data.Worldstates.emplace_back(
      uint32(FEL_REAVER_ALLIANCE_CONTROL),
      uint32(m_PointOwnedByTeam[FEL_REAVER] == ALLIANCE &&
             m_PointState[FEL_REAVER] == EY_POINT_UNDER_CONTROL));

  data.Worldstates.emplace_back(
      uint32(FEL_REAVER_UNCONTROL),
      uint32(m_PointState[FEL_REAVER] != EY_POINT_UNDER_CONTROL));

  data.Worldstates.emplace_back(
      uint32(BLOOD_ELF_HORDE_CONTROL),
      uint32(m_PointOwnedByTeam[BLOOD_ELF] == HORDE &&
             m_PointState[BLOOD_ELF] == EY_POINT_UNDER_CONTROL));

  data.Worldstates.emplace_back(
      uint32(BLOOD_ELF_ALLIANCE_CONTROL),
      uint32(m_PointOwnedByTeam[BLOOD_ELF] == ALLIANCE &&
             m_PointState[BLOOD_ELF] == EY_POINT_UNDER_CONTROL));

  data.Worldstates.emplace_back(
      uint32(BLOOD_ELF_UNCONTROL),
      uint32(m_PointState[BLOOD_ELF] != EY_POINT_UNDER_CONTROL));

  data.Worldstates.emplace_back(
      uint32(NETHERSTORM_FLAG),
      uint32(m_FlagState == BG_EY_FLAG_STATE_ON_BASE));

  data.Worldstates.emplace_back(uint32(0xad2), uint32(0x1));
  data.Worldstates.emplace_back(uint32(0xad1), uint32(0x1));
  data.Worldstates.emplace_back(uint32(0xabe),
                                uint32(GetTeamScore(TEAM_HORDE)));
  data.Worldstates.emplace_back(uint32(0xabd),
...
WorldSafeLocsEntry const *BattlegroundEY::GetClosestGraveyard(Player *player) {
  ...
  if (!entry) {
    LOG_ERROR("bg.battleground",
              ...
              "graveyard system will not be operational!");
    return nullptr;
  }
  ...
  distance = (entry->Loc.X - plr_x) * (entry->Loc.X - plr_x) +
             (entry->Loc.Y - plr_y) * (entry->Loc.Y - plr_y) +
             (entry->Loc.Z - plr_z) * (entry->Loc.Z - plr_z);
  ...
  for (uint8 i = 0; i < EY_POINTS_MAX; ++i) {
    if (m_PointOwnedByTeam[i] == player->GetTeam() &&
      ...
      if (!entry)
        LOG_ERROR("bg.battleground",
                  "BattlegroundEY: Graveyard {} could not be found.",
