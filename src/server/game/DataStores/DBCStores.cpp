...
// temporary hack until includes are sorted out (don't want to pull in
// Windows.h)
#ifdef GetClassName
#undef GetClassName
#endif
...
static std::map<EmotesTextSoundKey, EmotesTextSoundEntry const *>
    sEmotesTextSoundMap;
...
DBCStorage<ImportPriceQualityEntry>
    sImportPriceQualityStore(ImportPriceQualityfmt);
DBCStorage<ImportPriceShieldEntry>
    sImportPriceShieldStore(ImportPriceShieldfmt);
DBCStorage<ImportPriceWeaponEntry>
    sImportPriceWeaponStore(ImportPriceWeaponfmt);
...
DBCStorage<ItemDisenchantLootEntry>
    sItemDisenchantLootStore(ItemDisenchantLootfmt);
...
DBCStorage<LFGDungeonsGroupingMapEntry>
    sLFGDungeonsGroupingMapStore(LFGDungeonsGroupingMapfmt);
...
DBCStorage<NumTalentsAtLevelEntry>
    sNumTalentsAtLevelStore(NumTalentsAtLevelfmt);

typedef std::array<std::vector<Firelands::wregex>, TOTAL_LOCALES>
...
DBCStorage<SpellTargetRestrictionsEntry>
    sSpellTargetRestrictionsStore(SpellTargetRestrictionsEntryfmt);
DBCStorage<SpellPowerEntry> sSpellPowerStore(SpellPowerEntryfmt);
DBCStorage<SpellLevelsEntry> sSpellLevelsStore(SpellLevelsEntryfmt);
DBCStorage<SpellInterruptsEntry> sSpellInterruptsStore(SpellInterruptsEntryfmt);
DBCStorage<SpellEquippedItemsEntry>
    sSpellEquippedItemsStore(SpellEquippedItemsEntryfmt);
DBCStorage<SpellClassOptionsEntry>
    sSpellClassOptionsStore(SpellClassOptionsEntryfmt);
DBCStorage<SpellCooldownsEntry> sSpellCooldownsStore(SpellCooldownsEntryfmt);
DBCStorage<SpellAuraOptionsEntry>
    sSpellAuraOptionsStore(SpellAuraOptionsEntryfmt);
DBCStorage<SpellAuraRestrictionsEntry>
    sSpellAuraRestrictionsStore(SpellAuraRestrictionsEntryfmt);
DBCStorage<SpellCastingRequirementsEntry>
    sSpellCastingRequirementsStore(SpellCastingRequirementsEntryfmt);
...
DBCStorage<SpellShapeshiftFormEntry>
...
namespace {
std::unordered_map<uint32, std::vector<SkillLineAbilityEntry const *>>
    _skillLineAbilitiesBySkillupSkill;
}

template <class T>
inline void LoadDBC(uint32 &availableDbcLocales, StoreProblemList &errors,
                    DBCStorage<T> &storage, std::string const &dbcPath,
                    std::string const &filename, uint32 defaultLocale,
                    std::string const &customFormat = std::string(),
                    std::string const &customIndexName = std::string()) {
  ...
  std::string dbcFilename =
      dbcPath + localeNames[defaultLocale] + '/' + filename;
    ...
    if (FILE *f = fopen(dbcFilename.c_str(), "rb")) {
      ...
      stream << dbcFilename << " exists, and has " << storage.GetFieldCount()
             ...
             << "). Extracted file might be from wrong client version or a "
                "database-update has been forgotten. Search on forum for "
                "TCE00008 for more info.";
...
void DBCManager::LoadStores(const std::string &dataPath, uint32 defaultLocale) {
  ...
  DBCStorage<TalentTreePrimarySpellsEntry> sTalentTreePrimarySpellsStore(
...
#define LOAD_DBC(store, file)                                                  \
  ...
  LOAD_DBC(sCreatureDisplayInfoExtraStore,
           "CreatureDisplayInfoExtra.dbc");                           // 15595
  ...
  LOAD_DBC(sGtChanceToSpellCritBaseStore,
           "gtChanceToSpellCritBase.dbc");                        // 15595
  ...
  LOAD_DBC(sGtOCTClassCombatRatingScalarStore,
           "gtOCTClassCombatRatingScalar.dbc"); // 15595
  ...
  LOAD_DBC(sItemDamageOneHandCasterStore,
           "ItemDamageOneHandCaster.dbc");                    // 15595
  ...
  LOAD_DBC(sItemDamageTwoHandCasterStore,
  ...
  LOAD_DBC(sSpellTargetRestrictionsStore,
           "SpellTargetRestrictions.dbc");                            // 15595
  ...
  LOAD_DBC(sSpellCastingRequirementsStore,
           "SpellCastingRequirements.dbc");                         // 15595
...
#define LOAD_DBC_EXT(store, file, dbformat, dbpk)                              \
  LoadDBC(availableDbcLocales, bad_dbc_files, store, dbcPath, file,            \
          defaultLocale, dbformat, dbpk)

  LOAD_DBC_EXT(sAchievementStore, "Achievement.dbc", CustomAchievementfmt,
               CustomAchievementIndex); // 15595
  LOAD_DBC_EXT(sSpellStore, "Spell.dbc", CustomSpellEntryfmt,
               CustomSpellEntryIndex); //
  LOAD_DBC_EXT(sSpellEffectStore, "SpellEffect.dbc", CustomSpellEffectEntryfmt,
               CustomSpellEffectEntryIndex); // 15595
  LOAD_DBC_EXT(sSpellDifficultyStore, "SpellDifficulty.dbc",
               CustomSpellDifficultyfmt, CustomSpellDifficultyIndex); // 15595

#undef LOAD_DBC_EXT

  for (CharStartOutfitEntry const *outfit : sCharStartOutfitStore)
    sCharStartOutfitMap[outfit->RaceID | (outfit->ClassID << 8) |
                        (outfit->SexID << 16)] = outfit;

  for (CharSectionsEntry const *entry : sCharSectionsStore)
    if (entry->RaceID && ((1 << (entry->RaceID - 1)) & RACEMASK_ALL_PLAYABLE) !=
                             0) // ignore Nonplayable races
      sCharSectionMap.insert(
          {entry->BaseSection | (entry->SexID << 8) | (entry->RaceID << 16),
           entry});
  ...
  for (EmotesTextSoundEntry const *entry : sEmotesTextSoundStore)
    sEmotesTextSoundMap[EmotesTextSoundKey(entry->EmotesTextID, entry->RaceID,
                                           entry->SexID)] = entry;
  ...
  // fill data
  sMapDifficultyMap[MAKE_PAIR32(0, 0)] =
      MapDifficulty(0, 0, false); // map 0 is missing from MapDifficulty.dbc use
                                  // this till its ported to sql
  for (MapDifficultyEntry const *entry : sMapDifficultyStore)
    sMapDifficultyMap[MAKE_PAIR32(entry->MapID, entry->Difficulty)] =
        MapDifficulty(entry->RaidDuration, entry->MaxPlayers,
                      !std::string(entry->Message).empty());

  sMapDifficultyMap.erase(MAKE_PAIR32(
      189, 1)); // Scarlet Monastry has a incorrect heroic entry so we drop it

  for (NameGenEntry const *entry : sNameGenStore)
    sGenNameVectoArraysMap[entry->RaceID]
        .stringVectorArray[entry->Sex]
        .push_back(std::string(entry->Name));

  for (NamesProfanityEntry const *namesProfanity : sNamesProfanityStore) {
    ASSERT(namesProfanity->Language < TOTAL_LOCALES ||
           namesProfanity->Language == -1);
    std::wstring wname;
    bool conversionResult = Utf8toWStr(namesProfanity->Name, wname);
    ASSERT(conversionResult);

    if (namesProfanity->Language != -1)
      NamesProfaneValidators[namesProfanity->Language].emplace_back(
          wname, Firelands::regex::icase | Firelands::regex::optimize);
    else
      for (uint32 i = 0; i < TOTAL_LOCALES; ++i)
        NamesProfaneValidators[i].emplace_back(
            wname, Firelands::regex::icase | Firelands::regex::optimize);
  }

  for (NamesReservedEntry const *namesReserved : sNamesReservedStore) {
    ASSERT(namesReserved->Language < TOTAL_LOCALES ||
           namesReserved->Language == -1);
    std::wstring wname;
    bool conversionResult = Utf8toWStr(namesReserved->Name, wname);
    ASSERT(conversionResult);

    if (namesReserved->Language != -1)
      NamesReservedValidators[namesReserved->Language].emplace_back(
          wname, Firelands::regex::icase | Firelands::regex::optimize);
    else
      for (uint32 i = 0; i < TOTAL_LOCALES; ++i)
        NamesReservedValidators[i].emplace_back(
            wname, Firelands::regex::icase | Firelands::regex::optimize);
  }
  ...
  for (SkillLineAbilityEntry const *skillLineAbility : sSkillLineAbilityStore)
    _skillLineAbilitiesBySkillupSkill[skillLineAbility->SkillLine].push_back(
        skillLineAbility);
  ...
  // Must be done when sSkillLineAbilityStore, sSpellStore, sSpellLevelsStore
  // and sCreatureFamilyStore are all loaded
  for (SkillLineAbilityEntry const *skillLine : sSkillLineAbilityStore) {
    ...
    SpellLevelsEntry const *levels =
        sSpellLevelsStore.LookupEntry(spellInfo->LevelsID);
  ...
  for (SpellDifficultyEntry const *spellDiff : sSpellDifficultyStore) {
    ...
    for (uint32 x = 0; x < MAX_DIFFICULTY; ++x) {
      if (spellDiff->DifficultySpellID[x] <= 0 ||
          !sSpellStore.LookupEntry(spellDiff->DifficultySpellID[x])) {
        if (spellDiff->DifficultySpellID[x] >
            0) // don't show error if spell is <= 0, not all modes have spells
               // and there are unknown negative values
          LOG_ERROR("sql.sql",
                    "spelldifficulty_dbc: spell {} at field id:{} at spellid{} "
                    "does not exist in SpellStore (spell.dbc), loaded as 0",
                    spellDiff->DifficultySpellID[x], spellDiff->ID, x);
        newEntry.DifficultySpellID[x] = 0; // spell was <= 0 or invalid, set to
                                           // 0
      } else
        newEntry.DifficultySpellID[x] = spellDiff->DifficultySpellID[x];
    }

    if (newEntry.DifficultySpellID[0] <= 0 ||
        newEntry.DifficultySpellID[1] <= 0) // id0-1 must be always set!
      continue;

    for (uint32 x = 0; x < MAX_DIFFICULTY; ++x)
      if (newEntry.DifficultySpellID[x])
        sSpellMgr->SetSpellDifficultyId(uint32(newEntry.DifficultySpellID[x]),
                                        spellDiff->ID);
  }
  ...
  for (TalentEntry const *talentInfo : sTalentStore) {
    for (uint8 j = 0; j < MAX_TALENT_RANK; ++j)
      if (talentInfo->SpellRank[j])
        sTalentSpellPosMap[talentInfo->SpellRank[j]] =
            TalentSpellPos(talentInfo->ID, j);
  }
  ...
  for (TalentTreePrimarySpellsEntry const *talentSpell :
       sTalentTreePrimarySpellsStore)
    sTalentTreePrimarySpellsMap[talentSpell->TalentTabID].push_back(
        talentSpell->SpellID);

  for (TaxiPathEntry const *entry : sTaxiPathStore)
    sTaxiPathSetBySource[entry->FromTaxiNode][entry->ToTaxiNode] =
        TaxiPathBySourceAndDestination(entry->ID, entry->Cost);
  ...
  // path
  {
    ...
    for (TaxiNodesEntry const *node : sTaxiNodesStore) {
      TaxiPathSetBySource::const_iterator src_i =
          sTaxiPathSetBySource.find(node->ID);
      ...
      if (node->ContinentID < 2 || node->ID == 82 || node->ID == 83 ||
  ...
  for (WMOAreaTableEntry const *entry : sWMOAreaTableStore)
    sWMOAreaInfoByTripple[WMOAreaTableKey(entry->WMOID, entry->NameSetID,
                                          entry->WMOGroupID)] = entry;
  ...
  if (bad_dbc_files.size() >= DBCFileCount) {
    LOG_ERROR("misc",
              ...
              DBCFileCount, dataPath.c_str(), localeNames[defaultLocale]);
    exit(1);
  ...
  if (!sAreaTableStore.LookupEntry(5995) || // last area added in 4.3.4 (15595)
      !sCharTitlesStore.LookupEntry(
          287) || // last char title added in 4.3.4 (15595)
      !sGemPropertiesStore.LookupEntry(
          2250) ||                   // last gem property added in 4.3.4 (15595)
      !sMapStore.LookupEntry(980) || // last map added in 4.3.4 (15595)
      !sSpellStore.LookupEntry(121820)) // last spell added in 4.3.4 (15595)
  {
    LOG_ERROR("misc", "You have _outdated_ DBC files. Please extract correct "
                      "versions from current using client.");
    exit(1);
  }

  LOG_INFO("server.loading", ">> Initialized {} DBC data stores in {} ms",
           DBCFileCount, GetMSTimeDiffToNow(oldMSTime));
}

std::string const &DBCManager::GetRandomCharacterName(uint8 race,
                                                      uint8 gender) {
  return Firelands::Containers::SelectRandomContainerElement(
      sGenNameVectoArraysMap[race].stringVectorArray[gender]);
}
...
DBCManager::GetWMOAreaTableEntryByTripple(int32 rootid, int32 adtid,
                                          int32 groupid) {
...
*/
ContentLevels DBCManager::GetContentLevelsForMapAndZone(uint32 mapid,
                                                        uint32 zoneId) {
...
bool DBCManager::IsTotemCategoryCompatibleWith(uint32 itemTotemCategoryId,
  ...
  return (itemEntry->TotemCategoryMask & reqEntry->TotemCategoryMask) ==
         reqEntry->TotemCategoryMask;
}
...
void DBCManager::Zone2MapCoordinates(float &x, float &y, uint32 zone) {
  ...
  y = y * ((maEntry->LocRight - maEntry->LocLeft) / 100) +
      maEntry->LocLeft; // client y coord from top to down
}
...
void DBCManager::Map2ZoneCoordinates(float &x, float &y, uint32 zone) {
  ...
  y = (y - maEntry->LocLeft) / ((maEntry->LocRight - maEntry->LocLeft) /
                                100); // client y coord from top to down
  std::swap(x, y);                    // client have map coords swapped
}

MapDifficulty const *DBCManager::GetMapDifficultyData(uint32 mapId,
...
MapDifficulty const *
DBCManager::GetDownscaledMapDifficultyData(uint32 mapId,
                                           Difficulty &difficulty) {
...
DBCManager::GetBattlegroundBracketByLevel(uint32 mapid, uint32 level) {
...
PvPDifficultyEntry const *
DBCManager::GetBattlegroundBracketById(uint32 mapid, BattlegroundBracketId id) {
  for (uint32 i = 0; i < sPvPDifficultyStore.GetNumRows(); ++i)
    if (PvPDifficultyEntry const *entry = sPvPDifficultyStore.LookupEntry(i))
...
DBCManager::GetTalentTreePrimarySpells(uint32 talentTree) {
  TalentTreePrimarySpellsMap::const_iterator itr =
      sTalentTreePrimarySpellsMap.find(talentTree);
...
DBCManager::GetCharStartOutfitEntry(uint8 race, uint8 class_, uint8 gender) {
...
DBCManager::GetCharSectionEntry(uint8 race, CharSectionType genType,
                                uint8 gender, uint8 type, uint8 color) {
  std::pair<CharSectionsMap::const_iterator, CharSectionsMap::const_iterator>
      eqr = sCharSectionMap.equal_range(uint32(genType) | uint32(gender << 8) |
                                        uint32(race << 16));
  for (CharSectionsMap::const_iterator itr = eqr.first; itr != eqr.second;
       ++itr) {
...
uint32 ScalingStatValuesEntry::GetArmor(uint32 inventoryType,
                                        uint32 armorType) const {
...
uint32 ScalingStatValuesEntry::GetDPSAndDamageMultiplier(
    uint32 subClass, bool isCasterWeapon, float *damageMultiplier) const {
  if (!isCasterWeapon) {
    switch (subClass) {
...
/// Monastery)
LFGDungeonEntry const *DBCManager::GetLFGDungeon(uint32 mapId,
                                                 Difficulty difficulty) {
...
uint32 DBCManager::GetDefaultMapLight(uint32 mapId) {
  for (int32 i = sLightStore.GetNumRows(); i >= 0; --i) {
    ...
    if (light->ContinentID == mapId && light->X == 0.0f && light->Y == 0.0f &&
        light->Z == 0.0f)
      return light->ID;
  }
...
std::vector<SkillLineAbilityEntry const *> const *
DBCManager::GetSkillLineAbilitiesBySkill(uint32 skillId) const {
  return Firelands::Containers::MapGetValuePtr(
      _skillLineAbilitiesBySkillupSkill, skillId);
}

SkillRaceClassInfoEntry const *
DBCManager::GetSkillRaceClassInfo(uint32 skill, uint8 race, uint8 class_) {
  SkillRaceClassInfoBounds bounds =
      SkillRaceClassInfoBySkill.equal_range(skill);
  for (SkillRaceClassInfoMap::iterator itr = bounds.first; itr != bounds.second;
       ++itr) {
...
ResponseCodes DBCManager::ValidateName(std::wstring const &name,
  ...
  // regexes at TOTAL_LOCALES are loaded from NamesReserved which is not locale
  // specific
...
DBCManager::FindTextSoundEmoteFor(uint32 emote, uint32 race, uint32 gender) {
...
bool DBCManager::IsInArea(uint32 objectAreaId, uint32 areaId) {
  do {
    ...
    AreaTableEntry const *objectArea =
        sAreaTableStore.LookupEntry(objectAreaId);
    if (!objectArea)
      break;
