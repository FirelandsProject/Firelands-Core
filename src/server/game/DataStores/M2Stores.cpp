...
void LoadM2Cameras(std::string const &dataPath) {
  ...
  for (CinematicCameraEntry const *dbcentry : sCinematicCameraStore) {
    ...
    // Now we have a good file, read it all into a vector of char's, then close
    // the file.
    ...
    if (header->ofsCameras + sizeof(M2Camera) > static_cast<uint32>(fileSize)) {
      LOG_ERROR("server.loading",
                "Camera file {} is damaged. Camera references position beyond "
                "file end",
                filename.string().c_str());
      continue;
    }
    ...
    M2Camera const *cam =
        reinterpret_cast<M2Camera const *>(buffer.data() + header->ofsCameras);
  ...
  LOG_INFO("server.loading", ">> Loaded {} cinematic waypoint sets in {} ms",
           (uint32)sFlyByCameraStore.size(), GetMSTimeDiffToNow(oldMSTime));
}

std::vector<FlyByCamera> const *GetFlyByCameras(uint32 cinematicCameraId) {
  return Firelands::Containers::MapGetValuePtr(sFlyByCameraStore,
                                               cinematicCameraId);
}
