...
LFGDungeonData::LFGDungeonData(LFGDungeonEntry const *dbc)
    : id(dbc->ID), name(dbc->Name), map(dbc->MapID), type(dbc->TypeID),
      expansion(dbc->ExpansionLevel), group(dbc->Group_ID),
      minlevel(dbc->MinLevel), maxlevel(dbc->Maxlevel),
      difficulty(Difficulty(dbc->DifficultyID)),
      seasonal((dbc->Flags & LFG_FLAG_SEASONAL) != 0), x(0.0f), y(0.0f),
      z(0.0f), o(0.0f), requiredItemLevel(0), requiredTanks(dbc->Count_tank),
      requiredHealers(dbc->Count_healer),
      requiredDamageDealers(dbc->Count_damage) {}

LFGMgr::LFGMgr()
    : m_QueueTimer(0), m_lfgProposalId(1),
...
void LFGMgr::LoadRewards() {
  ...
  if (!result) {
    LOG_ERROR("server.loading", ">> Loaded 0 lfg dungeon rewards. DB table "
  ...
  do {
    ...
    if (!GetLFGDungeonEntry(dungeonId)) {
      LOG_ERROR(
          "sql.sql",
          "Dungeon {} specified in table `lfg_dungeon_rewards` does not exist!",
          dungeonId);
      continue;
    }
    ...
    if (shortageQuestId && !sObjectMgr->GetQuestTemplate(shortageQuestId)) {
      LOG_ERROR("sql.sql",
                "Shortage quest {} specified for dungeon {} in table "
                "`lfg_dungeon_rewards` does not exist!",
  ...
  LOG_INFO("server.loading", ">> Loaded {} lfg dungeon rewards in {} ms", count,
...
void LFGMgr::LoadLFGDungeons(bool reload /* = false */) {
  ...
  if (!result) {
    LOG_ERROR("server.loading", ">> Loaded 0 lfg dungeon templates. DB table "
  ...
  LOG_INFO("server.loading", ">> Loaded {} lfg dungeon templates in {} ms",
  ...
  for (LFGDungeonContainer::iterator itr = LfgDungeonStore.begin();
    ...
    if (dungeon.type != LFG_TYPE_RANDOM && dungeon.x == 0.0f &&
      ...
      if (!at) {
        LOG_ERROR("sql.sql",
                  "Failed to load dungeon {} (Id: {}), cant find areatrigger "
                  "for map {}",
...
void LFGMgr::Update(uint32 diff) {
  ...
  for (LfgQueueContainer::iterator it = QueuesStore.begin();
    ...
    if (uint8 newProposals = it->second.FindGroups())
      LOG_DEBUG("lfg.update", "Found {} new groups in queue {}", newProposals,
                it->first);
...
void LFGMgr::JoinLfg(Player *player, uint8 roles, LfgDungeonSet &dungeons,
    ...
    if (grp->GetMembersCount() > MAXGROUPSIZE)
      ...
      for (GroupReference *itr = grp->GetFirstMember();
        ...
        if (Player *plrg = itr->GetSource()) {
          if (!plrg->GetSession()->HasPermission(
  ...
  LOG_DEBUG("lfg.join", "{} joined ({}), Members: {}. Dungeons ({}): {}",
...
void LFGMgr::LeaveLfg(ObjectGuid guid, bool disconnected) {
  ...
  LOG_DEBUG("lfg.leave", "{} left ({})", guid.ToString().c_str(),
  ...
  case LFG_STATE_RAIDBROWSER: {
    ...
    if (gguid) {
      ...
      // the group is already in a dungeon This is required in case a LFG group
      // vote-kicks a player in a dungeon, queues, then leaves the queue (maybe
      // to queue later again)
  ...
  for (LfgRolesMap::const_iterator it = roleCheck.roles.begin();
    ...
    case LFG_ROLECHECK_FINISHED:
      ...
      SendLfgUpdateStatus(pguid,
                          LfgUpdateData(LFG_UPDATETYPE_ADDED_TO_QUEUE, dungeons,
                                        GetComment(pguid)),
                          true);
      break;
    default:
      ...
      SendLfgUpdateStatus(pguid, LfgUpdateData(LFG_UPDATETYPE_ROLECHECK_FAILED),
                          true);
...
void LFGMgr::UpdateProposal(uint32 proposalId, ObjectGuid guid, bool accept) {
  ...
  LOG_DEBUG("lfg.proposal.update", "{}, Proposal {}, Selection: {}",
...
void LFGMgr::RemoveProposal(LfgProposalContainer::iterator itProposal,
  ...
  LOG_DEBUG("lfg.proposal.remove", "Proposal {}, state FAILED, UpdateType {}",
  ...
  for (LfgProposalPlayerContainer::const_iterator it = proposal.players.begin();
                        ...
                        // didn't accept
    {
      ...
      LOG_DEBUG("lfg.proposal.remove", "Readding {} to queue.",
                guid.ToString().c_str());
...
void LFGMgr::FinishDungeon(ObjectGuid gguid, const uint32 dungeonId,
  ...
  if (gDungeonId != dungeonId) {
    LOG_DEBUG("lfg.dungeon.finish",
              "Group {} finished dungeon {} but queued for {}",
  ...
  {
    LOG_DEBUG("lfg.dungeon.finish", "Group: {} already rewarded",
  ...
  for (GuidSet::const_iterator it = players.begin(); it != players.end();
    ...
    if (GetState(guid) == LFG_STATE_FINISHED_DUNGEON) {
      LOG_DEBUG("lfg.dungeon.finish", "Group: {}, Player: {} already rewarded",
    ...
    if (!dungeon || (dungeon->type != LFG_TYPE_RANDOM && !dungeon->seasonal)) {
      LOG_DEBUG("lfg.dungeon.finish",
                "Group: {}, Player: {} dungeon {} is not random or seasonal",
    ...
    if (!player) {
      LOG_DEBUG("lfg.dungeon.finish",
                "Group: {}, Player: {} not found in world",
                gguid.ToString().c_str(), guid.ToString().c_str());
      continue;
    }
    ...
    if (player->FindMap() != currMap) {
      LOG_DEBUG("lfg.dungeon.finish",
                "Group: {}, Player: {} is in a different map",
                gguid.ToString().c_str(), guid.ToString().c_str());
      continue;
    }
    ...
    if (player->GetMapId() != mapId) {
      LOG_DEBUG("lfg.dungeon.finish",
                "Group: {}, Player: {} is in map {} and should be in {} to get "
                "reward",
                gguid.ToString().c_str(), guid.ToString().c_str(),
    ...
    // Give rewards
    LOG_DEBUG("lfg.dungeon.finish",
              "Group: {}, Player: {} done dungeon {}, {} previously done.",
              gguid.ToString().c_str(), guid.ToString().c_str(),
...
LfgState LFGMgr::GetState(ObjectGuid guid) {
  ...
  if (guid.IsGroup()) {
    ...
    LOG_TRACE("lfg.data.group.state.get", "Group: {}, State: {}",
              guid.ToString().c_str(), GetStateString(state).c_str());
  } else {
    state = PlayersStore[guid].GetState();
    LOG_TRACE("lfg.data.player.state.get", "Player: {}, State: {}",
              guid.ToString().c_str(), GetStateString(state).c_str());
  }
...
LfgState LFGMgr::GetOldState(ObjectGuid guid) {
  ...
  if (guid.IsGroup()) {
    ...
    LOG_TRACE("lfg.data.group.oldstate.get", "Group: {}, Old state: {}",
              guid.ToString().c_str(), state);
  } else {
    state = PlayersStore[guid].GetOldState();
    LOG_TRACE("lfg.data.player.oldstate.get", "Player: {}, Old state: {}",
              guid.ToString().c_str(), state);
  }
...
bool LFGMgr::IsVoteKickActive(ObjectGuid gguid) {
  ...
  LOG_TRACE("lfg.data.group.votekick.get", "Group: {}, Active: {}",
...
uint32 LFGMgr::GetDungeon(ObjectGuid guid, bool asId /*= true */) {
  ...
  LOG_TRACE("lfg.data.group.dungeon.get", "Group: {}, asId: {}, Dungeon: {}",
...
uint32 LFGMgr::GetDungeonMapId(ObjectGuid guid) {
  ...
  LOG_TRACE("lfg.data.group.dungeon.map",
            "Group: {}, MapId: {} (DungeonId: {})", guid.ToString().c_str(),
            mapId, dungeonId);
...
uint8 LFGMgr::GetRoles(ObjectGuid guid) {
  ...
  LOG_TRACE("lfg.data.player.role.get", "Player: {}, Role: {}",
            guid.ToString().c_str(), roles);
  return roles;
}

const std::string &LFGMgr::GetComment(ObjectGuid guid) {
  LOG_TRACE("lfg.data.player.comment.get", "Player: {}, Comment: {}",
...
LfgDungeonSet const &LFGMgr::GetSelectedDungeons(ObjectGuid guid) {
  LOG_TRACE(
      ...
      "Player: {}, Selected Dungeons: {}", guid.ToString().c_str(),
...
LfgLockMap const LFGMgr::GetLockedDungeons(ObjectGuid guid) {
  LOG_TRACE("lfg.data.player.dungeons.locked.get",
            "Player: {}, LockedDungeons.", guid.ToString().c_str());
  LfgLockMap lock;
  Player *player = ObjectAccessor::FindConnectedPlayer(guid);
  if (!player) {
    LOG_WARN("lfg.data.player.dungeons.locked.get",
             "Player: {} not ingame while retrieving his LockedDungeons.",
             guid.ToString().c_str());
    return lock;
  }
  ...
  LfgDungeonSet const &dungeons = GetDungeonsByRandom(0);
  bool denyJoin =
      !player->GetSession()->HasPermission(rbac::RBAC_PERM_JOIN_DUNGEON_FINDER);
...
uint8 LFGMgr::GetKicksLeft(ObjectGuid guid) {
  ...
  LOG_TRACE("lfg.data.group.kickleft.get", "Group: {}, Kicks left: {}",
...
void LFGMgr::SetVoteKick(ObjectGuid gguid, bool active) {
  ...
  LOG_TRACE("lfg.data.group.votekick.set",
            "Group: {}, New state: {}, Previous: {}", gguid.ToString().c_str(),
...
void LFGMgr::SetSelectedDungeons(ObjectGuid guid,
  ...
  LOG_TRACE("lfg.data.player.dungeon.selected.set", "Player: {}, Dungeons: {}",
...
void LFGMgr::DecreaseKicksLeft(ObjectGuid guid) {
  ...
  LOG_TRACE("lfg.data.group.kicksleft.decrease", "Group: {}, Kicks: {}",
...
void LFGMgr::RemovePlayerData(ObjectGuid guid) {
  LOG_TRACE("lfg.data.player.remove", "Player: {}", guid.ToString().c_str());
...
void LFGMgr::RemoveGroupData(ObjectGuid guid) {
  LOG_TRACE("lfg.data.group.remove", "Group: {}", guid.ToString().c_str());
...
uint8 LFGMgr::GetTeam(ObjectGuid guid) {
  ...
  LOG_TRACE("lfg.data.player.team.get", "Player: {}, Team: {}",
