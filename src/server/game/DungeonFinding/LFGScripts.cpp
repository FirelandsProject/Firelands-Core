...
void LFGPlayerScript::OnLogin(Player *player, bool /*loginFirst*/) {
  ...
  if (Group const *group = player->GetGroup()) {
    ...
    if (gguid != gguid2) {
      LOG_ERROR("lfg", "{} on group {} but LFG has group {} saved... Fixing.",
                player->GetSession()->GetPlayerInfo().c_str(),
                gguid2.ToString().c_str(), gguid.ToString().c_str());
...
void LFGPlayerScript::OnMapChanged(Player *player) {
  ...
  if (sLFGMgr->inLfgDungeonMap(player->GetGUID(), map->GetId(),
    ...
    if (group && group->GetMembersCount() == 1) {
      ...
      group->Disband();
      LOG_DEBUG("lfg",
                "LFGPlayerScript::OnMapChanged, Player {}({}) is last in the "
                "lfggroup so we disband the group.",
...
void LFGGroupScript::OnAddMember(Group *group, ObjectGuid guid) {
  ...
  if (leader == guid) {
    LOG_DEBUG("lfg", "LFGScripts::OnAddMember [{}]: added [{}] leader [{}]",
              gguid.ToString().c_str(), guid.ToString().c_str(),
              leader.ToString().c_str());
    sLFGMgr->SetLeader(gguid, guid);
  } else {
    LfgState gstate = sLFGMgr->GetState(gguid);
    LfgState state = sLFGMgr->GetState(guid);
    LOG_DEBUG("lfg",
              "LFGScripts::OnAddMember [{}]: added [{}] leader [{}] gstate: "
              "{}, state: {}",
              gguid.ToString().c_str(), guid.ToString().c_str(),
              leader.ToString().c_str(), gstate, state);
...
void LFGGroupScript::OnRemoveMember(Group *group, ObjectGuid guid,
  ...
  LOG_DEBUG("lfg",
            ...
            "[{}] Reason: {}",
            gguid.ToString().c_str(), guid.ToString().c_str(), method,
  ...
  if (Player *player = ObjectAccessor::FindPlayer(guid)) {
    ...
    player->GetSession()->SendLfgUpdateStatus(
        LfgUpdateData(LFG_UPDATETYPE_LEADER_UNK1), true);
    if (isLFG &&
        player->GetMap()->IsDungeon()) // Teleport player out the dungeon
  ...
  if (isLFG &&
    ...
    if (Player *leader =
            ObjectAccessor::FindConnectedPlayer(sLFGMgr->GetLeader(gguid)))
...
void LFGGroupScript::OnDisband(Group *group) {
  ...
  ObjectGuid gguid = group->GetGUID();
  LOG_DEBUG("lfg", "LFGScripts::OnDisband [{}]", gguid.ToString().c_str());

  sLFGMgr->RemoveGroupData(gguid);
}

void LFGGroupScript::OnChangeLeader(Group *group, ObjectGuid newLeaderGuid,
  ...
  LOG_DEBUG("lfg", "LFGScripts::OnChangeLeader [{}]: old [{}] new [{}]",
            gguid.ToString().c_str(), newLeaderGuid.ToString().c_str(),
            oldLeaderGuid.ToString().c_str());
...
void LFGGroupScript::OnInviteMember(Group *group, ObjectGuid guid) {
  ...
  ObjectGuid gguid = group->GetGUID();
  ObjectGuid leader = group->GetLeaderGUID();
  LOG_DEBUG("lfg", "LFGScripts::OnInviteMember [{}]: invite [{}] leader [{}]",
            gguid.ToString().c_str(), guid.ToString().c_str(),
            leader.ToString().c_str());
