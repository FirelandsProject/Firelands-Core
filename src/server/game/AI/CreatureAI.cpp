...
CreatureAI::CreatureAI(Creature *creature)
    : UnitAI(creature), me(creature), _boundary(nullptr),
      _negateBoundary(false), _isEngaged(false), _moveInLOSLocked(false) {}
...
void CreatureAI::MoveInLineOfSight(Unit *who) {
  ...
  if (me->HasReactState(REACT_AGGRESSIVE) && !me->IsIgnoringCombat() &&
...
void CreatureAI::TriggerAlert(Unit const *who) const {
  ...
  // If this unit isn't an NPC, is already distracted, is fighting, is confused,
  // stunned or fleeing, do nothing
...
static bool ShouldFollowOnSpawn(SummonPropertiesEntry const *properties) {
  // Summons without SummonProperties are generally scripted summons that don't
  // belong to any owner
  if (!properties)
    return false;
...
void CreatureAI::EngagementStart(Unit *who) {
  if (_isEngaged) {
    LOG_ERROR("scripts.ai", "CreatureAI::EngagementStart called even though "
                            "creature is already engaged.");
    return;
  }
  _isEngaged = true;
...
void CreatureAI::EngagementOver() {
  if (!_isEngaged) {
    LOG_ERROR("scripts.ai", "CreatureAI::EngagementOver called even though "
                            "creature is not currently engaged.");
    return;
  }
  _isEngaged = false;
...
int32 CreatureAI::VisualizeBoundary(uint32 duration, Unit *owner,
                                    bool fill) const {
  ...
  while (!Q.empty()) {
    ...
    for (coordinate off :
         std::initializer_list<coordinate>{{1, 0}, {0, 1}, {-1, 0}, {0, -1}}) {
      coordinate next(front.first + off.first, front.second + off.second);
      if (next.first > BOUNDARY_VISUALIZE_FAILSAFE_LIMIT ||
          next.first < -BOUNDARY_VISUALIZE_FAILSAFE_LIMIT ||
          next.second > BOUNDARY_VISUALIZE_FAILSAFE_LIMIT ||
          next.second < -BOUNDARY_VISUALIZE_FAILSAFE_LIMIT) {
        boundsWarning = true;
        continue;
      }
      if (alreadyChecked.find(next) ==
          alreadyChecked.end()) // never check a coordinate twice
      {
        Position nextPos(startPosition.GetPositionX() +
                             next.first * BOUNDARY_VISUALIZE_STEP_SIZE,
                         startPosition.GetPositionY() +
                             next.second * BOUNDARY_VISUALIZE_STEP_SIZE,
    ...
    if (fill || hasOutOfBoundsNeighbor)
      if (TempSummon *point = owner->SummonCreature(
              BOUNDARY_VISUALIZE_CREATURE,
              Position(startPosition.GetPositionX() +
                           front.first * BOUNDARY_VISUALIZE_STEP_SIZE,
                       startPosition.GetPositionY() +
                           front.second * BOUNDARY_VISUALIZE_STEP_SIZE,
                       spawnZ),
...
Creature *CreatureAI::DoSummon(uint32 entry, Position const &pos,
