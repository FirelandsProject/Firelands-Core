...
SmartAI::SmartAI(Creature *creature)
    : CreatureAI(creature), _isCharmed(false), _followCreditType(0),
      _followArrivedTimer(0), _followCredit(0), _followArrivedEntry(0),
      _followDist(0.f), _followAngle(0.f), _escortState(SMART_ESCORT_NONE),
      _escortNPCFlags(0), _escortInvokerCheckTimer(1000),
      _currentWaypointNode(0), _waypointReached(false), _waypointPauseTimer(0),
      _waypointPauseForced(false), _repeatWaypointPath(false),
      _OOCReached(false), _waypointPathEnded(false), _run(true),
      _evadeDisabled(false), _canAutoAttack(true), _canCombatMove(true),
      _invincibilityHpLevel(0), _despawnTime(0), _despawnState(0),
      _conditionsTimer(0), _gossipReturn(false) {
  _hasConditions = sConditionMgr->HasConditionsForNotGroupedEntry(
      CONDITION_SOURCE_TYPE_CREATURE_TEMPLATE_VEHICLE, creature->GetEntry());
}

bool SmartAI::IsAIControlled() const { return !_isCharmed; }
...
void SmartAI::StartPath(bool run /* = false*/, uint32 pathId /* = 0*/,
                        bool repeat /* = false*/, Unit *invoker /* = nullptr*/,
                        uint32 nodeId /* = 1*/) {
  ...
  GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_START, nullptr,
                                _currentWaypointNode, GetScript()->GetPathId());
...
void SmartAI::PausePath(uint32 delay, bool forced) {
  if (!HasEscortState(SMART_ESCORT_ESCORTING)) {
    ...
    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() ==
      ...
      GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_PAUSED, nullptr,
                                    waypointInfo.first, waypointInfo.second);
    }
    return;
  }
  ...
  if (HasEscortState(SMART_ESCORT_PAUSED)) {
    LOG_ERROR("misc",
              "SmartAI::PausePath: Creature entry {} wanted to pause waypoint "
              "(current waypoint: {}) movement while already paused, ignoring.",
              me->GetEntry(), _currentWaypointNode);
    return;
  }
...
void SmartAI::EndPath(bool fail) {
  ...
  // End Path events should be only processed if it was SUCCESSFUL stop or stop
  // called by SMART_ACTION_WAYPOINT_STOP
  if (fail)
    return;
...
void SmartAI::ReturnToLastOOCPos() {
  ...
  me->SetWalk(false);
  me->GetMotionMaster()->MovePoint(SMART_ESCORT_LAST_OOC_POINT,
                                   me->GetHomePosition());
}
...
void SmartAI::UpdatePath(uint32 diff) {
  ...
  if (_escortInvokerCheckTimer < diff) {
    if (!IsEscortInvokerInRange()) {
      ...
      // allow to properly hook out of range despawn action, which in most cases
      // should perform the same operation as dying
...
void SmartAI::UpdateAI(uint32 diff) {
  ...
  if (_followGuid) {
    if (_followArrivedTimer < diff) {
      if (me->FindNearestCreature(_followArrivedEntry, INTERACTION_DISTANCE,
                                  true)) {
        StopFollow(true);
        return;
      }
...
void SmartAI::WaypointPathStarted(uint32 pathId) {
  if (!HasEscortState(SMART_ESCORT_ESCORTING)) {
    ...
    GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_START, nullptr, 1,
                                  pathId);
    return;
  }
}
...
void SmartAI::WaypointReached(uint32 nodeId, uint32 pathId) {
  if (!HasEscortState(SMART_ESCORT_ESCORTING)) {
    GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_REACHED, nullptr, nodeId,
                                  pathId);
    return;
  }
...
void SmartAI::WaypointPathEnded(uint32 nodeId, uint32 pathId) {
  if (!HasEscortState(SMART_ESCORT_ESCORTING)) {
    GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_ENDED, nullptr, nodeId,
                                  pathId);
    return;
  }
}
...
void SmartAI::EnterEvadeMode(EvadeReason /*why*/) {
  ...
  GetScript()->ProcessEventsFor(
      SMART_EVENT_EVADE); // must be after _EnterEvadeMode (spells, auras, ...)
  ...
  if (Unit *owner = me->GetCharmerOrOwner()) {
    ...
    ReturnToLastOOCPos();
  } else if (Unit *target = _followGuid
                                ? ObjectAccessor::GetUnit(*me, _followGuid)
                                : nullptr) {
...
void SmartAI::JustReachedHome() {
  ...
  if (!formation || formation->getLeader() == me || !formation->isFormed()) {
    if (me->GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_IDLE) !=
            WAYPOINT_MOTION_TYPE &&
        me->GetWaypointPath())
      me->GetMotionMaster()->MovePath(me->GetWaypointPath(), true);
    else if (me->GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_IDLE) !=
                 CYCLIC_SPLINE_MOTION_TYPE &&
             me->GetCyclicSplinePathId())
...
void SmartAI::DamageTaken(Unit *doneBy, uint32 &damage) {
  ...
  if (_invincibilityHpLevel &&
      (damage >= me->GetHealth() - _invincibilityHpLevel))
    damage = me->GetHealth() -
             _invincibilityHpLevel; // damage should not be nullified, because
...
void SmartAI::InitializeAI() {
  ...
  _followGuid.Clear(); // do not reset follower on Reset(), we need it after
                       // combat evade
  _followDist = 0;
  _followAngle = 0;
  _followCredit = 0;
...
void SmartAI::OnCharmed(bool /*isNew*/) {
  ...
  if (charmed) // do this before we change charmed state, as charmed state might
               // prevent these things from processing
  {
  ...
  if (!charmed && !me->IsInEvadeMode()) {
    ...
    if (me->LastCharmerGUID) {
      if (!me->HasReactState(REACT_PASSIVE))
        if (Unit *lastCharmer =
                ObjectAccessor::GetUnit(*me, me->LastCharmerGUID))
...
void SmartAI::SetDisableGravity(bool fly) { me->SetDisableGravity(fly); }

void SmartAI::SetCanFly(bool fly) { me->SetCanFly(fly); }

void SmartAI::SetSwim(bool swim) { me->SetSwim(swim); }

void SmartAI::SetEvadeDisabled(bool disable) { _evadeDisabled = disable; }
...
bool SmartAI::GossipSelect(Player *player, uint32 menuId, uint32 gossipListId) {
  _gossipReturn = false;
  GetScript()->ProcessEventsFor(SMART_EVENT_GOSSIP_SELECT, player, menuId,
                                gossipListId);
  return _gossipReturn;
}

bool SmartAI::GossipSelectCode(Player * /*player*/, uint32 /*menuId*/,
                               uint32 /*gossipListId*/, char const * /*code*/) {
  return false;
}
...
void SmartAI::SetCombatMove(bool on) {
  ...
  if (me->IsInCombat()) {
    if (on && !me->HasReactState(REACT_PASSIVE) && me->GetVictim() &&
        me->GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_ACTIVE) ==
            MAX_MOTION_TYPE) {
      SetRun(_run);
      me->GetMotionMaster()->MoveChase(me->GetVictim());
    } else if (!on && me->GetMotionMaster()->GetMotionSlotType(
                          MOTION_SLOT_ACTIVE) == CHASE_MOTION_TYPE) {
      me->GetMotionMaster()->Clear(MOTION_SLOT_ACTIVE);
      me->StopMoving();
    }
  }
...
bool SmartGameObjectAI::OnReportUse(Player *player) {
  ...
  GetScript()->ProcessEventsFor(SMART_EVENT_GOSSIP_HELLO, player, 1, 0, false,
                                nullptr, me);
  return false;
}

// Called when a player selects a gossip item in the gameobject's gossip menu.
bool SmartGameObjectAI::GossipSelect(Player *player, uint32 menuId,
                                     uint32 gossipListId) {
  _gossipReturn = false;
  GetScript()->ProcessEventsFor(SMART_EVENT_GOSSIP_SELECT, player, menuId,
                                gossipListId, false, nullptr, me);
  return _gossipReturn;
}

// Called when a player selects a gossip with a code in the gameobject's gossip
// menu.
bool SmartGameObjectAI::GossipSelectCode(Player * /*player*/, uint32 /*menuId*/,
                                         uint32 /*gossipListId*/,
                                         char const * /*code*/) {
  return false;
}
...
void SmartGameObjectAI::QuestAccept(Player *player, Quest const *quest) {
  GetScript()->ProcessEventsFor(SMART_EVENT_ACCEPTED_QUEST, player,
                                quest->GetQuestId(), 0, false, nullptr, me);
}

// Called when a player selects a quest reward.
void SmartGameObjectAI::QuestReward(Player *player, Quest const *quest,
                                    uint32 opt) {
  GetScript()->ProcessEventsFor(SMART_EVENT_REWARD_QUEST, player,
                                quest->GetQuestId(), opt, false, nullptr, me);
}
...
public:
  ...
  bool OnTrigger(Player *player, AreaTriggerEntry const *trigger) override {
    ...
    LOG_DEBUG("scripts.ai", "AreaTrigger {} is using SmartTrigger script",
              trigger->ID);
    SmartScript script;
