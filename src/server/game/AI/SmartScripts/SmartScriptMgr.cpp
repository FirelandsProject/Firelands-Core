...
void SmartAIMgr::LoadSmartAIFromDB()
{
    ...
    do
    {
        ...
        temp.event.event_phase_mask = fields[5].Get<uint8>();
        temp.event.event_chance = fields[6].Get<uint8>();
        temp.event.event_flags = fields[7].Get<uint8>();
        ...
        temp.action.type = (SMART_ACTION)fields[12].Get<uint8>();
        temp.action.raw.param1 = fields[13].Get<uint32>();
        temp.action.raw.param2 = fields[14].Get<uint32>();
        temp.action.raw.param3 = fields[15].Get<uint32>();
        temp.action.raw.param4 = fields[16].Get<uint32>();
        temp.action.raw.param5 = fields[17].Get<uint32>();
        temp.action.raw.param6 = fields[18].Get<uint32>();

        temp.target.type = (SMARTAI_TARGETS)fields[19].Get<uint8>();
        temp.target.raw.param1 = fields[20].Get<uint32>();
        temp.target.raw.param2 = fields[21].Get<uint32>();
        temp.target.raw.param3 = fields[22].Get<uint32>();
        temp.target.x = fields[23].Get<float>();
        temp.target.y = fields[24].Get<float>();
        temp.target.z = fields[25].Get<float>();
        temp.target.o = fields[26].Get<float>();
...
bool SmartAIMgr::IsEventValid(SmartScriptHolder& e)
{
    ...
    {
        LOG_ERROR("sql.sql", "SmartAIMgr: EntryOrGuid {} using event({}) has invalid action type ({}), skipped.", e.entryOrGuid, e.event_id, e.GetActionType());
        return false;
    }
    ...
    else
    {
        ...
        switch (type)
        {
            ...
            case SMART_EVENT_DISTANCE_PLAYER:
            {
                ...
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr: Event SMART_EVENT_DISTANCE_PLAYER no aura set but isPresent is set, skipped.");
                    return false;
                }
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_SOUND:
            ...
            {
                LOG_ERROR("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses invalid Text Range {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.sound.range);
                return false;
            }
            break;
        ...
        case SMART_ACTION_EQUIP:
        {
            ...
            if (e.GetScriptType() == SMART_SCRIPT_TYPE_CREATURE)
            {
                ...
                if (!einfo)
                {
                    LOG_ERROR("sql.sql", "SmartScript: SMART_ACTION_EQUIP uses non-existent equipment info id {} for creature {}, skipped.", equipId, e.entryOrGuid);
                    return false;
                }
...
/*bool SmartAIMgr::IsTextValid(SmartScriptHolder const& e, uint32 id) // unused
{
    ...
    if (e.entryOrGuid >= 0)
        ...
        if (!data)
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} using non-existent Creature guid {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
            return false;
        }
        else
            entry = data->id;
    ...
    if (error)
    {
        LOG_ERROR("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} using non-existent Text id {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.source_type, e.GetActionType(), id);
        return false;
    }
    return true;
}*/
...
void SmartAIMgr::LoadHelperStores()
{
    ...
    LOG_INFO("server.loading", ">> Loaded SmartAIMgr Helpers in {} ms", GetMSTimeDiffToNow(oldMSTime));
}
