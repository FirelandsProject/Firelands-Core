...
void SmartWaypointMgr::LoadFromDB() {
  ...
  do {
    ...
    bool smoothTransition = fields[8].Get<bool>();
    ...
    if (lastId != id)
      LOG_ERROR("sql.sql",
                "SmartWaypointMgr::LoadFromDB: Path entry {}, unexpected point "
                "id {}, expected {}.",
                entry, id, lastId);
...
void SmartAIMgr::LoadSmartAIFromDB() {
  ...
  do {
    ...
    if (temp.entryOrGuid >= 0) {
      ...
      case SMART_SCRIPT_TYPE_CREATURE: {
        CreatureTemplate const *creatureInfo =
            sObjectMgr->GetCreatureTemplate((uint32)temp.entryOrGuid);
        if (!creatureInfo) {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: Creature entry ({}) does "
        ...
        if (creatureInfo->AIName != "SmartAI") {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: Creature entry ({}) is not "
                    "using SmartAI, skipped loading.",
                    uint32(temp.entryOrGuid));
          continue;
        }
        break;
      ...
      case SMART_SCRIPT_TYPE_GAMEOBJECT: {
        GameObjectTemplate const *gameObjectInfo =
            sObjectMgr->GetGameObjectTemplate((uint32)temp.entryOrGuid);
        if (!gameObjectInfo) {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: GameObject entry ({}) does "
                    "not exist, skipped loading.",
                    uint32(temp.entryOrGuid));
          continue;
        }
        ...
        if (gameObjectInfo->AIName != "SmartGameObjectAI") {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: GameObject entry ({}) is "
                    "not using SmartGameObjectAI, skipped loading.",
                    uint32(temp.entryOrGuid));
          continue;
        }
        break;
      ...
      case SMART_SCRIPT_TYPE_AREATRIGGER: {
        if (!sAreaTriggerStore.LookupEntry((uint32)temp.entryOrGuid)) {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: AreaTrigger entry ({}) "
      ...
      switch (source_type) {
      case SMART_SCRIPT_TYPE_CREATURE: {
        CreatureData const *creature =
            sObjectMgr->GetCreatureData(uint32(std::abs(temp.entryOrGuid)));
        if (!creature) {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: Creature guid ({}) does "
                    "not exist, skipped loading.",
        ...
        CreatureTemplate const *creatureInfo =
            sObjectMgr->GetCreatureTemplate(creature->id);
        if (!creatureInfo) {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: Creature entry ({}) guid "
                    "({}) does not exist, skipped loading.",
                    creature->id, uint32(std::abs(temp.entryOrGuid)));
          continue;
        }
        ...
        if (creatureInfo->AIName != "SmartAI") {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: Creature entry ({}) guid "
                    "({}) is not using SmartAI, skipped loading.",
                    creature->id, uint32(std::abs(temp.entryOrGuid)));
          continue;
        }
        break;
      ...
      case SMART_SCRIPT_TYPE_GAMEOBJECT: {
        GameObjectData const *gameObject =
            sObjectMgr->GetGameObjectData(uint32(std::abs(temp.entryOrGuid)));
        if (!gameObject) {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: GameObject guid ({}) does "
                    "not exist, skipped loading.",
                    uint32(std::abs(temp.entryOrGuid)));
          continue;
        }
        ...
        GameObjectTemplate const *gameObjectInfo =
            sObjectMgr->GetGameObjectTemplate(gameObject->id);
        if (!gameObjectInfo) {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: GameObject entry ({}) guid "
                    "({}) does not exist, skipped loading.",
                    gameObject->id, uint32(std::abs(temp.entryOrGuid)));
          continue;
        }
        ...
        if (gameObjectInfo->AIName != "SmartGameObjectAI") {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: GameObject entry ({}) guid "
                    "({}) is not using SmartGameObjectAI, skipped loading.",
                    gameObject->id, uint32(std::abs(temp.entryOrGuid)));
          continue;
        }
        break;
      ...
      default:
        LOG_ERROR("sql.sql",
                  "SmartAIMgr::LoadSmartAIFromDB: GUID-specific scripting not "
                  "yet implemented for source_type {}",
                  (uint32)source_type);
        continue;
      }
    }
    ...
    temp.target.x = fields[24].Get<float>();
    temp.target.y = fields[25].Get<float>();
    temp.target.z = fields[26].Get<float>();
    temp.target.o = fields[27].Get<float>();
    ...
    case SMART_EVENT_TARGET_BUFFED:
      if (temp.event.minMaxRepeat.repeatMin == 0 &&
        ...
        temp.event.event_flags |= SMART_EVENT_FLAG_NOT_REPEATABLE;
        LOG_ERROR("sql.sql",
                  "SmartAIMgr::LoadSmartAIFromDB: Entry {} SourceType {}, "
                  "Event {}, Missing Repeat flag.",
  ...
  for (uint8 i = 0; i < SMART_SCRIPT_TYPE_MAX; ++i) {
    for (SmartAIEventMap::iterator itr = mEventMap[i].begin();
         itr != mEventMap[i].end(); ++itr) {
      for (SmartScriptHolder const &e : itr->second) {
        if (e.link) {
          if (!FindLinkedEvent(itr->second, e.link)) {
            LOG_ERROR("sql.sql",
                      "SmartAIMgr::LoadSmartAIFromDB: Entry {} SourceType {}, "
                      "Event {}, Link Event {} not found or invalid.",
        ...
        if (e.GetEventType() == SMART_EVENT_LINK) {
          if (!FindLinkedSourceEvent(itr->second, e.event_id)) {
            LOG_ERROR("sql.sql",
                      "SmartAIMgr::LoadSmartAIFromDB: Entry {} SourceType {}, "
                      "Event {}, Link Source Event not found or invalid. Event "
                      "will never trigger.",
  ...
  LOG_INFO("server.loading", ">> Loaded {} SmartAI scripts in {} ms", count,
...
SmartAIEventList SmartAIMgr::GetScript(int32 entry, SmartScriptType type) {
  ...
  if (mEventMap[uint32(type)].find(entry) != mEventMap[uint32(type)].end())
    ...
    if (entry > 0) // first search is for guid (negative), do not drop error if
                   // not found
...
SmartScriptHolder &SmartAIMgr::FindLinkedSourceEvent(SmartAIEventList &list,
                                                     uint32 eventId) {
  SmartAIEventList::iterator itr = std::find_if(
      list.begin(), list.end(),
...
SmartScriptHolder &SmartAIMgr::FindLinkedEvent(SmartAIEventList &list,
                                               uint32 link) {
  SmartAIEventList::iterator itr =
      std::find_if(list.begin(), list.end(), [link](SmartScriptHolder &linked) {
        return linked.event_id == link &&
               linked.GetEventType() == SMART_EVENT_LINK;
      });
...
/*static*/ bool SmartAIMgr::EventHasInvoker(SMART_EVENT event) {
  switch (event) { // white list of events that actually have an invoker passed
                   // to them
...
bool SmartAIMgr::IsTargetValid(SmartScriptHolder const &e) {
  if (std::abs(e.target.o) > 2 * float(M_PI))
    LOG_ERROR(
        ...
        "abs(`target.o` = {}) > 2*PI (orientation is expressed in radians)",
  ...
  default:
    LOG_ERROR("sql.sql",
              "SmartAIMgr: Not handled target_type({}), Entry {} SourceType {} "
...
bool SmartAIMgr::IsMinMaxValid(SmartScriptHolder const &e, uint32 min,
                               uint32 max) {
  if (max < min) {
    LOG_ERROR("sql.sql",
              "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
...
bool SmartAIMgr::NotNULL(SmartScriptHolder const &e, uint32 data) {
  if (!data) {
    LOG_ERROR("sql.sql",
              "SmartAIMgr: Entry {} SourceType {} Event {} Action {} Parameter "
              "can not be nullptr, skipped.",
              e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
    return false;
  }
  return true;
...
bool SmartAIMgr::IsAnimKitValid(SmartScriptHolder const &e, uint32 entry) {
  if (!sAnimKitStore.LookupEntry(entry)) {
    LOG_ERROR("sql.sql",
              "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
              "non-existent AnimKit entry {}, skipped.",
              e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
              entry);
    return false;
  }
...
bool SmartAIMgr::IsEventValid(SmartScriptHolder &e) {
  if (e.event.type >= SMART_EVENT_END) {
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid {} using event({}) has invalid event "
              "type ({}), skipped.",
  ...
  if (e.GetScriptType() != SMART_SCRIPT_TYPE_TIMED_ACTIONLIST &&
    ...
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid {}, event type {} can not be used for "
              "Script type {}",
  ...
  if (e.action.type <= 0 || e.action.type >= SMART_ACTION_END) {
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid {} using event({}) has invalid action "
              "type ({}), skipped.",
  ...
  if (e.event.event_phase_mask > SMART_EVENT_PHASE_ALL) {
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid {} using event({}) has invalid phase "
              "mask ({}), skipped.",
  ...
  if (e.link && e.link == e.event_id) {
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid {} SourceType {}, Event {}, Event is "
  ...
  if (e.GetScriptType() == SMART_SCRIPT_TYPE_TIMED_ACTIONLIST) {
    ...
    case SMART_EVENT_SPELLHIT_TARGET:
      if (e.event.spellHit.spell) {
        ...
        if (e.event.spellHit.school &&
          ...
          LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} "
                    "uses Spell entry {} with invalid school mask, skipped.",
                    e.entryOrGuid, e.GetScriptType(), e.event_id,
                    e.GetActionType(), e.event.spellHit.spell);
          return false;
        }
    ...
    case SMART_EVENT_RESPAWN:
      if (e.event.respawn.type == SMART_SCRIPT_RESPAWN_CONDITION_MAP &&
        ...
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                  "non-existent Map entry {}, skipped.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id,
      ...
      if (e.event.respawn.type == SMART_SCRIPT_RESPAWN_CONDITION_AREA &&
        ...
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                  "non-existent Area entry {}, skipped.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id,
    ...
    case SMART_EVENT_REWARD_QUEST:
      ...
      if (!IsMinMaxValid(e, e.event.quest.cooldownMin,
                         e.event.quest.cooldownMax))
        return false;
      break;
    case SMART_EVENT_RECEIVE_EMOTE: {
    ...
    case SMART_EVENT_EVENT_PHASE_CHANGE: {
      if (!e.event.eventPhaseChange.phasemask) {
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has "
                  "no param set, event won't be executed!.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id,
                  e.GetActionType());
        return false;
      }
      ...
      if (e.event.eventPhaseChange.phasemask > SMART_EVENT_PHASE_ALL) {
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                  "invalid phasemask {}, skipped.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id,
                  e.GetActionType(), e.event.eventPhaseChange.phasemask);
        return false;
      }

      if (e.event.event_phase_mask &&
          !(e.event.event_phase_mask & e.event.eventPhaseChange.phasemask)) {
        LOG_ERROR(
            "sql.sql",
            "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses event "
            "phasemask {} and incompatible event_param1 {}, skipped.",
            e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
            e.event.event_phase_mask, e.event.eventPhaseChange.phasemask);
        return false;
      }
    ...
    case SMART_EVENT_DISTANCE_CREATURE:
      ...
      if (e.event.distance.entry != 0 &&
        ...
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Event SMART_EVENT_DISTANCE_CREATURE using "
                  "invalid creature entry {}, skipped.",
    ...
    case SMART_EVENT_DISTANCE_GAMEOBJECT:
      ...
      if (e.event.distance.guid != 0 &&
        ...
        LOG_ERROR("sql.sql",
                  ...
                  "invalid gameobject guid {}, skipped.",
                  e.event.distance.guid);
        return false;
      }
      ...
      if (e.event.distance.entry != 0 &&
        ...
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Event SMART_EVENT_DISTANCE_GAMEOBJECT using "
                  "invalid gameobject entry {}, skipped.",
                  e.event.distance.entry);
        return false;
      }
      break;
    ...
    default:
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Not handled event_type({}), Entry {} SourceType "
                "{} Event {} Action {}, skipped.",
  ...
  case SMART_ACTION_RANDOM_SOUND: {
    if (std::all_of(std::begin(e.action.randomSound.sounds),
                    std::end(e.action.randomSound.sounds),
                    [](uint32 sound) { return sound == 0; })) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} does "
                "not have any non-zero sound",
                e.entryOrGuid, e.GetScriptType(), e.event_id,
                e.GetActionType());
      return false;
    }
  ...
  case SMART_ACTION_CAST: {
    ...
    SpellInfo const *spellInfo =
        sSpellMgr->AssertSpellInfo(e.action.cast.spell);
    for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j) {
      if (spellInfo->Effects[j].IsEffect(SPELL_EFFECT_KILL_CREDIT) ||
          spellInfo->Effects[j].IsEffect(SPELL_EFFECT_KILL_CREDIT2)) {
        if (spellInfo->Effects[j].TargetA.GetTarget() == TARGET_UNIT_CASTER)
          LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} "
                    "Effect: SPELL_EFFECT_KILL_CREDIT: (SpellId: {} targetA: "
                    "{} - targetB: {}) has invalid target for this Action",
                    e.entryOrGuid, e.GetScriptType(), e.event_id,
                    e.GetActionType(), e.action.cast.spell,
                    spellInfo->Effects[j].TargetA.GetTarget(),
                    spellInfo->Effects[j].TargetB.GetTarget());
      }
    }
    break;
  }
  ...
  case SMART_ACTION_SET_EVENT_PHASE:
    if (e.action.setEventPhase.phase >= SMART_EVENT_PHASE_MAX) {
      LOG_ERROR(
          "sql.sql",
          "SmartAIMgr: Entry {} SourceType {} Event {} Action {} attempts to "
          "set phase {}. Phase mask cannot be used past phase {}, skipped.",
          e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
  ...
  case SMART_ACTION_INC_EVENT_PHASE:
    if (!e.action.incEventPhase.inc && !e.action.incEventPhase.dec) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} is "
  ...
  case SMART_ACTION_RANDOM_PHASE: {
    if (std::all_of(std::begin(e.action.randomPhase.phases),
                    std::end(e.action.randomPhase.phases),
                    [](uint32 phase) { return phase == 0; })) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} does "
                "not have any non-zero phase",
                e.entryOrGuid, e.GetScriptType(), e.event_id,
                e.GetActionType());
      return false;
    }

    if (std::any_of(std::begin(e.action.randomPhase.phases),
                    std::end(e.action.randomPhase.phases), [](uint32 phase) {
  ...
  case SMART_ACTION_SUMMON_CREATURE: {
    ...
    CacheSpellContainerBounds sBounds =
        GetSummonCreatureSpellContainerBounds(e.action.summonCreature.creature);
    for (CacheSpellContainer::const_iterator itr = sBounds.first;
         itr != sBounds.second; ++itr)
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} "
                "creature summon: There is a summon spell for creature entry "
                "{} (SpellId: {}, effect: {})",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.summonCreature.creature, itr->second.first,
                itr->second.second);
    ...
    if (e.action.summonCreature.type < TEMPSUMMON_TIMED_OR_DEAD_DESPAWN ||
      ...
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                "incorrect TempSummonType {}, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
  ...
  case SMART_ACTION_CALL_KILLEDMONSTER: {
    ...
    CacheSpellContainerBounds sBounds =
        GetKillCreditSpellContainerBounds(e.action.killedMonster.creature);
    for (CacheSpellContainer::const_iterator itr = sBounds.first;
         itr != sBounds.second; ++itr)
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} Kill "
                "Credit: There is a killcredit spell for creatureEntry {} "
                "(SpellId: {} effect: {})",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.killedMonster.creature, itr->second.first,
                itr->second.second);

    if (e.GetTargetType() == SMART_TARGET_POSITION) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                "incorrect TargetType {}, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.GetTargetType());
      return false;
    }
  ...
  case SMART_ACTION_SET_SHEATH:
    if (e.action.setSheath.sheath &&
      ...
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                "incorrect Sheath state {}, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
  ...
  case SMART_ACTION_SET_REACT_STATE: {
    if (e.action.react.state > REACT_AGGRESSIVE) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Creature {} Event {} Action {} uses invalid React "
                "State {}, skipped.",
                e.entryOrGuid, e.event_id, e.GetActionType(),
  ...
  case SMART_ACTION_SUMMON_GO: {
    ...
    CacheSpellContainerBounds sBounds =
        GetSummonGameObjectSpellContainerBounds(e.action.summonGO.entry);
    for (CacheSpellContainer::const_iterator itr = sBounds.first;
         itr != sBounds.second; ++itr)
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} "
                "gameobject summon: There is a summon spell for gameobject "
                "entry {} (SpellId: {}, effect: {})",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.summonGO.entry, itr->second.first, itr->second.second);
    break;
  }
  ...
  case SMART_ACTION_ADD_ITEM: {
    ...
    CacheSpellContainerBounds sBounds =
        GetCreateItemSpellContainerBounds(e.action.item.entry);
    for (CacheSpellContainer::const_iterator itr = sBounds.first;
         itr != sBounds.second; ++itr)
      LOG_ERROR(
          "sql.sql",
          "SmartAIMgr: Entry {} SourceType {} Event {} Action {} Create Item: "
          "There is a create item spell for item {} (SpellId: {} effect: {})",
          e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
          e.action.item.entry, itr->second.first, itr->second.second);
    break;
  }
  ...
  case SMART_ACTION_INSTALL_AI_TEMPLATE:
    if (e.action.installTtemplate.id >= SMARTAI_TEMPLATE_END) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Creature {} Event {} Action {} uses non-existent "
                "AI template id {}, skipped.",
                e.entryOrGuid, e.event_id, e.GetActionType(),
  ...
  case SMART_ACTION_WP_START: {
    WaypointPath const *path =
        sSmartWaypointMgr->GetPath(e.action.wpStart.pathID);
    if (!path || path->Nodes.empty()) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Creature {} Event {} Action {} uses non-existent "
                "WaypointPath id {}, skipped.",
                e.entryOrGuid, e.event_id, e.GetActionType(),
  ...
  case SMART_ACTION_CALL_RANDOM_RANGE_TIMED_ACTIONLIST: {
    if (!IsMinMaxValid(e, e.action.randTimedActionList.actionLists[0],
  ...
  case SMART_ACTION_GAME_EVENT_STOP: {
    ...
    if (eventId < 1 || eventId >= events.size()) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                "non-existent event, eventId {}, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
  ...
  case SMART_ACTION_SET_INGAME_PHASE_ID: {
    ...
    if (apply != 0 && apply != 1) {
      LOG_ERROR("sql.sql",
                "SmartScript: SMART_ACTION_SET_INGAME_PHASE_ID uses invalid "
                "apply value {} (Should be 0 or 1) for creature {}, skipped",
                apply, e.entryOrGuid);
      return false;
    }
    ...
    if (!sPhaseStore.LookupEntry(phaseId)) {
      LOG_ERROR("sql.sql",
                "SmartScript: SMART_ACTION_SET_INGAME_PHASE_ID uses invalid "
                "phaseid {} for creature {}, skipped",
                phaseId, e.entryOrGuid);
      return false;
    }
    break;
  }
  case SMART_ACTION_SET_INGAME_PHASE_GROUP: {
    ...
    if (apply != 0 && apply != 1) {
      LOG_ERROR("sql.sql",
                "SmartScript: SMART_ACTION_SET_INGAME_PHASE_GROUP uses invalid "
                "apply value {} (Should be 0 or 1) for creature {}, skipped",
                apply, e.entryOrGuid);
      return false;
    }
    ...
    if (!sDBCManager.GetPhasesForGroup(phaseGroup)) {
      LOG_ERROR("sql.sql",
                "SmartScript: SMART_ACTION_SET_INGAME_PHASE_GROUP uses invalid "
                "phase group id {} for creature {}, skipped",
                phaseGroup, e.entryOrGuid);
      return false;
    }
    break;
  }
  case SMART_ACTION_SET_INST_DATA: {
    if (e.action.setInstanceData.type > 1) {
      LOG_ERROR("sql.sql",
                "Entry {} SourceType {} Event {} Action {} uses invalid data "
                "type {} (value range 0-1), skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
      ...
      if (e.action.setInstanceData.data > TO_BE_DECIDED) {
        LOG_ERROR("sql.sql",
                  "Entry {} SourceType {} Event {} Action {} uses invalid boss "
                  "state {} (value range 0-5), skipped.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id,
  ...
  case SMART_ACTION_REMOVE_AURAS_BY_TYPE: {
    if (e.action.auraType.type >= TOTAL_AURAS) {
      LOG_ERROR("sql.sql",
                "Entry {} SourceType {} Event {} Action {} uses invalid data "
                "type {} (value range 0-TOTAL_AURAS), skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.auraType.type);
      return false;
    }
    break;
  }
  case SMART_ACTION_RESPAWN_BY_SPAWNID:
    if (!sObjectMgr->GetSpawnData(
            SpawnObjectType(e.action.respawnData.spawnType),
            e.action.respawnData.spawnId)) {
      LOG_ERROR("sql.sql",
                "Entry {} SourceType {} Event {} Action {} specifies invalid "
                "spawn data ({},{})",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
  ...
  case SMART_ACTION_ENABLE_TEMP_GOBJ:
    if (!e.action.enableTempGO.duration) {
      LOG_ERROR(
          "sql.sql",
          "Entry {} SourceType {} Event {} Action {} does not specify duration",
          e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
      return false;
    }
    break;
  case SMART_ACTION_PAUSE_MOVEMENT:
    if (!e.action.pauseMovement.pauseTimer) {
      LOG_ERROR("sql.sql",
                "Entry {} SourceType {} Event {} Action {} does not specify "
                "pause duration",
                e.entryOrGuid, e.GetScriptType(), e.event_id,
                e.GetActionType());
      return false;
  ...
  case SMART_ACTION_OVERRIDE_LIGHT: {
    AreaTableEntry const *areaEntry =
        sAreaTableStore.LookupEntry(e.action.overrideLight.zoneId);
    if (!areaEntry) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                "non-existent zoneId {}, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
    ...
    if (areaEntry->ParentAreaID != 0) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                "subzone (ID: {}) instead of zone, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.overrideLight.zoneId);
      return false;
    }
    ...
    if (!sLightStore.LookupEntry(e.action.overrideLight.areaLightId)) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                "non-existent areaLightId {}, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.overrideLight.areaLightId);
      return false;
    }

    if (e.action.overrideLight.overrideLightId &&
        !sLightStore.LookupEntry(e.action.overrideLight.overrideLightId)) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                "non-existent overrideLightId {}, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.overrideLight.overrideLightId);
      return false;
    }
  ...
  case SMART_ACTION_OVERRIDE_WEATHER: {
    AreaTableEntry const *areaEntry =
        sAreaTableStore.LookupEntry(e.action.overrideWeather.zoneId);
    if (!areaEntry) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                "non-existent zoneId {}, skipped.",
  ...
  case SMART_ACTION_SET_AI_ANIM_KIT:
    if (!sAnimKitStore.LookupEntry(e.action.setAIAnimKit.animKitId)) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses "
                "non-existent AI anim Kit {}, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
  ...
  default:
    LOG_ERROR("sql.sql",
              "SmartAIMgr: Not handled action_type({}), event_type({}), Entry "
              "{} SourceType {} Event {}, skipped.",
  ...
  for (uint32 i = 0; i < sSpellMgr->GetSpellInfoStoreSize(); ++i) {
    ...
    for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j) {
      if (spellInfo->Effects[j].IsEffect(SPELL_EFFECT_SUMMON))
        SummonCreatureSpellStore.insert(
            std::make_pair(uint32(spellInfo->Effects[j].MiscValue),
                           std::make_pair(i, SpellEffIndex(j))));

      else if (spellInfo->Effects[j].IsEffect(SPELL_EFFECT_SUMMON_OBJECT_WILD))
        SummonGameObjectSpellStore.insert(
            std::make_pair(uint32(spellInfo->Effects[j].MiscValue),
                           std::make_pair(i, SpellEffIndex(j))));

      else if (spellInfo->Effects[j].IsEffect(SPELL_EFFECT_KILL_CREDIT) ||
               spellInfo->Effects[j].IsEffect(SPELL_EFFECT_KILL_CREDIT2))
        KillCreditSpellStore.insert(
            std::make_pair(uint32(spellInfo->Effects[j].MiscValue),
                           std::make_pair(i, SpellEffIndex(j))));

      else if (spellInfo->Effects[j].IsEffect(SPELL_EFFECT_CREATE_ITEM))
        CreateItemSpellStore.insert(
            std::make_pair(uint32(spellInfo->Effects[j].ItemType),
                           std::make_pair(i, SpellEffIndex(j))));
    }
  }

  LOG_INFO("server.loading", ">> Loaded SmartAIMgr Helpers in {} ms",
           GetMSTimeDiffToNow(oldMSTime));
}
...
SmartAIMgr::GetSummonCreatureSpellContainerBounds(uint32 creatureEntry) const {
  return SummonCreatureSpellStore.equal_range(creatureEntry);
}

CacheSpellContainerBounds SmartAIMgr::GetSummonGameObjectSpellContainerBounds(
    uint32 gameObjectEntry) const {
  return SummonGameObjectSpellStore.equal_range(gameObjectEntry);
}

CacheSpellContainerBounds
SmartAIMgr::GetKillCreditSpellContainerBounds(uint32 killCredit) const {
  return KillCreditSpellStore.equal_range(killCredit);
}

CacheSpellContainerBounds
SmartAIMgr::GetCreateItemSpellContainerBounds(uint32 itemId) const {
