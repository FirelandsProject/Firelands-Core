...
void CombatAI::UpdateAI(uint32 diff) {
  ...
  if (uint32 spellId = events.ExecuteEvent()) {
    ...
    events.ScheduleEvent(spellId, AISpellInfo[spellId].cooldown +
                                      rand32() % AISpellInfo[spellId].cooldown);
  } else
    DoMeleeAttackIfReady();
}
...
ArcherAI::ArcherAI(Creature *c) : CreatureAI(c) {
  if (!me->m_spells[0])
    LOG_ERROR("misc",
              "ArcherAI set for creature (entry = {}) with spell1=0. AI will "
              "do nothing",
              me->GetEntry());
...
TurretAI::TurretAI(Creature *c) : CreatureAI(c) {
  if (!me->m_spells[0])
    LOG_ERROR("misc",
              "TurretAI set for creature (entry = {}) with spell1=0. AI will "
              "do nothing",
              me->GetEntry());
...
VehicleAI::VehicleAI(Creature *creature)
    : CreatureAI(creature), m_HasConditions(false),
...
void VehicleAI::OnCharmed(bool /*isNew*/) {
  ...
  if (!me->GetVehicleKit()->IsVehicleInUse() && !charmed &&
      m_HasConditions) // was used and has conditions
  {
...
void VehicleAI::LoadConditions() {
  m_HasConditions = sConditionMgr->HasConditionsForNotGroupedEntry(
