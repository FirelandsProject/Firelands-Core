...
FollowerAI::FollowerAI(Creature *creature)
    : ScriptedAI(creature), _updateFollowTimer(2500),
      _followState(STATE_FOLLOW_NONE), _questForFollow(0) {}
...
void FollowerAI::JustDied(Unit * /*killer*/) {
  if (!HasFollowState(STATE_FOLLOW_INPROGRESS) || !_leaderGUID ||
      !_questForFollow)
    return;
...
void FollowerAI::OwnerAttackedBy(Unit *other) {
  if (!me->HasReactState(REACT_PASSIVE) &&
      ShouldAssistPlayerInCombatAgainst(other))
    AttackStart(other);
}
  ...
  if (HasFollowState(STATE_FOLLOW_INPROGRESS) && !me->GetVictim()) {
    if (_updateFollowTimer <= uiDiff) {
      ...
      if (Player *player = GetLeaderForFollower()) {
        if (Group *group = player->GetGroup()) {
          for (GroupReference *groupRef = group->GetFirstMember();
               groupRef && (maxRangeExceeded || questAbandoned);
      ...
      if (maxRangeExceeded || questAbandoned) {
        LOG_DEBUG("scripts.ai.followerai",
                  "FollowerAI::UpdateAI: failed because player/group was to "
                  "far away or not found ({})",
...
void FollowerAI::StartFollow(Player *player, uint32 factionForFollower,
  ...
  LOG_DEBUG("scripts.ai.followerai",
            ...
            player->GetName().c_str(), _leaderGUID.ToString().c_str(),
            me->GetGUID().ToString().c_str());
}
