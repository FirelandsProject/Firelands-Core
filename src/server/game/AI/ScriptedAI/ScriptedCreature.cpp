...
void ScriptedAI::DoPlaySoundToSet(WorldObject* source, uint32 soundId)
{
    ...
    {
        LOG_ERROR("scripts", "Invalid soundId %u used in DoPlaySoundToSet (Source: TypeId %u, GUID %u)", soundId, source->GetTypeId(), source->GetGUIDLow());
        return;
    }
...
void ScriptedAI::DoTeleportPlayer(Unit* unit, float x, float y, float z, float o)
{
    ...
    if (Player* player = unit->ToPlayer())
        ...
        LOG_ERROR("scripts", "Creature " UI64FMTD " (Entry: %u) Tried to teleport non-player unit (Type: %u GUID: " UI64FMTD ") to x: %f y:%f z: %f o: %f. Aborted.",
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float minimumRange)
{
    ...
    CellCoord pair(Firelands::ComputeCellCoord(me->GetPositionX(), me->GetPositionY()));
    Cell cell(pair);
    cell.SetNoCreate();
    ...
    TypeContainerVisitor<Firelands::PlayerSearcher<Firelands::PlayerAtMinimumRangeAway>, GridTypeMapContainer> visitor(searcher);
...
bool ScriptedAI::EnterEvadeIfOutOfCombatArea(uint32 const diff)
{
    ...
    switch (me->GetEntry())
    {
        ...
        default: // For most of creatures that certain area is their home area.
            LOG_INFO("misc", "TSCR: EnterEvadeIfOutOfCombatArea used for creature entry %u, but does not have any definition. Using the default one.", me->GetEntry());
