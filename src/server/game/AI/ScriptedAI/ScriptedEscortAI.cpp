...
enum Points { POINT_LAST_POINT = 0xFFFFFF, POINT_HOME = 0xFFFFFE };

EscortAI::EscortAI(Creature *creature)
    : ScriptedAI(creature), _pauseTimer(2500), _playerCheckTimer(1000),
      _escortState(STATE_ESCORT_NONE),
      _maxPlayerDistance(DEFAULT_MAX_PLAYER_DISTANCE), _escortQuest(nullptr),
      _activeAttacker(true), _running(false), _instantRespawn(false),
      _returnToStart(false), _despawnAtEnd(true), _despawnAtFar(true),
      _manualPath(false), _hasImmuneToNPCFlags(false), _started(false),
      _ended(false), _resume(false) {}
...
void EscortAI::EnterEvadeMode(EvadeReason /*why*/) {
  ...
  if (HasEscortState(STATE_ESCORT_ESCORTING)) {
    ...
    ReturnToLastPoint();
    LOG_DEBUG("scripts", "EscortAI::EnterEvadeMode: left combat and is now "
...
void EscortAI::UpdateAI(uint32 diff) {
  // Waypoint Updating
  if (HasEscortState(STATE_ESCORT_ESCORTING) && !me->IsEngaged() &&
    ...
    if (_pauseTimer <= diff) {
      if (!HasEscortState(STATE_ESCORT_PAUSED)) {
        ...
        if (_ended) {
          ...
          if (_despawnAtEnd) {
            LOG_DEBUG("scripts", "EscortAI::UpdateAI: reached end of "
                                 "waypoints, despawning at end");
            if (_returnToStart) {
              Position respawnPosition;
              float orientation = 0.f;
              me->GetRespawnPosition(respawnPosition.m_positionX,
                                     respawnPosition.m_positionY,
                                     respawnPosition.m_positionZ, &orientation);
              ...
              LOG_DEBUG("scripts",
                        ...
                        respawnPosition.ToString().c_str());
  ...
  if (_despawnAtFar && HasEscortState(STATE_ESCORT_ESCORTING) && _playerGUID &&
      !me->IsEngaged() && !HasEscortState(STATE_ESCORT_RETURNING)) {
    if (_playerCheckTimer <= diff) {
      if (!IsPlayerOrGroupInRange()) {
        LOG_DEBUG("scripts", "EscortAI::UpdateAI: failed because player/group "
                             "was to far away or not found");

        bool isEscort = false;
        if (CreatureData const *creatureData = me->GetCreatureData())
          isEscort = (sWorld->getBoolConfig(CONFIG_RESPAWN_DYNAMIC_ESCORTNPC) &&
                      (creatureData->spawnGroupData->flags &
...
void EscortAI::MovementInform(uint32 type, uint32 id) {
  ...
  if (type == POINT_MOTION_TYPE) {
    ...
    if (id == POINT_LAST_POINT) {
      LOG_DEBUG("scripts",
                "EscortAI::MovementInform: returned to before combat position");
      me->SetWalk(!_running);
      RemoveEscortState(STATE_ESCORT_RETURNING);
    } else if (id == POINT_HOME) {
      LOG_DEBUG("scripts", "EscortAI::MovementInform: returned to home "
                           "location and restarting waypoint path");
      _started = false;
    }
  } else if (type == WAYPOINT_MOTION_TYPE) {
    ASSERT(id < _path.Nodes.size(),
           "EscortAI::MovementInform: referenced movement id ({}) points to "
           "non-existing node in loaded path",
           id);
    WaypointNode waypoint = _path.Nodes[id];

    LOG_DEBUG("scripts", "EscortAI::MovementInform: waypoint node {} reached",
              waypoint.Id);
...
void EscortAI::AddWaypoint(uint32 id, float x, float y, float z,
  ...
  waypoint.MoveType =
      _running ? WAYPOINT_MOVE_TYPE_RUN : WAYPOINT_MOVE_TYPE_WALK;
...
void EscortAI::Start(bool isActiveAttacker /* = true*/, bool run /* = false */,
  ...
  if (CreatureData const *cdata = me->GetCreatureData()) {
    if (sWorld->getBoolConfig(CONFIG_RESPAWN_DYNAMIC_ESCORTNPC) &&
        (cdata->spawnGroupData->flags & SPAWNGROUP_FLAG_ESCORTQUESTNPC))
      me->SaveRespawnTime(me->GetRespawnDelay());
  }

  if (me->IsEngaged()) {
    LOG_ERROR("scripts",
              "EscortAI::Start: (script: {}, creature entry: {}) attempts to "
              "Start while in combat",
  ...
  if (_path.Nodes.empty()) {
    LOG_ERROR(
        "scripts",
        "EscortAI::Start: (script: {}, creature entry: {}) starts with 0 "
        "waypoints (possible missing entry in script_waypoint. Quest: {}).",
        me->GetScriptName().c_str(), me->GetEntry(),
  ...
  if (_returnToStart && _instantRespawn)
    LOG_DEBUG("scripts",
              "EscortAI::Start: (script: {}, creature entry: {}) is set to "
              "return home after waypoint end and instant respawn at waypoint "
              "end. Creature will never despawn.",
              me->GetScriptName().c_str(), me->GetEntry());
  ...
  LOG_DEBUG("scripts",
            "EscortAI::Start: (script: {}, creature entry: {}) started with {} "
            "waypoints. ActiveAttacker = {}, Run = {}, Player = {}",
            me->GetScriptName().c_str(), me->GetEntry(),
            uint32(_path.Nodes.size()), _activeAttacker, _running,
            _playerGUID.ToString().c_str());
