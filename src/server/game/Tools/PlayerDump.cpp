...
inline auto FindColumnByName(TableStruct &tableStruct,
  ...
  return std::find_if(
      tableStruct.TableFields.begin(), tableStruct.TableFields.end(),
...
inline void MarkDependentColumn(TableStruct &tableStruct,
  ...
  if (itr == tableStruct.TableFields.end()) {
    LOG_FATAL("server.loading",
              "Column `%s` declared in table `%s` marked as dependent but "
              "doesn't exist, PlayerDump will not work properly, please update "
              "table definitions",
  ...
  if (itr->IsDependentField) {
    LOG_FATAL("server.loading",
              "Attempt to mark column `%s` in table `%s` as dependent column "
              "but already marked! please check your code.",
              columnName.c_str(), tableStruct.TableName.c_str());
    ABORT();
    return;
  }
...
inline void MarkWhereField(TableStruct &tableStruct,
  ...
  if (whereFieldItr == tableStruct.TableFields.end()) {
    LOG_FATAL("server.loading",
              "Column name `%s` set as 'WHERE' column for table `%s` doesn't "
              "exist. PlayerDump won't work properly",
...
inline void AssertBaseTable(BaseTable const &baseTable) {
  auto itr = std::find_if(CharacterTables.begin(), CharacterTables.end(),
...
void PlayerDump::InitializeTables() {
  ...
  for (DumpTable const &dumpTable : DumpTables) {
    ...
    default:
      LOG_FATAL("server.loading",
                "Wrong dump table type %u, probably added a new table type "
  ...
  for (TableStruct const &tableStruct : CharacterTables) {
    if (tableStruct.WhereFieldName.empty()) {
      LOG_FATAL(
          "server.loading",
          "Table `%s` defined in player dump doesn't have a WHERE query field",
          tableStruct.TableName.c_str());
      ABORT();
    }
  }
  ...
  LOG_INFO("server.loading", ">> Initialized tables for PlayerDump in %u ms.",
           GetMSTimeDiffToNow(oldMSTime));
}

// Low level functions
inline bool FindColumn(TableStruct const &ts, std::string const &str,
                       std::string const &column, std::string::size_type &s,
...
inline bool ValidateFields(TableStruct const &ts, std::string const &str,
                           size_t lineNumber) {
  std::string::size_type s = str.find("` VALUES (");
  if (s != std::string::npos) // old dump format (no column names)
    return true;

  // new format has insert with columns, need validation else we risk executing
  // an invalid query
  s = str.find("` (`");
  if (s == std::string::npos) {
    LOG_ERROR("misc",
              "LoadPlayerDump: (line " UI64FMTD ") dump format not recognized.",
              lineNumber);
    return false;
  }
  s += 4;
  ...
  if (e == std::string::npos || valPos == std::string::npos) {
    LOG_ERROR("misc",
              "LoadPlayerDump: (line " UI64FMTD ") unexpected end of line",
              lineNumber);
    return false;
  }
  ...
  do {
    ...
    if (columnIndex == -1) {
      LOG_ERROR("misc",
                "LoadPlayerDump: (line " UI64FMTD
                ") unknown column name `%s` for table `%s`, aborting due to "
                "incompatible DB structure.",
                lineNumber, column.c_str(), ts.TableName.c_str());
      return false;
    }
...
inline bool ChangeColumn(TableStruct const &ts, std::string &str,
                         std::string const &column, std::string const &with,
...
inline std::string GetColumn(TableStruct const &ts, std::string &str,
...
template <typename T, template <class, class, class...> class MapType,
          class... Rest>
inline T RegisterNewGuid(T oldGuid, MapType<T, T, Rest...> &guidMap,
                         T guidOffset) {
...
template <typename T, template <class, class, class...> class MapType,
          class... Rest>
inline bool ChangeGuid(TableStruct const &ts, std::string &str,
                       std::string const &column,
                       MapType<T, T, Rest...> &guidMap, T guidOffset,
                       bool allowZero = false) {
...
inline void AppendTableDump(StringTransaction &trans,
  ...
  do {
    ...
    for (uint32 i = 0; i < fieldSize;) {
      std::string _string = fields[i].Get<std::string>();
      ++i;
      ...
      if (_string.empty())
        ...
        CharacterDatabase.EscapeString(_string);
        ss << '\'' << _string << '\'';
      }
...
void PlayerDumpWriter::PopulateGuids(ObjectGuid::LowType guid) {
  for (BaseTable const &baseTable : BaseTables) {
    ...
    QueryResult result = CharacterDatabase.PQuery(
        "SELECT %s FROM %s WHERE %s", baseTable.PrimaryKey, baseTable.TableName,
        whereStr.c_str());
...
bool PlayerDumpWriter::AppendTable(StringTransaction &trans,
  ...
  QueryResult result = CharacterDatabase.PQuery(
      "SELECT * FROM %s WHERE %s", dumpTable.Name, whereStr.c_str());
  switch (dumpTable.Type) {
  case DTT_CHARACTER:
    if (result) {
...
DumpReturn PlayerDumpReader::LoadDump(std::string const &file, uint32 account,
                                      std::string name,
                                      ObjectGuid::LowType guid) {
  ...
  if (ObjectMgr::CheckPlayerName(name, sWorld->GetDefaultDbcLocale(), true) ==
      CHAR_NAME_SUCCESS) {
    CharacterDatabasePreparedStatement *stmt =
  ...
  while (!feof(fin.get())) {
    ...
    if (tn.empty()) {
      LOG_ERROR("misc",
                "LoadPlayerDump: (line " UI64FMTD ") Can't extract table name!",
                lineNumber);
      return DUMP_FILE_BROKEN;
    }
    ...
    if (i == DUMP_TABLE_COUNT) {
      LOG_ERROR("misc",
                "LoadPlayerDump: (line " UI64FMTD ") Unknown table: `%s`!",
                lineNumber, tn.c_str());
      return DUMP_FILE_BROKEN;
    }
    ...
    for (TableField const &field : ts.TableFields) {
      ...
      case GUID_TYPE_MAIL:
        ...
        break;
      case GUID_TYPE_ITEM:
        if (!ChangeGuid(ts, line, field.FieldName, items, itemLowGuidOffset,
                        true))
          return DUMP_FILE_BROKEN;
        break;
      case GUID_TYPE_EQUIPMENT_SET:
        if (!ChangeGuid(ts, line, field.FieldName, equipmentSetIds,
