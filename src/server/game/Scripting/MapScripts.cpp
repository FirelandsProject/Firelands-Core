...
inline Unit* Map::_GetScriptUnit(Object* obj, bool isSource, const ScriptInfo* scriptInfo) const
{
    ...
    else
    {
        unit = obj->ToUnit();
        if (!unit)
            LOG_ERROR("scripts", "%s %s object could not be casted to unit.",
...
inline void Map::_ScriptProcessDoor(Object* source, Object* target, const ScriptInfo* scriptInfo) const
{
    ...
    else
    {
        ...
        if (!wSource)
            LOG_ERROR("scripts", "%s source object could not be casted to world object (TypeId: %u, Entry: %u, GUID: %u), skipping.",
        ...
        else
        {
            ...
            if (!pDoor)
                ...
                LOG_ERROR("scripts", "%s gameobject is not a door or a trap door (GoType: %u, Entry: %u, GUID: %u).",
...
inline GameObject* Map::_FindGameObject(WorldObject* searchObject, uint32 guid) const
{
    ...
    CellCoord p(Firelands::ComputeCellCoord(searchObject->GetPositionX(), searchObject->GetPositionY()));
    Cell cell(p);

    Firelands::GameObjectWithDbGUIDCheck goCheck(*searchObject, guid);
    Firelands::GameObjectSearcher<Firelands::GameObjectWithDbGUIDCheck> checker(searchObject, gameobject, goCheck);

    TypeContainerVisitor<Firelands::GameObjectSearcher<Firelands::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > objectChecker(checker);
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld->GetGameTime()))
    {
        ...
        switch (step.script->command)
        {
                ...
                else
                {
                    ...
                    if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
                    {
                        ...
                        switch (step.script->Talk.ChatType)
                        {
                            ...
                            case CHAT_MSG_RAID_BOSS_WHISPER:
                                if (!targetGUID || !IS_PLAYER_GUID(targetGUID))
                                    LOG_ERROR("scripts", "%s attempt to raidbosswhisper to non-player unit, skipping.", step.script->GetDebugInfo().c_str());
                                else
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
            {
                ...
                {
                    CellCoord p(Firelands::ComputeCellCoord(wSource->GetPositionX(), wSource->GetPositionY()));
                    Cell cell(p);

                    Firelands::CreatureWithDbGUIDCheck target_check(wSource, step.script->CallScript.CreatureEntry);
                    Firelands::CreatureSearcher<Firelands::CreatureWithDbGUIDCheck> checker(wSource, cTarget, target_check);

                    TypeContainerVisitor<Firelands::CreatureSearcher <Firelands::CreatureWithDbGUIDCheck>, GridTypeMapContainer > unit_checker(checker);
