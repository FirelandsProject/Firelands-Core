        ...
        void DoAction(int32 const action) override
        {
            ...
            else if (action == ACTION_SUMMON_BLOOD)
            {
                ...
                if (!targetList.empty() && targetList.size() > 1)
                    Firelands::Containers::RandomResizeList(targetList, 1);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_IMPALE:
                    {
                        ...
                        Firelands::WorldObjectListSearcher<Firelands::AllWorldObjectsInRange> searcher(me, playerList, objects, GRID_MAP_TYPE_MASK_PLAYER);
                        me->VisitNearbyObject(200.0f, searcher);
                        playerList.sort(Firelands::ObjectDistanceOrderPred(me));
...
public:
    ...
    class spell_ds_carrying_winds_SpellScript : public SpellScript
    {
        ...
        void ModDestHeight(SpellEffIndex /*effIndex*/)
        {
            ...
            creatureList.sort(Firelands::ObjectDistanceOrderPred(GetCaster()));
