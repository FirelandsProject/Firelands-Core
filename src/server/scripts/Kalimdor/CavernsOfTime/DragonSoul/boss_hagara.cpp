        ...
        void DoAction(int32 const actionId) override
        {
            switch (actionId)
            {
                ...
                case ACTION_FINISH_SPECIAL_PHASE:
                {
                    ...
                    Firelands::AllWorldObjectsInRange objects(me, 100.0f);
            ...
            while (uint32 eventid = events.ExecuteEvent())
            {
                switch (eventid)
                {
                    ...
                    case EVENT_SHATTERED_ICE:
                    {
                        Unit* caster = me;
                        std::list<WorldObject*> targetList;
                        Firelands::AllWorldObjectsInRange objects(me, 40.0f);
                        Firelands::WorldObjectListSearcher<Firelands::AllWorldObjectsInRange> searcher(me, targetList, objects, GRID_MAP_TYPE_MASK_PLAYER);
                        ...
                        {
                            if (WorldObject* target = Firelands::Containers::SelectRandomContainerElement(targetList))
...
public:
    ...
    class spell_ds_ice_tomb_selector_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            targets.remove_if(Firelands::UnitAuraCheck(true, SPELL_ICE_TOMB_MISSILE));
            ...
            if (!targets.empty())
                Firelands::Containers::RandomResizeList(targets, targetCount);
        }
...
public:
    ...
    class spell_ds_frostflake_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            targets.remove_if(Firelands::UnitAuraCheck(true, SPELL_FROSTFLAKE));
...
public:
    ...
    class spell_ds_lightning_conduit_periodic_AuraScript : public AuraScript
    {
        ...
        void OnPeriodicTick(AuraEffect const* /*aurEff*/)
        {
            ...
            Firelands::AllCreaturesOfEntryInRange checker(GetTarget(), NPC_CRYSTAL_CONDUCTOR, 12.00f);
            Firelands::CreatureListSearcher<Firelands::AllCreaturesOfEntryInRange> searcher(GetTarget(), CreatureList, checker);
