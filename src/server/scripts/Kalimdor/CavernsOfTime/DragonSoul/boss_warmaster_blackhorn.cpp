...
public:
    ...
    struct npc_ds_sky_captain_swayzeAI : public Scripted_NoMovementAI
    {
        ...
        void setCannonsCombatState(bool active, bool phaseTwo = false)
        {
            ...
            Firelands::AllWorldObjectsInRange objects(me, 70.0f);
            Firelands::WorldObjectListSearcher<Firelands::AllWorldObjectsInRange> searcher(me, targetList, objects);
        ...
        void SpawnDeckFire(bool initial)
        {
            if (initial)
            {
                if (Creature* deckFireController = me->SummonCreature(NPC_DECK_FIRE_CONTROLLER, deckFireSpawnPos, TEMPSUMMON_MANUAL_DESPAWN))
                {
                    ...
                    if (!triggers.empty())
                    {
                        auto trigger = Firelands::Containers::SelectRandomContainerElement(triggers);
                        Position pos;
                            ...
                            for (int8 i = 0; i < 2; i++)
                            {
                                if (DynamicObject* lastDynObj = summon->GetLastDynObject())
                                {
                                    ...
                                    for (uint8 i = 0; i < 16; ++i)
                                    {
                                        ...
                                        std::list<WorldObject*> targetList;
                                        Firelands::AllWorldObjectsInRange objects(lastDynObj, 8.0f);
                                        Firelands::WorldObjectListSearcher<Firelands::AllWorldObjectsInRange> searcher(lastDynObj, targetList, objects, GRID_MAP_TYPE_MASK_DYNAMICOBJECT);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_MASSIVE_EXPLOSION:
                        if (Creature* explosion = Firelands::Containers::SelectRandomContainerElement(massiveExplosion))
                    ...
                    case EVENT_CHECK_BRIGADE_TARGET:
                    {
                        ...
                        Firelands::AllWorldObjectsInRange objects(me, 200.0f);
                        Firelands::WorldObjectListSearcher<Firelands::AllWorldObjectsInRange> searcher(me, dynObjList, objects, GRID_MAP_TYPE_MASK_DYNAMICOBJECT);
...
public:
    ...
    class spell_ds_consuming_shroud_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            targets.remove_if(Firelands::UnitAuraCheck(true, sSpellMgr->GetSpellIdForDifficulty(SPELL_CONSUMING_SHROUD, GetCaster())));
            targets.remove_if([](WorldObject* target){
                return target->ToPlayer() && target->ToPlayer()->HasTankSpec();
            });

            if (!targets.empty())
                Firelands::Containers::RandomResizeList(targets, 1);
        }
...
struct DeckFireTargetCheck : public Firelands::WorldObjectSpellAreaTargetCheck
{
    DeckFireTargetCheck(float range, Position const* position, Unit* caster, Unit* referer, SpellInfo const* spellInfo)
        : Firelands::WorldObjectSpellAreaTargetCheck(range, position, caster, referer, spellInfo, TARGET_CHECK_ALLY, nullptr)
    {}

    bool operator()(Unit* target)
    {
        if (!Firelands::WorldObjectSpellAreaTargetCheck::operator()(target))
            return false;
...
public:
    ...
    class spell_ds_deck_fire_periodic_AuraScript : public AuraScript
    {
        ...
        void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            Firelands::UnitListSearcher<DeckFireTargetCheck> searcher(me, targets, check);
