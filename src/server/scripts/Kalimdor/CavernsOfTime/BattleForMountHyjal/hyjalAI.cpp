...
void hyjalAI::HideNearPos(float x, float y)
{
    ...
    TypeContainerVisitor <Firelands::CreatureListSearcher<Firelands::AllFriendlyCreaturesInGrid>, GridTypeMapContainer> creature_visitor(creature_searcher);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    TypeContainerVisitor<Firelands::WorldObjectWorker<Firelands::RespawnDo>, GridTypeMapContainer > obj_worker(worker);
...
void hyjalAI::WaypointReached(uint32 waypointId)
{
    if (waypointId == 1 || (waypointId == 0 && me->GetEntry() == THRALL))
    {
        ...
        TypeContainerVisitor
            <Firelands::CreatureListSearcher<Firelands::AllFriendlyCreaturesInGrid>,
