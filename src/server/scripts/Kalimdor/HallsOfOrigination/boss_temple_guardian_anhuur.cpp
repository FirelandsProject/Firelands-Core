    ...
    struct boss_temple_guardian_anhuurAI : public BossAI
    {
        ...
        void DamageTaken(Unit* /*attacker*/, uint32& damage)
        {
                ...
                (me->HealthBelowPctDamaged(33, damage) && _phase == PHASE_SECOND_SHIELD))
            {
                ...
                stalkers.remove_if(Firelands::HeightDifferenceCheck(door, 0.0f, false)); // Target only the bottom ones
                for (std::list<Creature*>::iterator itr = stalkers.begin(); itr != stalkers.end(); ++itr)
                {
        ...
        void DoAction(int32 const action)
        {
            if (action == ACTION_CHECK_BEACONS)
            {
                ...
                GameObject* door = ObjectAccessor::GetGameObject(*me, instance->GetData64(DATA_ANHUUR_DOOR));
                GetCreatureListWithEntryInGrid(stalkers, me, NPC_CAVE_IN_STALKER, 100.0f);
                stalkers.remove_if(Firelands::HeightDifferenceCheck(door, 0.0f, false));
                for (std::list<Creature*>::iterator itr = stalkers.begin(); itr != stalkers.end(); ++itr)
                {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_SEAR:
                    {
                        ...
                        std::list<Creature*> stalkers;
                        GetCreatureListWithEntryInGrid(stalkers, me, NPC_CAVE_IN_STALKER, 100.0f);
                        stalkers.remove_if(Firelands::HeightDifferenceCheck(ObjectAccessor::GetGameObject(*me, instance->GetData64(DATA_ANHUUR_DOOR)), 5.0f, true));
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                if (InstanceMap* instance = GetCaster()->GetMap()->ToInstanceMap())
                {
                    if (InstanceScript* const script = instance->GetInstanceScript())
                    {
                        ...
                        {
                            targets.remove_if(Firelands::HeightDifferenceCheck(go, 5.0f, false));
