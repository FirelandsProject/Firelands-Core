...
public:
    ...
    Unit* GetTargetByType(uint8 uiType, float fRange, float fArc = M_PI)
    {
        ...
        return Firelands::Containers::SelectRandomContainerElement(vTargets);
    }
    ...
    Unit* GetMovementSquare()
    {
        ...
        lEnemies.sort(Firelands::ObjectDistanceOrderPred(me));
        lSquaresList.sort(Firelands::ObjectDistanceOrderPred(lEnemies.front()));
...
public:
    ...
    class spell_medivh_cheat_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            if (targets.size() > 3)
                Firelands::Containers::RandomResizeList(targets, 3);
        }
