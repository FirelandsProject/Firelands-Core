...
public:
    ...
    static bool HandleNpcAddCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (chr->GetTransport())
        {
            ...
            if (tguid > 0)
            {
                ...
                stmt->SetData(0, int32(tguid));
                stmt->SetData(1, int32(id));
                stmt->SetData(2, int32(chr->GetTransport()->GetEntry()));
                stmt->SetData(3, chr->GetTransOffsetX());
                stmt->SetData(4, chr->GetTransOffsetY());
                stmt->SetData(5, chr->GetTransOffsetZ());
                stmt->SetData(6, chr->GetTransOffsetO());
    ...
    static bool HandleNpcDeleteCommand(ChatHandler* handler, char const* args)
    {
        ...
        // Delete the creature
        LOG_DEBUG("sql.dev", "DELETE FROM creature WHERE guid = '%u';", unit->GetDBTableGUIDLow());
        unit->CombatStop();
        unit->DeleteFromDB();
    ...
    static bool HandleNpcDeleteAllCommand(ChatHandler* handler, char const* args)
    {
        ...
        LOG_DEBUG("sql.dev","DELETE FROM creature WHERE id = '%u' AND map = '%u';", unit->GetEntry(), unit->GetMapId());
        handler->SendSysMessage(LANG_COMMAND_DELCREATMESSAGE);
    ...
    static bool HandleNpcSetFactionIdCommand(ChatHandler* handler, char const* args)
    {
        ...
        stmt->SetData(1, uint16(factionId));
        stmt->SetData(2, creature->GetEntry());
    ...
    static bool HandleNpcPlayEmoteCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (target->GetTransport() && target->GetGUIDTransport())
        {
            ...
            stmt->SetData(0, int32(emote));
            stmt->SetData(1, target->GetTransport()->GetEntry());
            stmt->SetData(2, target->GetGUIDTransport());
