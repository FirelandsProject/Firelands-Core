...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    static std::vector<ChatCommand> serverIdleRestartCommandTable = {
        {"cancel", rbac::RBAC_PERM_COMMAND_SERVER_IDLERESTART_CANCEL, true,
         &HandleServerShutDownCancelCommand, ""},
        {"", rbac::RBAC_PERM_COMMAND_SERVER_IDLERESTART, true,
         &HandleServerIdleRestartCommand, ""},
    };

    static std::vector<ChatCommand> serverIdleShutdownCommandTable = {
        {"cancel", rbac::RBAC_PERM_COMMAND_SERVER_IDLESHUTDOWN_CANCEL, true,
         &HandleServerShutDownCancelCommand, ""},
    ...
    static std::vector<ChatCommand> serverRestartCommandTable = {
        {"cancel", rbac::RBAC_PERM_COMMAND_SERVER_RESTART_CANCEL, true,
         &HandleServerShutDownCancelCommand, ""},
        {"force", rbac::RBAC_PERM_COMMAND_SERVER_RESTART_FORCE, true,
         &HandleServerForceRestartCommand, ""},
        {"", rbac::RBAC_PERM_COMMAND_SERVER_RESTART, true,
         &HandleServerRestartCommand, ""},
    };

    static std::vector<ChatCommand> serverShutdownCommandTable = {
        {"cancel", rbac::RBAC_PERM_COMMAND_SERVER_SHUTDOWN_CANCEL, true,
         &HandleServerShutDownCancelCommand, ""},
        {"force", rbac::RBAC_PERM_COMMAND_SERVER_SHUTDOWN_FORCE, true,
         &HandleServerForceShutDownCommand, ""},
        {"", rbac::RBAC_PERM_COMMAND_SERVER_SHUTDOWN, true,
         &HandleServerShutDownCommand, ""},
    };

    static std::vector<ChatCommand> serverSetCommandTable = {
        {"difftime", rbac::RBAC_PERM_COMMAND_SERVER_SET_DIFFTIME, true,
         &HandleServerSetDiffTimeCommand, ""},
        {"loglevel", rbac::RBAC_PERM_COMMAND_SERVER_SET_LOGLEVEL, true,
         &HandleServerSetLogLevelCommand, ""},
        {"motd", rbac::RBAC_PERM_COMMAND_SERVER_SET_MOTD, true,
         &HandleServerSetMotdCommand, ""},
        {"closed", rbac::RBAC_PERM_COMMAND_SERVER_SET_CLOSED, true,
         &HandleServerSetClosedCommand, ""},
    };

    static std::vector<ChatCommand> serverCommandTable = {
        {"corpses", rbac::RBAC_PERM_COMMAND_SERVER_CORPSES, true,
         &HandleServerCorpsesCommand, ""},
        {"debug", rbac::RBAC_PERM_COMMAND_SERVER_DEBUG, true,
         &HandleServerDebugCommand, ""},
        {"exit", rbac::RBAC_PERM_COMMAND_SERVER_EXIT, true,
         &HandleServerExitCommand, ""},
        {"idlerestart", rbac::RBAC_PERM_COMMAND_SERVER_IDLERESTART, true,
         nullptr, "", serverIdleRestartCommandTable},
        {"idleshutdown", rbac::RBAC_PERM_COMMAND_SERVER_IDLESHUTDOWN, true,
         nullptr, "", serverIdleShutdownCommandTable},
        {"info", rbac::RBAC_PERM_COMMAND_SERVER_INFO, true,
         &HandleServerInfoCommand, ""},
        {"motd", rbac::RBAC_PERM_COMMAND_SERVER_MOTD, true,
         &HandleServerMotdCommand, ""},
        {"plimit", rbac::RBAC_PERM_COMMAND_SERVER_PLIMIT, true,
         &HandleServerPLimitCommand, ""},
        {"restart", rbac::RBAC_PERM_COMMAND_SERVER_RESTART, true, nullptr, "",
         serverRestartCommandTable},
        {"shutdown", rbac::RBAC_PERM_COMMAND_SERVER_SHUTDOWN, true, nullptr, "",
         serverShutdownCommandTable},
        {"set", rbac::RBAC_PERM_COMMAND_SERVER_SET, true, nullptr, "",
         serverSetCommandTable},
    };

    static std::vector<ChatCommand> commandTable = {
        {"server", rbac::RBAC_PERM_COMMAND_SERVER, true, nullptr, "",
         serverCommandTable},
    };
    return commandTable;
  }
  ...
  static bool HandleServerDebugCommand(ChatHandler *handler,
...

    {
      ...
      if (QueryResult res = LoginDatabase.PQuery(
              "SELECT port FROM realmlist WHERE id = %u", realm.Id.Realm))
        dbPort = (*res)[0].Get<uint16>();

      if (dbPort)
        dbPortOutput = Firelands::StringFormat(
            "Realmlist (Realm Id: %u) configured in port %" PRIu16,
            realm.Id.Realm, dbPort);
      else
        dbPortOutput =
            Firelands::StringFormat("Realm Id: %u not found in `realmlist` "
    ...
    uint32 updateFlags = sConfigMgr->GetOption<int32>(
        "Updates.EnableDatabases", DatabaseLoader::DATABASE_NONE);
    if (!updateFlags)
      ...
      handler->PSendSysMessage("Automatic database updates are enabled for the "
                               "following databases: %s",
                               availableUpdateDatabases.c_str());
    }
    ...
    bool vmapHeightCheck =
        VMAP::VMapFactory::createOrGetVMapManager()->isHeightCalcEnabled();
    ...
    if (vmapIndoorCheck || vmapLOSCheck || vmapHeightCheck) {
      handler->PSendSysMessage("VMAPs status: Enabled. LineOfSight: %i, "
                               ...
                               vmapLOSCheck, vmapHeightCheck, vmapIndoorCheck);
    ...
    for (std::string const &subDir : subDirs) {
      ...
      if (!boost::filesystem::exists(mapPath)) {
        handler->PSendSysMessage("%s directory doesn't exist!. Using path: %s",
                                 subDir.c_str(),
      ...
      std::size_t folderSize = std::accumulate(
          boost::filesystem::directory_iterator(mapPath), end, std::size_t(0),
          [](std::size_t val, boost::filesystem::path const &mapFile) {
      ...
      handler->PSendSysMessage(
          "%s directory located in %s. Total size: " SZFMTD " bytes",
          subDir.c_str(), mapPath.generic_string().c_str(), folderSize);
    }
  ...
  static bool HandleServerInfoCommand(ChatHandler *handler,
    ...
    handler->PSendSysMessage(LANG_CONNECTED_USERS, activeClientsNum,
                             maxActiveClientsNum, queuedClientsNum,
                             maxQueuedClientsNum);
    ...
    if (sWorld->IsShuttingDown())
      handler->PSendSysMessage(
          ...
          secsToTimeString(sWorld->GetShutDownTimeLeft()).c_str());
  ...
  static bool HandleServerPLimitCommand(ChatHandler *handler,
                                        char const *args) {
    if (*args) {
      ...
      if (strncmp(paramStr, "player", limit) == 0)
        ...
        sWorld->SetPlayerAmountLimit(
            sConfigMgr->GetOption<int32>("PlayerLimit", 100));
    ...
    handler->PSendSysMessage(
        "Player limits: amount %u, min. security level %s.", playerAmountLimit,
        secName);
  ...
  static bool HandleServerRestartCommand(ChatHandler *handler,
                                         char const *args) {
    return ShutdownServer(handler, args, SHUTDOWN_MASK_RESTART,
                          RESTART_EXIT_CODE);
  }

  static bool HandleServerForceShutDownCommand(ChatHandler *handler,
                                               char const *args) {
    return ShutdownServer(handler, args, SHUTDOWN_MASK_FORCE,
                          SHUTDOWN_EXIT_CODE);
  }

  static bool HandleServerForceRestartCommand(ChatHandler *handler,
                                              char const *args) {
    return ShutdownServer(handler, args,
                          SHUTDOWN_MASK_FORCE | SHUTDOWN_MASK_RESTART,
                          RESTART_EXIT_CODE);
  }

  static bool HandleServerIdleShutDownCommand(ChatHandler *handler,
                                              char const *args) {
    return ShutdownServer(handler, args, SHUTDOWN_MASK_IDLE,
                          SHUTDOWN_EXIT_CODE);
  }
  ...
  static bool HandleServerSetLogLevelCommand(ChatHandler * /*handler*/,
    ...
    if (!type || !name || !level || *name == '\0' || *level == '\0' ||
        (*type != 'a' && *type != 'l'))
      return false;
...
private:
  ...
  static bool ShutdownServer(ChatHandler *handler, char const *args,
                             uint32 shutdownMask, int32 defaultExitCode) {
    if (!*args)
      return false;
    ...
    if (isNumeric(delayStr)) {
      ...
      if ((delay == 0 && (delayStr[0] != '0' || delayStr[1] != '\0')) ||
          delay < 0)
        return false;
    } else {
    ...
    // Override parameter "delay" with the configuration value if there are
    // still players connected and "force" parameter was not specified
    if (delay < (int32)sWorld->getIntConfig(CONFIG_FORCE_SHUTDOWN_THRESHOLD) &&
        !(shutdownMask & SHUTDOWN_MASK_FORCE) &&
    ...
    sWorld->ShutdownServ(delay, shutdownMask, static_cast<uint8>(exitCode),
                         std::string(reason));
