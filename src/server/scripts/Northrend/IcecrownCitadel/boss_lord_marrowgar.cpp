            ...
            void HandleSpikes(SpellEffIndex effIndex)
            {
                ...
                if (Creature* marrowgar = GetCaster()->ToCreature())
                {
                    ...
                    if (Unit* tank = marrowgar->getVictim())
                    {
                        ...
                        std::list<Player*> playerList;
                        float range = 5;
                        Firelands::AnyPlayerInObjectRangeCheck checker(tank, range, true);
                        Firelands::PlayerListSearcher<Firelands::AnyPlayerInObjectRangeCheck> searcher(tank, playerList, checker);
                        tank->VisitNearbyWorldObject(range, searcher);
                       // playerList = tank->GetNearestPlayersList(5);
                        playerList.remove_if(IsGameMaster());
                        playerList.sort(Firelands::ObjectDistanceOrderPred(tank));
                    ...
                    Firelands::Containers::RandomResizeList(targets, boneSpikeCount);
