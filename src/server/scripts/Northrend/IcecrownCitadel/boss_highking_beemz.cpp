...
public:
    ...
    struct boss_gunship_ersatzAI : public BossAI
    {
        ...
        void SummonAdds()
        {
            ...
            Firelands::Containers::RandomResizeList(entries, uint32(IsHeroic() ? 4 : 3));
            uint8 i = 0;
        ...
        void SummonAllAdds()
        {
            ...
            Firelands::Containers::RandomResizeList(entries, 10);
            uint8 i = 0;
            for (std::list<uint32>::const_iterator itr = entries.begin(); itr != entries.end(); ++itr, ++i)
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_SWARMING_SHADOWS:
                    {
                        ...
                        Firelands::Containers::RandomResizeList<Player*>(targets, uint32(Is25ManRaid() ? 4 : 2));
                    ...
                    case EVENT_FATAL_ATRACTION:
                    {
                        ...
                        if (Is25ManRaid())
                            ++targetCount;
                        Firelands::Containers::RandomResizeList<Player*>(targets, targetCount);
                        if (targets.size() > 1)
                            for (std::list<Player*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)
                    ...
                    case EVENT_FIRE:
                    {
                        ...
                        if (Is25ManRaid())
                            targetCount += 2;
                        Firelands::Containers::RandomResizeList<Player*>(targets, uint32(Is25ManRaid() ? 6 : 2));
                        for (std::list<Player*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)
