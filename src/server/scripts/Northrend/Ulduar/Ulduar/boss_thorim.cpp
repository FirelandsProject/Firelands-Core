                ...
                while (uint32 event = events.ExecuteEvent())
                {
                    switch (event)
                    {
                        case EVENT_CHECK_PLAYER_IN_RANGE:
                            ...
                            {
                                Player* player = 0;
                                Firelands::AnyPlayerInObjectRangeCheck u_check(me, 30.0f, true);
                                Firelands::PlayerSearcher<Firelands::AnyPlayerInObjectRangeCheck> searcher(me, player, u_check);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CHARGE_ORB:
                            if (Creature *orb = Firelands::Containers::SelectRandomContainerElement(orbList))
                            {
    ...
    public:
        ...
        class spell_stormhammer_targeting_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& unitList)
            {
                ...
                _target = Firelands::Containers::SelectRandomContainerElement(unitList)->ToUnit();
                SetTarget(unitList);
            }
