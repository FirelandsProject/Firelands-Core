    ...
    struct npc_ebon_gargoyleAI : PetAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            if (!init && me->GetOwnerGUID())
            {
                ...
                Firelands::AnyUnfriendlyUnitInObjectRangeCheck u_check(me, me, 30.00f);
                ...
                if (!targetFound)
                {
                    if (!targets.empty())
                    {
                        targets.sort(Firelands::ObjectDistanceOrderPred(me));
                ...
                switch (eventId)
                {
                    case EVENT_SEARING_BOLT:
                    {
                        if (Unit* owner = me->GetOwner())
                        {
                            ...
                            std::list<Unit*> targets;
                            Firelands::AnyUnfriendlyUnitInObjectRangeCheck u_check(me, me, 20.0f);
                            Firelands::UnitListSearcher<Firelands::AnyUnfriendlyUnitInObjectRangeCheck> searcher(me, targets, u_check);
        ...
        void UpdateAI(const uint32 diff)
        {
            if (doomboltTimer < diff)
            {
                if (Unit* owner = me->GetOwner())
                {
                    targetFound = false;
                    std::list<Unit*> targets;
                    Firelands::AnyUnfriendlyUnitInObjectRangeCheck u_check(me, me, range);
                    Firelands::UnitListSearcher<Firelands::AnyUnfriendlyUnitInObjectRangeCheck> searcher(me, targets, u_check);
        ...
        void UpdateAI(const uint32 diff)
        {
            if (fireboltTimer < diff)
            {
                if (Unit* owner = me->GetOwner())
                {
                    targetFound = false;
                    std::list<Unit*> targets;
                    Firelands::AnyUnfriendlyUnitInObjectRangeCheck u_check(me, me, 40);
                    Firelands::UnitListSearcher<Firelands::AnyUnfriendlyUnitInObjectRangeCheck> searcher(me, targets, u_check);
        ...
        void UpdateAI(const uint32 diff)
        {
            if (targetCheckTimer < diff)
            {
                if (Unit* owner = me->GetOwner())
                {
                    ...
                    std::list<Unit*> targets;
                    Firelands::AnyUnfriendlyUnitInObjectRangeCheck u_check(me, me, 40.0f);
                    Firelands::UnitListSearcher<Firelands::AnyUnfriendlyUnitInObjectRangeCheck> searcher(me, targets, u_check);
