...
uint32 const Builds[] = {13164, 13205, 13287, 13329, 13596, 13623,
                         13914, 14007, 14333, 14480, 14545, 15005,
                         15050, 15211, 15354, 15595, 0};
#define LAST_DBC_IN_DATA_BUILD                                                 \
  13623 // after this build mpqs with dbc are back to locale folder
#define NEW_BASE_SET_BUILD 15211

#define LOCALES_COUNT 15

char const *Locales[LOCALES_COUNT] = {"enGB", "enUS", "deDE", "esES", "frFR",
                                      "koKR", "zhCN", "zhTW", "enCN", "enTW",
                                      "esMX", "ruRU", "ptBR", "ptPT", "itIT"};

uint8 const MpqToWowLocale[LOCALES_COUNT] = {
    LOCALE_enUS, LOCALE_enUS, LOCALE_deDE, LOCALE_esES, LOCALE_frFR,
...
TCHAR const *LocalesT[LOCALES_COUNT] = {
    _T("enGB"), _T("enUS"), _T("deDE"), _T("esES"), _T("frFR"),
    _T("koKR"), _T("zhCN"), _T("zhTW"), _T("enCN"), _T("enTW"),
    _T("esMX"), _T("ruRU"), _T("ptBR"), _T("ptPT"), _T("itIT"),
};
...
uint32 ReadBuild(int locale) {
  ...
  std::string filename =
      std::string("component.wow-") + Locales[locale] + ".txt";
  // printf("Read %s file... ", filename.c_str());

  HANDLE dbcFile;
  if (!SFileOpenFileEx(LocaleMpq, filename.c_str(), SFILE_OPEN_PATCHED_FILE,
                       &dbcFile)) {
...
uint32 ReadMapDBC() {
  ...
  HANDLE dbcFile;
  if (!SFileOpenFileEx(LocaleMpq, "DBFilesClient\\Map.dbc",
                       SFILE_OPEN_PATCHED_FILE, &dbcFile)) {
...
void ReadLiquidMaterialTable() {
  ...
  HANDLE dbcFile;
  if (!SFileOpenFileEx(LocaleMpq, "DBFilesClient\\LiquidMaterial.dbc",
                       SFILE_OPEN_PATCHED_FILE, &dbcFile)) {
  ...
  for (uint32 x = 0; x < dbc.getRecordCount(); ++x) {
    LiquidMaterialEntry &liquidType =
        LiquidMaterials[dbc.getRecord(x).getUInt(0)];
...
void ReadLiquidObjectTable() {
  ...
  HANDLE dbcFile;
  if (!SFileOpenFileEx(LocaleMpq, "DBFilesClient\\LiquidObject.dbc",
                       SFILE_OPEN_PATCHED_FILE, &dbcFile)) {
...
void ReadLiquidTypeTable() {
  ...
  HANDLE dbcFile;
  if (!SFileOpenFileEx(LocaleMpq, "DBFilesClient\\LiquidType.dbc",
                       SFILE_OPEN_PATCHED_FILE, &dbcFile)) {
...
LiquidVertexFormatType adt_MH2O::GetLiquidVertexFormat(
    adt_liquid_instance const *liquidInstance) const {
  if (liquidInstance->LiquidVertexFormat < 42)
    return static_cast<LiquidVertexFormatType>(
        liquidInstance->LiquidVertexFormat);
...
bool ConvertADT(std::string const &inputPath, std::string const &outputPath,
                int /*cell_y*/, int /*cell_x*/, uint32 build,
  ...
  for (std::multimap<std::string, FileChunk *>::const_iterator itr =
           adt.chunks.lower_bound("MCNK");
    ...
    if (FileChunk *chunk = itr->second->GetSubChunk("MCVT")) {
      ...
      for (int y = 0; y < ADT_CELL_SIZE; y++) {
        ...
        for (int x = 0; x < ADT_CELL_SIZE; x++) {
          int cx = mcnk->ix * ADT_CELL_SIZE + x;
          V8[cy][cx] += mcvt->height_map[y * (ADT_CELL_SIZE * 2 + 1) +
                                         ADT_CELL_SIZE + 1 + x];
        }
      }
    }
    ...
    if (mcnk->sizeMCLQ > 8) {
      if (FileChunk *chunk = itr->second->GetSubChunk("MCLQ")) {
        ...
        if (c_flag & (1 << 4)) {
          ...
          liquid_flags[mcnk->iy][mcnk->ix] |=
              MAP_LIQUID_TYPE_MAGMA; // magma/slime
        }
  ...
  // Store area data
  outFile.write(reinterpret_cast<const char *>(&areaHeader),
                sizeof(areaHeader));
  if (!(areaHeader.flags & MAP_AREA_NO_AREA))
    outFile.write(reinterpret_cast<const char *>(area_ids), sizeof(area_ids));

  // Store height data
  outFile.write(reinterpret_cast<const char *>(&heightHeader),
                sizeof(heightHeader));
  if (!(heightHeader.flags & MAP_HEIGHT_NO_HEIGHT)) {
    if (heightHeader.flags & MAP_HEIGHT_AS_INT16) {
      outFile.write(reinterpret_cast<const char *>(uint16_V9),
                    sizeof(uint16_V9));
      outFile.write(reinterpret_cast<const char *>(uint16_V8),
                    sizeof(uint16_V8));
  ...
  if (heightHeader.flags & MAP_HEIGHT_HAS_FLIGHT_BOUNDS) {
    outFile.write(reinterpret_cast<char *>(flight_box_max),
                  sizeof(flight_box_max));
    outFile.write(reinterpret_cast<char *>(flight_box_min),
                  sizeof(flight_box_min));
  }

  // Store liquid data if need
  if (map.liquidMapOffset) {
    outFile.write(reinterpret_cast<const char *>(&liquidHeader),
                  sizeof(liquidHeader));
    if (!(liquidHeader.flags & MAP_LIQUID_NO_TYPE)) {
      outFile.write(reinterpret_cast<const char *>(liquid_entry),
                    sizeof(liquid_entry));
      outFile.write(reinterpret_cast<const char *>(liquid_flags),
                    sizeof(liquid_flags));
    }
      ...
      for (int y = 0; y < liquidHeader.height; y++)
        outFile.write(
            reinterpret_cast<const char *>(
                &liquid_height[y + liquidHeader.offsetY][liquidHeader.offsetX]),
...
bool IsDeepWaterIgnored(uint32 mapId, uint32 x, uint32 y) {
  if (mapId == 0) {
    ...
    // GRID(41, 18) || GRID(41, 19) || GRID(41, 20) || GRID(41, 21) || GRID(41,
    // 22) || GRID(41, 23) || GRID(41, 24) || GRID(41, 25) || GRID(41, 26) ||
    // GRID(42, 18) || GRID(42, 19) || GRID(42, 20) || GRID(42, 21) || GRID(42,
    // 22) || GRID(42, 23) || GRID(42, 24) || GRID(42, 25) || GRID(42, 26) ||
    // GRID(43, 18) || GRID(43, 19) || GRID(43, 20) || GRID(43, 21) || GRID(43,
    // 22) || GRID(43, 23) || GRID(43, 24) || GRID(43, 25) || GRID(43, 26) ||
    // GRID(44, 18) || GRID(44, 19) || GRID(44, 20) || GRID(44, 21) || GRID(44,
    // 22) || GRID(44, 23) || GRID(44, 24) || GRID(44, 25) || GRID(44, 26) ||
    // GRID(45, 18) || GRID(45, 19) || GRID(45, 20) || GRID(45, 21) || GRID(45,
    // 22) || GRID(45, 23) || GRID(45, 24) || GRID(45, 25) || GRID(45, 26) ||
    // GRID(46, 18) || GRID(46, 19) || GRID(46, 20) || GRID(46, 21) || GRID(46,
    // 22) || GRID(46, 23) || GRID(46, 24) || GRID(46, 25) || GRID(46, 26)
...
void ExtractMapsFromMpq(uint32 build) {
  ...
  for (uint32 z = 0; z < map_count; ++z) {
    ...
    if (wdt.loadFile(WorldMpq, mpq_map_name, false)) {
      ...
      for (uint32 y = 0; y < WDT_MAP_SIZE; ++y) {
        for (uint32 x = 0; x < WDT_MAP_SIZE; ++x) {
          ...
          sprintf(output_filename, "%s/maps/%03u%02u%02u.map", output_path,
                  map_ids[z].id, y, x);
          bool ignoreDeepWater = IsDeepWaterIgnored(map_ids[z].id, y, x);
          existingTiles[y * WDT_MAP_SIZE + x] = ConvertADT(
              mpq_filename, output_filename, y, x, build, ignoreDeepWater);
        }
    ...
    if (FILE *tileList =
            fopen(Firelands::StringFormat("%s/maps/%03u.tilelist", output_path,
                                          map_ids[z].id)
                      .c_str(),
                  "wb")) {
      ...
      fwrite(existingTiles.to_string().c_str(), 1, existingTiles.size(),
             tileList);
      fclose(tileList);
    }
  }
...
void ExtractDBCFiles(int l) {
  ...
  HANDLE listFile =
      SFileFindFirstFile(LocaleMpq, "DBFilesClient\\*dbc", &foundFile, nullptr);
  ...
  if (listFile) {
    ...
    do {
      if (!SFileOpenFileEx(LocaleMpq, foundFile.cFileName,
                           SFILE_OPEN_PATCHED_FILE, &dbcFile)) {
...
void ExtractDB2Files(int l) {
  ...
  memset(&foundFile, 0, sizeof(foundFile));
  HANDLE listFile =
      SFileFindFirstFile(LocaleMpq, "DBFilesClient\\*db2", &foundFile, nullptr);
  HANDLE dbcFile = nullptr;
  uint32 count = 0;
  if (listFile) {
    std::string outputPath = output_path;
...
void ExtractCameraFiles() {
  ...
  HANDLE dbcFile;
  if (!SFileOpenFileEx(LocaleMpq, "DBFilesClient\\CinematicCamera.dbc",
                       SFILE_OPEN_PATCHED_FILE, &dbcFile)) {
    printf("Fatal error: Cannot find Map.dbc in archive!\n");
    exit(1);
  }
  ...
  for (std::string thisFile : camerafiles) {
    ...
    if (!SFileOpenFileEx(WorldMpq, thisFile.c_str(), SFILE_OPEN_PATCHED_FILE,
                         &dbcFile)) {
      printf("Unable to open file %s in the archive\n", thisFile.c_str());
      continue;
    }
...
bool LoadLocaleMPQFile(int locale) {
  ...
  _stprintf(buff, _T("%s/Data/%s/locale-%s.MPQ"), input_path, LocalesT[locale],
            LocalesT[locale]);
  ...
  for (int i = 0; Builds[i] && Builds[i] <= CONF_TargetBuild; ++i) {
    // Do not attempt to read older MPQ patch archives past this build, they
    // were merged with base and trying to read them together with new base will
    // not end well
    if (CONF_TargetBuild >= NEW_BASE_SET_BUILD &&
        Builds[i] < NEW_BASE_SET_BUILD)
      continue;

    memset(buff, 0, sizeof(buff));
    if (Builds[i] > LAST_DBC_IN_DATA_BUILD) {
      prefix = "";
      _stprintf(buff, _T("%s/Data/%s/wow-update-%s-%u.MPQ"), input_path,
                LocalesT[locale], LocalesT[locale], Builds[i]);
    } else {
...
void LoadCommonMPQFiles(uint32 build) {
  ...
  for (int i = 1; i < count; ++i) {
    if (build < NEW_BASE_SET_BUILD &&
        !strcmp("world2.MPQ", CONF_mpq_list[i])) // 4.3.2 and higher MPQ
      continue;
  ...
  for (int i = 0; Builds[i] && Builds[i] <= CONF_TargetBuild; ++i) {
    ...
    if (Builds[i] > LAST_DBC_IN_DATA_BUILD) {
      prefix = "";
      _stprintf(filename, _T("%s/Data/wow-update-base-%u.MPQ"), input_path,
                Builds[i]);
    } else {
      prefix = "base";
      _stprintf(filename, _T("%s/Data/wow-update-%u.MPQ"), input_path,
                Builds[i]);
    }
  ...
  for (int i = 0; i < LOCALES_COUNT; ++i) {
    ...
    if ((CONF_extract & EXTRACT_DBC) == 0) {
      ...
      if (build > CONF_TargetBuild) {
        printf("Base locale-%s.MPQ has build higher than target build (%u > "
               "%u), nothing extracted!\n",
    ...
    if (tempBuild > CONF_TargetBuild) {
      ...
      printf("Base locale-%s.MPQ has build higher than target build (%u > %u), "
             "nothing extracted!\n",
