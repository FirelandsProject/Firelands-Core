...
char const *CONF_mpq_list[] = {
    "world.MPQ",      "art.MPQ",        "expansion1.MPQ",
    "expansion2.MPQ", "expansion3.MPQ", "world2.MPQ",
};
...
bool LoadLocaleMPQFile(int locale) {
  ...
  _stprintf(buff, _T("%s%s/locale-%s.MPQ"), input_path, LocalesT[locale],
            LocalesT[locale]);
  ...
  for (int i = 0; Builds[i] && Builds[i] <= CONF_TargetBuild; ++i) {
    ...
    if (Builds[i] > LAST_DBC_IN_DATA_BUILD) {
      prefix = "";
      _stprintf(buff, _T("%s%s/wow-update-%s-%u.MPQ"), input_path,
...
void LoadCommonMPQFiles(uint32 build) {
  ...
  for (int i = 0; Builds[i] && Builds[i] <= CONF_TargetBuild; ++i) {
    ...
    if (Builds[i] > LAST_DBC_IN_DATA_BUILD) {
      prefix = "";
      _stprintf(filename, _T("%swow-update-base-%u.MPQ"), input_path,
                Builds[i]);
    } else {
      prefix = "base";
...
bool ExtractWmo() {
  ...
  // char const* ParsArchiveNames[] = {"patch-2.MPQ", "patch.MPQ", "common.MPQ",
  // "expansion.MPQ"};
...
void ParsMapFiles() {
  ...
  auto getWDT = [&wdts](uint32 mapId) -> WDTFile * {
    ...
    if (itr == wdts.end()) {
      ...
      itr = wdts.emplace(std::piecewise_construct, std::forward_as_tuple(mapId),
                         std::forward_as_tuple(
                             fn, name, maps_that_are_parents.count(mapId) > 0))
                .first;
  ...
  for (auto itr = map_ids.begin(); itr != map_ids.end(); ++itr) {
    if (WDTFile *WDT = getWDT(itr->first)) {
      WDTFile *parentWDT =
          itr->second.parent_id >= 0 ? getWDT(itr->second.parent_id) : nullptr;
...
bool processArgv(int argc, char **argv, const char *versionString) {
  ...
  if (!result) {
    printf("Extract %s.\n", versionString);
    printf("%s [-?][-s][-l][-d <path>]\n", argv[0]);
    printf("   -s : (default) small size (data size optimization), ~500MB less "
           "vmap data.\n");
...
int main(int argc, char **argv) {
  Firelands::Banner::Show(
      "VMAP data extractor", [](std::string text) { printf("%s\n", text); },
      nullptr);
  ...
  // some simple check if working dir is dirty
    ...
    if (!stat(sdir.c_str(), &status) || !stat(sdir_bin.c_str(), &status)) {
      printf("Your output directory seems to be polluted, please use an empty "
             "directory!\n");
