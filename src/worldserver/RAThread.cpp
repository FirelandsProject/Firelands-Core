...
class RASocket: protected ACE_Svc_Handler < ACE_SOCK_STREAM, ACE_NULL_SYNCH>
{
    ...
    protected:
        ...
        virtual int open(void* unused) override
        {
            ...
            sendf("\r\n");
            sendf(sObjectMgr.GetFirelandsStringForDBCLocale(LANG_RA_USER));
            ...
            if (gotenter)
            {
                ...
                switch (stage)
                {
                    case NONE:
                    {
                        ...
                        stage = LG;
                        sendf(sObjectMgr.GetFirelandsStringForDBCLocale(LANG_RA_PASS));
                        break;
                    }
                    case LG:
                    {
...
int RAThread::open(void* unused)
{
    ...
    {
        sLog.outError("Firelands RA can not bind to port %d on %s\n", listen_addr.get_port_number(), listen_addr.get_host_addr());
        return -1;
    }
    activate();
    return 0;
