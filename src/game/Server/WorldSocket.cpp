...
{
    FIRELANDS_ASSERT(m_RecvWPct == NULL);

    FIRELANDS_ASSERT(m_Header.length() == sizeof(ClientPktHeader));
    ...
    {
        FIRELANDS_ASSERT(m_RecvPct.space() == 0);
    }
...
int WorldSocket::handle_input_payload(void)
{
    ...
    FIRELANDS_ASSERT(m_RecvPct.space() == 0);
    FIRELANDS_ASSERT(m_Header.space() == 0);
    FIRELANDS_ASSERT(m_RecvWPct != NULL);
    ...
    while (message_block.length() > 0)
    {
        if (m_Header.space() > 0)
        {
            ...
            if (m_Header.space() > 0)
            {
                ...
                FIRELANDS_ASSERT(message_block.length() == 0);
                errno = EWOULDBLOCK;
                return -1;
            }
            ...
            {
                FIRELANDS_ASSERT((errno != EWOULDBLOCK) && (errno != EAGAIN));
                return -1;
            }
        }
