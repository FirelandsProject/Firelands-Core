...
inline void LoadDBC(LocalData& localeData, BarGoLink& bar, StoreProblemList& errlist, DBCStorage<T>& storage, const std::string& dbc_path, const std::string& filename)
{
    ...
    FIRELANDS_ASSERT(DBCFileLoader::GetFormatRecordSize(storage.GetFormat()) == sizeof(T) || LoadDBC_assert_print(DBCFileLoader::GetFormatRecordSize(storage.GetFormat()), sizeof(T), filename));
...
void LoadDBCStores(const std::string& dataPath)
{
    ...
    for (uint32 i = 0; i < sChrPowerTypesStore.GetNumRows(); ++i)
    {
        ...
        FIRELANDS_ASSERT(entry->classId < MAX_CLASSES && "MAX_CLASSES not updated");
        FIRELANDS_ASSERT(entry->power < MAX_POWERS && "MAX_POWERS not updated");
        ...
        FIRELANDS_ASSERT(index < MAX_STORED_POWERS && "MAX_STORED_POWERS not updated");
    ...
    for (uint32 i = 0; i < sPvPDifficultyStore.GetNumRows(); ++i)
        if (PvPDifficultyEntry const* entry = sPvPDifficultyStore.LookupEntry(i))
            ...
            {
                FIRELANDS_ASSERT(false && "Need update MAX_BATTLEGROUND_BRACKETS by DBC data");
            }
    ...
    for (uint32 i = 1; i < sSpellEffectStore.GetNumRows(); ++i)
    {
        if (SpellEffectEntry const* spellEffect = sSpellEffectStore.LookupEntry(i))
        {
            ...
            case SPELL_AURA_POWER_BURN_MANA:
                FIRELANDS_ASSERT(spellEffect->EffectMiscValue >= 0 && spellEffect->EffectMiscValue < MAX_POWERS);
                break;
            }
