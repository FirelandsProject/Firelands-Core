    ...
    for(uint32 i = 0; i < MSE_COUNT; ++i)
    {
        ...
        switch (element)
        {
            ...
            case MSEByteParam:
                 ...
                 break;
            default:
                FIRELANDS_ASSERT(false && "Wrong movement status element");
                break;
        }
    }
}
...
Unit::~Unit()
{
    ...
    FIRELANDS_ASSERT(m_gameObj.size() == 0);
    FIRELANDS_ASSERT(m_dynObjGUIDs.size() == 0);
    FIRELANDS_ASSERT(m_deletedAuras.size() == 0);
    FIRELANDS_ASSERT(m_deletedHolders.size() == 0);
}
...
uint32 Unit::DealDamage(Unit* pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* spellProto, bool durabilityLoss)
{
    ...
    else                                                    // if (health <= damage)
    {
        ...
        if (duel_hasEnded)
        {
            FIRELANDS_ASSERT(pVictim->GetTypeId() == TYPEID_PLAYER);
            Player* he = (Player*)pVictim;

            FIRELANDS_ASSERT(he->duel);
...
{
    FIRELANDS_ASSERT(pSpell);                                  // NULL may be never passed here, use InterruptSpell or InterruptNonMeleeSpells
...
{
    FIRELANDS_ASSERT(spellType < CURRENT_MAX_SPELL);
...
{
    FIRELANDS_ASSERT(gameObj && !gameObj->GetOwnerGuid());
...
{
    FIRELANDS_ASSERT(gameObj && gameObj->GetOwnerGuid().IsEmpty());
...
{
    FIRELANDS_ASSERT(gameObj && gameObj->GetOwnerGuid() == GetObjectGuid());
...
{
    FIRELANDS_ASSERT(GetTypeId() == TYPEID_UNIT);
...
{
    FIRELANDS_ASSERT(pTarget && GetTypeId() == TYPEID_UNIT);
...
{
    FIRELANDS_ASSERT(powerId < MAX_POWERS);
    FIRELANDS_ASSERT(classId < MAX_CLASSES);
...
{
    FIRELANDS_ASSERT(index < MAX_STORED_POWERS);
    FIRELANDS_ASSERT(classId < MAX_CLASSES);
...
Unit* Unit::SelectRandomUnfriendlyTarget(Unit* except /*= NULL*/, float radius /*= ATTACK_DISTANCE*/) const
{
    ...
    Firelands::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, radius);
    Firelands::UnitListSearcher<Firelands::AnyUnfriendlyUnitInObjectRangeCheck> searcher(targets, u_check);
...
Unit* Unit::SelectRandomFriendlyTarget(Unit* except /*= NULL*/, float radius /*= ATTACK_DISTANCE*/) const
{
    ...
    Firelands::AnyFriendlyUnitInObjectRangeCheck u_check(this, radius);
    Firelands::UnitListSearcher<Firelands::AnyFriendlyUnitInObjectRangeCheck> searcher(targets, u_check);
...
class RelocationNotifyEvent : public BasicEvent
{
    public:
        ...
        bool Execute(uint64 /*e_time*/, uint32 /*p_time*/)
        {
            ...
            {
                Firelands::PlayerRelocationNotifier notify((Player&)m_owner);
                Cell::VisitAllObjects(&m_owner, notify, radius);
            }
            else // if(m_owner.GetTypeId() == TYPEID_UNIT)
            {
                Firelands::CreatureRelocationNotifier notify((Creature&)m_owner);
                Cell::VisitAllObjects(&m_owner, notify, radius);
            }
