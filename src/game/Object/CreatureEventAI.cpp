...
Unit* CreatureEventAI::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff)
{
    ...
    Firelands::MostHPMissingInRangeCheck u_check(m_creature, range, MinHPDiff);
...
{
    Firelands::FriendlyCCedInRangeCheck u_check(m_creature, range);
    Firelands::CreatureListSearcher<Firelands::FriendlyCCedInRangeCheck> searcher(_list, u_check);
    Cell::VisitGridObjects(m_creature, searcher, range);
}

void CreatureEventAI::DoFindFriendlyMissingBuff(std::list<Creature*>& _list, float range, uint32 spellid)
{
    Firelands::FriendlyMissingBuffInRangeCheck u_check(m_creature, range, spellid);
    Firelands::CreatureListSearcher<Firelands::FriendlyMissingBuffInRangeCheck> searcher(_list, u_check);
    Cell::VisitGridObjects(m_creature, searcher, range);
}
