...
// helper function, mainly for script side, but can be used for simple task in firelands also.
...
void Player::Regenerate(Powers power, uint32 diff)
{
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            ...
            if (IsInCombat())
            {
                // firelands Updates Mana in intervals of 2s, which is correct
...
void Player::RegenerateHealth(uint32 diff)
{
    ...
    // This needs fixing
    HealthIncreaseRate = 1.0f; // having to do this as the above constantly results in 0 - worldserver.conf is obviously not being read properly
...
int32 Player::CalculateReputationGain(ReputationSource source, int32 rep, int32 faction, uint32 creatureOrQuestLevel, bool noAuraBonus)
{
    ...
    if (rate != 1.0f && creatureOrQuestLevel <= Firelands::XP::GetGrayLevel(getLevel()))
    {
        percent *= rate;
    }
...
bool Player::RewardHonor(Unit* uVictim, uint32 groupsize, float honor)
{
    ...
    if (honor <= 0)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            k_grey = Firelands::XP::GetGrayLevel(k_level);
    ...
    for (GossipMenuItemsMap::const_iterator itr = pMenuItemBounds.first; itr != pMenuItemBounds.second; ++itr)
    {
        ...
        if (hasMenuItem)
        {
            ...
            if (isGMSkipConditionCheck)
            {
                ...
                strOptionText.append(GetSession()->GetFirelandsString(LANG_GM_ON));
                strOptionText.append(")");
            }
...
void Player::AddQuest(Quest const* pQuest, Object* questGiver)
{
    ...
    FIRELANDS_ASSERT(log_slot < MAX_QUEST_LOG_SIZE);
    ...
    for (Quest::PrevQuests::const_iterator iter = qInfo->prevQuests.begin(); iter != qInfo->prevQuests.end(); ++iter)
    {
        ...
        if (qPrevInfo && i_prevstatus != mQuestStatus.end())
        {
            ...
            if (*iter > 0 && i_prevstatus->second.m_rewarded)
            {
                ...
                FIRELANDS_ASSERT(bounds.first != bounds.second); // always must be found if qPrevInfo->ExclusiveGroup != 0
...
bool Player::SatisfyQuestExclusiveGroup(Quest const* qInfo, bool msg) const
{
    ...
    FIRELANDS_ASSERT(bounds.first != bounds.second);           // must always be found if qInfo->ExclusiveGroup != 0
...
void Player::SendQuestReward(Quest const* pQuest, uint32 XP)
{
    ...
    data << uint32(10 * Firelands::Honor::hk_honor_at_level(getLevel(), pQuest->GetRewHonorAddition()));
...
{
    FIRELANDS_ASSERT(count < 65536 && "mob/GO count store in 16 bits 2^16 = 65536 (0..65536)");
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder* holder)
{
    ...
    if (uint32 node_id = m_taxi.GetTaxiSource())
    {
        ...
        FIRELANDS_ASSERT(nodeEntry);                           // checked in m_taxi.LoadTaxiDestinationsFromString
...
void Player::_LoadInventory(QueryResult* result, uint32 timediff)
{
    ...
    if (result)
    {
        ...
        {
            std::string subject = GetSession()->GetFirelandsString(LANG_NOT_EQUIPPED_ITEM);
...
void Player::HandleStealthedUnitsDetection()
{
    ...
    Firelands::AnyStealthedCheck u_check(this);
    Firelands::UnitListSearcher<Firelands::AnyStealthedCheck > searcher(stealthedUnits, u_check);
...
void Player::SendTransferAbortedByLockStatus(MapEntry const* mapEntry, AreaLockStatus lockStatus, uint32 miscRequirement)
{
    ...
    switch (lockStatus)
    {
        case AREA_LOCKSTATUS_TOO_LOW_LEVEL:
            GetSession()->SendAreaTriggerMessage(GetSession()->GetFirelandsString(LANG_LEVEL_MINREQUIRED), miscRequirement);
            break;
        ...
        case AREA_LOCKSTATUS_QUEST_NOT_COMPLETED:
            ...
            {
                GetSession()->SendAreaTriggerMessage("%s", GetSession()->GetFirelandsString(LANG_TELEREQ_QUEST_BLACK_MORASS));
                break;
            }
...
{
    uint32 v_level = pVictim->getLevel();
    uint32 k_grey  = Firelands::XP::GetGrayLevel(getLevel());
...
void Player::RewardSinglePlayerAtKill(Unit* pVictim)
{
    ...
    uint32 xp = PvP ? 0 : Firelands::XP::Gain(this, pVictim);
...
{
    FIRELANDS_ASSERT((!GetGroup() || pRewardSource) && "Player::RewardPlayerAndGroupAtEvent called for Group-Case but no source for range searching provided");
