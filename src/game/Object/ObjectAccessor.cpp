...
Corpse* ObjectAccessor::GetCorpseForPlayerGUID(ObjectGuid guid)
{
    ...
    FIRELANDS_ASSERT(c->GetType() != CORPSE_BONES);
    return c;
}
...
void Player2Corpse::Remove(Corpse* corpse)
{
    ...
    CellPair cell_pair = Firelands::ComputeCellPair(corpse->GetPositionX(), corpse->GetPositionY());
...
{
    FIRELANDS_ASSERT(corpse && corpse->GetType() != CORPSE_BONES);
