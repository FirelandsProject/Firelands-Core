...
void ObjectMgr::AddCreatureToGrid(uint32 guid, CreatureData const* data)
{
    ...
    for (uint8 i = 0; mask != 0; ++i, mask >>= 1)
    {
        if (mask & 1)
        {
            CellPair cell_pair = Firelands::ComputeCellPair(data->posX, data->posY);
...
static char SERVER_SIDE_SPELL[] = "Firelands server-side spell";
...
inline void _DoStringError(int32 entry, char const* text, ...)
{
    ...
    else if (entry < MIN_DB_SCRIPT_STRING_ID)               // firelands string error
    {
        sLog.outError("%s", buf);
    }
...
bool ObjectMgr::LoadFirelandsStrings(DatabaseType& db, char const* table, int32 min_value, int32 max_value, bool extra_content)
{
    ...
    for (FirelandsStringLocaleMap::iterator itr = mFirelandsStringLocaleMap.begin(); itr != mFirelandsStringLocaleMap.end();)
    {
        if (itr->first >= start_value && itr->first < end_value)
        {
            mFirelandsStringLocaleMap.erase(itr++);
        }
        else
        {
            ++itr;
    ...
    if (!result)
    {
        ...
        {
            sLog.outErrorDb(">> Loaded 0 firelands strings. DB table `%s` is empty. Can not continue.", table);
        }
        else
        {
    ...
    do
    {
        ...
        FirelandsStringLocale& data = mFirelandsStringLocaleMap[entry];
    ...
    {
        sLog.outString(">> Loaded %u Firelands strings from table %s", count, table);
    }
    else
    {
...
const char* ObjectMgr::GetFirelandsString(int32 entry, int locale_idx) const
{
    // locale_idx==-1 -> default, locale_idx >= 0 in to idx+1
    // Content[0] always exist if exist FirelandsStringLocale
    if (FirelandsStringLocale const* msl = GetFirelandsStringLocale(entry))
    {
...
bool PlayerCondition::Meets(Player const* player, Map const* map, WorldObject const* source, ConditionSource conditionSourceType) const
{
    ...
    switch (m_condition)
    {
        ...
        case CONDITION_CREATURE_IN_RANGE:
        {
            ...
            Firelands::NearestCreatureEntryWithLiveStateInObjectRangeCheck creature_check(*player, m_value1, true, false, m_value2, true);
            Firelands::CreatureLastSearcher<Firelands::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(creature, creature_check);
        ...
        case CONDITION_GAMEOBJECT_IN_RANGE:
        {
            ...
            if (source)
            {
                Firelands::NearestGameObjectEntryInObjectRangeCheck go_check(*source, m_value1, m_value2);
...
bool LoadFirelandsStrings(DatabaseType& db, char const* table, int32 start_value, int32 end_value, bool extra_content)
{
    // MAX_DB_SCRIPT_STRING_ID is max allowed negative value for scripts (scrpts can use only more deep negative values
    // start/end reversed for negative values
    if (start_value > MAX_DB_SCRIPT_STRING_ID || end_value >= start_value)
    {
        sLog.outErrorDb("Table '%s' attempt loaded with reserved by firelands range (%d - %d), strings not loaded.", table, start_value, end_value + 1);
        return false;
    }

    return sObjectMgr.LoadFirelandsStrings(db, table, start_value, end_value, extra_content);
}
...
FirelandsStringLocale const* GetFirelandsStringData(int32 entry)
{
    return sObjectMgr.GetFirelandsStringLocale(entry);
}
...
{
    FirelandsStringLocale const* data = sObjectMgr.GetFirelandsStringLocale(entry);
