        ...
        case GO_READY:
        {
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)   // traps
                {
                    ...
                    Firelands::UnitSearcher<Firelands::AnyUnfriendlyUnitInObjectRangeCheck> checker(enemy, u_check);
...
void GameObject::TriggerLinkedGameObject(Unit* target)
{
...

    {
        ...
        Firelands::NearestGameObjectEntryInObjectRangeCheck go_check(*this, trapEntry, range);
        Firelands::GameObjectLastSearcher<Firelands::NearestGameObjectEntryInObjectRangeCheck> checker(trapGO, go_check);
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    Firelands::NearestGameObjectFishingHoleCheck u_check(*this, range);
    Firelands::GameObjectSearcher<Firelands::NearestGameObjectFishingHoleCheck> checker(ok, u_check);
    Cell::VisitGridObjects(this, checker, range);
...
void GameObject::Use(Unit* user)
{
    ...
    FIRELANDS_ASSERT(user || PrintEntryError("GameObject::Use (without user)"));
...
void GameObject::TickCapturePoint()
{
    ...
    Firelands::AnyPlayerInCapturePointRange u_check(this, radius);
    Firelands::PlayerListSearcher<Firelands::AnyPlayerInCapturePointRange> checker(capturingPlayers, u_check);
...
{
    FIRELANDS_ASSERT(GetGoType() == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING);
    FIRELANDS_ASSERT(spell && sSpellStore.LookupEntry(spell) && caster);
...
{
    FIRELANDS_ASSERT(GetGoType() == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING);
    FIRELANDS_ASSERT(caster || diff >= 0);
