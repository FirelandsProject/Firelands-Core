    ...
    do
    {
        ...
        if (auction->owner)
        {
            ...
            if (plWName.empty())
            {
                ...
                {
                    plName = sObjectMgr.GetFirelandsStringForDBCLocale(LANG_UNKNOWN);
                }
...
{
    FIRELANDS_ASSERT(it);
    FIRELANDS_ASSERT(mAitems.find(it->GetGUIDLow()) == mAitems.end());
    ...
    for (AuctionEntryMap::iterator itr = AuctionsMap.begin(); itr != AuctionsMap.end();)
    {
        if (itr->second->moneyDeliveryTime)                 // pending auction
        {
            if (curTime > itr->second->moneyDeliveryTime)
            {
                ...
                FIRELANDS_ASSERT(!itr->second->itemGuidLow);   // already removed or send in mail at won
                delete itr->second;
