...
{
    // Load EventAI Text
    sObjectMgr.LoadFirelandsStrings(WorldDatabase, "creature_ai_texts", MIN_CREATURE_AI_TEXT_STRING_ID, MAX_CREATURE_AI_TEXT_STRING_ID, true);
...
void CreatureEventAIMgr::LoadCreatureEventAI_Summons(bool check_entry_use)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (!Firelands::IsValidMapCoord(temp.position_x, temp.position_y, temp.position_z, temp.orientation))
            {
                    ...
                    case ACTION_T_TEXT:
                    {
                        ...
                        for (int k = firstTextParam; k < 3; ++k)
                        {
                            if (action.text.TextId[k])
                            {
                                ...
                                else if (!sObjectMgr.GetFirelandsStringLocale(action.text.TextId[k]))
                                {
