...
void CreatureAI::SendAIEventAround(AIEventType eventType, Unit* pInvoker, uint32 uiDelay, float fRadius, uint32 miscValue /*=0*/) const
{
    if (fRadius > 0)
    {
        ...
        {
            Firelands::AnyUnitInObjectRangeCheck u_check(m_creature, fRadius);
            Firelands::CreatureListSearcher<Firelands::AnyUnitInObjectRangeCheck> searcher(receiverList, u_check);
        ...
        else
        {
            ...
            Firelands::AnyAssistCreatureInRangeCheck u_check(m_creature, eventType == AI_EVENT_CALL_ASSISTANCE ? pInvoker : NULL, fRadius);
            Firelands::CreatureListSearcher<Firelands::AnyAssistCreatureInRangeCheck> searcher(receiverList, u_check);
            Cell::VisitGridObjects(m_creature, searcher, fRadius);
        }
