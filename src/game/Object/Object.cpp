...
void Object::BuildValuesUpdate(uint8 updatetype, ByteBuffer* data, UpdateMask* updateMask, Player* target) const
{
    ...
    FIRELANDS_ASSERT(updateMask && updateMask->GetCount() == m_valuesCount);
...
{
    FIRELANDS_ASSERT(index < m_valuesCount || PrintIndexError(index, true));
...
{
    FIRELANDS_ASSERT(index + 1 < m_valuesCount || PrintIndexError(index, true));
...
float WorldObject::GetAngle(const WorldObject* obj) const
{
    ...
    // FIRELANDS_ASSERT(obj != this || PrintEntryError("GetAngle (for self)"));
    if (obj == this)
    {
...
{
    return Firelands::IsValidMapCoord(m_position.x, m_position.y, m_position.z, m_position.o);
}
...
namespace Firelands
{
    ...
    {
        public:
            MonsterChatBuilder(WorldObject const& obj, ChatMsg msgtype, FirelandsStringLocale const* textData, Language language, Unit const* target)
        ...
        private:
            ...
            ChatMsg i_msgtype;
            FirelandsStringLocale const* i_textData;
            Language i_language;
            Unit const* i_target;
    };
}                                                           // namespace Firelands
...
void _DoLocalizedTextAround(WorldObject const* source, FirelandsStringLocale const* textData, ChatMsg msgtype, Language language, Unit const* target, float range)
{
    Firelands::MonsterChatBuilder say_build(*source, msgtype, textData, language, target);
    Firelands::LocalizedPacketDo<Firelands::MonsterChatBuilder> say_do(say_build);
    Firelands::CameraDistWorker<Firelands::LocalizedPacketDo<Firelands::MonsterChatBuilder> > say_worker(source, range, say_do);
    Cell::VisitWorldObjects(source, say_worker, range);
}

/// Function that sends a text associated to a FirelandsString
void WorldObject::MonsterText(FirelandsStringLocale const* textData, Unit const* target) const
{
    FIRELANDS_ASSERT(textData);
    ...
    switch (textData->Type)
    {
        ...
        case CHAT_TYPE_WHISPER:
        {
            ...
            Firelands::MonsterChatBuilder say_build(*this, CHAT_MSG_MONSTER_WHISPER, textData, LANG_UNIVERSAL, target);
            Firelands::LocalizedPacketDo<Firelands::MonsterChatBuilder> say_do(say_build);
        ...
        case CHAT_TYPE_BOSS_WHISPER:
        {
            ...
            Firelands::MonsterChatBuilder say_build(*this, CHAT_MSG_RAID_BOSS_WHISPER, textData, LANG_UNIVERSAL, target);
            Firelands::LocalizedPacketDo<Firelands::MonsterChatBuilder> say_do(say_build);
            say_do((Player*)target);
            break;
        }
        ...
        {
            Firelands::MonsterChatBuilder say_build(*this, CHAT_MSG_MONSTER_YELL, textData, textData->LanguageId, target);
            Firelands::LocalizedPacketDo<Firelands::MonsterChatBuilder> say_do(say_build);
...
void WorldObject::SendMessageToSetExcept(WorldPacket* data, Player const* skipped_receiver) const
{
    ...
    if (IsInWorld())
    {
        Firelands::MessageDelivererExcept notifier(this, data, skipped_receiver);
...
{
    CellPair p = Firelands::ComputeCellPair(GetPositionX(), GetPositionY());
    Cell cell(p);
