...
#ifndef FIRELANDS_H_OBJECTMGR
#define FIRELANDS_H_OBJECTMGR
...
struct AreaTrigger
{
    ...
    {
        FIRELANDS_ASSERT(target_mapId == l->target_mapId);
...
struct FirelandsStringLocale
{
    FirelandsStringLocale() : SoundId(0), Type(0), LanguageId(LANG_UNIVERSAL), Emote(0) { }
...
typedef UNORDERED_MAP<int32, FirelandsStringLocale> FirelandsStringLocaleMap;
...
class ObjectMgr
{
    ...
    public:
        ...
        bool LoadFirelandsStrings(DatabaseType& db, char const* table, int32 min_value, int32 max_value, bool extra_content);
        bool LoadFirelandsStrings()
        {
            return LoadFirelandsStrings(WorldDatabase, "firelands_string", MIN_FIRELANDS_STRING_ID, MAX_FIRELANDS_STRING_ID, false);
        }
        ...
        FirelandsStringLocale const* GetFirelandsStringLocale(int32 entry) const
        {
            FirelandsStringLocaleMap::const_iterator itr = mFirelandsStringLocaleMap.find(entry);
            if (itr == mFirelandsStringLocaleMap.end())
            {
                return NULL;
            }
            return &itr->second;
        ...
        const char* GetFirelandsString(int32 entry, int locale_idx) const;
        const char* GetFirelandsStringForDBCLocale(int32 entry) const { return GetFirelandsString(entry, DBCLocaleIndex); }
    ...
    private:
        ...
        FirelandsStringLocaleMap mFirelandsStringLocaleMap;
...
#define sObjectMgr Firelands::Singleton<ObjectMgr>::Instance()

/// generic text function
 bool DoDisplayText(WorldObject* source, int32 entry, Unit const* target = NULL);

// scripting access functions
 bool LoadFirelandsStrings(DatabaseType& db, char const* table, int32 start_value = MAX_CREATURE_AI_TEXT_STRING_ID, int32 end_value = std::numeric_limits<int32>::min(), bool extra_content = false);
