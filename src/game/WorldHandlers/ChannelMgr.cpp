...
ChannelMgr* channelMgr(Team team)
{
    ...
    {
        return &Firelands::Singleton<AllianceChannelMgr>::Instance();         // cross-faction
    }

    if (team == ALLIANCE)
    {
        return &Firelands::Singleton<AllianceChannelMgr>::Instance();
    }
    if (team == HORDE)
    {
        return &Firelands::Singleton<HordeChannelMgr>::Instance();
    }
