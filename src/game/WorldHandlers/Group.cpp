...
uint32 Group::RemoveMember(ObjectGuid guid, uint8 method)
{
    ...
    // Used by Eluna
#ifdef ENABLE_ELUNA
    sEluna->OnRemoveMember(this, guid, method); // Kicker and Reason not a part of Firelands, implement?
#endif /* ENABLE_ELUNA */
...
void Group::StartLootRoll(WorldObject* lootTarget, LootMethod method, Loot* loot, uint8 itemSlot, uint32 maxEnchantingSkill)
{
    ...
    if (r->totalPlayersRolling > 0)                         // has looters
    {
        ...
        else
        {
            ...
            FIRELANDS_ASSERT(lootTarget->isType(TYPEMASK_CREATURE_OR_GAMEOBJECT));
...
static void GetDataForXPAtKill_helper(Player* player, Unit const* victim, uint32& sum_level, Player*& member_with_max_level, Player*& not_gray_member_with_max_level)
{
    ...
    uint32 gray_level = Firelands::XP::GetGrayLevel(player->getLevel());
...
void Group::RewardGroupAtKill(Unit* pVictim, Player* player_tap)
{
    ...
    if (member_with_max_level)
    {
        ...
        xp = (PvP || !not_gray_member_with_max_level) ? 0 : Firelands::XP::Gain(not_gray_member_with_max_level, pVictim);
        ...
        float group_rate = Firelands::XP::xp_in_group_rate(count, is_raid);
