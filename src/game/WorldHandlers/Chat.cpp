...
ChatCommand* ChatHandler::getCommandTable()
{
    ...
    static ChatCommand reloadCommandTable[] =
    {
        ...
        { "firelands_string",             SEC_ADMINISTRATOR, true,  &ChatHandler::HandleReloadFirelandsStringCommand,            "", NULL },
...
{
    return value ?  GetFirelandsString(LANG_ON) : GetFirelandsString(LANG_OFF);
}
...
std::string ChatHandler::PrepareStringNpcOrGoSpawnInformation(uint32 guid)
{
    ...
    if (uint16 pool_id = sPoolMgr.IsPartOfAPool<T>(guid))
    {
        ...
        {
            char buffer[100];
            const char* format = GetFirelandsString(LANG_NPC_GO_INFO_POOL_EVENT_STRING);
        ...
        {
            char buffer[100];
            const char* format = GetFirelandsString(LANG_NPC_GO_INFO_POOL_STRING);
    ...
    {
        char buffer[100];
        const char* format = GetFirelandsString(LANG_NPC_GO_INFO_EVENT_STRING);
                                  ...
                                  char const* channelName /*= NULL*/, uint32 achievementId /*= 0*/, const char* addonPrefix /*= NULL*/)
{
    ...
    switch (msgtype)
    {
        ...
        default:
            ...
            {
                FIRELANDS_ASSERT(channelName);
                data << channelName;
                data << ObjectGuid(targetGuid);
            }
            else if (msgtype == CHAT_MSG_ADDON)
            {
                FIRELANDS_ASSERT(addonPrefix);
                data << addonPrefix;
            }
            else
            {
