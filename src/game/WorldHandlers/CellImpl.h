...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellPair center = Firelands::ComputeCellPair(x, y).normalize();
        return CellArea(center, center);
    }

    return CellArea
           (
               Firelands::ComputeCellPair(x - radius, y - radius).normalize(),
               Firelands::ComputeCellPair(x + radius, y + radius).normalize()
           );
}
...
{
    CellPair p(Firelands::ComputeCellPair(center_obj->GetPositionX(), center_obj->GetPositionY()));
    Cell cell(p);
    if (dont_load)
    {
        cell.SetNoCreate();
