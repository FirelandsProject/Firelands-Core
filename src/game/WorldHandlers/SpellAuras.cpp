    ...
    m_isPersistent(false), m_in_use(0), m_spellAuraHolder(holder)
{
    ...
    FIRELANDS_ASSERT(spellproto && spellproto == sSpellStore.LookupEntry(spellproto->Id) && "`info` must be pointer to sSpellStore element");

    m_spellEffect = spellproto->GetSpellEffect(m_effIndex);

    FIRELANDS_ASSERT(m_spellEffect);                           // need testing...
    ...
    if (GetCasterGuid() == GetTarget()->GetObjectGuid())
    {
        ...
        if (!caster->hasUnitState(UNIT_STAT_ISOLATED))
        {
            ...
            switch (m_areaAuraType)
            {
                ...
                {
                    Firelands::AnyFriendlyUnitInObjectRangeCheck u_check(caster, m_radius);
                ...
                {
                    Firelands::AnyAoETargetUnitInObjectRangeCheck u_check(caster, m_radius); // No GetCharmer in searcher
                    Firelands::UnitListSearcher<Firelands::AnyAoETargetUnitInObjectRangeCheck> searcher(targets, u_check);
                    Cell::VisitAllObjects(caster, searcher, m_radius);
                    break;
                }
...
//                    case 21916: break;
                    ...
                    case 37125:                             // Mark of Death
                    {
                        ...
                        Firelands::AnyPlayerInObjectRangeWithAuraCheck u_check(GetTarget(), 100.0f, markSpellId);
                        Firelands::PlayerListSearcher<Firelands::AnyPlayerInObjectRangeWithAuraCheck > checker(targets, u_check);
...
//                    case 24018: break;
                    case 24210:                             // Mark of Arlokk
                    {
                        ...
                        Firelands::AllCreaturesOfEntryInRangeCheck check(triggerTarget, 15101, 15.0f);
            ...
            case SPELLFAMILY_GENERIC:
            {
                switch (GetId())
                {
                    ...
                    case 58600:                             // Restricted Flight Area
                    {
                        ...
                        const char* text = sObjectMgr.GetFirelandsString(LANG_NO_FLY_ZONE, ((Player*)target)->GetSession()->GetSessionDbLocaleIndex());
            ...
            switch (spell->Id)
            {
                ...
                case 51690:
                {
                    ...
                    Spell::UnitList targets;
                    {
                        ...
                        Firelands::AnyUnfriendlyVisibleUnitInObjectRangeCheck u_check(target, target, radius);
                        Firelands::UnitListSearcher<Firelands::AnyUnfriendlyVisibleUnitInObjectRangeCheck> checker(targets, u_check);
    ...
    m_permanent(false), m_isRemovedOnShapeLost(true), m_deleted(false), m_in_use(0)
{
    ...
    else
    {
        ...
        FIRELANDS_ASSERT(caster->isType(TYPEMASK_UNIT))
