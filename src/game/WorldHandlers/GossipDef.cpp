...
{
    FIRELANDS_ASSERT(m_gItems.size() <= GOSSIP_MAX_MENU_ITEMS);
...
void GossipMenu::AddMenuItem(uint8 Icon, int32 itemText, uint32 dtSender, uint32 dtAction, int32 boxText, uint32 BoxMoney, bool Coded)
{
    ...
    char const* item_text = itemText ? sObjectMgr.GetFirelandsString(itemText, loc_idx) : "";
    char const* box_text = boxText ? sObjectMgr.GetFirelandsString(boxText, loc_idx) : "";
...
void QuestMenu::AddMenuItem(uint32 QuestId, uint8 Icon)
{
    ...
    FIRELANDS_ASSERT(m_qItems.size() <= GOSSIP_MAX_MENU_ITEMS);
