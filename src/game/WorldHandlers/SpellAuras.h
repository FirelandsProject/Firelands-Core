...
//
// Other case choice: each code line moved under if(Real) check is firelands speedup,
//      each setting object update field code line moved under if(Real) check is significant firelands speedup, and less server->client data sends
//      each packet sending code moved under if(Real) check is _large_ firelands speedup, and lot less server->client data sends
