    ...
    do
    {
        ...
        switch (tmp.command)
        {
            case SCRIPT_COMMAND_TALK:                       // 0
            {
                ...
                // if (!GetFirelandsStringLocale(tmp.dataint)) will be checked after db_script_string loading
                break;
            }
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:                // 6
            {
                ...
                if (!Firelands::IsValidMapCoord(tmp.x, tmp.y, tmp.z, tmp.o))
                {
...
{
    sObjectMgr.LoadFirelandsStrings(WorldDatabase, "db_script_string", MIN_DB_SCRIPT_STRING_ID, MAX_DB_SCRIPT_STRING_ID, true);

    std::set<int32> ids;

    for (int32 i = MIN_DB_SCRIPT_STRING_ID; i < MAX_DB_SCRIPT_STRING_ID; ++i)
        if (sObjectMgr.GetFirelandsStringLocale(i))
        {
            ids.insert(i);
        }
        ...
        for (ScriptChainMap::const_iterator itrCM = m_dbScripts[t].begin(); itrCM != m_dbScripts[t].end(); ++itrCM)
        {
            for (ScriptChain::const_iterator itrC = itrCM->second.begin(); itrC != itrCM->second.end(); ++itrC)
            {
                if (itrC->command == SCRIPT_COMMAND_TALK)
                {
                    for (int i = 0; i < MAX_TEXT_ID; ++i)
                    {
                        if (itrC->textId[i] && !sObjectMgr.GetFirelandsStringLocale(itrC->textId[i]))
                        {
    ...
    if (m_script->buddyEntry)
    {
        ...
        else                                                // Buddy by entry
        {
            ...
            if (m_script->IsCreatureBuddy())
            {
                ...
                {
                    Firelands::AllCreaturesOfEntryInRangeCheck u_check(pSearcher, m_script->buddyEntry, m_script->searchRadiusOrGuid);
                    Firelands::CreatureLastSearcher<Firelands::AllCreaturesOfEntryInRangeCheck> searcher(pCreatureBuddy, u_check);
                    Cell::VisitGridObjects(pSearcher, searcher, m_script->searchRadiusOrGuid);
                }
                else
                {
                    Firelands::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSearcher, m_script->buddyEntry, true, false, m_script->searchRadiusOrGuid, true);
                    Firelands::CreatureLastSearcher<Firelands::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pCreatureBuddy, u_check);
            ...
            else
            {
                ...
                Firelands::NearestGameObjectEntryInObjectRangeCheck u_check(*pSearcher, m_script->buddyEntry, m_script->searchRadiusOrGuid);
                Firelands::GameObjectLastSearcher<Firelands::NearestGameObjectEntryInObjectRangeCheck> searcher(pGOBuddy, u_check);
    ...
    switch (m_script->command)
    {
        ...
        case SCRIPT_COMMAND_TERMINATE_SCRIPT:               // 31
        {
            ...
            if (m_script->terminateScript.npcEntry)
            {
                ...
                Creature* pCreatureBuddy = NULL;
                Firelands::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSearcher, m_script->terminateScript.npcEntry, true, false, m_script->terminateScript.searchDist, true);
                Firelands::CreatureLastSearcher<Firelands::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pCreatureBuddy, u_check);
...
{
#ifdef ENABLE_SD3
    if (std::strcmp(libName, FIRELANDS_SCRIPT_NAME) == 0)
    {
