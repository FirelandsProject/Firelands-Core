...
void World::SetInitialWorldSettings()
{
    ...
    sLog.outString();
    sLog.outString("Loading Firelands strings...");
...
#ifdef ENABLE_SD3
    switch (sScriptMgr.LoadScriptLibrary(FIRELANDS_SCRIPT_NAME))
    {
        case SCRIPT_LOAD_OK:
...
void World::showFooter()
{
    ...
    // PLAYERBOTS can be included or excluded but also disabled via worldserver.conf
#ifdef ENABLE_PLAYERBOTS
    ...
    // Remote Access can be activated / deactivated via worldserver.conf
    ...
    // SOAP can be included or excluded but also disabled via worldserver.conf
#ifdef ENABLE_SOAP
    ...
    // Warden is always included, set active or disabled via worldserver.conf
    ...
    sLog.outString("\n"
        ...
        "\n"
        " Firelands Server: World Initialization Complete\n"
        "_______________________________________________________\n"
        "\n"
        "        Server Version : %s\n"
...
void World::SendDefenseMessage(uint32 zoneId, int32 textId)
{
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        if (WorldSession* session = itr->second)
        {
            ...
            {
                char const* message = session->GetFirelandsString(textId);
