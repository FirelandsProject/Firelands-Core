...
#define CLASS_LOCK Firelands::ClassLevelLockable<MapManager, ACE_Recursive_Thread_Mutex>
...
Map* MapManager::CreateMap(uint32 id, const WorldObject* obj)
{
    ...
    {
        FIRELANDS_ASSERT(obj && obj->GetTypeId() == TYPEID_PLAYER);
...
Map* MapManager::CreateInstance(uint32 id, Player* player)
{
    ...
    if (entry->IsBattleGroundOrArena())
    {
        ...
        FIRELANDS_ASSERT(NewInstanceId);
...
BattleGroundMap* MapManager::CreateBattleGroundMap(uint32 id, uint32 InstanceId, BattleGround* bg)
{
    ...
    FIRELANDS_ASSERT(map->IsBattleGroundOrArena());
    map->SetBG(bg);
    bg->SetBgMap(map);
