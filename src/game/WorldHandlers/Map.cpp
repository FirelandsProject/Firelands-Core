...
void Map::ForceLoadGrid(float x, float y)
{
    if (!IsLoaded(x, y))
    {
        CellPair p = Firelands::ComputeCellPair(x, y);
        Cell cell(p);
...
bool Map::Add(Player* player)
{
    ...
    CellPair p = Firelands::ComputeCellPair(player->GetPositionX(), player->GetPositionY());
    Cell cell(p);
...
Map::Add(T* obj)
{
    FIRELANDS_ASSERT(obj);

    CellPair p = Firelands::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY());
...
void Map::MessageBroadcast(Player const* player, WorldPacket* msg, bool to_self)
{
    ...
    Firelands::MessageDeliverer post_man(*player, msg, to_self);
    TypeContainerVisitor<Firelands::MessageDeliverer, WorldTypeMapContainer > message(post_man);
...
void Map::MessageBroadcast(WorldObject const* obj, WorldPacket* msg)
{
    ...
    Firelands::ObjectMessageDeliverer post_man(*obj, msg);
    TypeContainerVisitor<Firelands::ObjectMessageDeliverer, WorldTypeMapContainer > message(post_man);
...
void Map::MessageDistBroadcast(Player const* player, WorldPacket* msg, float dist, bool to_self, bool own_team_only)
{
    ...
    Firelands::MessageDistDeliverer post_man(*player, msg, dist, to_self, own_team_only);
    TypeContainerVisitor<Firelands::MessageDistDeliverer , WorldTypeMapContainer > message(post_man);
...
void Map::MessageDistBroadcast(WorldObject const* obj, WorldPacket* msg, float dist)
{
    ...
    Firelands::ObjectMessageDistDeliverer post_man(*obj, msg, dist);
    TypeContainerVisitor<Firelands::ObjectMessageDistDeliverer, WorldTypeMapContainer > message(post_man);
...
void Map::Update(const uint32& t_diff)
{
    ...
    if (!IsBattleGroundOrArena())
    {
        for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end();)
        {
            ...
            FIRELANDS_ASSERT(grid->GetGridState() >= 0 && grid->GetGridState() < MAX_GRID_STATE);
...
{
    FIRELANDS_ASSERT(player);

    CellPair old_val = Firelands::ComputeCellPair(player->GetPositionX(), player->GetPositionY());
    CellPair new_val = Firelands::ComputeCellPair(x, y);
...
{
    FIRELANDS_ASSERT(CheckGridIntegrity(creature, false));

    Cell old_cell = creature->GetCurrentCell();
    Cell new_cell(Firelands::ComputeCellPair(x, y));
    ...
    FIRELANDS_ASSERT(CheckGridIntegrity(creature, true));
}
...
bool Map::CreatureRespawnRelocation(Creature* c)
{
    ...
    CellPair resp_val = Firelands::ComputeCellPair(resp_x, resp_y);
...
bool Map::CheckGridIntegrity(Creature* c, bool moved) const
{
    ...
    CellPair xy_val = Firelands::ComputeCellPair(c->GetPositionX(), c->GetPositionY());
    Cell xy_cell(xy_val);
    if (xy_cell != cur_cell)
    {
...
{
    FIRELANDS_ASSERT(obj->GetMapId() == GetId() && obj->GetInstanceId() == GetInstanceId());
...
{
    FIRELANDS_ASSERT(x < MAX_NUMBER_OF_GRIDS);
    FIRELANDS_ASSERT(y < MAX_NUMBER_OF_GRIDS);
    ...
    for (MapRefManager::const_iterator iter = m_mapRefManager.begin(); iter != m_mapRefManager.end(); ++iter)
    {
        ...
        CellPair p = Firelands::ComputeCellPair(plr->GetPositionX(), plr->GetPositionY());
...
void Map::AddToActive(WorldObject* obj)
{
    ...
    Cell cell = Cell(Firelands::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY()));
    ...
    if (obj->GetTypeId() == TYPEID_UNIT)
    {
        ...
        if (!c->IsPet() && c->HasStaticDBSpawnData())
        {
            ...
            GridPair p = Firelands::ComputeGridPair(x, y);
            if (getNGrid(p.x_coord, p.y_coord))
            {
            ...
            {
                GridPair p2 = Firelands::ComputeGridPair(c->GetPositionX(), c->GetPositionY());
...
{
    FIRELANDS_ASSERT(i_mapEntry->IsDungeon());
    ...
    else
    {
        ...
        else
        {
            ...
            if (!playerBind)
            {
                ...
                // can not jump to a different instance without resetting it
            {
                FIRELANDS_ASSERT(playerBind->state == GetPersistentState());
            }
        }
    }
...
void Map::MonsterYellToMap(CreatureInfo const* cinfo, int32 textId, Language language, Unit const* target, uint32 senderLowGuid /*= 0*/) const
{
    ...
    Firelands::LocalizedPacketDo<StaticMonsterChatBuilder> say_do(say_build);
