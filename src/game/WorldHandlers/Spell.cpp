...
void SpellCastTargets::read(ByteBuffer& data, Unit* caster)
{
    ...
    if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION)
    {
        ...
        if (!Firelands::IsValidMapCoord(m_srcX, m_srcY, m_srcZ))
        {
    ...
    if (m_targetMask & TARGET_FLAG_DEST_LOCATION)
    {
        ...
        if (!Firelands::IsValidMapCoord(m_destX, m_destY, m_destZ))
        {
            throw ByteBufferException(false, data.rpos(), 0, data.size());
        }
    }
...
{
    FIRELANDS_ASSERT(caster != NULL && info != NULL);
    FIRELANDS_ASSERT(info == sSpellStore.LookupEntry(info->Id) && "`info` must be pointer to sSpellStore element");
...
WorldObject* Spell::FindCorpseUsing()
{
    ...
    Firelands::WorldObjectSearcher<T> searcher(result, u_check);
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    switch (targetMode)
    {
        ...
        case TARGET_RANDOM_CIRCUMFERENCE_POINT:
        {
            ...
            // and no unitTarget (e.g. summon effects). As Firelands always needs a unitTarget we add just the caster here.
        ...
        case TARGET_RANDOM_UNIT_CHAIN_IN_AREA:
        {
            ...
            switch (targetMode)
            {
                case TARGET_RANDOM_ENEMY_CHAIN_IN_AREA:
                {
                    Firelands::AnyAoETargetUnitInObjectRangeCheck u_check(m_caster, max_range);
                    Firelands::UnitListSearcher<Firelands::AnyAoETargetUnitInObjectRangeCheck> searcher(tempTargetUnitMap, u_check);
                    Cell::VisitAllObjects(m_caster, searcher, max_range);
                    break;
                }
                case TARGET_RANDOM_FRIEND_CHAIN_IN_AREA:
                {
                    Firelands::AnyFriendlyUnitInObjectRangeCheck u_check(m_caster, max_range);
                    Firelands::UnitListSearcher<Firelands::AnyFriendlyUnitInObjectRangeCheck> searcher(tempTargetUnitMap, u_check);
                    Cell::VisitAllObjects(m_caster, searcher, max_range);
                    break;
                }
                case TARGET_RANDOM_UNIT_CHAIN_IN_AREA:
                {
                    Firelands::AnyUnitInObjectRangeCheck u_check(m_caster, max_range);
                    Firelands::UnitListSearcher<Firelands::AnyUnitInObjectRangeCheck> searcher(tempTargetUnitMap, u_check);
                    Cell::VisitAllObjects(m_caster, searcher, max_range);
                    break;
                }
            }
        ...
        case TARGET_CHAIN_DAMAGE:
        {
            ...
            else
            {
                ...
                UnitList tempTargetUnitMap;
                {
                    Firelands::AnyAoEVisibleTargetUnitInObjectRangeCheck u_check(pUnitTarget, originalCaster, max_range);
                    Firelands::UnitListSearcher<Firelands::AnyAoEVisibleTargetUnitInObjectRangeCheck> searcher(tempTargetUnitMap, u_check);
                    Cell::VisitAllObjects(m_caster, searcher, max_range);
                }
        ...
        case TARGET_GO_IN_FRONT_OF_CASTER_90:
        {
            ...
            for (SQLMultiStorage::SQLMultiSIterator<SpellTargetEntry> i_spellST = bounds.first; i_spellST != bounds.second; ++i_spellST)
            {
                ...
                if (i_spellST->type == SPELL_TARGET_TYPE_GAMEOBJECT)
                {
                    ...
                    Firelands::GameObjectEntryInPosRangeCheck go_check(*m_caster, i_spellST->targetEntry, x, y, z, radius);
                    Firelands::GameObjectListSearcher<Firelands::GameObjectEntryInPosRangeCheck> checker(tempTargetGOList, go_check);
            ...
            if (!fixedTargetExist)
            {
                ...
                {
                    Firelands::GameObjectTypeInPosRangeCheck go_check(*m_caster, GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING, x, y, z, radius, spellEffect->Effect == SPELL_EFFECT_WMO_DAMAGE, spellEffect->Effect == SPELL_EFFECT_WMO_REPAIR);
                    Firelands::GameObjectListSearcher<Firelands::GameObjectTypeInPosRangeCheck> checker(tempTargetGOList, go_check);
                    Cell::VisitGridObjects(m_caster, checker, radius + GetSpellMaxRange(sSpellRangeStore.LookupEntry(m_spellInfo->rangeIndex)));
                }
            }
            ...
            switch(spellEffect->Effect)
            {
                case SPELL_EFFECT_DUMMY:
                {
                    switch (m_spellInfo->Id)
                    {
                        ...
                        {
                            WorldObject* result = FindCorpseUsing<Firelands::CannibalizeObjectCheck> ();
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    {
        GameObject* ok = NULL;
        Firelands::GameObjectFocusCheck go_check(m_caster, m_spellInfo->GetRequiresSpellFocus());
        Firelands::GameObjectSearcher<Firelands::GameObjectFocusCheck> checker(ok, go_check);
                ...
                for (SQLMultiStorage::SQLMultiSIterator<SpellTargetEntry> i_spellST = bounds.first; i_spellST != bounds.second; ++i_spellST)
                {
                    ...
                    switch (i_spellST->type)
                    {
                        case SPELL_TARGET_TYPE_GAMEOBJECT:
                        {
                            ...
                            {
                                Firelands::NearestGameObjectEntryInObjectRangeCheck go_check(*m_caster, i_spellST->targetEntry, range);
                                Firelands::GameObjectLastSearcher<Firelands::NearestGameObjectEntryInObjectRangeCheck> checker(p_GameObject, go_check);
                        ...
                        default:
                        {
                            ...
                            if (!targetExplicit)
                            {
                                Firelands::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*m_caster, i_spellST->targetEntry, i_spellST->type != SPELL_TARGET_TYPE_DEAD, i_spellST->type == SPELL_TARGET_TYPE_DEAD, range);
                                Firelands::CreatureLastSearcher<Firelands::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(p_Creature, u_check);
        ...
        switch(spellEffect->Effect)
        {
            ...
            case SPELL_EFFECT_DUMMY:
            {
                ...
                else if (m_spellInfo->Id == 51690)          // Killing Spree
                {
                    ...
                    Firelands::AnyUnfriendlyVisibleUnitInObjectRangeCheck unitCheck(m_caster, m_caster, radius);
                    Firelands::UnitListSearcher<Firelands::AnyUnfriendlyVisibleUnitInObjectRangeCheck> checker(targets, unitCheck);
...
{
    Firelands::SpellNotifierCreatureAndPlayer notifier(*this, targetUnitMap, radius, pushType, spellTargets, originalCaster);
