 ...
 * This file contains the code needed for Firelands to provide abstract support for transported entities
...
{
    FIRELANDS_ASSERT(m_owner);
}

TransportBase::~TransportBase()
{
    FIRELANDS_ASSERT(m_passengers.size() == 0);
}
...
TransportInfo::TransportInfo(WorldObject* owner, TransportBase* transport, float lx, float ly, float lz, float lo, uint8 seat) :
    ...
    m_seat(seat)
{
    FIRELANDS_ASSERT(owner && m_transport);
}
