...
bool ChatHandler::HandlePInfoCommand(char* args)
{
    ...
    std::string username = GetFirelandsString(LANG_ERROR);
    std::string email = GetFirelandsString(LANG_ERROR);
    std::string last_ip = GetFirelandsString(LANG_ERROR);
    AccountTypes security = SEC_PLAYER;
    std::string last_login = GetFirelandsString(LANG_ERROR);
    ...
    PSendSysMessage(LANG_PINFO_ACCOUNT, (target ? "" : GetFirelandsString(LANG_OFFLINE)), nameLink.c_str(), target_guid.GetCounter(), username.c_str(), accId, security, email.c_str(), last_ip.c_str(), last_login.c_str(), latency);
...
bool ChatHandler::HandleGMVisibleCommand(char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_YOU_ARE, m_session->GetPlayer()->isGMVisible() ?  GetFirelandsString(LANG_VISIBLE) : GetFirelandsString(LANG_INVISIBLE));
        return true;
    }
...
bool ChatHandler::HandleGMListIngameCommand(char* /*args*/)
{
    ...
    if (!names.empty())
    {
        ...
        char const* accepts = GetFirelandsString(LANG_GM_ACCEPTS_WHISPER);
        char const* not_accept = GetFirelandsString(LANG_GM_NO_WHISPER);
