...
bool ChatHandler::HandleAHBotStatusCommand(char* args)
{
    ...
    PSendSysMessage(fmtId, GetFirelandsString(LANG_AHBOT_STATUS_ITEM_COUNT),
    ...
    if (all)
    {
        PSendSysMessage(fmtId, GetFirelandsString(LANG_AHBOT_STATUS_ITEM_RATIO),
        ...
        for (int i = 0; i < MAX_AUCTION_QUALITY; ++i)
            PSendSysMessage(fmtId, GetFirelandsString(ahbotQualityIds[i]),
...
bool ChatHandler::HandleAHBotItemsAmountCommand(char* args)
{
    ...
    for (int i = 0; i < MAX_AUCTION_QUALITY; ++i)
    {
        PSendSysMessage(LANG_AHBOT_ITEMS_AMOUNT, GetFirelandsString(ahbotQualityIds[i]), sAuctionBotConfig.getConfigItemQualityAmount(AuctionQuality(i)));
    }
...
bool ChatHandler::HandleAHBotItemsAmountQualityCommand(char* args)
{
    ...
    PSendSysMessage(LANG_AHBOT_ITEMS_AMOUNT, GetFirelandsString(ahbotQualityIds[Q]),
