...
void ChatHandler::ShowTriggerTargetListHelper(uint32 id, AreaTrigger const* at, bool subpart /*= false*/)
{
    if (m_session)
    {
        ...
        if (!subpart)
        {
            ...
            snprintf(dist_buf, 50, GetFirelandsString(LANG_TRIGGER_DIST), dist);
        }
        else
        {
            dist_buf[0] = '\0';
...
{
    char const* tavern = sObjectMgr.IsTavernAreaTrigger(atEntry->id) ? GetFirelandsString(LANG_TRIGGER_TAVERN) : "";
    char const* quest = sObjectMgr.GetQuestForAreaTrigger(atEntry->id) ? GetFirelandsString(LANG_TRIGGER_QUEST) : "";
