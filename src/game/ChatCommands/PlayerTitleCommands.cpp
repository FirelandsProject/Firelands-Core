...
bool ChatHandler::HandleLookupTitleCommand(char* args)
{
    ...
    for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); ++id)
    {
        ...
        if (titleInfo)
        {
            ...
            if (loc < MAX_LOCALE)
            {
                char const* knownStr = target && target->HasTitle(titleInfo) ? GetFirelandsString(LANG_KNOWN) : "";

                char const* activeStr = target && target->GetUInt32Value(PLAYER_CHOSEN_TITLE) == titleInfo->bit_index
                                        ? GetFirelandsString(LANG_ACTIVE)
                                        : "";
...
bool ChatHandler::HandleCharacterTitlesCommand(char* args)
{
    ...
    char const* targetName = target->GetName();
    char const* knownStr = GetFirelandsString(LANG_KNOWN);
