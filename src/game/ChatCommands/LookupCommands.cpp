...
void ChatHandler::ShowQuestListHelper(uint32 questId, int32 loc_idx, Player* target /*= NULL*/)
{
    ...
    if (target)
    {
        ...
        if (status == QUEST_STATUS_COMPLETE)
        {
            ...
            {
                statusStr = GetFirelandsString(LANG_COMMAND_QUEST_REWARDED);
            }
            else
            {
                statusStr = GetFirelandsString(LANG_COMMAND_QUEST_COMPLETE);
            }
        }
        else if (status == QUEST_STATUS_INCOMPLETE)
        {
            statusStr = GetFirelandsString(LANG_COMMAND_QUEST_ACTIVE);
        }
    }
...
void ChatHandler::ShowItemListHelper(uint32 itemId, int loc_idx, Player* target /*=NULL*/)
{
    ...
    if (target)
    {
        ...
        {
            usableStr = GetFirelandsString(LANG_COMMAND_ITEM_USABLE);
        }
    }
...
bool ChatHandler::HandleLookupEventCommand(char* args)
{
    ...
    for (uint32 id = 1; id < events.size(); ++id)
    {
        ...
        {
            char const* active = sGameEventMgr.IsActiveEvent(id) ? GetFirelandsString(LANG_ACTIVE) : "";
    ...
    for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); ++id)
    {
        ...
        if (skillInfo)
        {
            ...
            if (loc < MAX_LOCALE)
            {
                ...
                {
                    knownStr = GetFirelandsString(LANG_KNOWN);
                    uint32 curValue = target->GetPureSkillValue(id);
                    uint32 maxValue = target->GetPureMaxSkillValue(id);
                    uint32 permValue = target->GetSkillPermBonusValue(id);
                    uint32 tempValue = target->GetSkillTempBonusValue(id);

                    char const* valFormat = GetFirelandsString(LANG_SKILL_VALUES);
