...
bool ChatHandler::HandleSummonCommand(char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), GetFirelandsString(LANG_OFFLINE));
...
bool ChatHandler::HandleGPSCommand(char* args)
{
    ...
    CellPair cell_val = Firelands::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY());
    Cell cell(cell_val);
    ...
    GridPair p = Firelands::ComputeGridPair(obj->GetPositionX(), obj->GetPositionY());
    ...
    DEBUG_LOG("Player %s GPS call for %s '%s' (%s: %u):",
              m_session ? GetNameLink().c_str() : GetFirelandsString(LANG_CONSOLE_COMMAND),
              (obj->GetTypeId() == TYPEID_PLAYER ? "player" : "creature"), obj->GetName(),
              (obj->GetTypeId() == TYPEID_PLAYER ? "GUID" : "Entry"), (obj->GetTypeId() == TYPEID_PLAYER ? obj->GetGUIDLow() : obj->GetEntry()));

    DEBUG_LOG(GetFirelandsString(LANG_MAP_POSITION),
...
bool ChatHandler::HandleNearGraveCommand(char* args)
{
    ...
    if (graveyard)
    {
        ...
        {
            team_name = GetFirelandsString(LANG_COMMAND_GRAVEYARD_ANY);
        }
        else if (data->team == HORDE)
        {
            team_name = GetFirelandsString(LANG_COMMAND_GRAVEYARD_HORDE);
        }
        else if (data->team == ALLIANCE)
        {
            team_name = GetFirelandsString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
        }
        else                                                // Actually, this case can not happen
        {
            team_name = GetFirelandsString(LANG_COMMAND_GRAVEYARD_NOTEAM);
        }
...
bool ChatHandler::HandleTeleNameCommand(char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_TELEPORTING_TO, nameLink.c_str(), GetFirelandsString(LANG_OFFLINE), tele->name.c_str());
