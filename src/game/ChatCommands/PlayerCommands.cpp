...
bool ChatHandler::HandleCooldownCommand(char* args)
{
    ...
    else
    {
        ...
        {
            PSendSysMessage(LANG_UNKNOWN_SPELL, target == m_session->GetPlayer() ? GetFirelandsString(LANG_YOU) : tNameLink.c_str());
            SetSentErrorMessage(true);
            return false;
        }

        target->RemoveSpellCooldown(spell_id, true);
        PSendSysMessage(LANG_REMOVE_COOLDOWN, spell_id, target == m_session->GetPlayer() ? GetFirelandsString(LANG_YOU) : tNameLink.c_str());
    }
    return true;
}
...
bool ChatHandler::HandleAddItemCommand(char* args)
{
    ...
    DETAIL_LOG(GetFirelandsString(LANG_ADDITEM), itemId, count);
...
bool ChatHandler::HandleAddItemSetCommand(char* args)
{
    ...
    DETAIL_LOG(GetFirelandsString(LANG_ADDITEMSET), itemsetId);
...
bool ChatHandler::HandleModifyEnergyCommand(char* args)
{
    ...
    DETAIL_LOG(GetFirelandsString(LANG_CURRENT_ENERGY), chr->GetMaxPower(POWER_ENERGY));
...
bool ChatHandler::HandleModifyRepCommand(char* args)
{
    ...
    if (!ExtractInt32(&args, amount))
    {
        ...
        {
            std::string rank = GetFirelandsString(ReputationRankStrIndex[r]);
            if (rank.empty())
            {
                continue;
            }
