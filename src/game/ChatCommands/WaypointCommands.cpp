...
inline void UnsummonVisualWaypoints(Player const* player, ObjectGuid ownerGuid)
{
    ...
    Firelands::AllCreaturesOfEntryInRangeCheck checkerForWaypoint(player, VISUAL_WAYPOINT, SIZE_OF_GRIDS);
    Firelands::CreatureListSearcher<Firelands::AllCreaturesOfEntryInRangeCheck> searcher(waypoints, checkerForWaypoint);
...
bool ChatHandler::HandleWpAddCommand(char* args)
{
    ...
    {
        return false;                                       // must exist as normal creature in firelands.sql 'creature_template'
    }
...
bool ChatHandler::HandleWpShowCommand(char* args)
{
    ...
    if (subCmd == "info")
    {
        ...
        if (WaypointBehavior* behaviour = point->second.behavior)
        {
            ...
            {
                PSendSysMessage(" TextId%i: %i \'%s\'", i + 1, behaviour->textid[i], (behaviour->textid[i] ? GetFirelandsString(behaviour->textid[i]) : ""));
            }
        }
