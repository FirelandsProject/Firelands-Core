...
bool ChatHandler::HandleListAurasCommand(char* /*args*/)
{
    ...
    char const* talentStr = GetFirelandsString(LANG_TALENT);
    char const* passiveStr = GetFirelandsString(LANG_PASSIVE);
...
void ChatHandler::ShowSpellListHelper(Player* target, SpellEntry const* spellInfo, LocaleConstant loc)
{
    ...
    if (rank)
    {
        ss << GetFirelandsString(LANG_SPELL_RANK) << rank;
    }
    ...
    if (talent)
    {
        ss << GetFirelandsString(LANG_TALENT);
    }
    if (passive)
    {
        ss << GetFirelandsString(LANG_PASSIVE);
    }
    if (learn)
    {
        ss << GetFirelandsString(LANG_LEARN);
    }
    if (known)
    {
        ss << GetFirelandsString(LANG_KNOWN);
    }
    if (active)
    {
        ss << GetFirelandsString(LANG_ACTIVE);
    }
...
void ChatHandler::ShowCurrencyListHelper(Player* target, CurrencyTypesEntry const* currency, LocaleConstant loc)
{
    ...
    if (target)
    {
        ss << " (" << GetFirelandsString(LANG_CURRENCY_AMOUNT) << ": " << count << ")";
    }
...
void ChatHandler::ShowFactionListHelper(FactionEntry const* factionEntry, LocaleConstant loc, FactionState const* repState /*= NULL*/, Player* target /*= NULL */)
{
    ...
    if (repState)                               // and then target!=NULL also
    {
        ...
        std::string rankName = GetFirelandsString(ReputationRankStrIndex[rank]);

        ss << " " << rankName << "|h|r (" << target->GetReputationMgr().GetReputation(factionEntry) << ")";

        if (repState->Flags & FACTION_FLAG_VISIBLE)
        {
            ss << GetFirelandsString(LANG_FACTION_VISIBLE);
        }
        if (repState->Flags & FACTION_FLAG_AT_WAR)
        {
            ss << GetFirelandsString(LANG_FACTION_ATWAR);
        }
        if (repState->Flags & FACTION_FLAG_PEACE_FORCED)
        {
            ss << GetFirelandsString(LANG_FACTION_PEACE_FORCED);
        }
        if (repState->Flags & FACTION_FLAG_HIDDEN)
        {
            ss << GetFirelandsString(LANG_FACTION_HIDDEN);
        }
        if (repState->Flags & FACTION_FLAG_INVISIBLE_FORCED)
        {
            ss << GetFirelandsString(LANG_FACTION_INVISIBLE_FORCED);
        }
        if (repState->Flags & FACTION_FLAG_INACTIVE)
        {
            ss << GetFirelandsString(LANG_FACTION_INACTIVE);
        }
    }
    else if (target)
    {
        ss << GetFirelandsString(LANG_FACTION_NOREPUTATION);
    }
