...
void BattleGroundAV::HandleQuestComplete(uint32 questid, Player* player)
{
    ...
    FIRELANDS_ASSERT(teamIdx != BG_AV_TEAM_NEUTRAL);
...
void BattleGroundAV::UpdateScore(PvpTeamIndex teamIdx, int32 points)
{
    ...
    FIRELANDS_ASSERT(teamIdx == TEAM_INDEX_ALLIANCE || teamIdx == TEAM_INDEX_HORDE);
...
void BattleGroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    FIRELANDS_ASSERT(m_Nodes[node].Owner != BG_AV_TEAM_NEUTRAL)
...
void BattleGroundAV::ChangeMineOwner(uint8 mine, BattleGroundAVTeamIndex teamIdx)
{
    ...
    FIRELANDS_ASSERT(mine == BG_AV_NORTH_MINE || mine == BG_AV_SOUTH_MINE);
...
{
    FIRELANDS_ASSERT(GetStatus() == STATUS_IN_PROGRESS);
    ...
    if (m_Nodes[node].TotalOwner == BG_AV_TEAM_NEUTRAL)     // initial snowfall capture
    {
        ...
        FIRELANDS_ASSERT(node == BG_AV_NODES_SNOWFALL_GRAVE);  // currently the only neutral grave
...
{
    FIRELANDS_ASSERT(m_Nodes[node].TotalOwner != BattleGroundAVTeamIndex(teamIdx));
    FIRELANDS_ASSERT(m_Nodes[node].Owner != BattleGroundAVTeamIndex(teamIdx));
    // only assault an assaulted node if no totalowner exists:
    FIRELANDS_ASSERT(m_Nodes[node].State != POINT_ASSAULTED || m_Nodes[node].TotalOwner == BG_AV_TEAM_NEUTRAL);
...
{
    FIRELANDS_ASSERT(m_Nodes[node].State == POINT_ASSAULTED);
...
void BattleGroundAV::DefendNode(BG_AV_Nodes node, PvpTeamIndex teamIdx)
{
    FIRELANDS_ASSERT(m_Nodes[node].TotalOwner == BattleGroundAVTeamIndex(teamIdx));
    FIRELANDS_ASSERT(m_Nodes[node].Owner != BattleGroundAVTeamIndex(teamIdx));
    FIRELANDS_ASSERT(m_Nodes[node].State != POINT_CONTROLLED);
    m_Nodes[node].PrevOwner  = m_Nodes[node].Owner;
    m_Nodes[node].Owner      = BattleGroundAVTeamIndex(teamIdx);
    m_Nodes[node].PrevState  = m_Nodes[node].State;
    m_Nodes[node].State      = POINT_CONTROLLED;
