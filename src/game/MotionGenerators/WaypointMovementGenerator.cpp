...
void WaypointMovementGenerator<Creature>::OnArrived(Creature& creature)
{
    ...
    FIRELANDS_ASSERT(currPoint != i_path->end());
    ...
    if (WaypointBehavior* behavior = node.behavior)
    {
        ...
        if (behavior->textid[0])
        {
            ...
            if (FirelandsStringLocale const* textData = sObjectMgr.GetFirelandsStringLocale(textId))
            {
...
bool WaypointMovementGenerator<Creature>::GetResetPosition(Creature&, float& x, float& y, float& z, float& o) const
{
    ...
    FIRELANDS_ASSERT(lastPoint != i_path->end());
