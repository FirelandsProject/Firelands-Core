...
CREATE TABLE IF NOT EXISTS `game_event_mail` (
  `event` smallint NOT NULL DEFAULT '0' COMMENT 'Event ID (See game_events.entry).',
  `raceMask` mediumint unsigned NOT NULL DEFAULT '0' COMMENT 'Races of affected players.',
  `quest` mediumint unsigned NOT NULL DEFAULT '0' COMMENT 'Quest (See quest_template.entry) which should be rewarded.',
  `mailTemplateId` mediumint unsigned NOT NULL DEFAULT '0' COMMENT 'Mail template ID (See MailTemplate.dbc).',
  `senderEntry` mediumint unsigned NOT NULL DEFAULT '0' COMMENT 'NPC entry (See creature_template.entry).',
  PRIMARY KEY (`event`,`raceMask`,`quest`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 ROW_FORMAT=DYNAMIC COMMENT='Game event system';
