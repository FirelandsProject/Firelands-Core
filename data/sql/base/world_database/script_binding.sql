...
CREATE TABLE IF NOT EXISTS `script_binding` (
  `type` tinyint unsigned NOT NULL COMMENT 'enum ScriptedObjectType in ScriptMgr.h',
  `ScriptName` char(64) NOT NULL COMMENT 'Script name, to be unique across all types',
  `bind` mediumint NOT NULL COMMENT 'Bound to entry (>0) or GUID (<0)',
  `data` tinyint unsigned DEFAULT '0' COMMENT 'Misc data; Effect number for spellscripts',
  PRIMARY KEY (`ScriptName`,`bind`),
  KEY `type` (`type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 ROW_FORMAT=DYNAMIC COMMENT='SD2 Script Names and Binding';
