...
CREATE TABLE IF NOT EXISTS `dungeonfinder_requirements` (
  ...
  `min_item_level` smallint unsigned NOT NULL DEFAULT '0',
  `item` mediumint unsigned NOT NULL DEFAULT '0',
  `item_2` mediumint unsigned NOT NULL DEFAULT '0',
  `alliance_quest` mediumint unsigned NOT NULL DEFAULT '0',
  `horde_quest` mediumint unsigned NOT NULL DEFAULT '0',
  `achievement` mediumint unsigned NOT NULL DEFAULT '0',
  `quest_incomplete_text` text,
  `comment` text,
  PRIMARY KEY (`mapId`,`difficulty`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 ROW_FORMAT=DYNAMIC COMMENT='Dungeon Finder Requirements';

DELETE FROM `dungeonfinder_requirements`;
